...
def update_local_view_data(fig1, bag_loader, bag_time, local_view_data):
  ...
  if bag_loader.plan_debug_msg['enable'] == True:
    ...
    if bag_loader.road_msg['enable'] == True:
      ...
      meb_traj_dphi_vector =planning_json['meb_traj_dphi_vector']

      if len(meb_traj_y_vector) and len(meb_traj_x_vector):
        lat_car_xb_traj = []
        lat_car_yb_traj = []
        for i in range(len(meb_traj_x_vector) - 1, -1, -1):
          car_xb_traj_point = []
          car_yb_traj_point = []
          for j in range(len(car_xb)):
            tmp_x, tmp_y = local2global(car_xb[j], car_yb[j], meb_traj_x_vector[i], meb_traj_y_vector[i], meb_traj_dphi_vector[i])
