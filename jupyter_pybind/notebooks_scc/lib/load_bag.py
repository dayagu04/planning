...
class LoadCyberbag:
  def __init__(self, path) -> None:
    ...
    # loclization msg
    self.loc_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    self.old_loc_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # road msg
    self.road_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # fusion object msg
    self.fus_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # me object msg
    self.mobileye_objects_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}
    # rdg object msg
    self.rdg_objects_msg = {'t':[], 'data':[], 'enable':[], 'timestamp':[]}
    self.lidar_msg = {'t':[], 'data':[], 'enable':[], 'timestamp':[]}

    # radar_fm object msg
    self.radar_fm_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # radar_fl object msg
    self.radar_fl_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # radar_fr object msg
    self.radar_fr_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # radar_rl object msg
    self.radar_rl_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # radar_rr object msg
    self.radar_rr_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # vehicle service msg
    self.vs_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}
    # car pos in local coordinates

    # prediction_msg
    self.prediction_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # planning msg
    self.plan_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # planning debug msg
    self.plan_debug_msg = {'abs_t':[], 't':[], 'data':[], 'json':[], 'enable':[], 'timestamp':[]}

    # control msg
    self.ctrl_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # control debug msg
    self.ctrl_debug_msg = {'abs_t':[], 't':[], 'data':[], 'json':[], 'enable':[], 'timestamp':[]}

    # parking fusion msg
    self.fus_parking_msg = {'abs_t':[], 't':[], 'data':[], 'json':[], 'enable':[], 'timestamp':[]}

    # soc state machine
    self.soc_state_msg = {'abs_t':[], 't':[], 'data':[], 'json':[], 'enable':[], 'timestamp':[]}

    # ehr static map msg
    self.ehr_static_map_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # ehr parking map msg
    self.ehr_parking_map_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # ground_line_msg
    self.ground_line_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    # planning hmi msg
    self.planning_hmi_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}

    self.mobileye_lane_lines_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}
    self.rdg_lane_lines_msg = {'abs_t':[], 't':[], 'data':[], 'enable':[], 'timestamp':[]}
  ...
  def load_all_data(self, normal_print = True):
    ...
    try:
      ...
      for topic, msg, t in self.bag.read_messages("/mobileye/camera_perception/lane_lines"):
        mobileye_lane_lines_msg_dict[msg.header.timestamp / 1e6] = msg
      mobileye_lane_lines_msg_dict = {key: val for key, val in sorted(mobileye_lane_lines_msg_dict.items(), key = lambda ele: ele[0])}
      for t, msg in mobileye_lane_lines_msg_dict.items():
        self.mobileye_lane_lines_msg['t'].append(t)
        self.mobileye_lane_lines_msg['abs_t'].append(t)
    ...
    try:
      ...
      for topic, msg, t in self.bag.read_messages("/mobileye/camera_perception/objects"):
        mobileye_objects_msg_dict[msg.header.timestamp / 1e6] = msg
      mobileye_objects_msg_dict = {key: val for key, val in sorted(mobileye_objects_msg_dict.items(), key = lambda ele: ele[0])}
      for t, msg in mobileye_objects_msg_dict.items():
        self.mobileye_objects_msg['t'].append(t)
        self.mobileye_objects_msg['abs_t'].append(t)
    ...
    try:
      ...
      for topic, msg, t in self.bag.read_messages("/iflytek/camera_perception/objects"):
        rdg_objects_msg_dict[msg.header.timestamp / 1e6] = msg
      rdg_objects_msg_dict = {key: val for key, val in sorted(rdg_objects_msg_dict.items(), key = lambda ele: ele[0])}
      for t, msg in rdg_objects_msg_dict.items():
        self.rdg_objects_msg['t'].append(t)
        self.rdg_objects_msg['abs_t'].append(t)
    ...
    try:
      ...
      for topic, msg, t in self.bag.read_messages("/iflytek/lidar_objects"):
        lidar_msg_dict[msg.header.timestamp / 1e6] = msg
    ...
    try:
      json_value_list = ["replan_status", "ego_pos_x", "ego_pos_y", "ego_pos_yaw", \
                         ...
                         "lane_width", "smooth_lateral_offset", "normal_left_avoid_threshold","normal_right_avoid_threshold","lat_offset", "avoid_way", "is_use_ego_position", "allow_side_max_opposite_offset", "allow_side_max_opposite_offset_id", \
    ...
    try:
      ...
      for t, msg in fus_parking_msg_dict.items():
        ...
        self.fus_parking_msg['timestamp'].append(msg.header.timestamp)
    ...
    try:
      ...
      for t, msg in soc_state_msg_dict.items():
        ...
        self.soc_state_msg['timestamp'].append(msg.header.timestamp)
    ...
    try:
      ...
      for t, msg in planning_hmi_msg_dict.items():
        ...
        self.planning_hmi_msg['timestamp'].append(msg.header.timestamp)
