...
def load_lc_path_figure():
  ...
  fig2 = bkp.figure(x_axis_label='t', y_axis_label='s', x_range = [-0.1, 7.0], width=600, height=400, match_aspect = True, aspect_scale=1)
  fig2.line('t', 'front_obj_s', source = data_st, legend_label="FObj S", line_width=2, color = "red")
...
def update_lc_path_figure(data_st,bag_loader, bag_time, local_view_data):
  ...
  data_st.data.update({
    ...
    'front_obj_s_tar_lane': plan_debug_json_info['front_obj_s_tar_lane_vec'],
    'rear_obj_s': plan_debug_json_info['rear_obj_s_vec'],
