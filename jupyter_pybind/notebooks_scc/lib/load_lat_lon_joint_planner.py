...
def update_joint_plan_data(bag_loader, bag_time, local_view_data, joint_plan_data):
    ...
    jp_in = plan_debug.joint_motion_planning_input
    ...
    for i in range(10):
        if i < len(obs_ref_trajectories):
            ...
            joint_plan_data[f'data_obs_ref_{i}'].data.update({
                ...
                'curv_factor_vec': list(obs_traj.curv_factor_vec) if len(obs_traj.curv_factor_vec) > 0 else [0.0] * len(time_vec),
...
def load_joint_planner_cost_time_figure(bag_loader):
    """Create cost time figure for joint planner pipeline."""
    cost_time_fig = bkp.figure(
        title='Joint Planner Pipeline Cost Time',
        x_axis_label='time/s',
    ...
    for ind in range(len(bag_loader.plan_debug_msg['json'])):
        ...
        joint_motion_planner_cost_vec.append(
            round(plan_json.get('LatLonJointPlannerDeciderTime', 0.0), 2))
    ...
    if len(t_plan_debug) > 0:
        ...
        lat_lon_motion_average_cost = sum(joint_motion_planner_cost_vec) / len(t_plan_debug)
        ...
        print('lat_lon_motion_average_cost', lat_lon_motion_average_cost)
    ...
    hover_total_cost = HoverTool(
        tooltips=[
            ...
            ('JointMotionPlanner', '@joint_motion_planner_cost{0.00}ms'),
