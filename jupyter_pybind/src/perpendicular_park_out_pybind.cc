...
std::vector<Eigen::Vector3d> Update(Eigen::Vector3d ego_pose_offset,
                                    std::vector<Eigen::Vector2d> raw_pt,
                                    double ds, bool is_complete_path,
                                    size_t direction, double inside_dx,
  ...
  Eigen::Vector2d pos_ego(
      ego_pose_offset.x() + apa_param.GetParam().terminal_target_x,
      ego_pose_offset.y() + apa_param.GetParam().terminal_target_y);
  if (direction == 0) {
    frame.current_arc_steer = pnc::geometry_lib::SEG_STEER_LEFT;
    ego_slot_info.ego_heading_slot =
        apa_param.GetParam().terminal_target_heading + ego_pose_offset.z();
  } else if (direction == 1) {
    frame.current_arc_steer = pnc::geometry_lib::SEG_STEER_STRAIGHT;
    ego_slot_info.ego_heading_slot =
        apa_param.GetParam().terminal_target_heading - ego_pose_offset.z();
  } else if (direction == 2) {
  ...
  ego_slot_info.ego_pos_slot << pos_ego.x(), pos_ego.y();

  std::cout << "ego_pos_slot = " << ego_slot_info.ego_pos_slot.transpose()
            << "  ego_heading_slot = " << ego_slot_info.ego_heading_slot * 57.3
            << std::endl;

  const Eigen::Vector2d &ego_pos_global =
      ego_slot_info.l2g_tf.GetPos(ego_slot_info.ego_pos_slot);
  const double &ego_heading_global =
      ego_slot_info.l2g_tf.GetHeading(ego_slot_info.ego_heading_slot);

  global_ego_pose_ << ego_pos_global.x(), ego_pos_global.y(),
      ego_heading_global;
  std::cout << "global_ego_pose_ " << ego_pos_global.transpose() << std::endl;
  std::cout << "ego_heading_global " << ego_heading_global * 57.3 << std::endl;
  ...
  planning::apa_planner::PerpendicularPathOutPlanner::Tlane slot_t_lane;
  ...
  Eigen::Vector2d corner_left_slot(ego_slot_info.slot_length,
                                   0.5 * slot_width + 1);
  ...
  planning::apa_planner::PerpendicularPathOutPlanner::Input input;
  ...
  bool success = pBase->UpdatePyband(input, collision_detector_ptr);
  ...
  // cal target pos
  Eigen::Vector3d global_last_pose =
      current_path_point_global_vec_[current_path_point_global_vec_.size() - 1];

  global_target_pose_ << global_last_pose.x(), global_last_pose.y(),
      global_last_pose.z();
  // std::cout << "target_pose = " << last_pos.transpose() << std::endl;
  // std::cout << "target_pose handing = " << global_target_pose_.z() <<
  // std::endl;
  return current_path_point_global_vec_;
}
...
Eigen::Vector3d GetEgoPose() { return global_ego_pose_; }
