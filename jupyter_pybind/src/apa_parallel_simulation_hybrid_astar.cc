...
#include "src/modules/apa_function/parking_scenario/parallel/parallel_park_in_scenario.h"
...
std::shared_ptr<apa_planner::ParallelParkInScenario> hybrid_astar_park_;
...
int Init() {
  ...
  hybrid_astar_park_ =
      std::dynamic_pointer_cast<apa_planner::ParallelParkInScenario>(planner);
...
const bool PlanOnce(
    ...
    double sample_ds, int pybind_state, std::vector<double> target_managed_slot_x_vec,
...
PYBIND11_MODULE(parallel_simulation_hybrid_astar, m) {
  m.doc() = "m";
