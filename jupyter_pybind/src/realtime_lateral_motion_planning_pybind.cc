...
#include "motion_planners/realtime_lateral_motion_planner/src/realtime_lateral_motion_planning_problem.h"
#include "planning_debug_info.pb.h"

namespace py = pybind11;
using namespace pnc::realtime_lateral_planning;

static RealtimeLateralMotionPlanningProblem *pBase = nullptr;

int Init() {
  pBase = new RealtimeLateralMotionPlanningProblem();
  pBase->Init();
  return 0;
}
...
int UpdateByParams(py::bytes &planning_input_bytes, double q_ref_xy,
  ...
  //       i, planning_input.soft_upper_bound_x0_vec(i) + upper_unit_vector.x()
  //       * upper_safe_bound);
  //   planning_input.mutable_soft_upper_bound_y0_vec()->Set(
  //       i, planning_input.soft_upper_bound_y0_vec(i) + upper_unit_vector.y()
  //       * upper_safe_bound);
  //   planning_input.mutable_soft_lower_bound_x0_vec()->Set(
  //       i, planning_input.soft_lower_bound_x0_vec(i) + lower_unit_vector.x()
  //       * lower_safe_bound);
  //   planning_input.mutable_soft_lower_bound_y0_vec()->Set(
  //       i, planning_input.soft_lower_bound_y0_vec(i) + lower_unit_vector.y()
  //       * lower_safe_bound);
  // }
...
PYBIND11_MODULE(realtime_lateral_motion_planning_py, m) {
  m.doc() = "m";
