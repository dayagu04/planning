...
size_t real_path_index_ = 0;
std::vector<Eigen::Vector3d> global_path_;
std::vector<Eigen::Vector3d> revised_path_;
...
int Init() {
  FilePath::SetName("hybrid_astar_out_pybind");
...
int GetPathFromHybridAstar(const EgoInfoUnderSlot &ego_slot_info,
  ...
  if (result.x.size() > 0) {
    ...
    for (i = 0; i < result.x.size(); i++) {
      ...
      if (fabs(result.phi[i]) <= 85 * kDeg2Rad && !sample_finish) {
        real_path_index_ = i;
        revised_path_.emplace_back(
  ...
  for (int i = 0; i < real_time_node_list_.size(); i++) {
    for (int j = 0; j < real_time_node_list_[i].size(); j++) {
      ...
      real_time_node_list_[i][j] = Eigen::Vector2f(
          real_time_node_list_[i][j].x(), real_time_node_list_[i][j].y());
    }
  }
...
int GenerateObstacleByJupyter(
  ...
  const double channel_length = 40.0;

  const double obs_length = (channel_length - vec_01.norm()) * 0.5;

  const Eigen::Vector2d vritual_obs_center(obs_params[7], obs_params[8]);
  const Eigen::Vector2d vritual_obs_center_1(obs_params[11], obs_params[12]);
  ...
  // set vritual obs
  Eigen::Vector2d vritual_obs_temp0(
      vritual_obs_center.x() - 0.5 * obs_params[9],
      vritual_obs_center.y() + 0.5 * obs_params[10]);
  Eigen::Vector2d vritual_obs_temp1(
      vritual_obs_center.x() + 0.5 * obs_params[9],
      vritual_obs_center.y() + 0.5 * obs_params[10]);
  line.SetPoints(vritual_obs_temp0, vritual_obs_temp1);
  line_vec.emplace_back(line);

  Eigen::Vector2d vritual_obs_temp2(
      vritual_obs_center.x() + 0.5 * obs_params[9],
      vritual_obs_center.y() - 0.5 * obs_params[10]);

  line.SetPoints(vritual_obs_temp1, vritual_obs_temp2);
  line_vec.emplace_back(line);

  Eigen::Vector2d vritual_obs_temp3(
      vritual_obs_center.x() - 0.5 * obs_params[9],
      vritual_obs_center.y() - 0.5 * obs_params[10]);

  line.SetPoints(vritual_obs_temp2, vritual_obs_temp3);
  line_vec.emplace_back(line);

  line.SetPoints(vritual_obs_temp3, vritual_obs_temp0);
  line_vec.emplace_back(line);

  Eigen::Vector2d vritual_obs_temp4(
      vritual_obs_center_1.x() - 0.5 * obs_params[13],
      vritual_obs_center_1.y() + 0.5 * obs_params[14]);
  Eigen::Vector2d vritual_obs_temp5(
      vritual_obs_center_1.x() + 0.5 * obs_params[13],
      vritual_obs_center_1.y() + 0.5 * obs_params[14]);
  line.SetPoints(vritual_obs_temp4, vritual_obs_temp5);
  line_vec.emplace_back(line);

  Eigen::Vector2d vritual_obs_temp6(
      vritual_obs_center_1.x() + 0.5 * obs_params[13],
      vritual_obs_center_1.y() - 0.5 * obs_params[14]);

  line.SetPoints(vritual_obs_temp5, vritual_obs_temp6);
  line_vec.emplace_back(line);

  Eigen::Vector2d vritual_obs_temp7(
      vritual_obs_center_1.x() - 0.5 * obs_params[13],
      vritual_obs_center_1.y() - 0.5 * obs_params[14]);

  line.SetPoints(vritual_obs_temp6, vritual_obs_temp7);
  line_vec.emplace_back(line);

  line.SetPoints(vritual_obs_temp7, vritual_obs_temp4);
  line_vec.emplace_back(line);
...
std::vector<Eigen::Vector3d> Update(
    ...
    const int turn_direction, const std::vector<double> offset_vec) {
  obs_global_points_.clear();
  ...
  const double target_pos_x = 8.0;
  const double target_pos_y = 5.0;
  const double target_heading = 90.0;

  ego_slot_info.target_pose.pos = Eigen::Vector2d(target_pos_x + offset_vec[0],
                                                  target_pos_y + offset_vec[1]);

  ego_slot_info.target_pose.heading =
      (target_heading + offset_vec[2]) * M_PI / 180;
  ...
  end[1] = ego_slot_info.target_pose.pos[1] + 1.0;
  ...
  if (hybrid_astar_interface_ != nullptr && trigger_plan) {
    ...
    request.rs_request = RSPathRequestType::FIRST_PATH_FORBID_REVERSE;
    ...
    GetPathFromHybridAstar(
        ego_slot_info,
        parking_param.astar_config.vertical_head_out_end_straight_dist,
        ego_global_pose);
...
const std::vector<Eigen::Vector3d> &GetRevisedPath() { return revised_path_; }
...
const Eigen::Vector2d GetAstarTargetLine() {
  Eigen::Vector2d extend_pos;
  extend_pos << astar_end_pose_.x(), astar_end_pose_.y();
  const Eigen::Vector2d line_tangent_vec =
      pnc::geometry_lib::GenHeadingVec(astar_end_pose_.z());
  return extend_pos + 1.0 * line_tangent_vec;
}
...
PYBIND11_MODULE(hybrid_astar_out_py, m) {
  m.doc() = "m";

  m.def("Init", &Init)
      .def("Update", &Update)
      .def("GetRevisedPath", &GetRevisedPath)
      ...
      .def("GetAstarTargetLine", &GetAstarTargetLine)
