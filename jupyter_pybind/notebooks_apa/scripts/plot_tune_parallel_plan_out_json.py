...
from jupyter_pybind import parallel_planning_out_py
...
parallel_planning_out_py.Init()

### sliders config
class LocalViewSlider:
  def __init__(self,  slider_callback):

    self.ref_gear_slider = ipywidgets.IntSlider(layout=ipywidgets.Layout(width='35%'), description= "ref gear",min=0, max=2, value= 0, step=1)
    self.is_dirve_out_left_slider = ipywidgets.IntSlider(layout=ipywidgets.Layout(width='35%'), description= "is park out left",min=0, max=1, value= 1, step=1)
    ...
    # ego pose
    self.ego_x_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "ego_x",min=-15, max=15, value= 1.3, step=0.01)
    ...
    ipywidgets.interact(slider_callback,
                                         ...
                                         ds = self.ds_slider,
                                         ref_gear = self.ref_gear_slider,
                                         is_dirve_out_left = self.is_dirve_out_left_slider)
...
def slider_callback(is_front_occupied, is_rear_occupied, is_all_path, ego_x, ego_y, ego_heading,
  ...
  if parallel_planning_out_py.UpdateByJson(obs_x_vec, obs_y_vec, slot_width, slot_length, ego_x, ego_y,
                ego_heading_rad, ds, ref_gear, is_dirve_out_left):

    path_x_vec = parallel_planning_out_py.GetPathEle(0)
    path_y_vec = parallel_planning_out_py.GetPathEle(1)
    path_theta_vec = parallel_planning_out_py.GetPathEle(2)
    ...
    if is_all_path:
      parallel_planning_out_py.SampleAllDebugPaths()
      x_debug_paths = parallel_planning_out_py.GetDebugPathsX()
      y_debug_paths = parallel_planning_out_py.GetDebugPathsY()
    ...
    # tra_search_out_path = parallel_planning_out_py.GetTraSearchOutPath()
    # data_tra_search_out_path.data.update({
    #   'x_vec': tra_search_out_path[0],
    #   'y_vec': tra_search_out_path[1],
    # })

  obs_vec = parallel_planning_out_py.GetParkPlannerObs()
