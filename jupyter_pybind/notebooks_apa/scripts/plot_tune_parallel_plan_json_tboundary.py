...
def load_car_box(car_xb, car_yb, ego_x, ego_y, ego_heading_rad):
  car_xn = []
  car_yn = []
...
data_preparing_step_end_pos =  ColumnDataSource(data = {'x':[], 'y':[]})
data_preparing_step_end_car = ColumnDataSource(data = {'car_yn':[], 'car_xn':[]})

data_preparing_line_end_pos =  ColumnDataSource(data = {'x':[], 'y':[]})
data_preparing_line_end_car = ColumnDataSource(data = {'car_yn':[], 'car_xn':[]})

data_inversed_parking_out_end_pos =  ColumnDataSource(data = {'x':[], 'y':[]})
data_inversed_parking_out_end_car = ColumnDataSource(data = {'car_yn':[], 'car_xn':[]})
...
fig1 = bkp.figure(width=800, height=500, match_aspect = True, aspect_scale=1)
...
fig1.x_range = Range1d(start = -6.0, end = 13.0)
fig1.y_range = Range1d(start = -2.2, end = 7.0)
...
# fig1.line('x_vec','y_vec',source =data_path,  line_width = 3.0, line_color = 'green', line_dash = 'solid',legend_label = 'Car Path')
...
fig1.patch('x_vec', 'y_vec', source = data_slot, fill_color= None, line_color='grey', fill_alpha=0.2, line_width = 0.5)
...
fig1.patches('x_vec', 'y_vec', source = data_other_slot, fill_color = None, line_color ='grey', fill_alpha=0.2, line_width = 0.5)
...
# fig1.patches('x_vec', 'y_vec', source = data_car_box, fill_color = "#98FB98", fill_alpha = 0.0, line_color = "black", line_width = 0.2)


# car box at start pose
# fig1.circle('x', 'y', source = data_start_pos, size=8, color='palegreen', legend_label = 'Start pose')
# fig1.patch( 'car_xn', 'car_yn', source = data_start_car, fill_color = "palegreen", fill_alpha = 0.2, line_color = "black", line_width = 0.5, legend_label = 'Start pose')
fig1.circle('x', 'y', source = data_start_pos, size=8, fill_color = 'red', line_color = 'red')
fig1.patch( 'car_xn', 'car_yn', source = data_start_car, fill_color = None, fill_alpha = 0.3, line_color = "red", line_width = 2.0)

# target pose
# fig1.circle('x', 'y', source = data_target_pos, size=8, color='red', legend_label = 'Target pose')
...
fig1.circle('x', 'y', source = data_target_pos, size=8, fill_color = 'black', line_color = 'black')
fig1.patch( 'car_xn', 'car_yn', source = data_target_car, fill_color = None, fill_alpha = 0.3, line_color = "black", line_width = 2.0)
# obstacles
...
fig1.scatter("x_vec", "y_vec", source=data_fus_obs, size=3, color='grey')
...
fig1.line('x_vec','y_vec',source =data_preparing_step_path,  line_width = 2.5, line_color = 'red', line_dash = 'solid',legend_label = 'Preparing step')
fig1.line('x_vec','y_vec',source =data_preparing_line_path,  line_width = 2.5, line_color = 'green', line_dash = 'solid',legend_label = 'Preparing line')
fig1.line('x_vec','y_vec',source =data_parking_out_path,  line_width = 2.5, line_color = 'blue', line_dash = 'dashed',legend_label = 'Inversed parking out step')
fig1.line('x_vec','y_vec',source =data_in_slot_path,  line_width = 2.5, line_color = 'black', line_dash = 'solid',legend_label = 'Inversed trials in slot')

# each end pose and car box
fig1.circle('x', 'y', source = data_preparing_step_end_pos, size=8, fill_color = 'red', line_color = 'red')
fig1.patch( 'car_xn', 'car_yn', source = data_preparing_step_end_car, fill_color = None, fill_alpha = 0.3, line_color = "red", line_width = 2.5)

fig1.circle('x', 'y', source = data_preparing_line_end_pos, size=8, fill_color = 'green', line_color = 'green')
fig1.patch( 'car_xn', 'car_yn', source = data_preparing_line_end_car, fill_color = None, fill_alpha = 0.3, line_color = "green", line_width = 2.5)

fig1.circle('x', 'y', source = data_inversed_parking_out_end_pos, size=8, fill_color = 'blue', line_color = 'blue')
fig1.patch( 'car_xn', 'car_yn', source = data_inversed_parking_out_end_car, fill_color = None, fill_alpha = 0.3, line_color = "blue", line_width = 2.5)
...
class LocalViewSlider:
  def __init__(self,  slider_callback):
    ...
    self.ego_y_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "ego_y",min=-10, max=10, value= 3.52, step=0.01)
    self.ego_heading_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "ego_heading",min=-180, max=180, value= -30.7, step=0.1)
    # slot pt
    ...
    # obs
    self.lon_space_dx_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "lon_space_dx",min=-1.0, max=4.0, value=0.93, step=0.01)
    ...
    self.rear_car_heading_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "rear obs heading",min=-180.0, max=180.0, value=-2.0, step=0.1)

    self.ds_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "path ds",min=0.025, max=1.0, value=0.01, step=0.01)
...
def slider_callback(is_front_occupied, is_rear_occupied, is_all_path, ego_x, ego_y, ego_heading,
  ...
  if parallel_planning_py.UpdateByJson(obs_x_vec, obs_y_vec, slot_width, slot_length, ego_x, ego_y,
    ...
    in_slot_x_vec = path_x_vec[parking_out_end_idx - 1: in_slot_end_idx]
    in_slot_y_vec = path_y_vec[parking_out_end_idx - 1: in_slot_end_idx]
    ...
    # preparing step end pose
    data_preparing_step_end_pos.data.update({
      'x': [preparing_step_x_vec[-1]],
      'y': [preparing_step_y_vec[-1]]
    })
    preparing_end_box_x_vec, preparing_end_box_y_vec = load_car_box(car_xb, car_yb,
      preparing_step_x_vec[-1], preparing_step_y_vec[-1],
      path_theta_vec[preparing_step_end_idx])

    data_preparing_step_end_car.data.update({
      'car_xn': preparing_end_box_x_vec,
      'car_yn': preparing_end_box_y_vec
    })

    # prepring line end pose
    data_preparing_line_end_pos.data.update({
      'x': [preparing_line_x_vec[-1]],
      'y': [preparing_line_y_vec[-1]]
    })
    preparing_line_end_box_x_vec, preparing_line_end_box_y_vec = load_car_box(car_xb, car_yb,
      preparing_line_x_vec[-1], preparing_line_y_vec[-1],
      path_theta_vec[preparing_line_end_idx])
    data_preparing_line_end_car.data.update({
      'car_xn': preparing_line_end_box_x_vec,
      'car_yn': preparing_line_end_box_y_vec
    })

    # inversed parking out end pose
    data_inversed_parking_out_end_pos.data.update({
      'x': [parking_out_x_vec[-1]],
      'y': [parking_out_y_vec[-1]]
    })
    inversed_parking_out_end_box_x_vec, inversed_parking_out_end_box_y_vec = load_car_box(car_xb, car_yb,
      parking_out_x_vec[-1], parking_out_y_vec[-1],
      path_theta_vec[parking_out_end_idx])
    data_inversed_parking_out_end_car.data.update({
      'car_xn': inversed_parking_out_end_box_x_vec,
      'car_yn': inversed_parking_out_end_box_y_vec
    })
