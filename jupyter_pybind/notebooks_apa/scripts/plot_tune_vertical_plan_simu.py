...
from jupyter_pybind import vertical_planning_py
...
class LocalViewSlider:
  def __init__(self,  slider_callback):
    ...
    self.slot_heading_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "slot_heading",min=0, max=360, value=90.0, step=1)
    ...
    ipywidgets.interact(slider_callback, ego_x = self.ego_x_slider,
                                         ...
                                         slot_heading = self.slot_heading_slider,
                                         slot_width = self.slot_width_slider,
                                         slot_length = self.slot_length_slider,
                                         inside_dx = self.inside_dx_slider,)

### sliders callback
def slider_callback(ego_x, ego_y, ego_heading, slot_pt0_x, slot_pt0_y, slot_heading, slot_width, slot_length, inside_dx):
  kwargs = locals()
  ...
  heading_rad = slot_heading/57.3
  a = math.sin(heading_rad)
  b = math.cos(heading_rad)

  if slot_heading != 90:
    # (x0,y0)-->(x1,y1)
    delta_x1 = slot_width * a
    delta_y1 = slot_width * b
    # (x0,y0)-->(x2,y2)
    delta_x2 = slot_length * b
    delta_y2 = slot_length * a
    # (x2,y2)-->(x3,y3)
    delta_x3 = slot_width * a
    delta_y3 = slot_width * b
  ...
  current_path_point_global_vec_ = vertical_planning_py.Update(ego_pose, slot_pt, 0.2, True, inside_dx)

  # update end point pos
  car_end_pose = vertical_planning_py.GetTargetPose()
  ...
  pt_inside_pos = vertical_planning_py.GetPtInsidePose()
  ...
  safe_circle_tang_pose = vertical_planning_py.GetCircleTangentPose()
