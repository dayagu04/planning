...
car_xb, car_yb, wheel_base = load_car_params_patch_parking(
    vehicle_type=CHERY_M32T, car_lat_inflation=0.0)
...
fig1.line('x_vec', 'y_vec', source=data_tra_search_out_path,  line_width=3.0,
          line_color='red', line_dash='solid', legend_label='tra_search_out', visible=False)
fig1.multi_line('x_vec', 'y_vec', source=data_all_debug_path, line_width=2,
                line_color='orange', line_dash='solid', legend_label='all debug path', visible=True)
fig1.multi_line('x_vec', 'y_vec', source=data_car_in_slot_path, line_width=1.0,
                line_color='blue', line_dash='solid', legend_label='car in slot line', visible=True)
# fig1.scatter("x_vec", "y_vec", source=data_all_prepare_path, size=1, color='grey',legend_label = 'all prepare line pt')
fig1.multi_line('x_vec', 'y_vec', source=data_all_prepare_path, line_width=1.0,
                line_color='pink', line_dash='dashed', legend_label='all prepare line pt', visible=True)
fig1.line('x_vec', 'y_vec', source=data_path,  line_width=3.0,
          line_color='green', line_dash='solid', legend_label='Car Path', visible=True)
...
fig1.line('x_vec', 'y_vec', source=data_slot,  line_width=2.0,
          line_color='black', line_dash='solid', legend_label='slot', visible=True)
...
fig1.multi_line('x_vec', 'y_vec', source=data_other_slot,  line_width=2.0,
                line_color='black', line_dash='solid', legend_label='slot', visible=True)

fig1.patches("x_vec", "y_vec", source=data_extra_region, color='green',
             fill_alpha=0.6, line_color=None, legend_label='Extra available space')
fig1.patches("x_vec", "y_vec", source=data_obs_car_polygon, color='white',
             fill_alpha=1, line_color=None, legend_label='obs obstacles')

# obstacles
fig1.scatter("x_vec", "y_vec", source=data_fus_obs, size=3,
             color='grey', legend_label='External obstacles')
fig1.scatter("x_vec", "y_vec", source=data_obs_pt, size=3,
             color='red', legend_label='Internal obstacles')
...
fig1.patch('x_vec', 'y_vec', source=data_tra_tb_pt,
           fill_color='skyblue', fill_alpha=0.2, line_color=None)

fig1.patches('x_vec', 'y_vec', source=data_car_box, fill_color="#98FB98",
             fill_alpha=0.0, line_color="black", line_width=0.2, legend_label='Envelope')
# car box at start pose
fig1.circle('x', 'y', source=data_start_pos, size=8,
            color='black', legend_label='Start pose')
fig1.patch('car_xn', 'car_yn', source=data_start_car, fill_color="red",
           fill_alpha=0.2, line_color="black", line_width=0.5, legend_label='Start pose')

# target pose
fig1.circle('x', 'y', source=data_target_pos, size=8,
            color='black', legend_label='Target pose')
fig1.patch('car_xn', 'car_yn', source=data_target_car, fill_color="red",
           fill_alpha=0.3, line_color="black", line_width=1, legend_label='Target pose')

fig1.scatter("x_vec", "y_vec", source=data_virtual_obs_pt, size=8, color='red',
             marker='star', legend_label='virtual obstacles', visible=False)
fig1.circle(x='cx_vec', y='cy_vec', radius='radius_vec', source=data_debug_arc, line_alpha=1,
            line_width=2, line_color="red", fill_alpha=0, legend_label='data_debug_arc', visible=False)
...
def slider_callback(is_front_occupied, is_rear_occupied, is_all_path, ego_x, ego_y, ego_heading,
    ...
    json_data = construct_scenario(slot_width, slot_length, curb_offset, lon_space_dx, channel_width,
                                   front_car_y_offset, front_car_heading_rad, rear_car_y_offset, rear_car_heading_rad,
                                   is_front_occupied, is_rear_occupied, "bush")
    ...
    slot_y_vec = [half_slot_width_sgn, half_slot_width_sgn, -
                  half_slot_width_sgn, -half_slot_width_sgn]
    ...
    if parallel_planning_py.UpdateByJson(obs_x_vec, obs_y_vec, slot_width, slot_length, ego_x, ego_y,
        ...
        for i in range(len(car_xb)):
            tmp_x, tmp_y = local2global(
                car_xb[i], car_yb[i], target_pos_x, target_pos_y, 0.00)
        ...
        for k in range(len(path_x_vec)):
            ...
            for i in range(len(car_xb)):
                tmp_x, tmp_y = local2global(
                    car_xb[i], car_yb[i], path_x_vec[k], path_y_vec[k], path_theta_vec[k])
                car_xn.append(tmp_x)
                car_yn.append(tmp_y)
    ...
    for i in range(arc_size):
        print(i, ": [", res[info_len * i], res[info_len * i + 1],
              res[info_len * i + 2], res[info_len * i + 3], res[info_len * i + 4], "]")
