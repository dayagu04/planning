...
car_xb, car_yb, wheel_base = load_car_params_patch_parking(vehicle_type = BESTUNE_E541, car_lat_inflation = 0.0)
...
data_slot_new = ColumnDataSource(data = {'x_vec':[], 'y_vec':[]})
...
data_pa_line_pt = ColumnDataSource(data = {'x_vec':[], 'y_vec':[]})
data_slot_new_pa = ColumnDataSource(data = {'x_vec':[], 'y_vec':[]})
data_slot_origin_pt = ColumnDataSource(data = {'x_vec':[], 'y_vec':[]})
...
fig1.line('x_vec','y_vec',source =data_slot_new,  line_width = 2.0, line_color = 'red', line_dash = 'solid',legend_label = 'new slot', visible = True)
...
fig1.line("x_vec", "y_vec", source=data_pa_line_pt, line_width = 2.0, line_color='blue', line_dash = 'solid', legend_label = 'PA line obs', visible = True)
fig1.line('x_vec','y_vec',source =data_slot_new_pa,  line_width = 2.0, line_color = 'pink', line_dash = 'solid',legend_label = 'pa new slot', visible = True)
fig1.scatter("x_vec", "y_vec", source=data_slot_origin_pt, size=8, color='white',legend_label = 'slot origin pt')
...
class LocalViewSlider:
  def __init__(self,  slider_callback):
    ...
    # ego pose
    self.ego_x_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "ego_x",min=-15, max=15, value= 1.50, step=0.01)
    self.ego_y_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "ego_y",min=-10, max=10, value= 0.3, step=0.01)
    ...
    # slot pt
    self.slot_length_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "slot length",min=5.0, max=8.0, value=5.8, step=0.01)
    ...
    self.new_slot_x_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "new_slot_x",min=-10.0, max=10.0, value=0.0, step=0.1)
    self.new_slot_y_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "new_slot_y",min=-10.0, max=10.0, value=0.35, step=0.1)
    self.new_slot_heading_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "new_slot_heading",min=-180, max=180, value= 0, step=0.01)
    # obs
    ...
    self.front_car_x_offset_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "front obs x",min=-2.0, max=4.0, value=-0.3, step=0.01)
    self.front_car_y_offset_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "front obs y",min=-2.0, max=4.0, value=0, step=0.01)
    ...
    self.rear_car_x_offset_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "rear obs x",min=-2.0, max=4.0, value=0.3, step=0.01)
    ...
    ipywidgets.interact(slider_callback,
                                         ...
                                         new_slot_x = self.new_slot_x_slider,
                                         new_slot_y = self.new_slot_y_slider,
                                         new_slot_heading = self.new_slot_heading_slider,

                                         lon_space_dx  =  self.lon_space_dx_slider,
                                         curb_offset = self.curb_offset_slider,
                                         channel_width = self.channel_width_slider,

                                         front_car_x_offset = self.front_car_x_offset_slider,
                                         front_car_y_offset = self.front_car_y_offset_slider,
                                         front_car_heading = self.front_car_heading_slider,
                                         rear_car_x_offset = self.rear_car_x_offset_slider,
...
def slider_callback(is_front_occupied, is_rear_occupied, is_all_path, ego_x, ego_y, ego_heading,
                    slot_length, slot_width, new_slot_x, new_slot_y, new_slot_heading, lon_space_dx, curb_offset, channel_width,
                    front_car_x_offset, front_car_y_offset, front_car_heading, rear_car_x_offset, rear_car_y_offset, rear_car_heading,
                    ds):
  kwargs = locals()
  ...
  print(f"front_car_x_offset {front_car_x_offset} rear_car_x_offset {rear_car_x_offset}")
  construct_scenario(slot_width, slot_length, curb_offset, lon_space_dx, channel_width, front_car_x_offset,
      front_car_y_offset, front_car_heading_rad, rear_car_x_offset, rear_car_y_offset, rear_car_heading_rad,
  ...
  slot_x_new_vec = []
  slot_y_new_vec = []
  for i in range(len(slot_x_vec)):
      tmp_x, tmp_y = local2global(slot_x_vec[i], slot_y_vec[i], new_slot_x, new_slot_y, new_slot_heading)
      slot_x_new_vec.append(tmp_x)
      slot_y_new_vec.append(tmp_y)
  # slot_x_new_vec = [coord + new_slot_x for coord in slot_x_new_vec]
  # slot_y_new_vec = [coord + new_slot_y for coord in slot_y_new_vec]
  data_slot_new.data.update({'x_vec': slot_x_new_vec, 'y_vec': slot_y_new_vec})
  ...
  # ego start position
  # ego_x, ego_y = local2global(ego_x, ego_y, new_slot_x, new_slot_y, 0.00)
  ...
  data_slot_origin_pt.data.update({
      'x_vec': [],
      'y_vec': [],
  })
  print(f"slot_x_new_vec: {slot_x_new_vec}")
  print(f"slot_y_new_vec: {slot_y_new_vec}")
  data_pa_line_pt.data.update({
      'x_vec': [],
      'y_vec': [],
  })
  data_slot_new_pa.data.update({
      'x_vec': [],
      'y_vec': [],
  })

  if parallel_planning_py.UpdateByJson(obs_x_vec, obs_y_vec, slot_width, slot_length, ego_x, ego_y,
                ego_heading_rad, ds, slot_x_new_vec, slot_y_new_vec):
    ...
    slot_new_pa = parallel_planning_py.GetPASlot()
    data_slot_new_pa.data.update({
        'x_vec': slot_new_pa[0],
        'y_vec': slot_new_pa[1],
    })
    new_slot_x = (slot_new_pa[0][1] + slot_new_pa[0][2]) * 0.5
    new_slot_y = (slot_new_pa[1][1] + slot_new_pa[1][2]) * 0.5
    new_slot_heading = math.atan2(slot_new_pa[1][0] - slot_new_pa[1][1], slot_new_pa[0][0] - slot_new_pa[0][1])
    print(f"slot_new_pa : {slot_new_pa}")
    print(f"new_slot_x: {new_slot_x} new_slot_y: {new_slot_y} new_slot_heading: {new_slot_heading}")
    data_slot_origin_pt.data.update({
        'x_vec': [new_slot_x],
        'y_vec': [new_slot_y],
    })
    ...
    target_pos_x, target_pos_y = local2global(target_pos_x, target_pos_y, new_slot_x, new_slot_y, new_slot_heading)
    ...
    target_car_yn = []
    for i in range(len(car_xb)):
        tmp_x, tmp_y = local2global(car_xb[i], car_yb[i], target_pos_x, target_pos_y, new_slot_heading)
    ...
    for i in range(len(path_x_vec)):
        tmp_x, tmp_y = local2global(path_x_vec[i], path_y_vec[i], new_slot_x, new_slot_y, new_slot_heading)
        path_x_vec_new.append(tmp_x)
        path_y_vec_new.append(tmp_y)
    path_x_vec = path_x_vec_new
    path_y_vec = path_y_vec_new
    ...
    pa_line_points = parallel_planning_py.GetPALine()
    data_pa_line_pt.data.update({
        'x_vec': pa_line_points[0],
        'y_vec': pa_line_points[1],
    })
    ...
    if is_all_path:
      ...
      x_debug_paths_new = []
      y_debug_paths_new = []
      for i in range(len(x_debug_paths)):
        cur_x = []
        cur_y = []
        for j in range(len(x_debug_paths[i])):
          tmp_x, tmp_y = local2global(x_debug_paths[i][j], y_debug_paths[i][j], new_slot_x, new_slot_y, new_slot_heading)
          cur_x.append(tmp_x)
          cur_y.append(tmp_y)
        x_debug_paths_new.append(cur_x)
        y_debug_paths_new.append(cur_y)
      x_vec = [list(x_path) for x_path in x_debug_paths_new]
      y_vec = [list(y_path) for y_path in y_debug_paths_new]
  ...
  car_in_slot_path = [[], []]
  car_in_slot_path_old = parallel_planning_py.GetDebugInSlotPath()
  for i in range(len(car_in_slot_path_old[0])):
      car_in_slot_path_x = []
      car_in_slot_path_y = []
      for j in range(len(car_in_slot_path_old[0][i])):
          tmp_x, tmp_y = local2global(car_in_slot_path_old[0][i][j], car_in_slot_path_old[1][i][j], new_slot_x, new_slot_y, new_slot_heading)
          car_in_slot_path_x.append(tmp_x)
          car_in_slot_path_y.append(tmp_y)
      car_in_slot_path[0].append(car_in_slot_path_x)
      car_in_slot_path[1].append(car_in_slot_path_y)
  data_car_in_slot_path.data.update({
  ...
  obs_in_tboundary_old = parallel_planning_py.GetParkPlannerObs()
  obs_in_tboundary_x = []
  obs_in_tboundary_y = []
  for i in range(len(obs_in_tboundary_old[0])):
      tmp_x, tmp_y = local2global(obs_in_tboundary_old[0][i], obs_in_tboundary_old[1][i], new_slot_x, new_slot_y, new_slot_heading)
      obs_in_tboundary_x.append(tmp_x)
      obs_in_tboundary_y.append(tmp_y)
  # obstacles
  obs_in_tboundary = [obs_in_tboundary_x, obs_in_tboundary_y]
