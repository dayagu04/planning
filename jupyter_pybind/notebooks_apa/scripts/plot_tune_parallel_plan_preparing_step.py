...
class LocalViewSlider:
  def __init__(self,  slider_callback):
    ...
    self.ds_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "path ds",min=0.01, max=1.0, value=0.05, step=0.001)
    ...
    if (len(preparing_x_vec) > 0):
      ...
      if preparing_heading_deg == 0.0:
        data_preparing_line.data.update({
          'x_vec': [-0.5, 16.0],
          'y_vec': [preparing_y_vec[-1], preparing_y_vec[-1]]
        })
      else:
        data_preparing_line.data.update({
