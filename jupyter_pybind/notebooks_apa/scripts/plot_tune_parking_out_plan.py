...
from jupyter_pybind import perpendicular_parking_out_py
...
perpendicular_parking_out_py.Init()
...
class LocalViewSlider:
  def __init__(self,  slider_callback):
    ...
    self.lon_pos_dif_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "lon_pos_dif",min=-10, max=10, value=0.0, step=0.01)
    self.lat_pos_dif_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "lat_pos_dif",min=-10, max=10, value=0.0, step=0.01)
    self.heading_dif_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "heading_dif",min=-5, max=10, value=0.0, step=0.2)
    self.direction_slider = ipywidgets.IntSlider(layout=ipywidgets.Layout(width='15%'), description= "direction",min=0, max=2, value=2, step=1)
    ...
    self.left_obj_dx_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "left_obj_dx",min=-2.0, max=2.0, value=0.0, step=0.05)
    ...
    ipywidgets.interact(slider_callback, vehicle_type = self.vehicle_type_slider,
                                         direction = self.direction_slider,
...
### sliders callback
def slider_callback(vehicle_type, car_inflation,lon_pos_dif, lat_pos_dif, heading_dif, direction, is_left, is_astar, is_complete, slot_phi, slot_inside_obs, right_obj_dx,
  ...
  ego_pose_offset = [lon_pos_dif, lat_pos_dif, heading_dif / 57.3]
  ...
  current_path_point_global_vec_ = perpendicular_parking_out_py.Update(ego_pose_offset, slot_pt, 0.2, is_complete, direction, inside_dx, obs_params)

  rectangle_solt_pos_vec_ = perpendicular_parking_out_py.GetRectangleSoltPos()
  ...
  car_end_pose = perpendicular_parking_out_py.GetTargetPose()
  print("car_end_pose = ",car_end_pose)
  data_car_end_pos.data.update({
    'x': [car_end_pose[0]],
    'y': [car_end_pose[1]],
  })

  ego_pose = perpendicular_parking_out_py.GetEgoPose()
  ego_x = ego_pose[0]
  ego_y = ego_pose[1]
  ...
  print('plan last point = ', current_path_point_global_vec_[-1])
  ...
  pt_inside_pos = perpendicular_parking_out_py.GetPtInsidePose()
  ...
  safe_circle_tang_pose = perpendicular_parking_out_py.GetCircleTangentPose()
  ...
  obs_pts = perpendicular_parking_out_py.GetObstacles()
  obs_pt_x, obs_pt_y = [], []
