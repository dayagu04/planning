...
from jupyter_pybind import slant_planning_py
...
class LocalViewSlider:
  def __init__(self,  slider_callback):
    ...
    self.left_obj_dx_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "left_obj_dx",min=-2.0, max=2.0, value=0.5, step=0.5)
...
def slider_callback(vehicle_type, car_inflation, ego_x, ego_y, ego_heading, is_left, is_astar, is_itervative_solu, is_complete, slot_phi, slot_inside_obs, right_obj_dx,
  ...
  car_end_pose = slant_planning_py.GetTargetPose()
  ...
  pt_inside_pos = slant_planning_py.GetPtInsidePose()
  ...
  obs_pts = slant_planning_py.GetObstacles()
  obs_pt_x, obs_pt_y = [], []
