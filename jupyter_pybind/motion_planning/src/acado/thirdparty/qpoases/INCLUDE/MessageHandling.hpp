...
public:
  ...
  typedef struct {
    ...
    VisibilityStatus globalVisibilityStatus; /**< Determines if message can be
                                              * printed. If this value is set to
                                              * VS_HIDDEN, no message is
                                              * printed! */
  } ReturnValueList;
...
public:
   ...
   * \n Errors are definied as abnormal events which cause an immediate
   * termination of the current (sub) function. Errors of a sub function should
   * be commented by the calling function by means of a warning message (if
   * this error does not cause an error of the calling function, either)!
  ...
  returnValue throwError(
      ...
      VisibilityStatus localVisibilityStatus /**< Determines (locally) if error
                                              * message can be printed to
                                              * myStderr. If GLOBAL visibility
                                              * status of the message is set
                                              * to VS_HIDDEN, no message is
                                              * printed, anyway! */
  );
   ...
   * immediate termination of the current (sub) function. \return Warning
   * number returned by sub function call
   */
  returnValue throwWarning(
      ...
      VisibilityStatus localVisibilityStatus /**< Determines (locally) if
                                              * warning message can be printed
                                              * to myStderr. If GLOBAL
                                              * visibility status of the
                                              * message is set to VS_HIDDEN, no
  ...
  returnValue throwInfo(
      ...
      VisibilityStatus localVisibilityStatus /**< Determines (locally) if info
                                              * message can be printed to
                                              * myStderr. If GLOBAL visibility
                                              * status of the message is set
                                              * to VS_HIDDEN, no message is
  ...
  inline void
  setOutputFile(myFILE *_outputFile /**< New output file for messages. */
  );
  ...
  inline returnValue
  setErrorCount(int _errorCount /**< New error count value. */
  );
...
#define THROWERROR(retval)                                                     \
  (getGlobalMessageHandler()->throwError((retval), 0, __FUNCTION__, __FILE__,  \
                                         __LINE__, VS_VISIBLE))

/** Short version of throwWarning with default values, only returnValue is
 * needed */
#define THROWWARNING(retval)                                                   \
  (getGlobalMessageHandler()->throwWarning((retval), 0, __FUNCTION__,          \
...
#define THROWINFO(retval)                                                      \
  (getGlobalMessageHandler()->throwInfo((retval), 0, __FUNCTION__, __FILE__,   \
                                        __LINE__, VS_VISIBLE))
