...
public:
  ...
  inline BooleanType
  isMember(int _number /**< Number to be tested for membership. */
           ) const;
   ...
   */
protected:
  int number[INDEXLISTFACTOR *
             (NVMAX +
              NCMAX)]; /**< Array to store numbers of constraints or bounds. */
  int next[INDEXLISTFACTOR *
           (NVMAX + NCMAX)]; /**< Array to store physical index of successor. */
  int previous[INDEXLISTFACTOR *
               (NVMAX +
                NCMAX)]; /**< Array to store physical index of predecossor. */
