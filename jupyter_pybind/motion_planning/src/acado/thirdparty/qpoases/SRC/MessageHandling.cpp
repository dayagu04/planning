...
MessageHandling::ReturnValueList returnValueList[] = {
    ...
    {TERMINAL_LIST_ELEMENT, "",
     VS_HIDDEN} /* IMPORTANT: Terminal list element! */
};
...
MessageHandling::MessageHandling()
    : errorVisibility(VS_VISIBLE), warningVisibility(VS_VISIBLE),
      infoVisibility(VS_VISIBLE), outputFile(myStdout), errorCount(0) {}
...
MessageHandling::MessageHandling(myFILE *_outputFile)
    : errorVisibility(VS_VISIBLE), warningVisibility(VS_VISIBLE),
      infoVisibility(VS_VISIBLE), outputFile(_outputFile), errorCount(0) {}
...
MessageHandling::MessageHandling(VisibilityStatus _errorVisibility,
    ...
    : errorVisibility(_errorVisibility), warningVisibility(_warningVisibility),
      infoVisibility(_infoVisibility), outputFile(myStderr), errorCount(0) {}
...
MessageHandling::MessageHandling(myFILE *_outputFile,
                                 ...
                                 VisibilityStatus _infoVisibility)
    : errorVisibility(_errorVisibility), warningVisibility(_warningVisibility),
      infoVisibility(_infoVisibility), outputFile(_outputFile), errorCount(0) {}
...
MessageHandling::MessageHandling(const MessageHandling &rhs)
    : errorVisibility(rhs.errorVisibility),
      ...
      infoVisibility(rhs.infoVisibility), outputFile(rhs.outputFile),
...
MessageHandling::throwError(returnValue Enumber, const char *additionaltext,
  ...
  if (Enumber <= SUCCESSFUL_RETURN)
    return throwError(RET_ERROR_UNDEFINED, 0, __FUNCTION__, __FILE__, __LINE__,
                      VS_VISIBLE);
...
MessageHandling::throwWarning(returnValue Wnumber, const char *additionaltext,
  ...
  if (Wnumber <= SUCCESSFUL_RETURN)
    return throwError(RET_WARNING_UNDEFINED, 0, __FUNCTION__, __FILE__,
                      __LINE__, VS_VISIBLE);
...
returnValue MessageHandling::throwInfo(returnValue Inumber,
  ...
  if (Inumber < SUCCESSFUL_RETURN)
    return throwError(RET_INFO_UNDEFINED, 0, __FUNCTION__, __FILE__, __LINE__,
                      VS_VISIBLE);
...
returnValue MessageHandling::throwMessage(
  ...
  if (returnValueList[keypos].key == TERMINAL_LIST_ELEMENT) {
    throwError(RET_EWI_UNDEFINED, 0, __FUNCTION__, __FILE__, __LINE__,
               VS_VISIBLE);
    return RETnumber;
  }
  ...
  if ((returnValueList[keypos].globalVisibilityStatus == VS_VISIBLE) &&
    ...
    if (additionaltext == 0) {
      sprintf(myPrintfString, "%s (%s, %s:%d): \t%s\n", RETstring, functionname,
              filename, (int)linenumber, returnValueList[keypos].data);
      myPrintf(myPrintfString);
    } else {
      sprintf(myPrintfString, "%s (%s, %s:%d): \t%s %s\n", RETstring,
              functionname, filename, (int)linenumber,
...
MessageHandling globalMessageHandler(myStderr, VS_VISIBLE, VS_VISIBLE,
                                     VS_VISIBLE);
