 ...
 *	\date 2007-2008
 *
 *	Implementation of some inlined utilities for working with the different
 *QProblem classes.
 */
...
returnValue printCopyrightNotice() {
  return myPrintf(
      ...
      "Strategy.\nCopyright (C) 2007-2008 by Hans Joachim Ferreau et al. All "
      "rights reserved.\n\nqpOASES is distributed under the terms of the \nGNU "
      "Lesser General Public License 2.1 in the hope that it will be \nuseful, "
    ...
    for (j = 0; j < ncol; ++j) {
      if (fscanf(datafile, "%f ", &float_data) == 0) {
        ...
        return getGlobalMessageHandler()->throwError(
            RET_UNABLE_TO_READ_FILE, errstr, __FUNCTION__, __FILE__, __LINE__,
            VS_VISIBLE);
      }
...
returnValue writeIntoFile(const real_t *const data, int nrow, int ncol,
  ...
  if (append == BT_TRUE) {
    ...
    if ((datafile = fopen(datafilename, "a")) == 0) {
      ...
      sprintf(errstr, "(%s)", datafilename);
      return getGlobalMessageHandler()->throwError(
          RET_UNABLE_TO_OPEN_FILE, errstr, __FUNCTION__, __FILE__, __LINE__,
          VS_VISIBLE);
    }
  } else {
    /* do not append data */
