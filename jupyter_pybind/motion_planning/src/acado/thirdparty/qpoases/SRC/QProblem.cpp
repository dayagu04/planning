...
returnValue QProblem::init(const real_t *const _H, const real_t *const _g,
                           ...
                           const real_t *const _ubA, int &nWSR,
                           const real_t *const yOpt, real_t *const cputime) {
  /* 1) Setup QP data. */
  if (setupQPdata(_H, 0, _g, _A, _lb, _ub, _lbA, _ubA) != SUCCESSFUL_RETURN)
    return THROWERROR(RET_INVALID_ARGUMENTS);

  /* 2) Call to main initialisation routine (without any additional
   * information). */
  return solveInitialQP(0, yOpt, 0, 0, nWSR, cputime);
}

returnValue QProblem::init(const real_t *const _H, const real_t *const _R,
                           const real_t *const _g, const real_t *const _A,
                           const real_t *const _lb, const real_t *const _ub,
                           const real_t *const _lbA, const real_t *const _ubA,
                           int &nWSR, const real_t *const yOpt,
...
returnValue QProblem::hotstart(const real_t *const g_new,
                               ...
                               const real_t *const ubA_new, int &nWSR,
                               real_t *const cputime) {
  int l;
  ...
  for (l = 0; l < nWSR; ++l) {
    ...
    returnvalue = hotstart_determineDataShift(
        FX_idx, AC_idx, g_new, lbA_new, ubA_new, lb_new, ub_new, delta_g,
        delta_lbA, delta_ubA, delta_lb, delta_ub, Delta_bC_isZero,
        Delta_bB_isZero);
    ...
    returnvalue = hotstart_determineStepDirection(
        FR_idx, FX_idx, AC_idx, delta_g, delta_lbA, delta_ubA, delta_lb,
        delta_ub, Delta_bC_isZero, Delta_bB_isZero, delta_xFX, delta_xFR,
        delta_yAC, delta_yFX);
    if (returnvalue != SUCCESSFUL_RETURN) {
      nWSR = l;
    ...
    returnvalue = hotstart_determineStepLength(
        FR_idx, FX_idx, AC_idx, IAC_idx, delta_lbA, delta_ubA, delta_lb,
        delta_ub, delta_xFX, delta_xFR, delta_yAC, delta_yFX, delta_Ax, BC_idx,
        BC_status, BC_isBound);
    if (returnvalue != SUCCESSFUL_RETURN) {
      nWSR = l;
    ...
    returnvalue = hotstart_performStep(
        FR_idx, FX_idx, AC_idx, IAC_idx, delta_g, delta_lbA, delta_ubA,
        delta_lb, delta_ub, delta_xFX, delta_xFR, delta_yAC, delta_yFX,
        delta_Ax, BC_idx, BC_status, BC_isBound);
...
#ifdef PC_DEBUG
        if (printIteration(l, BC_idx, BC_status, BC_isBound) !=
            SUCCESSFUL_RETURN)
          THROWERROR(RET_PRINT_ITERATION_FAILED); /* do not pass this as return
                                                     value! */
#endif
  ...
  if (printlevel == PL_HIGH) {
    ...
    return getGlobalMessageHandler()->throwWarning(
        RET_MAX_NWSR_REACHED, messageString, __FUNCTION__, __FILE__, __LINE__,
        VS_VISIBLE);
#endif
  }
...
QProblem::solveInitialQP(const real_t *const xOpt, const real_t *const yOpt,
                         ...
                         const Constraints *const guessedConstraints, int &nWSR,
                         real_t *const cputime) {
  int i;
  ...
  returnValue returnvalue = hotstart(g_original, lb_original, ub_original,
                                     lbA_original, ubA_original, nWSR, 0);
...
returnValue QProblem::setupAuxiliaryQPsolution(const real_t *const xOpt,
   ...
   *  old solution vector is kept if pointer to internal solution vevtor is
   * passed. */
  if (xOpt != 0) {
    if (xOpt != x)
      for (i = 0; i < nV; ++i)
...
returnValue QProblem::addConstraint(int number, SubjectToStatus C_status,
  ...
  if (nZ > 0) {
    ...
    for (j = 0; j < nZ - 1; ++j) {
      ...
      for (i = 0; i < nFR; ++i) {
        ii = FR_idx[i];
        applyGivens(c, s, Q[ii * NVMAX + 1 + j], Q[ii * NVMAX + j],
                    Q[ii * NVMAX + 1 + j], Q[ii * NVMAX + j]);
      }

      if ((updateCholesky == BT_TRUE) && (hessianType != HST_IDENTITY)) {
        for (i = 0; i <= j + 1; ++i)
          applyGivens(c, s, R[i * NVMAX + 1 + j], R[i * NVMAX + j],
                      R[i * NVMAX + 1 + j], R[i * NVMAX + j]);
      }
    }
    ...
    if ((updateCholesky == BT_TRUE) && (hessianType != HST_IDENTITY)) {
       ...
       * R. */
      for (i = 0; i < nZ - 1; ++i) {
        computeGivens(R[i * NVMAX + i], R[(1 + i) * NVMAX + i],
                      R[i * NVMAX + i], R[(1 + i) * NVMAX + i], c, s);

        for (j = (1 + i); j < (nZ - 1);
             ++j) /* last column of R is thrown away */
          applyGivens(c, s, R[i * NVMAX + j], R[(1 + i) * NVMAX + j],
                      R[i * NVMAX + j], R[(1 + i) * NVMAX + j]);
      }
...
/* setup output preferences */
  ...
  if (y_min_number >= 0) {
    ...
    if ((cyclingManager.getCyclingStatus(number, BT_FALSE) ==
         CYC_PREV_REMOVED) &&
        (cyclingManager.getCyclingStatus(y_min_number, y_min_isBound) ==
         CYC_PREV_ADDED)) {
      infeasible = BT_TRUE;
      ...
      cyclingManager.setCyclingStatus(y_min_number, y_min_isBound,
                                      CYC_PREV_REMOVED);
    }
    ...
    if (y_min_isBound == BT_TRUE) {
      ...
      getGlobalMessageHandler()->throwInfo(
          RET_REMOVE_FROM_ACTIVESET, messageString, __FUNCTION__, __FILE__,
          __LINE__, visibilityStatus);
#endif
  ...
  return getGlobalMessageHandler()->throwInfo(RET_LI_RESOLVED, 0, __FUNCTION__,
                                              __FILE__, __LINE__, VS_HIDDEN);
}
...
returnValue QProblem::addBound(int number, SubjectToStatus B_status,
                  ...
                  nothing to do */
  {
...

    {
      ...
      applyGivens(c, s, T[(nAC - 1) * NVMAX + tcol], tmp[nAC - 1], tmp[nAC - 1],
                  T[(nAC - 1) * NVMAX + tcol]);
    }
    ...
    for (j = nZ; j < nFR - 1; ++j) {
      ...
      for (i = (nFR - 2 - j); i < nAC; ++i)
        applyGivens(c, s, T[i * NVMAX + 1 + tcol - nZ + j], tmp[i], tmp[i],
  ...
  if ((updateCholesky == BT_TRUE) && (hessianType != HST_IDENTITY)) {
     ...
     */
    for (i = 0; i < nZ - 1; ++i) {
      computeGivens(R[i * NVMAX + i], R[(1 + i) * NVMAX + i], R[i * NVMAX + i],
                    R[(1 + i) * NVMAX + i], c, s);
...
/* setup output preferences */
  ...
  if (y_min_number >= 0) {
    /* 1) Check for cycling due to infeasibility. */
    if ((cyclingManager.getCyclingStatus(number, BT_TRUE) ==
         CYC_PREV_REMOVED) &&
        (cyclingManager.getCyclingStatus(y_min_number, y_min_isBound) ==
         CYC_PREV_ADDED)) {
      infeasible = BT_TRUE;
...
returnValue QProblem::removeConstraint(int number, BooleanType updateCholesky) {
  ...
  if (number_idx < nAC - 1) {
    ...
    for (j = (nAC - 2 - number_idx); j >= 0; --j) {
      computeGivens(T[(nAC - 2 - j) * NVMAX + tcol + 1 + j],
                    T[(nAC - 2 - j) * NVMAX + tcol + j],
                    T[(nAC - 2 - j) * NVMAX + tcol + 1 + j],
                    T[(nAC - 2 - j) * NVMAX + tcol + j], c, s);

      for (i = (nAC - j - 1); i < (nAC - 1); ++i)
        applyGivens(c, s, T[i * NVMAX + tcol + 1 + j], T[i * NVMAX + tcol + j],
                    T[i * NVMAX + tcol + 1 + j], T[i * NVMAX + tcol + j]);

      for (i = 0; i < nFR; ++i) {
        ii = FR_idx[i];
        applyGivens(c, s, Q[ii * NVMAX + nZ + 1 + j], Q[ii * NVMAX + nZ + j],
                    Q[ii * NVMAX + nZ + 1 + j], Q[ii * NVMAX + nZ + j]);
      }
    }
  } else {
    /* gimmick: write zeros into the last row of T */
...
returnValue QProblem::removeBound(int number, BooleanType updateCholesky) {
  ...
  if (nAC > 0) {
    ...
    for (j = (nAC - 1); j >= 0; --j) {
      computeGivens(tmp[nAC - 1 - j], T[(nAC - 1 - j) * NVMAX + tcol + j],
                    T[(nAC - 1 - j) * NVMAX + tcol + j], tmp[nAC - 1 - j], c,
                    s);

      for (i = (nAC - j); i < nAC; ++i)
        applyGivens(c, s, tmp[i], T[i * NVMAX + tcol + j],
                    T[i * NVMAX + tcol + j], tmp[i]);
  ...
  if ((updateCholesky == BT_TRUE) && (hessianType != HST_IDENTITY)) {
    ...
    if (nFR > 0) {
      ...
      for (j = 0; j < nFR; ++j) {
        ...
        /* z1' * ( Hz + 2*z2*h1 ) */
        rho2 +=
            Q[jj * NVMAX + nZ] * (Hz[j] + 2.0 * z2 * H[nnFRp1 * NVMAX + jj]);
      }
    }
 ...
 */
returnValue QProblem::backsolveR(const real_t *const b, BooleanType transposed,
                                 BooleanType removingBound, real_t *const a) {
  int i, j;
  int nR = getNZ();
  ...
  if (nFR > 0) {
    ...
    if (nAC > 0) {
      if ((Delta_bC_isZero == BT_TRUE) && (Delta_bB_isZero == BT_TRUE)) {
        ...
        if (backsolveT(delta_xFRy_TMP, BT_FALSE, delta_xFRy) !=
            SUCCESSFUL_RETURN)
      ...
      if (nZ > 0) {
        ...
        if ((nAC > 0) && (nFX > 0) && (Delta_bB_isZero == BT_FALSE)) {
          ...
          if ((nAC == 0) && (Delta_bB_isZero == BT_TRUE)) {
            ...
            if (nAC > 0) /* => Delta_bB_isZero == BT_TRUE, as BT_FALSE would
                            imply nFX>0 */
            {
              for (j = 0; j < nFR; ++j) {
                jj = FR_idx[j];
        ...
        if (backsolveR(delta_xFRz, BT_TRUE, delta_xFRz_TMP) !=
            SUCCESSFUL_RETURN)
          return THROWERROR(RET_STEPDIRECTION_FAILED_CHOLESKY);

        if (backsolveR(delta_xFRz_TMP, BT_FALSE, delta_xFRz) !=
            SUCCESSFUL_RETURN)
          return THROWERROR(RET_STEPDIRECTION_FAILED_CHOLESKY);
...
returnValue QProblem::hotstart_determineStepLength(
    ...
    SubjectToStatus &BC_status, BooleanType &BC_isBound) {
  int i, j, ii, jj;
  int nV = getNV();
  int nC = getNC();
  int nFR = getNFR();
  ...
  if (printlevel == PL_HIGH) {
    ...
    getGlobalMessageHandler()->throwInfo(RET_STEPSIZE_NONPOSITIVE,
                                         messageString, __FUNCTION__, __FILE__,
                                         __LINE__, VS_VISIBLE);
  }
#endif
...
returnValue QProblem::hotstart_performStep(
  ...
  if (areBoundsConsistent(delta_lb, delta_ub, delta_lbA, delta_ubA) ==
      BT_FALSE) {
    infeasible = BT_TRUE;
    tau = 0.0;
...
/* print a stepsize warning if stepsize is zero */
    ...
    char messageString[80];
    sprintf(messageString, "Stepsize is %.6e", tau);
    getGlobalMessageHandler()->throwWarning(RET_STEPSIZE, messageString,
                                            __FUNCTION__, __FILE__, __LINE__,
                                            VS_VISIBLE);
#endif
  }
 ...
 */
returnValue
QProblem::setupQPdata(const real_t *const _H, const real_t *const _R,
                      const real_t *const _g, const real_t *const _A,
                      const real_t *const _lb, const real_t *const _ub,
                      const real_t *const _lbA, const real_t *const _ubA) {
  int i, j;
 ...
 */
returnValue QProblem::printIteration(int iteration, int BC_idx,
                                     SubjectToStatus BC_status,
                                     BooleanType BC_isBound) {
