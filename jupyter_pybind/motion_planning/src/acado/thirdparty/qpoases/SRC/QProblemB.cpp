...
returnValue QProblemB::init(const real_t *const _H, const real_t *const _R,
                            const real_t *const _g, const real_t *const _lb,
                            const real_t *const _ub, int &nWSR,
...
returnValue QProblemB::hotstart(const real_t *const g_new,
                                ...
                                const real_t *const ub_new, int &nWSR,
                                real_t *const cputime) {
  int l;
  ...
  for (l = 0; l < nWSR; ++l) {
    ...
    returnvalue =
        hotstart_determineDataShift(FX_idx, g_new, lb_new, ub_new, delta_g,
    ...
    returnvalue = hotstart_determineStepDirection(
        FR_idx, FX_idx, delta_g, delta_lb, delta_ub, Delta_bB_isZero, delta_xFX,
        delta_xFR, delta_yFX);
    if (returnvalue != SUCCESSFUL_RETURN) {
      nWSR = l;
    ...
    returnvalue =
        hotstart_determineStepLength(FR_idx, FX_idx, delta_lb, delta_ub,
                                     delta_xFR, delta_yFX, BC_idx, BC_status);
    if (returnvalue != SUCCESSFUL_RETURN) {
      nWSR = l;
    ...
    returnvalue = hotstart_performStep(FR_idx, FX_idx, delta_g, delta_lb,
                                       delta_ub, delta_xFX, delta_xFR,
...
returnValue QProblemB::solveInitialQP(const real_t *const xOpt,
  ...
  returnValue returnvalue =
      hotstart(g_original, lb_original, ub_original, nWSR, 0);
...
returnValue QProblemB::addBound(int number, SubjectToStatus B_status,
  ...
  for (i = number_idx + 1; i < nFR; ++i) {
    computeGivens(R[(i - 1) * NVMAX + i], R[i * NVMAX + i],
                  R[(i - 1) * NVMAX + i], R[i * NVMAX + i], c, s);

    for (j = (1 + i); j < nFR; ++j) /* last column of R is thrown away */
      applyGivens(c, s, R[(i - 1) * NVMAX + j], R[i * NVMAX + j],
                  R[(i - 1) * NVMAX + j], R[i * NVMAX + j]);
  }
...
returnValue QProblemB::hotstart_determineStepLength(
  ...
  if (printlevel == PL_HIGH) {
    ...
    if (BC_status == ST_UNDEFINED)
      ...
      sprintf(messageString, "Stepsize is %.6e! (BC_idx = %d, BC_status = %d)",
 ...
 */
returnValue QProblemB::printIteration(int iteration, int BC_idx,
                                      SubjectToStatus BC_status) {
