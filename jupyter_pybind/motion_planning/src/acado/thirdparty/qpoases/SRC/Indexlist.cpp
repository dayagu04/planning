 ...
 */
Indexlist::Indexlist()
    : length(0), first(-1), last(-1), lastusedindex(-1),
...
Indexlist::Indexlist(const Indexlist &rhs)
    : length(rhs.length), first(rhs.first), last(rhs.last),
      lastusedindex(rhs.lastusedindex), physicallength(rhs.physicallength) {
  int i;
