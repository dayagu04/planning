...
#             fusion_obs_info['is_cipv'].append(obstacle_list[i].target_selection_type)
  ...
  # print(num) 
  return obs_info_all
def load_obstacle_paramsv2(obstacle_list):
  obs_info = {
    'obstacles_x_rel': [],
    'obstacles_y_rel': [],
    'obstacles_x': [],
  ...
  for i in range(obs_num):
    ...
    if obstacle_list[i].common_info.relative_position.x == 0 and obstacle_list[i].common_info.relative_position.y == 0:
      ...
      num = num + 1
      obs_info['obstacles_x_rel'].append(obs_x_rel)
      obs_info['obstacles_y_rel'].append(obs_y_rel)
      obs_info['pos_x_rel'].append(long_pos_rel)
      obs_info['pos_y_rel'].append(lat_pos_rel)
