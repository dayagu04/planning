...
TEST(TestJerkLimitedTrajectory, jerk_limited_trajectory) {
  ...
  JLT.positionTargetSolver(extrem_, PS_0.p_, PS_0.v_, PS_0.a_, p_desire,
                           v_desire);
  ...
  for (int i = 0; i < total_time / delta_t; ++i) {
    Point_State PS_f =
        JLT.updateFinalTrajectory(PS_0.p_, PS_0.v_, PS_0.a_, time_stamp);
