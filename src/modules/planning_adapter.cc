...
void PlanningAdapter::Proc() {
  ...
  // 1.update all inputs to local_view
  {
    ...
    input_topic_latency->set_prediction(
        start_time - prediction_result_msg_.header().timestamp() / US_PER_MS);
    ...
    input_topic_latency->set_fusion_road(
        start_time - road_info_msg_.header().timestamp() / US_PER_MS);
    ...
    input_topic_latency->set_localization(
        start_time -
        localization_estimate_msg_.header().timestamp() / US_PER_MS);
    ...
    input_topic_latency->set_fusion_object(
        start_time - fusion_objects_info_msg_.header().timestamp() / US_PER_MS);
    ...
    input_topic_latency->set_vehicle_service(
        start_time -
    ...
    input_topic_latency->set_control_output(
        start_time - control_output_msg_.header().timestamp() / US_PER_MS);
    ...
    input_topic_latency->set_hmi(
        start_time - hmi_mcu_inner_info_msg_.header().timestamp() / US_PER_MS);
    ...
    input_topic_latency->set_parking_fusion(
        start_time - parking_fusion_info_msg_.header().timestamp() / US_PER_MS);
    ...
    input_topic_latency->set_function_state_machine(
        start_time - func_state_machine_msg_.header().timestamp() / US_PER_MS);
  ...
  std::cout << "==============The planning enters RunOnce============="
            << std::endl;
  bool run_success = planning_base_->RunOnce(local_view_, &planning_output,
                                             debug_output, &planning_hmi_info);
