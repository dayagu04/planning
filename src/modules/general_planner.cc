...
void GeneralPlanner::InitContext() {
  ...
  // init state machine
  scenario_state_machine_ = std::make_shared<ScenarioStateMachine>(config_builder, session_);
  scenario_state_machine_->init();
  session_->mutable_planning_context()->set_scenario_state_machine(scenario_state_machine_);
  session_->mutable_planning_context()->set_vehicle_param(session_->vehicle_config_context().get_vehicle_param());
  ...
  // SCC Function
  adaptive_cruise_control_ = std::make_shared<AdaptiveCruiseControl>(config_builder, session_);
  session_->mutable_planning_context()->set_adaptive_cruise_control_function(adaptive_cruise_control_);
...
void GeneralPlanner::UpdateFixLaneVirtualId() {
  ...
  if (fix_reference_path == nullptr) {
    session_->mutable_planning_context()
        ...
        ->reset_lc_lanes();
    session_->mutable_planning_context()->mutable_lat_behavior_state_machine_output().fix_lane_virtual_id =
        current_lane_virtual_id;
    session_->mutable_planning_context()->mutable_lat_behavior_state_machine_output().origin_lane_virtual_id =
        current_lane_virtual_id;
    session_->mutable_planning_context()->mutable_lat_behavior_state_machine_output().target_lane_virtual_id =
        current_lane_virtual_id;
  }
}
...
bool GeneralPlanner::Run(planning::framework::Frame *frame) {
  ...
  common::PlanningStatus *pnc_status = session_->mutable_planning_output_context()->mutable_planning_status();
  ...
  object_selector_->update(session_->planning_context().lat_behavior_state_machine_output().curr_state,
                           session_->planning_context().scenario_state_machine()->get_start_move_dist_lane(), false,
                           80., false, false, false, false, false, -1);
  ...
  auto &ego_planning_result = session_->mutable_planning_context()->mutable_planning_result();
  bool planning_failed = !session_->mutable_planning_context()->planning_success();
...
void GeneralPlanner::SetPlanningResult(const PlanningResult &ego_planning_result, common::PlanningResult &pnc_result) {
  // 这个函数需要好好整理下
  ...
  if (ego_planning_result.turn_signal == NO_CHANGE) {
    pnc_result.planning_output.mutable_turn_signal_command()->set_turn_signal_value(
