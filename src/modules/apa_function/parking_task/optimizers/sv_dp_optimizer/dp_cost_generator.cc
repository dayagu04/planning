...
double DPCostGenerator::CalcSpeedUpCost(const double acc, const double node_v,
  ...
  double acc_diff;
  if (node_v <= speed_limit) {
    acc_diff =
        (acc - config_->advised_acceleration) / config_->advised_acceleration;
  } else {
...
void DPCostGenerator::CalcSpeedCost(const double speed_limit,
  ...
  double speed_diff = (speed - speed_limit) / speed_limit;
  if (speed_diff > 0) {
...
double DPCostGenerator::CalcSpeedDownCost(const double dec) {
  if (dec < config_->deceleration_limit) {
    return 10000.0;
  }

  double acc_diff = dec - config_->advised_deceleration;
...
void DPCostGenerator::CalcAccCost(SVGraphNode* point) {
  if (std::fabs(point->GetSVPoint().acc) < 1e-2) {
    ...
    point->MutableCost()->acc_cost = CalcSpeedDownCost(point->GetSVPoint().acc);
  } else {
