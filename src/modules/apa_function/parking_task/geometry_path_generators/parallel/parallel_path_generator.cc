...
static const double kColSmallBufferInSlot = 0.12;  // in slot
static const double kColTinyBufferInSlot = 0.0;
static const double kColPABuffer = 0.2;
...
static const size_t kMaxParallelShiftNumsToPA = 4;
...
static const double kMaxDriveLengthInclineInSlot = 0.5;
...
void ParallelPathGenerator::Preprocess() {
  ...
  if (tlane_length >= 6.8) {
    ...
    calc_params_.lon_buffer_rev_trials = kColTinyBufferInSlot;
  }
...
ParallelPathGenerator::CheckPaParkCondition() {
  ...
  const double ratio = input_.ego_info_under_slot.slot_occupied_ratio;
  const double sec_ratio = ratio / input_.tlane.first_occupied_ratio + kEps;
  ILOG_INFO << "slot_len = " << slot_len
...
const bool ParallelPathGenerator::Update() {
  ...
  if (enable_pa_park_ && is_pa_condition == PaPlanMethod::PaSturnPlan) {
    collision_detector_ptr_->SetParam(CollisionDetector::Paramters(0.0, false));
    if (!PAParallelAdjustPlan()) {
      ILOG_INFO << "PAParallelAdjustPlan failed!";
      return false;
    }
    ILOG_INFO << "PAParallelAdjustPlan success!";
    return true;
  } else if ((!enable_pa_park_ && !is_in_slot) ||
             (enable_pa_park_ &&
              is_pa_condition == PaPlanMethod::ApaOutSlotPlan)) {
    ...
    for (size_t i = 0; i < calc_params_.inversed_path_vec_in_slot.size(); i++) {
      std::vector<pnc::geometry_lib::PathSegment> path_segment_vec =
          calc_params_.inversed_path_vec_in_slot[i].path_segment_vec;
      ReversePathSegVec(path_segment_vec);
      path_in_slot_to_astar_.emplace_back(path_segment_vec);
    }
    ...
    if (input_.astar_state != AstarSearchState::NONE) {
      ILOG_INFO << "using astar, so geometryunc path is not calculated";
      return false;
    }
    ...
    if (success) {
      ...
      return success;
             ...
             (enable_pa_park_ &&
              is_pa_condition == PaPlanMethod::ApaInSlotPlan)) {
    ILOG_INFO << "ego is in slot";

    bool is_multi_align_success = false;
    std::vector<pnc::geometry_lib::PathSegment> path_seg_vec;
    collision_detector_ptr_->SetParam(CollisionDetector::Paramters(0.0, false));
...
const bool ParallelPathGenerator::PlanFromTargetToLine(
  ...
  for (const auto& target_pose : calc_params_.valid_target_pt_vec) {
    ...
    for (; i < arc2_radius_vec.size(); i++) {
      ...
      ILOG_INFO << "diverse arc idx = " << i << "  radius = " << arc2_radius;
    ...
    if (is_arc_2_set && i > 3) {
      ...
      if (PlanFromTargetToLineInNarrowChannel(narrow_path_seg_vec, arc_1,
        ...
        ILOG_INFO << "PlanFromTargetToLineInNarrowChannel failed!";
      }
    }
...
const bool ParallelPathGenerator::CalSinglePathInNarrowChannel(
  ...
  const double min_radius = apa_param.GetParam().min_turn_radius + 0.4;
...
const bool ParallelPathGenerator::OutsideSlotPlan() {
  ...
  for (int k = tiled_line_size; k < big_ang_tiled_line_size; k++) {
    pnc::geometry_lib::LineSegment line_seg(
        preparing_pose_vec[k].pos, preparing_pose_vec[k].heading, 1, 1);
    debug_info_.debug_line_vec.emplace_back(line_seg);
  }
  ILOG_INFO << "big_ang_tiled_line_size = "
            << debug_info_.debug_line_vec.size();
  ...
  for (size_t i = 0; i < preparing_pose_vec.size(); i++) {
    ...
    if ((i >= tiled_line_size &&
         aligned_success_cnt + parallel_success_cnt + tiled_success_cnt > 0) &&
        !short_channel_flag) {
...
const bool ParallelPathGenerator::CheckEgoInSlot()  {
  ...
  if (input_.astar_state != AstarSearchState::NONE) {
    threhold = kAstarToGeometryEgoRatio;
  }
...
const bool ParallelPathGenerator::CalMinSafeCircle() {
  ...
  bool success_adv = AdvancedInversedTrialsInSlot(adv_search_out_res,
  ...
  if (apa_param.GetParam().is_parallel_advanced_method) {
    if (success_adv) {
      ...
      for (size_t i = 1; i < 4; i++) {
        calc_params_.target_pose.pos.y() += 0.1 * calc_params_.slot_side_sgn;
        adv_search_out_res.clear();
        success_adv = AdvancedInversedTrialsInSlot(adv_search_out_res,
                                                   calc_params_.target_pose);
        if (success_adv) {
          search_out_res = adv_search_out_res;
          break;
  ...
  pnc::geometry_lib::PrintPose("corrected safe_circle_root_pose",
                               calc_params_.safe_circle_root_pose);
...
const bool ParallelPathGenerator::SortPathByGearShiftHeadingAndLength(
  ...
  if (sorted_path_vec.size() > 1) {
    ...
    double rl_min_x = base_x;
    size_t rl_min_x_idx = 0;
    bool found_reverse_line = false;

    for (size_t i = 1; i < sorted_path_vec.size(); ++i) {
      const auto& path = sorted_path_vec[i];
      if (path.path_segment_vec.empty()) continue;
      const double curr_x = path.path_segment_vec.back().GetStartPos().x();
      const double curr_y =
          std::fabs(path.path_segment_vec.back().GetStartPos().y());
      ...
      bool is_reverse_line = false;
      const auto& segs = path.path_segment_vec;
      if (!segs.empty() && segs.front().seg_gear ==
                               uint8_t(pnc::geometry_lib::SEG_GEAR_REVERSE)) {
        is_reverse_line = true;
        for (size_t j = 0; j < segs.size(); ++j) {
          const auto& seg = segs[j];
          if (seg.seg_gear != segs.front().seg_gear) {
            break;
          }
          if (seg.seg_type != uint8_t(pnc::geometry_lib::SEG_TYPE_LINE)) {
      ...
      if (is_reverse_line && curr_x < rl_min_x) {
        rl_min_x = curr_x;
        rl_min_x_idx = i;
        found_reverse_line = true;
      }
    }
    if (std::fabs(min_x - rl_min_x) < 0.5) {
      min_x_idx = found_reverse_line ? rl_min_x_idx : min_x_idx;
      min_x = found_reverse_line ? rl_min_x : min_x;
    }
...
const bool ParallelPathGenerator::CheckTargetPoseValid(
    const pnc::geometry_lib::PathPoint target_pose) const {
  const auto pos_error =
      (target_pose.pos - calc_params_.target_pose.pos).cwiseAbs();

  const bool heading_cond =
      std::fabs(kRad2Deg *
                pnc::geometry_lib::NormalizeAngle(
                    target_pose.heading - calc_params_.target_pose.heading)) <
...
const bool ParallelPathGenerator::MultiAlignBody(
  ...
  path_seg_vec.clear();
  path_seg_vec.reserve(kMaxMultiStepNums);
  ...
  while (std::fabs(current_pose.heading * kRad2Deg) > 1.0 && i++ < max_iter) {
    ...
    auto col_res = TrimPathByCollisionDetection(single_aligned_path.back(),
                                                kColSmallBufferInSlot);
    if (col_res == PATH_COL_INVALID) {
      ILOG_WARN << "first buffer failed, trying fallback buffer";
      col_res = TrimPathByCollisionDetection(single_aligned_path.back(), 0.05);
    }

    if (col_res == PATH_COL_SHORTEN || col_res == PATH_COL_NORMAL) {
      path_seg_vec.emplace_back(single_aligned_path.back());
  ...
  if (success) {
    Eigen::Vector2d last_pt_start = path_seg_vec.back().GetEndPos();
...
const bool ParallelPathGenerator::OneLinePlanToPA(
  ...
  for (double i = max_line_length; i > min_line_length; i -= by_step) {
    ...
    a_line.heading = current_pose.heading;
    pnc::geometry_lib::PathPoint r_point(current_pose.GetPos(), a_line.heading);
    if (gear == static_cast<int>(pnc::geometry_lib::SEG_GEAR_REVERSE)) {
      r_point.pos.x() -= i;
    } else {
      r_point.pos.x() += i;
    }
    if (OneLinePlan(a_line, r_point)) {
      pnc::geometry_lib::PathSegment a_path_line(
          pnc::geometry_lib::CalLineSegGear(a_line), a_line);
      if (!CheckPathSegCollided(a_path_line, lon_buffer)) {
        ILOG_INFO << "calc line success length: " << a_line.length;
        // AddPathSegToOutPut(a_path_line);
        out_path_line = a_path_line;
        current_pose = a_path_line.GetEndPose();
        return true;
      }
    }
    ILOG_INFO << "calc line failed length: " << a_line.length;
  }
  return false;
}

const bool ParallelPathGenerator::PAParallelAdjustPlan() {
  ILOG_INFO << "Enter PAParallelAdjustPlan";
  // set init state
  input_.ego_info_under_slot.cur_pose.heading =
      pnc::geometry_lib::NormalizeAngle(
          input_.ego_info_under_slot.cur_pose.heading);
  ...
  ILOG_INFO << "current pose x =" << current_pose.pos.x()
  ...
  const double debug_param =
      apa_param.GetParam().debug_parallel_angle_threshold;
  ILOG_INFO << "debug_param =" << debug_param;
  if (std::fabs(pnc::geometry_lib::NormalizeAngle(
          current_pose.heading - calc_params_.target_line.heading)) >=
      debug_param * kDeg2Rad) {
    // need align body
    tmp_path_seg_vec.clear();
    tmp_path_seg_vec.reserve(5);
    ...
    if (!success) {
      ILOG_INFO << "align body Collided failed!";
      return false;
    }
    ...
    pnc::geometry_lib::PrintPose("align body success! start pose",
                                 current_pose);
    current_pose = tmp_path_seg_vec.back().GetEndPose();
    pnc::geometry_lib::PrintPose("align body success! end pose", current_pose);
    pnc::geometry_lib::PrintSegmentsVecInfo(tmp_path_seg_vec);
  }

  ILOG_INFO << "body already align";

  if (!success && !pnc::mathlib::IsDoubleEqual(
                      current_pose.heading, calc_params_.target_line.heading)) {
    tmp_path_seg_vec.clear();
    if (AlignBodyPlan(tmp_path_seg_vec, current_pose, current_gear)) {
      if (!CheckPathSegCollided(tmp_path_seg_vec.back(),
                                kColSmallBufferInSlot)) {
        ILOG_INFO << "second align body success!";
        AddPathSegToOutPut(tmp_path_seg_vec.back());
        current_pose = tmp_path_seg_vec.back().GetEndPose();
      }
    }
  ...
  pnc::geometry_lib::LineSegment update_target_line =
      pnc::geometry_lib::BuildLineSegByPose(calc_params_.target_line.pA,
                                            current_pose.heading);

  double kSTurnSampleValue = 0.01;
  bool loop_success = false;
  ...
  for (size_t i = 0; i < kMaxParallelShiftNumsToPA; ++i) {
    ILOG_INFO << "-------- No." << i << " in paralle adjust-plan--------";

    bool s_turn_success = false;
    std::vector<pnc::geometry_lib::PathSegment> s_turn_vec;
    double ratio = 1.0;
    for (; ratio > kSTurnSampleValue; ratio -= kSTurnSampleValue) {
      s_turn_vec.clear();
      s_turn_vec.reserve(2);
      if (STurnParallelPlan(s_turn_vec, current_pose, update_target_line,
                            current_gear, ratio,
                            apa_param.GetParam().min_turn_radius)) {
        if (!CheckSTurnPathSegVecCollided(
                s_turn_vec, apa_param.GetParam().s_turn_plan_pa_buffer_1,
                apa_param.GetParam().s_turn_plan_pa_buffer_2)) {
          s_turn_success = true;
          parallel_shift_path_vec.insert(parallel_shift_path_vec.end(),
                                         s_turn_vec.begin(), s_turn_vec.end());
          ILOG_INFO << "s turn not Collided! ratio = " << ratio;
          pnc::geometry_lib::PrintPose(s_turn_vec[0].GetStartPose());
          pnc::geometry_lib::PrintPose(s_turn_vec[1].GetEndPose());
          current_pose = s_turn_vec[1].GetEndPose();
          pnc::geometry_lib::PathSegment out_path_line;
          if (OneLinePlanToPA(out_path_line, current_pose, 0.25, 0.05,
                              current_gear,
                              apa_param.GetParam().line_plan_pa_buffer)) {
            parallel_shift_path_vec.insert(parallel_shift_path_vec.end(),
                                           out_path_line);
          }
          break;
        }
        // ILOG_INFO << "s turn Collided!";
        // pnc::geometry_lib::PrintPose(s_turn_vec[0].GetStartPose());
        // pnc::geometry_lib::PrintPose(s_turn_vec[1].GetEndPose());
      }
    }
    ...
    if (!s_turn_success) {
      pnc::geometry_lib::PathSegment out_path_line;
      if (OneLinePlanToPA(out_path_line, current_pose, 0.3, 0.15, current_gear,
                          apa_param.GetParam().line_plan_pa_buffer)) {
        parallel_shift_path_vec.insert(parallel_shift_path_vec.end(),
                                       out_path_line);
      }
    ...
    if (ratio == 1.0) {
      ...
      break;
    } else if (!s_turn_vec.empty() &&
               IsOnTargetLinePA(s_turn_vec.back().GetEndPose(), 0.05)) {
      loop_success = true;
      ILOG_INFO << "close to target line, try last line!";
      break;
    }
    if (i == (kMaxParallelShiftNumsToPA - 1) &&
        IsOnTargetLinePA(s_turn_vec.back().GetEndPose(), 0.5)) {
      loop_success = true;
      ILOG_INFO << "loop over, clost to line";
      break;
    }
  ...
  if (loop_success && !parallel_shift_path_vec.empty()) {
    AddPathSegVecToOutput(parallel_shift_path_vec);
    loop_end_pose = parallel_shift_path_vec.back().GetEndPose();
  } else if (!tmp_path_seg_vec.empty()) {
    current_pose = tmp_path_seg_vec.back().GetEndPose();
    if (!parallel_shift_path_vec.empty()) {
      AddPathSegVecToOutput(parallel_shift_path_vec);
      current_pose = parallel_shift_path_vec.back().GetEndPose();
    }
    const double res_len =
        input_.tlane.pt_terminal_pos.x() - current_pose.pos.x() + 0.05;
    pnc::geometry_lib::PathSegment out_path_line;
    if (OneLinePlanToPA(out_path_line, current_pose, res_len, 0.15,
                        current_gear,
                        apa_param.GetParam().line_plan_pa_buffer)) {
      AddPathSegToOutPut(out_path_line);
    }
    loop_end_pose = current_pose;
  }
  pnc::geometry_lib::PathPoint target_pose(input_.tlane.pt_terminal_pos,
                                           calc_params_.target_line.heading);
  if (!CheckSamePose(current_pose, target_pose)) {
...
const bool ParallelPathGenerator::SearchArcNode(
  ...
  // ILOG_INFO << "max available angle = " << max_search_angle_rad_mag *
  // kRad2Deg;
...
const bool ParallelPathGenerator::OneArcPlan(
  ...
  bool success = pnc::geometry_lib::CalOneArcWithLineAndGear(arc, target_line,
                                                             current_gear);
  if (!success) {
    ILOG_INFO << "CalOneArcWithLineAndGear fail!";
  }
  if (success) {
...
const bool ParallelPathGenerator::STurnParallelPlan(
  ...
  const auto steer_change_ratio =
      pnc::mathlib::Clamp(steer_change_ratio1, 0.01, 1.0);
  ...
  if (success) {
    ...
    if (success) {
      // ILOG_INFO << "s turn parallel plan success! ratio: "
      //           << steer_change_ratio1;
  ...
  if (!success) {
    ILOG_INFO << "s turn parallel plan fail! ratio: " << steer_change_ratio1;
  }
  return success;
}
...
const bool ParallelPathGenerator::CheckPathSegCollided(
  ...
  if (safe_remain_dist <= 0.0 || remain_car_dist > safe_remain_dist) {
    // ILOG_INFO << " remain_car_dist: " << col_res.remain_car_dist
    //           << " remain_obstacle_dist: " << col_res.remain_obstacle_dist
    //           << " col_pt_obs_global: " << col_res.col_pt_obs_global.transpose()
    //           << " col_pt_ego_global: "
...
const bool ParallelPathGenerator::CheckSTurnPathSegVecCollided(
  ...
  if (CheckPathSegCollided(path_seg_vec[0], first_buffer)) {
    return true;
  }
  if (CheckPathSegCollided(path_seg_vec[1], second_buffer)) {
    return true;
  }
  return false;
}
...
const bool ParallelPathGenerator::IsOnTargetLinePA(
    const pnc::geometry_lib::PathPoint& current_pose, const double lat_buffer) {
  const auto& target_pose = calc_params_.target_pose;
  ...
  const double lat_err_mag =
      std::fabs(current_pose.pos.y() - target_pose.pos.y());
  lat_condition = (lat_err_mag <= lat_buffer);
...
const bool ParallelPathGenerator::CheckGearChangeProjection(
    const std::vector<pnc::geometry_lib::PathPoint>& path_point_vec,
    size_t& third_gear_change_idx) {
  if (path_point_vec.size() < 2) {
    third_gear_change_idx = path_point_vec.size();
    return false;
  }

  uint8_t current_gear = path_point_vec[0].gear;
  std::vector<size_t> gear_change_indices;

  for (size_t i = 1; i < path_point_vec.size(); ++i) {
    if (path_point_vec[i].gear != current_gear) {
      gear_change_indices.push_back(i);
      current_gear = path_point_vec[i].gear;
      if (gear_change_indices.size() >= 3) {
        break;
      }
    }
  }

  if (gear_change_indices.size() < 2) {
    third_gear_change_idx = path_point_vec.size() - 1;
    return false;
  }

  if (gear_change_indices.size() >= 3) {
    third_gear_change_idx = gear_change_indices[2];
  } else {
    third_gear_change_idx = path_point_vec.size() - 1;
  }

  size_t first_idx = gear_change_indices[0];
  size_t second_idx = gear_change_indices[1];

  const auto& pt1 = path_point_vec[first_idx];
  const auto& pt2 = path_point_vec[second_idx];

  pt1.PrintInfo();
  pt2.PrintInfo();

  double distance_diff = (pt2.pos - pt1.pos).norm();

  double heading_diff =
      pnc::geometry_lib::NormalizeAngle(pt2.heading - pt1.heading);

  ILOG_INFO << "distance_diff = " << distance_diff
            << ", heading_diff = " << heading_diff * kRad2Deg;

  return (heading_diff < 3 * kDeg2Rad) && (distance_diff < 1.2);
}

const bool ParallelPathGenerator::OptZigZagPathByDubins(
  ...
  if (complete_path_point_vec.size() < 2) {
    ILOG_INFO << "Input complete_path_point_vec size is less than 2, skip "
                 "optimization.";
    return false;
  }

  size_t third_gear_change_idx = 0;
  if (!CheckGearChangeProjection(complete_path_point_vec,
                                 third_gear_change_idx)) {
    ILOG_INFO
  ...
  std::vector<double> radius_vec = {
      min_turn_radius + 0.5, min_turn_radius + 1.0, min_turn_radius + 1.5,
      min_turn_radius + 2.0, min_turn_radius + 2.5, min_turn_radius + 3.0,
      min_turn_radius + 5.0, min_turn_radius + 7.0, min_turn_radius + 8.0,
      min_turn_radius,       min_turn_radius - 0.5, min_turn_radius - 1.0,
      min_turn_radius - 1.5};

  const pnc::geometry_lib::PathPoint start_pose =
      complete_path_point_vec.front();
  const pnc::geometry_lib::PathPoint target_pose =
      complete_path_point_vec[third_gear_change_idx];

  std::vector<pnc::geometry_lib::PathSegment> path_seg_vec;
  for (const double radius : radius_vec) {
    if (OneStepDubinsPlan(start_pose, target_pose, radius,
                          kLonBufferTrippleStep)) {
      ILOG_INFO << "OptZigZagPathByDubins: Dubins plan succeeded with radius: "
                << radius;
      ...
      std::vector<pnc::geometry_lib::PathPoint> new_path_point_vec =
          pnc::geometry_lib::SamplePathSegVec(path_seg_vec, sample_ds);

      std::vector<pnc::geometry_lib::PathPoint> updated_complete_path;
      updated_complete_path.reserve(
          new_path_point_vec.size() +
          (complete_path_point_vec.size() - third_gear_change_idx - 1));

      updated_complete_path.insert(updated_complete_path.end(),
                                   new_path_point_vec.begin(),
                                   new_path_point_vec.end());

      if (third_gear_change_idx + 1 < complete_path_point_vec.size()) {
        updated_complete_path.insert(
            updated_complete_path.end(),
            complete_path_point_vec.begin() + third_gear_change_idx + 1,
            complete_path_point_vec.end());
      }

      complete_path_point_vec = std::move(updated_complete_path);

      current_path_point_vec.clear();
      current_path_point_vec = std::move(new_path_point_vec);

      return true;
    }
  }
  ILOG_INFO
      << "OptZigZagPathByDubins: All radii failed to generate a valid path.";
  return false;
}
