...
class ApaSlotManager final {
 public:
  ...
  const std::unordered_map<size_t, int>& GetPerpendicularRedefineInfoMap() {
 ...
 private:
  ...
  const bool GetRecommendApaParkingOperationType(
      const iflyauto::ParkingFusionInfo& parking_fusion_info);
  const bool LongitudinalConditions(const double dot_product,
 ...
 private:
  ...
  std::unordered_map<size_t, int> perpendicular_redefine_info_map_;
