...
const bool RuleBasedSlotRelease::IsPerpendicularSlotCoarseRelease(
  ...
  if (IsPerpendicularSlotAndPassageAreaOccupied(slot)) {
    slot->set_is_release(false);
    slot->set_is_occupied(true);

    if (slot_release_voter_.count(slot->id()) != 0) {
      slot_release_voter_[slot->id()] = 0;
    }
    ...
    if (slot_release_voter_.count(slot->id()) == 0) {
      slot_release_voter_[slot->id()] = 1;
    } else {
      if (slot_release_voter_[slot->id()] < kSlotReleaseVoteCount + 5) {
        slot_release_voter_[slot->id()]++;
      }
      if (slot_release_voter_[slot->id()] < kSlotReleaseVoteCount) {
        slot->set_is_release(false);
        slot->set_is_occupied(true);
        return false;
      }
...
const bool RuleBasedSlotRelease::IsPerpendicularSlotAndPassageAreaOccupied(
  ...
  polygon.vertexes[1].x = (pt_0 - 3.0 * n).x();
  polygon.vertexes[1].y = (pt_0 - 3.0 * n).y();

  polygon.vertexes[1].x = (pt_1 - 3.0 * n).x();
  polygon.vertexes[1].y = (pt_1 - 3.0 * n).y();
