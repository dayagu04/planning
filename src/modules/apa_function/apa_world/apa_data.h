...
struct ApaObstacle {
  ...
  ParkObstacleType obs_type = ParkObstacleType::INVALID;
};
...
struct ApaData {
  ...
  // 暂时用这个 无需改变太多之前代码
  std::unordered_map<ParkObstacleType, std::vector<Eigen::Vector2d>>
      apa_obs_map;

  // 后面需要用这个
  // std::unordered_map<ParkObstacleType, std::vector<ApaObstacle>> apa_obs_map;
