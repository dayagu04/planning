...
void EDTCollisionDetector::UpdateCarWithOutMirrorSafeBuffer() {
  car_without_mirror_circles_list_with_buffer_.Reset();
  car_without_mirror_circles_list_with_buffer_ =
      car_without_mirror_circles_list_;
  ...
  for (const geometry_lib::PathPoint &pt : path_pt_vec_) {
    if (!IsPoseInClearZone(pt)) {
      col_flag = need_cal_obs_dist
                     ? IsCollisionForPoint(pt, &car_with_mirror_circles_list_,
                                           &obs_dist, &circle_id)
                     : IsCollisionForPoint(pt, &car_with_mirror_circles_list_);
      if (col_flag) {
        break;
      ...
      col_flag =
          need_cal_obs_dist
              ? IsCollisionForPoint(pt, &car_without_mirror_circles_list_,
                                    &obs_dist, &circle_id)
              : IsCollisionForPoint(pt, &car_without_mirror_circles_list_);
      if (col_flag) {
        break;
      ...
      col_flag = need_cal_obs_dist
                     ? IsCollisionForPoint(pt, &car_chassis_circles_list_,
                                           &obs_dist, &circle_id)
                     : IsCollisionForPoint(pt, &car_chassis_circles_list_);
#endif
    }
