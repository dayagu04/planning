...
void UssObstacleAvoidance::Update(
  ...
  if (apa_param.GetParam().enable_corner_uss_process) {
    ...
    if (dist + param_.lat_inflation < 0.268) {
      ...
      vel_target = std::max(1.168, measure_data_ptr_->GetVel() + 0.28 * 0.1);
    }
  }
...
const bool UssObstacleAvoidance::CheckIsDirectlyFrontUss() {
  if (!remain_dist_info_.is_available) {
    return false;
  }
  for (size_t i = 0; i < apa_param.GetParam().uss_directly_front_index.size();
       ++i) {
    if (remain_dist_info_.uss_index ==
        apa_param.GetParam().uss_directly_front_index[i]) {
      return true;
    }
  }
  return false;
