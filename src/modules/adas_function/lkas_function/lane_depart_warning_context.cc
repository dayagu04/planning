...
void LaneDepartWarning::Update() {
  ...
  measurement_str_.main_switch =
      lkas_input_->vehicle_info.ldw_main_switch;  // LDW功能开关状态 0:Off  1:On
  // 依据驾驶员选择的灵敏度等级,设定触发阈值
  if (lkas_input_->vehicle_info.ldw_tlc_level == 1) {
    measurement_str_.tlc_line_threshold = calribration_str_.tlc_line_near;
  } else if (lkas_input_->vehicle_info.ldw_tlc_level == 2) {
...
void LaneDepartWarning::RunOnce() {
  ...
  JSON_DEBUG_VALUE("lkas_function::ldw::left_intervention",
                   measurement_str_.left_intervention);
  JSON_DEBUG_VALUE("lkas_function::ldw::right_intervention",
                   measurement_str_.right_intervention);
  JSON_DEBUG_VALUE("lkas_function::ldw::main_switch",
                   measurement_str_.main_switch);
  JSON_DEBUG_VALUE("lkas_function::ldw::enable_code",
                   measurement_str_.enable_code);
  JSON_DEBUG_VALUE("lkas_function::ldw::disable_code",
                   measurement_str_.disable_code);
  JSON_DEBUG_VALUE("lkas_function::ldw::fault_code",
                   measurement_str_.fault_code);
  JSON_DEBUG_VALUE("lkas_function::ldw::left_suppression_code",
                   measurement_str_.left_suppression_code);
  JSON_DEBUG_VALUE("lkas_function::ldw::left_kickdown_code",
                   measurement_str_.left_kickdown_code);
  JSON_DEBUG_VALUE("lkas_function::ldw::right_suppression_code",
                   measurement_str_.right_suppression_code);
  JSON_DEBUG_VALUE("lkas_function::ldw::right_kickdown_code",
                   measurement_str_.right_kickdown_code);
  JSON_DEBUG_VALUE("lkas_function::ldw::state", measurement_str_.state);
  JSON_DEBUG_VALUE("ldw_tlc_threshold", measurement_str_.tlc_line_threshold);
...
uint8 LaneDepartWarning::StateMachine() {
  static uint8 ldw_state_machine_init_flag =
      0;  // LDW状态机初始化状态 0:未初始化过 1:已完成过初始化
  static uint8 ldw_state_fault_off_standby_active =
      0;  // LDW一级主状态 FAULT OFF STANDBY ACTIVE
  static uint8 ldw_state_active_substate =
      0;  // LDW ACTIVE状态子状态 NO_ACTIVE_CHILD NoIntervention
