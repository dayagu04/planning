...
uint32 LdpCore::UpdateLdpLeftSuppressionCode(void) {
  ...
  if (left_suppress_repeat_warning_flag_ == false) {
    left_not_suppress_repeat_warning_flag_duration_ +=
  ...
  if (ldp_state_ != iflyauto::LDPFunctionFSMWorkState::
      ...
      ldp_state_ != iflyauto::LDPFunctionFSMWorkState::
                        LDP_FUNCTION_FSM_WORK_STATE_ACTIVE_RIGHT_INTERVENTION) {
    ldp_left_coolingtime_duration_ += GetContext.get_param()->dt;
    if (ldp_left_coolingtime_duration_ > 60.0) {
      ldp_left_coolingtime_duration_ = 60.0;
    } else {
      /*do nothing*/
    }
  } else {
  ...
  // :横摆角速度绝对值<12deg/s，持续1s
  if (yaw_rate_supp_recover_duration_ < 1.0) {
    ldp_left_suppression_code += uint32_bit[13];
  } else {
    /*do nothing*/
  }
  ...
  // PS1:1.8：mps转换为kph   PS2:使用实际车速
  if (str_wheel_ang_speed_recover_duration_ > 1.0) {
    ldp_left_suppression_code += uint32_bit[14];
  } else {
    /*do nothing*/
  }
  ...
  // 驾驶员未踩下制动踏板:制动力<3bar，持续2s.后续再讨论
  if (brake_pedal_pressed_supp_recover_duration_ < 2.0) {
    ldp_left_suppression_code += uint32_bit[15];
  } else {
    /*do nothing*/
  }
  ...
  if (acc_pedal_pos_rate_supp_recover_duration_ <
      GetContext.get_param()->ldp_enable_accel_pedal_pos_rate_dur) {
    ldp_left_suppression_code += uint32_bit[16];
  } else {
    /*do nothing*/
  }
  ...
  if ((vehicle_service_output_info_ptr->abs_active == true) ||
      (vehicle_service_output_info_ptr->tcs_active == true) ||
      (vehicle_service_output_info_ptr->esp_vdc_active == true)) {
      ldp_left_suppression_code += uint32_bit[19];
  } else {
    /*do nothing*/
  }
...
uint32 LdpCore::UpdateLdpLeftKickDownCode(void) {
  ...
  // bit 8
  if (ldp_left_warning_time_ > GetContext.get_param()->kickdown_warning_time &&
    ...
    ldp_left_kickdown_code += uint16_bit[8];
  } else {
    /*do nothing*/
  }
  ...
  if ((GetContext.get_road_info()->current_lane.left_sideway_exist_flag ||
      ...
      (GetContext.get_param()->force_no_sideway_switch == false)) {
    // 左侧有分流口
    ldp_left_kickdown_code += uint16_bit[9];
  }
  // bit 10
  if (elk_state_ == iflyauto::ELKFunctionFSMWorkState::
                        ELK_FUNCTION_FSM_WORK_STATE_ACTIVE_LEFT_INTERVENTION ||
      elk_state_ == iflyauto::ELKFunctionFSMWorkState::
                        ELK_FUNCTION_FSM_WORK_STATE_ACTIVE_RIGHT_INTERVENTION) {
    ldp_left_kickdown_code += uint16_bit[10];
  } else {
    /*do nothing*/
  }
  // bit 11
  ...
  if (yaw_rate_supp_duration_ > 5.0) {
    ldp_left_kickdown_code += uint32_bit[11];
  } else {
    /*do nothing*/
  }
  ...
  if ((fabs(vehicle_service_output_info_ptr->steering_wheel_angle_speed) >=
    ...
    ldp_left_kickdown_code += uint32_bit[12];
  } else {
    /*do nothing*/
  }
  ...
  if (brake_pedal_pressed_supp_duration_ > 0.2) {
    ldp_left_kickdown_code += uint32_bit[13];
  } else {
    /*do nothing*/
  }
  // bit 14
  ...
  if (GetContext.get_state_info()->accelerator_pedal_pos_rate >
      GetContext.get_param()->ldp_disable_accel_pedal_pos_rate) {
    ldp_left_kickdown_code += uint32_bit[14];
  } else {
    /*do nothing*/
  }
  ...
  // AEB未激活
  if (vehicle_service_output_info_ptr->aeb_actuator_status == 2) {
    ldp_left_kickdown_code += uint32_bit[15];
  } else { /*do nothing*/
  }
...
uint32 LdpCore::UpdateLdpRightSuppressionCode(void) {
  ...
  if (right_suppress_repeat_warning_flag_ == false) {
    right_not_suppress_repeat_warning_flag_duration_ +=
        GetContext.get_param()->dt;
  ...
  if (ldp_state_ != iflyauto::LDPFunctionFSMWorkState::
      ...
      ldp_state_ != iflyauto::LDPFunctionFSMWorkState::
                        LDP_FUNCTION_FSM_WORK_STATE_ACTIVE_RIGHT_INTERVENTION) {
    ldp_right_coolingtime_duration_ += GetContext.get_param()->dt;
    if (ldp_right_coolingtime_duration_ > 60.0) {
      ldp_right_coolingtime_duration_ = 60.0;
    } else {
      /*do nothing*/
    }
  } else {
  ...
  if (elk_state_ == iflyauto::ELKFunctionFSMWorkState::
      ...
      elk_state_ == iflyauto::ELKFunctionFSMWorkState::
                        ELK_FUNCTION_FSM_WORK_STATE_ACTIVE_RIGHT_INTERVENTION) {
    ldp_right_suppression_code += uint16_bit[12];
  } else {
    /*do nothing*/
  }
  // bit 14
  ...
  // :横摆角速度绝对值<12deg/s，持续1s
  if (yaw_rate_supp_recover_duration_ < 1.0) {
    ldp_right_suppression_code += uint32_bit[13];
  } else {
    /*do nothing*/
  }
  ...
  // PS1:1.8：mps转换为kph   PS2:使用实际车速
  if (str_wheel_ang_speed_recover_duration_ > 1.0) {
    ldp_right_suppression_code += uint32_bit[14];
  } else {
    /*do nothing*/
  }
  ...
  // 驾驶员未踩下制动踏板:制动力<3bar，持续2s.后续再讨论
  if (brake_pedal_pressed_supp_recover_duration_ < 2.0) {
    ldp_right_suppression_code += uint32_bit[15];
  } else {
    /*do nothing*/
  }
  ...
  if (acc_pedal_pos_rate_supp_recover_duration_ <
      GetContext.get_param()->ldp_enable_accel_pedal_pos_rate_dur) {
    ldp_right_suppression_code += uint32_bit[16];
  } else {
    /*do nothing*/
  }
  ...
  if ((vehicle_service_output_info_ptr->abs_active == true) ||
    (vehicle_service_output_info_ptr->tcs_active == true) ||
    (vehicle_service_output_info_ptr->esp_vdc_active == true)) {
    ldp_right_suppression_code += uint32_bit[19];
  } else {
    /*do nothing*/
  }
...
uint32 LdpCore::UpdateLdpRightKickDownCode(void) {
  ...
  if ((GetContext.get_road_info()->current_lane.left_sideway_exist_flag ||
      ...
      (GetContext.get_param()->force_no_sideway_switch == false)) {
    // 左侧有分流口
    ldp_right_kickdown_code += uint16_bit[9];
  }
  // bit 10
  if (elk_state_ == iflyauto::ELKFunctionFSMWorkState::
                        ELK_FUNCTION_FSM_WORK_STATE_ACTIVE_LEFT_INTERVENTION ||
      elk_state_ == iflyauto::ELKFunctionFSMWorkState::
                        ELK_FUNCTION_FSM_WORK_STATE_ACTIVE_RIGHT_INTERVENTION) {
    ldp_right_kickdown_code += uint16_bit[10];
  } else {
    /*do nothing*/
  }
  // bit 11
  ...
  if (yaw_rate_supp_duration_ > 5.0) {
    ldp_right_kickdown_code += uint32_bit[11];
  } else {
    /*do nothing*/
  }
  ...
  if ((fabs(vehicle_service_output_info_ptr->steering_wheel_angle_speed) >=
    ...
    ldp_right_kickdown_code += uint32_bit[12];
  } else {
    /*do nothing*/
  }
  ...
  if (brake_pedal_pressed_supp_duration_ > 0.2) {
    ldp_right_kickdown_code += uint32_bit[13];
  } else {
    /*do nothing*/
  }
  // bit 14
  ...
  if (GetContext.get_state_info()->accelerator_pedal_pos_rate >
      GetContext.get_param()->ldp_disable_accel_pedal_pos_rate) {
    ldp_right_kickdown_code += uint32_bit[14];
  } else {
    /*do nothing*/
  }
  ...
  // AEB未激活
  if (vehicle_service_output_info_ptr->aeb_actuator_status == 2) {
    ldp_right_kickdown_code += uint32_bit[15];
  } else { /*do nothing*/
  }
