...
DiscretizedPath::DiscretizedPath(const std::vector<PathPoint>& path_points) : std::vector<PathPoint>(path_points) {}
...
PathPoint DiscretizedPath::Evaluate(const double path_s) const {
  ...
  return planning_math::InterpolateUsingLinearApproximation(*(it_lower - 1), *it_lower, path_s);
}
...
PathPoint DiscretizedPath::EvaluateReverse(const double path_s) const {
  ...
  return planning_math::InterpolateUsingLinearApproximation(*(it_upper - 1), *it_upper, path_s);
}
