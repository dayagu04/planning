#include "src/modules/common/path/path_data.h"

namespace planning {

PolygonalLine::PolygonalLine(std::vector<SLPoint> sl_points)
    : std::vector<SLPoint>(std::move(sl_points)) {
  std::sort(begin(), end(), [](const SLPoint& p1, const SLPoint& p2) {
    return p1.s < p2.s;
  });
}
...
const PolygonalLine& PathData::LeftBorder() const {
  return left_border_;
}

const PolygonalLine& PathData::RightBorder() const {
