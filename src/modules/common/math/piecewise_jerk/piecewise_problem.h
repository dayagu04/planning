...
typedef int (*Osqp_run_para)(c_float *, c_int, c_int *, c_int *, c_float *, c_float *, c_int, c_int *, c_int *,
                             c_float *, c_float *, c_int, c_int, c_float *);
...
class PiecewiseProblem {
 public:
  PiecewiseProblem(const size_t num_of_knots, const std::vector<double> &s, const std::array<double, 3> &x_init,
                   bool hard_constraint = true);
  virtual ~PiecewiseProblem() = default;

  // bounds
  void set_x_bounds(const std::vector<WeightedBounds> &x_bounds);
  void set_dx_bounds(const double dx_lower_bound, const double dx_upper_bound, const double weight = -1);
  ...
  void set_x_infer_bounds(const std::vector<WeightedLonLeadBounds> &x_infer_bounds);
  ...
  void set_weight_slack(const double weight_slack) { weight_slack_ = weight_slack; }

  void set_x_ref(const std::vector<double> &xref_basic, const std::vector<double> &x_reference,
                 const std::vector<double> &obs_weight);

  void set_dx_ref(const std::vector<std::pair<double, double>> &dx_ref, const double weight_dxref_basic = 1.0);
   ...
   */
  void set_x_ref(const std::vector<std::pair<double, double>> &x_ref, const double weight_xref_basic = 1.0);
 ...
 private:
  ...
  void calculate_kernel(std::vector<c_float> *p_data, std::vector<c_int> *p_indices, std::vector<c_int> *p_indptr);

  virtual void calculate_affine_constraint(std::vector<c_float> *a_data, std::vector<c_int> *a_indices,
