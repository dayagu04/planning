...
class CubicPolynomialCurve1d : public PolynomialCurve1d {
 public:
  ...
  // CubicPolynomialCurve1d(const std::array<double, 3>& start, const double
  // end,
  //                        const double param);

  CubicPolynomialCurve1d(const std::array<double, 2>& start,
                         const std::array<double, 2>& end, const double param);

  CubicPolynomialCurve1d(const double x0, const double dx0, const double x1,
   ...
   * f(x = param) = x1
   */
  // CubicPolynomialCurve1d(const double x0, const double dx0, const double
  // ddx0,
  ...
  std::array<double, 4> CoefArray() { return coef_; };

  size_t Order() const override { return 3; }

 private:
  void ComputeCoefficients(const double x0, const double dx0, const double x1,
                           const double dx1, const double param);

  std::array<double, 4> coef_ = {{0.0, 0.0, 0.0, 0.0}};
  std::array<double, 2> start_condition_ = {{0.0, 0.0}};
