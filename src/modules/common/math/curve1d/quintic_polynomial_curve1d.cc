...
double QuinticPolynomialCurve1d::EvaluateKappa(const double p) const {
  const double dx = Evaluate(1, p);
  const double ddx = Evaluate(2, p);

  return ddx / std::pow((1.0 + dx * dx), 1.5);
}
