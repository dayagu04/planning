...
bool DiscretePointsMath::ComputePathProfile(
  ...
  const std::size_t points_size = xy_points.size();
  if (points_size < 2) {
    return false;
  }
  std::vector<double> dxs;
  dxs.reserve(points_size);
  std::vector<double> dys;
  dys.reserve(points_size);
  ...
  y_over_s_second_derivatives.reserve(points_size);
  std::vector<double> x_over_s_second_derivatives;
  x_over_s_second_derivatives.reserve(points_size);
