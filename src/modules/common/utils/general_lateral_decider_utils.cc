...
namespace planning {
  bool ConstructLinePolygons(const std::vector<planning_math::Vec2d> &line,
    ...
    for (size_t i = 0; i < line.size() - 1; i++) {
      ...
      std::vector<planning_math::Vec2d> points{planning_math::Vec2d{x_start, y_start}, planning_math::Vec2d{x_end, y_end},
                                               planning_math::Vec2d{x_end, y_edge}, planning_math::Vec2d{x_start, y_edge}};
  ...
  bool Vec2dsToFrenet2ds(const std::shared_ptr<FrenetCoordinateSystem> &frenet_coord,
    ...
    for (size_t i = 0; i < pts.size(); i++) {
      ...
      frenet_pts.emplace_back(planning_math::Vec2d{frenet_point.x, frenet_point.y});
    }
  ...
  void MakeLinePolygons(const std::shared_ptr<FrenetCoordinateSystem> &frenet_coord,
