...
FrenetCoordinateSystem(double length, const spline& x_s,
    const spline& y_s, const FrenetCoordinateSystemParameters& fcs_params):
...
FrenetCoordinateSystem::
FrenetCoordinateSystem(double length, const spline& x_s,
    const spline& y_s, const spline& k_s,
...
FrenetCoordinateSystem::
FrenetCoordinateSystem(const vector<Point2D>& vec_pts,
    const FrenetCoordinateSystemParameters& fcs_params):
...
FrenetCoordinateSystem::
FrenetCoordinateSystem(const vector<double>& vec_x, const vector<double>& vec_y,
    const FrenetCoordinateSystemParameters& fcs_params):
    m_fcs_params(fcs_params)
{
    ConstructFrenetCoordinateSystem(vec_x, vec_y);
...
void FrenetCoordinateSystem::
ConstructFrenetCoordinateSystem(const vector<double>& vec_x,
                                const vector<double>& vec_y, double s_polyfit_start, double s_polyfit_end)
{
    vector<double> vec_s;
...
double FrenetCoordinateSystem::
SquaredDistanceToOffSetOnCurve(double s, const Point2D& cart0) const {
...
double FrenetCoordinateSystem::
GradientSquaredDistanceToOffSetOnCurve(double s, const Point2D& cart0) const {
...
double FrenetCoordinateSystem::
HessianSquaredDistanceToPointOnCurve(double s, const Point2D& cart0) const {
    return 2.0 * m_x_s.deriv(2, s) * (m_x_s(s) - cart0.x)
        + 2.0 * m_y_s.deriv(2, s) * (m_y_s(s) - cart0.y)
        + 2.0 * pow(m_x_s.deriv(1, s), 2)
        + 2.0 * pow(m_y_s.deriv(1, s), 2);

}

//Find the offset that has the minimal distance to the given point
double FrenetCoordinateSystem::
FindMinDistanceOffsetOnCurve(const Point2D& cart0, double yaw, bool has_yaw,
    ...
    int count = 0;
    while (!(std::abs(s_new - s_old) <= m_fcs_params.coord_transform_precision || s_new - begin_s <= 0 || s_new - end_s >= 0 || count > m_fcs_params.max_iter)) {
		s_old = s_new;
...
TRANSFORM_STATUS FrenetCoordinateSystem::
CartCoord2FrenetCoord(const Point2D& cart, Point2D& frenet, bool has_heuristics, double s_begin, double s_end) const {
    double min_point_s;
    if (has_heuristics) {
    ...
    double cos_yaw_diff = std::abs<double>(
...
TRANSFORM_STATUS FrenetCoordinateSystem::
CartCoord2FrenetCoord(const Point2D& cart, Point2D& frenet, double yaw) const {
...
TRANSFORM_STATUS FrenetCoordinateSystem::
FrenetCoord2CartCoord(const Point2D& frenet, Point2D& cart) const {
    ...
    double angle = atan2(m_y_s.deriv(1, frenet.x),
        m_x_s.deriv(1, frenet.x));
...
TRANSFORM_STATUS FrenetCoordinateSystem::
CartState2FrenetState(const CartesianState& cart_state, FrenetState& frenet_state) const {
	Point2D cart_coord;
	...
	frenet_state.ds = cart_state.speed * cos(yaw_diff)
        / (1 - ref_curvature * frenet_state.r);
	frenet_state.dds = (cart_state.acceleration
		- pow(frenet_state.ds, 2)*(term_b*term_c*tan(yaw_diff) - term_a)
        / cos(yaw_diff)) / (term_c / cos(yaw_diff));

	//Get ddr
	frenet_state.ddr = frenet_state.dds * dr_ds
        + pow(frenet_state.ds, 2)*ddr_dsds;
    return TRANSFORM_SUCCESS;
}

//Coupled frenet state to cartesian state
TRANSFORM_STATUS FrenetCoordinateSystem::
FrenetState2CartState(const FrenetState& frenet_state, CartesianState& cart_state) const {
	Point2D frenet_coord;
	frenet_coord.x = frenet_state.s;
	frenet_coord.y = frenet_state.r;
	Point2D cart_coord;
    TRANSFORM_STATUS stat
     = FrenetCoord2CartCoord(frenet_coord, cart_coord);
    if (stat == TRANSFORM_FAILED)
        return TRANSFORM_FAILED;
	//Cartesian position
	...
	cart_state.speed = sqrt(pow((1 - ref_curvature*frenet_state.r)*frenet_state.ds, 2)
	...
	cart_state.curvature = ((frenet_state.ddr_dsds + term_a * tan(yaw_diff))
        * (cos(yaw_diff)* cos(yaw_diff)) / term_b + ref_curvature)
		* cos(yaw_diff) / term_b;
	cart_state.acceleration = frenet_state.dds * term_b / cos(yaw_diff) +
		(pow(frenet_state.ds, 2) / cos(yaw_diff))*
		(term_b * tan(yaw_diff)*(cart_state.curvature * term_b / cos(yaw_diff) - ref_curvature)
			- term_a);
    return TRANSFORM_SUCCESS;
}
//-----------------------------------------------------------------//
...
void FrenetCoordinateSystem::VerifyCurveFit()
{
    ...
    // std::cout << "****** curvature error max: " << cur_error_max << "percent:" << cur_error_max * 100.0 / cur_base << "******" << std::endl;
