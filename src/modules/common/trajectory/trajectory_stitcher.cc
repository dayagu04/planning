...
//   prev_planning_result->v_target) >
//       FLAGS_replan_velocity_threshold) {
//     MSD_LOG(INFO, "trajectory stitcher: vehicle-vel control-error is bigger
//     than: %f ",
//         FLAGS_replan_velocity_threshold);
...
//     MSD_LOG(INFO, "trajectory stitcher: Projected trajectory at time [%f]
...
//       veh_rel_time + 1.e-2 < prev_trajectory.StartPoint().relative_time -
//       planning_cycle_time) {
//     MSD_LOG(INFO, "trajectory stitcher: current time smaller than the
//     previous trajectory's first time"); *replan_reason =
...
//     MSD_LOG(INFO, "trajectory stitcher: current time beyond the previous
//     trajectory's last time"); *replan_reason =
...
//   prev_trajectory.QueryNearestPointWithBuffer(frenet_sd.first, 1.0e-6); auto
//   position_project_matched_point = prev_trajectory.TrajectoryPointAt(
...
//   prev_trajectory.at(forward_time_index); auto forward_time_trajectory_point
...
//     MSD_LOG(INFO, "trajectory stitcher: Control lateral diff: %f,
//     longitudinal diff: %f longitudinal time diff: %f",
...
//       *replan_reason = msg;
//       auto reinit_traj =
//       ComputeReinitStitchingTrajectory(planning_cycle_time,
//                                               vehicle_state);
...
//           std::to_string(lon_diff) + " lon_time_diff = " +
//           std::to_string(lon_time_diff));
//       MSD_LOG(INFO, "trajectory stitcher: %s", msg.c_str());
//       *replan_reason = msg;
//       auto reinit_traj =
//       ComputeReinitStitchingTrajectory(planning_cycle_time,
...
//     }
//   } else {
//     MSD_LOG(INFO, "trajectory stitcher: replan according to certain amount of
//     lat and lon offset is "
//               "disabled");
//   }

//   MSD_LOG(INFO, "trajectory stitcher: Position matched index: %d",
//   position_matched_index); MSD_LOG(INFO, "trajectory stitcher: Time matched
//   index: %d", time_matched_index);
...
//   // MSD_LOG(INFO, "trajectory stitcher: stitching_trajectory size: %d",
