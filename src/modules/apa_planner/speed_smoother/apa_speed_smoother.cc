...
bool ApaSpeedSmoother::Smooth(const std::vector<LineSegment2d>& obstacles,
  ...
  speed_limit_decider_.GetSpeedLimits(smoothed_planning_output, obstacles,
                                      &speed_limit_data);

  // construct piecewise jerk problem
  const std::array<double, 3> init_state = {kMinSpd, 0.0, 0.0};
  planning_math::PiecewiseProblem piecewise_jerk_problem(
      resample_point_num, s_vev, init_state, true);
  ...
  // calculate bounds
  std::vector<WeightedBounds> v_bounds(
      resample_point_num, std::vector<WeightedBound>(1, WeightedBound()));
  ...
  for (int i = 0; i < resample_point_num; ++i) {
    ...
    AINFO << "smoothed traj pt [" << i << "], x:" << pt.x() << ", y:" << pt.y()
          << ", theta:" << pt.heading_yaw() << ", kappa:" << pt.curvature()
          << ", v:" << pt.v() << ", v limit:" << v_limit
          << ", s:" << pt.distance();
  }
