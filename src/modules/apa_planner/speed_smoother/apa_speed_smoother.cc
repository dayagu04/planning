...
ApaSpeedSmoother::ApaSpeedSmoother() {
  const std::string config_file = "/asw/planning/res/conf/apa_speed_smoother_config.pb.txt";
  common::util::GetProtoFromFile(config_file, &config_);
}

bool ApaSpeedSmoother::Smooth(const std::vector<LineSegment2d>& obstacles, PlanningOutput* const planning_output) {
  const int traj_point_num = planning_output->trajectory().trajectory_points_size();
  if (traj_point_num < 2) {
    return false;
  }

  // resample path by same delta_s
  const double path_length = planning_output->trajectory().trajectory_points().rbegin()->distance();
  ...
  double path_s = 0.0;
  const auto& trajectory_points = planning_output->trajectory().trajectory_points();
  PlanningOutput smoothed_planning_output = *planning_output;
  auto smooothed_trajectory = smoothed_planning_output.mutable_trajectory();
  smooothed_trajectory->mutable_trajectory_points()->Clear();
  while (s_vev.size() < resample_point_num) {
    while (traj_point_index < traj_point_num && trajectory_points[traj_point_index].distance() < path_s) {
      ++traj_point_index;
    }
    traj_point_index = std::min(traj_point_index, traj_point_num - 1);

    const int pre_traj_point_index = std::max(traj_point_index - 1, 0);
    ::PlanningOutput::TrajectoryPoint traj_point = planning_math::InterpolateUsingLinearApproximation(
        trajectory_points[pre_traj_point_index], trajectory_points[traj_point_index], path_s);
    smooothed_trajectory->add_trajectory_points()->CopyFrom(traj_point);

    s_vev.emplace_back(path_s);
    path_s += delta_s_;
  }
  AINFO << "traj_point_num:" << traj_point_num << ", resample_point_num:" << resample_point_num;

  // get speed limit data
  ApaSpeedLimit speed_limit_data;
  speed_limit_decider_.GetSpeedLimits(smoothed_planning_output, obstacles, &speed_limit_data);

  // construct piecewise jerk problem
  const std::array<double, 3> init_state = {kMinSpd, 0.0, 0.0};
  planning_math::PiecewiseProblem piecewise_jerk_problem(resample_point_num, s_vev, init_state, true);
  ...
  AINFO << "weight_v:" << config_.weight_v() << ", weight_dv:" << config_.weight_dv()
        << ", set_weight_ddx:" << config_.weight_ddv() << ", weight_dddv:" << config_.weight_dddv()
        << ", weight_slack:" << config_.weight_slack() << ", weight_ref_v:" << config_.weight_ref_v();

  // calculate bounds
  std::vector<WeightedBounds> v_bounds(resample_point_num, std::vector<WeightedBound>(1, WeightedBound()));
  ...
  // solve problem
  int status = 0;
  bool success = piecewise_jerk_problem.optimize(config_.max_iter(), status);
  if (!success) {
  ...
  for (int i = 0; i < resample_point_num; ++i) {
    ...
    const double v_limit = speed_limit_data.speed_limit_points()[i].second * motion_sign;
    smooothed_trajectory->mutable_trajectory_points(i)->set_v(v);
    const auto& pt = smooothed_trajectory->trajectory_points()[i];
    AINFO << "smoothed traj pt [" << i << "], x:" << pt.x() << ", y:" << pt.y() << ", theta:" << pt.heading_yaw()
          << ", kappa:" << pt.curvature() << ", v:" << pt.v() << ", v limit:" << v_limit << ", s:" << pt.distance();
  }
