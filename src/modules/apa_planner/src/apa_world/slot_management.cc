...
void SlotManagement::UpdateOccupiedInfo() {
  ...
  const auto &upa_dis_info_buf = frame_.uss_wave_info_ptr_->upa_dis_info_buf();

  frame_.uss_raw_dist_vec_.clear();
  frame_.uss_raw_dist_vec_.reserve(uss_vertex_x_vec.size());
  ...
  for (size_t i = 0; i < wdis_index_front.size(); ++i) {
    frame_.uss_raw_dist_vec_.emplace_back(
  ...
  const double radius0 = frame_.uss_raw_dist_vec_[0];
  const double radius5 = frame_.uss_raw_dist_vec_[5];
  const double radius6 = frame_.uss_raw_dist_vec_[6];
  const double radius11 = frame_.uss_raw_dist_vec_[11];
  ...
  const auto car_global = pnc::geometry_lib::LocalToGlobalTf(
      frame_.measurement_.ego_pos, frame_.measurement_.heading);
  ...
  for (auto &slot_info :
    ...
    if (frame_.occupied_info_map_.count(slot_info.id()) > 0 &&
    ...
    const auto slot_invade_coner2 =
        slot_coner0 +
        frame_.param_.invade_area_ratio * (slot_coner2 - slot_coner0);

    const auto slot_invade_coner3 =
        slot_coner1 +
        frame_.param_.invade_area_ratio * (slot_coner3 - slot_coner1);
    ...
    const bool is_invade =
        (pnc::geometry_lib::IsPointInPolygon(polygon, uss0_global) &&
         radius0 < 3.5) ||
        (pnc::geometry_lib::IsPointInPolygon(polygon, uss5_global) &&
         radius5 < 3.5) ||
        (pnc::geometry_lib::IsPointInPolygon(polygon, uss6_global) &&
         radius6 < 3.5) ||
        (pnc::geometry_lib::IsPointInPolygon(polygon, uss11_global) &&
         radius11 < 3.5);

    if (frame_.occupied_info_map_.count(slot_info.id()) == 0) {
      frame_.occupied_info_map_.insert(
          std::make_pair(slot_info.id(), OccupiedInfo(0, false)));
    }

    if (is_invade) {
      // std::cout << slot_info.id() << " is_invade\n";
      frame_.occupied_info_map_[slot_info.id()].count++;
      if (frame_.occupied_info_map_[slot_info.id()].count >=
          kMaxOccupiedCount) {
        // modify  SlotInfo : is_occupied
        frame_.occupied_info_map_[slot_info.id()].is_occupied = true;
      }
    }
  }
