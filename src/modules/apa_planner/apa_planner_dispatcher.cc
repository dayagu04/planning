...
bool ApaPlannerDispatcher::Update(Frame* const frame) {
  ...
  const auto& planning_output =
      frame->session()->planning_output_context().planning_status().planning_result.planning_output;
  if (planning_output.has_planning_status() && planning_output.planning_status().has_apa_planning_status()) {
    const auto& apa_planning_status = planning_output.planning_status().apa_planning_status();
