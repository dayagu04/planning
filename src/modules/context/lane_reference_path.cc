...
void LaneReferencePath::update(planning::framework::Session *session) {
  ...
  auto virtual_lane =
      session->mutable_environmental_model()->mutable_virtual_lane_manager()->mutable_lane_with_virtual_id(
          lane_virtual_id_);
...
bool LaneReferencePath::is_obstacle_ignorable(const std::shared_ptr<FrenetObstacle> obstacle) {
  ...
  if (get_reference_point_by_lon(obstacle->frenet_s(), obstacle_matched_lane_point)) {
    if (obstacle->frenet_obstacle_boundary().l_start > -obstacle_matched_lane_point.distance_to_right_lane_border &&
        obstacle->frenet_obstacle_boundary().l_end < obstacle_matched_lane_point.distance_to_left_lane_border) {
...
void LaneReferencePath::update_obstacles() {
  ...
  free_space_ground_lines_ = obstacle_manager->get_groundline_obstacles().Items();
  road_edges_ = obstacle_manager->get_road_edge_obstacles().Items();
}

bool LaneReferencePath::get_points_by_lane_id(int target_lane_virtual_id, ReferencePathPoints &ref_path_points) {
  auto virtual_lane_manager = session_->mutable_environmental_model()->get_virtual_lane_manager();
  auto virtual_lane = virtual_lane_manager->get_lane_with_virtual_id(target_lane_virtual_id);
  ...
  for (auto &refline_pt : lane_points) {
    ...
    ref_path_pt.distance_to_left_lane_border =
        std::fmin(refline_pt.distance_to_left_lane_border(), kDefaultLaneBorderDis);
    ref_path_pt.distance_to_right_lane_border =
        std::fmin(refline_pt.distance_to_right_lane_border(), kDefaultLaneBorderDis);
    ref_path_pt.distance_to_left_road_border =
        std::fmin(refline_pt.distance_to_left_road_border(), kDefaultLaneBorderDis);
    ref_path_pt.distance_to_right_road_border =
        std::fmin(refline_pt.distance_to_right_road_border(), kDefaultLaneBorderDis);
    ...
    if (not ref_path_points.empty()) {
      ...
      Vec2d delta{ref_path_pt.path_point.x - pre_pt.path_point.x, ref_path_pt.path_point.y - pre_pt.path_point.y};
...
void LaneReferencePath::assign_obstacles_to_lane() {
  ...
  std::sort(sorted_obstacles.begin(), sorted_obstacles.end(), compare_obstacle_s_ascend);
