...
StopLine::StopLine(int id,
                   const planning::planning_math::LineSegment2d& stop_line)
    : id_(id), stop_line_(stop_line) {
  // id_ = id;
  // stop_line_ = planning::planning_math::LineSegment2d(stop_line.start(),
  // stop_line.end());
}
...
int StopLine::track_id() const { return id_; }
