...
ConstructionSceneManager::ConstructionSceneManager(
...
bool ConstructionSceneManager::update() {
  ILOG_INFO << "=======ConstructionSceneManager=======";
  ...
  JSON_DEBUG_VALUE("ConstructionSceneManagerCostTime", end_time - start_time);
  return true;
}
...
void ConstructionSceneManager::UpdateConstructionAgentClusters() {
  // 施工障碍物聚类
  // 自车状态
...
void ConstructionSceneManager::DbScan(ConstructionAgentPoints& cone_points,
...
bool ConstructionSceneManager::ConstructionAgentDistance(
    const ConstructionAgentPoint& a, const ConstructionAgentPoint& b,
...
double ConstructionSceneManager::CalcClusterToBoundaryDist(
...
bool ConstructionSceneManager::IsConstructionAgent(iflyauto::ObjectType type) {
...
void ConstructionSceneManager::GetOriginLaneWidthByConstructionAgent(
...
double ConstructionSceneManager::QueryLaneWidth(
    const double s0,
...
void ConstructionSceneManager::IdentifyConstructionScene() {
  // 判定是否存在施工区域
...
void ConstructionSceneManager::IsExistConstructionArea() {
...
void ConstructionSceneManager::JudgeConstructionIntrusionLevel() {
  ...
  construction_scene_output_.is_left_left_lane_available =
      is_left_left_lane_available;
  construction_scene_output_.is_left_lane_available =
      is_left_lane_available;
  construction_scene_output_.is_right_right_lane_available =
      is_right_right_lane_available;
  construction_scene_output_.is_right_lane_available =
...
void ConstructionSceneManager::SaveLatDebugInfo() {
  ...
  JSON_DEBUG_VALUE("construction_intrusion_level",
                    static_cast<uint32_t>(construction_scene_output_
                    .construction_intrusion_level));
  // 转成 double 类型
  std::vector<double> available_virtual_lane_ids_double;
  available_virtual_lane_ids_double.reserve(construction_scene_output_
      .available_virtual_lane_ids.size());
  std::transform(construction_scene_output_.available_virtual_lane_ids.begin(),
                 construction_scene_output_.available_virtual_lane_ids.end(),
