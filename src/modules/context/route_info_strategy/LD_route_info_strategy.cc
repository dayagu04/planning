...
bool LDRouteInfoStrategy::UpdateFeasibleLaneGraph() {
  ...
  if (mlc_scene_type == SPLIT_SCENE &&
      mlc_decider_scene_type_info_.dis_to_link_topo_change_point < 300) {
  ...
  // 如果在split场景中计算了merge约束，优先使用该结果
  if (!feasible_lane_graph_for_merge_after_split_.lane_topo_groups.empty()) {
    feasible_lane_graph_.lane_topo_groups.insert(
        feasible_lane_graph_.lane_topo_groups.begin(),
        feasible_lane_graph_for_merge_after_split_.lane_topo_groups.begin(),
        feasible_lane_graph_for_merge_after_split_.lane_topo_groups.end());
  } else if (!feasible_lane_graph_after_topo_change_vec.lane_topo_groups.empty()) {
    feasible_lane_graph_.lane_topo_groups.insert(
        feasible_lane_graph_.lane_topo_groups.begin(),
  ...
  while (current_link != nullptr) {
    ...
    for (const auto& lane : current_lane_vec) {
      ...
      if (pre_lane_count == 0) {
        ...
        MergeLaneType merge_link_direction = NONE_MERGE;
        while (link != nullptr) {
          link_id = link->id();
          // 是否来自合流道路
          if (link->predecessor_link_ids_size() > 1) {
            pre_topo_change_is_merge = true;
            if (link->predecessor_link_ids_size() == 2) {
              const auto* pred1 =
                  ld_map_.GetLinkOnRoute(link->predecessor_link_ids()[0]);
              const auto* pred2 =
                  ld_map_.GetLinkOnRoute(link->predecessor_link_ids()[1]);
              if (pred1 != nullptr && pred2 != nullptr) {
                const auto* closer_pred =
                    GetCloserPredecessorLinkByLateralDistance(link, pred1,
                                                              pred2);
                const auto* other_pred =
                    (closer_pred->id() == pred1->id()) ? pred2 : pred1;
                merge_link_direction = GetPredecessorLink2LateralSide(
                    link, closer_pred, other_pred);
              }
            }
            break;
          }
        ...
        for (const auto& pre_lane_id : lane.predecessor_lane_ids()) {
          ...
          if (IsMergeLane(pre_lane) && pre_topo_change_is_merge) {
            const auto merge_lane_direction = CalculateMergeLaneType(pre_lane);
            if (merge_link_direction == merge_lane_direction &&
                IsInvalidLaneMergeLaneOppositeSide(pre_lane)) {
              continue;
              // merge lane的反方向是无效车道，既这条merge lane不应该在feasible
              // lane中，continue掉，不加入到feasible lane中
            }
          }
...
bool LDRouteInfoStrategy::CalculateExtenedFeasibleLane(
  ...
  for (int i = 0; i < link_size; i++) {
    ...
    for (const auto& lane_id : temp_link->lane_ids()) {
      ...
      // 设置前继车道
      topo_lane.predecessor_lane_ids.clear();
      for (int j = 0; j < temp_lane->predecessor_lane_ids_size(); ++j) {
        topo_lane.predecessor_lane_ids.emplace(temp_lane->predecessor_lane_ids()[j]);
      }

      // 设置后继车道
      topo_lane.successor_lane_ids.clear();
      for (int j = 0; j < temp_lane->successor_lane_ids_size(); ++j) {
        topo_lane.successor_lane_ids.emplace(temp_lane->successor_lane_ids()[j]);
      }
  ...
  for (const auto& split_info : split_info_vec_) {
    ...
    for (const auto& suc_link_id: split_link->successor_link_ids()) {
      ...
      if (!CalculateSplitLinkExitLane(split_link, out_link, exit_lane_vec)) {
        // 没有找到出口车道，则不满足被过滤的条件。应该先置fasle，再break
...
bool LDRouteInfoStrategy::CalculateFeasibleLaneInRampScene(
    ...
    for (const auto& merge_info : merge_info_vec_) {
      ...
      if (merge_info.second > ramp_info_vec_[0].second &&
        ...
        has_merge_after_split =
            CalculateFeasibleLaneForMergeAfterSplit(
                feasible_lane_graph_for_merge_after_split_);
        break;
      }
    }
  }
  ...
  // 根据是否有merge约束选择使用哪个feasible_lane_graph
  const TopoLinkGraph* source_graph = nullptr;
  if (has_merge_after_split &&
      !feasible_lane_graph_for_merge_after_split_.lane_topo_groups.empty()) {
    source_graph = &feasible_lane_graph_for_merge_after_split_;
  } else {
    source_graph = &feasible_lane_graph_after_topo_change_vec;
  }

  if (source_graph->lane_topo_groups.empty()) {
    return false;
  }

  const auto& split_next_link_lane_info = source_graph->lane_topo_groups.back();
...
bool LDRouteInfoStrategy::CalculateFeasibleLaneForMergeAfterSplit(
    TopoLinkGraph& feasible_lane_graph) {
  ...
  // 3、从merge link向前搜索确定target link
  iflymapdata::sdpro::LinkInfo_Link first_merge_link = *first_merge_link_info;
  iflymapdata::sdpro::LinkInfo_Link* target_link = &first_merge_link;
  std::vector<iflymapdata::sdpro::Lane> start_lane_vec;
  if (!CalculateFrontTargetLinkBaseFixDis(
          target_link, start_lane_vec, first_merge_link_info,
          route_info_output_.mlc_decider_scene_type_info.mlc_scene_type)) {
    return false;
  }

  // 4、计算merge之后的feasible lane
  TopoLinkGraph after_merge_feasible_lane_graph;
  ...
  // 5、计算merge_pre_link上的feasible lane
  std::vector<iflymapdata::sdpro::Lane> merge_pre_link_feasible_lane;
  if (!CalculateMergePreFeasibleLane(merge_pre_link_feasible_lane,
  ...
  // 6、反向遍历至split_next_link，计算feasible lane
  if (!CalculateFeasibleLaneGraph(feasible_lane_graph,
...
void LDRouteInfoStrategy::Erase1Split2FeasibleLane(
  ...
  // 收集所有不在导航路线上的后继 link ID (out_links)
  std::unordered_set<uint64_t> out_link_ids;
  for (int i = 0; i < front_first_ramp_info->successor_link_ids_size(); ++i) {
    uint64_t successor_id = front_first_ramp_info->successor_link_ids()[i];
    if (successor_id != split_next_link->id()) {
      out_link_ids.insert(successor_id);
    }
  }

  // 遍历 max_distance_lanes，区分"直连车道"和"非直连车道"
  // 直连车道：从该车道开始，所有后继分支都能到达 split_next_link
  // 非直连车道：从该车道开始，至少有一个后继分支不能到达 split_next_link
  std::vector<uint64_t> non_directly_connected_lane_ids;
  bool has_directly_connected_lane = false;
  for (const auto& lane : max_distance_lanes) {
    const auto* start_lane = ld_map_.GetLaneInfoByID(lane.id);
    if (start_lane == nullptr) {
      continue;
    }
    std::unordered_set<uint64_t> visited;
    if (IsSuccessorReachingSplitNextLink(start_lane, split_next_link,
                                         out_link_ids, visited)) {
      has_directly_connected_lane = true;
    } else {
      non_directly_connected_lane_ids.push_back(lane.id);
    }
  }

  // 如果没有直连车道，保持原状（避免 feasible lane 为空）
  if (!has_directly_connected_lane) {
    return;
  }

  // 有直连车道，从 feasible_lane_graph 的当前 link 中移除所有非直连车道
  auto& topo_lanes = feasible_lane_graph.lane_topo_groups.back().topo_lanes;
  topo_lanes.erase(
      std::remove_if(topo_lanes.begin(), topo_lanes.end(),
                     [&](const TopoLane& topo_lane) {
                       return std::find(non_directly_connected_lane_ids.begin(),
                                        non_directly_connected_lane_ids.end(),
                                        topo_lane.id) !=
                              non_directly_connected_lane_ids.end();
                     }),
      topo_lanes.end());
...
bool LDRouteInfoStrategy::IsSuccessorReachingSplitNextLink(
  ...
  // 避免重复访问（处理环路）
  if (visited.count(lane->id()) > 0) {
    return false;
  }
  visited.insert(lane->id());

  // 如果已经到达 split_next_link，返回 true
  if (lane->link_id() == split_next_link->id()) {
    return true;
  }

  // 如果到达了 out_link，返回 false
  if (out_link_ids.count(lane->link_id()) > 0) {
    return false;
  }
  ...
  // 如果不在 route 上，返回 false
  if (!ld_map_.isOnRouteLinks(lane->link_id())) {
    return false;
  }

  // 递归检查所有后继，所有后继都必须到达 split_next_link
  for (const auto& suc_lane_id : lane->successor_lane_ids()) {
    const auto* suc_lane = ld_map_.GetLaneInfoByID(suc_lane_id);
    if (!IsSuccessorReachingSplitNextLink(suc_lane, split_next_link, out_link_ids, visited)) {
      return false;  // 有任何一个后继不到达 split_next_link，返回 false
    }
  }

  return true;  // 所有后继都到达 split_next_link
}
...
const iflymapdata::sdpro::LinkInfo_Link*
LDRouteInfoStrategy::GetCloserPredecessorLinkByLateralDistance(
    const iflymapdata::sdpro::LinkInfo_Link* input_link,
    const iflymapdata::sdpro::LinkInfo_Link* predecessor_link1,
    const iflymapdata::sdpro::LinkInfo_Link* predecessor_link2) const {
  if (input_link == nullptr || predecessor_link1 == nullptr ||
      predecessor_link2 == nullptr) {
    return nullptr;
  }
  if (input_link->points().boot().points_size() < 2) {
    return nullptr;
  }
  if (predecessor_link1->points().boot().points_size() < 1 ||
      predecessor_link2->points().boot().points_size() < 1) {
    return nullptr;
  }

  // 使用输入link的起点和反向方向向量
  const auto& input_points = input_link->points().boot();
  const double start_x = input_points.points(0).x();
  const double start_y = input_points.points(0).y();

  // 方向向量：从第二个点指向第一个点（反向）
  const double dx = start_x - input_points.points(1).x();
  const double dy = start_y - input_points.points(1).y();
  const double length = std::sqrt(dx * dx + dy * dy);
  if (length < kEpsilon) {
    return nullptr;
  }
  ...
  // 计算到predecessor1终点的横向距离
  const auto& pred1_points = predecessor_link1->points().boot();
  const int pred1_last = pred1_points.points_size() - 1;
  const double pred1_end_x = pred1_points.points(pred1_last).x();
  const double pred1_end_y = pred1_points.points(pred1_last).y();
  const double vec1_x = pred1_end_x - start_x;
  const double vec1_y = pred1_end_y - start_y;
  const double lateral_dist1 = std::abs(vec1_x * unit_dy - vec1_y * unit_dx);

  // 计算到predecessor2终点的横向距离
  const auto& pred2_points = predecessor_link2->points().boot();
  const int pred2_last = pred2_points.points_size() - 1;
  const double pred2_end_x = pred2_points.points(pred2_last).x();
  const double pred2_end_y = pred2_points.points(pred2_last).y();
  const double vec2_x = pred2_end_x - start_x;
  const double vec2_y = pred2_end_y - start_y;
  const double lateral_dist2 = std::abs(vec2_x * unit_dy - vec2_y * unit_dx);

  // 返回横向距离更小的前继link
  return (lateral_dist1 < lateral_dist2) ? predecessor_link1 : predecessor_link2;
}

MergeLaneType LDRouteInfoStrategy::GetPredecessorLink2LateralSide(
    const iflymapdata::sdpro::LinkInfo_Link* input_link,
    const iflymapdata::sdpro::LinkInfo_Link* predecessor_link1,
    const iflymapdata::sdpro::LinkInfo_Link* predecessor_link2) const {
  if (input_link == nullptr || predecessor_link1 == nullptr ||
  ...
  if (input_link->points().boot().points_size() < 2 ||
      predecessor_link1->points().boot().points_size() < 1 ||
      predecessor_link2->points().boot().points_size() < 1) {
    return NONE_MERGE;
  }

  // input_link 起点作为参考原点，反向方向向量（index1→index0）
  const auto& input_points = input_link->points().boot();
  const double ref_x = input_points.points(0).x();
  const double ref_y = input_points.points(0).y();
  const double dir_x = ref_x - input_points.points(1).x();
  const double dir_y = ref_y - input_points.points(1).y();
  const double dir_len = std::sqrt(dir_x * dir_x + dir_y * dir_y);
  if (dir_len < kEpsilon) {
    return NONE_MERGE;
  }
  const double unit_dx = dir_x / dir_len;
  const double unit_dy = dir_y / dir_len;

  // predecessor_link1 终点相对于参考点的向量
  const auto& pred1_points = predecessor_link1->points().boot();
  const int pred1_last = pred1_points.points_size() - 1;
  const double v1_x = pred1_points.points(pred1_last).x() - ref_x;
  const double v1_y = pred1_points.points(pred1_last).y() - ref_y;

  // predecessor_link2 终点相对于参考点的向量
  const auto& pred2_points = predecessor_link2->points().boot();
  const int pred2_last = pred2_points.points_size() - 1;
  const double v2_x = pred2_points.points(pred2_last).x() - ref_x;
  const double v2_y = pred2_points.points(pred2_last).y() - ref_y;

  const double lat1 = -v1_x * unit_dy + v1_y * unit_dx;
  const double lat2 = -v2_x * unit_dy + v2_y * unit_dx;
...
void LDRouteInfoStrategy::ProcessEraseFeasibleLaneForSplitScene(
  ...
  // 收集所有有效的后继 link
  std::vector<const iflymapdata::sdpro::LinkInfo_Link*> valid_successors;
  for (int i = 0; i < front_first_topo_change_link->successor_link_ids_size(); ++i) {
    uint64 successor_id = front_first_topo_change_link->successor_link_ids()[i];
    const auto& successor_link = ld_map_.GetLinkOnRoute(successor_id);

    if (successor_link != nullptr) {
      valid_successors.push_back(successor_link);
    }
  }

  // 至少需要2个有效后继才有比较的意义
  if (valid_successors.size() < 2) {
    return;
  }

  // 找出横向距离最近的后继 link
  const iflymapdata::sdpro::LinkInfo_Link* closest_successor = valid_successors[0];

  for (size_t i = 1; i < valid_successors.size(); ++i) {
    const auto& closer = GetCloserSuccessorLinkByLateralDistance(
        front_first_topo_change_link, closest_successor, valid_successors[i]);

    if (closer != nullptr && closer->id() == valid_successors[i]->id()) {
      closest_successor = valid_successors[i];
    }
  }

  // 如果导航路线（split_next_link）就是横向距离最近的后继，说明是直行，不执行移除
  if (closest_successor->id() == split_next_link->id()) {
    return;
  }
