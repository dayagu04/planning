...
bool LDRouteInfoStrategy::CalculateCurrentLink () {
  ...
  current_link_ =
      ld_map_.GetNearestLinkWithHeading(current_point, search_distance,
...
bool LDRouteInfoStrategy::CalculateFeasibleLaneGraph(
  ...
  while (current_link != nullptr) {
    ...
    // -------------------------- 7. 获取下一个pre link并更新车道列表（修复原逻辑漏洞） --------------------------
    const auto* next_pre_link = ld_map_.GetPreviousLinkOnRoute(current_link->id());
    if (!next_pre_link) {
      return false;
    }
...
bool LDRouteInfoStrategy::CalculateExtenedFeasibleLane(
  ...
  const int link_size = before_split_feasible_lane_graph.lane_topo_groups.size();
  ...
  for (int i = 0; i < link_size; i++) {
    ...
    const auto& temp_topo_lanes =
        temp_link_topo.topo_lanes;
    ...
    if (temp_link->id() == current_link_->id()) {
      link_sum_dis = link_sum_dis + temp_topo_lanes[0].length - ego_on_cur_link_s_;
    } else {
    ...
    for (const auto& lane_id: temp_link->lane_ids()) {
      ...
      if (route_info_output_.baidu_mlc_scene == SPLIT_SCENE &&
        ...
        if (other_link_ids.find(successor_lane->link_id()) != other_link_ids.end()) {
          continue;
        }
      }
...
void LDRouteInfoStrategy::UpdateLCNumTask(
  ...
  for (auto relative_id_lane: relative_id_lanes) {
    ...
    // 计算出原始的order
    std::vector<int> origin_order_id_seq;
    ...
    for (int i = cur_lane_size - 1; i >= 0; i--) {
      ...
      if (IsDiversionLane(temp_lane)) {
        ...
        for (int j = origin_order_size - 1; j >= 0; j--) {
          if (origin_order_id_seq[j] > temp_lane->sequence()) {
            origin_order_id_seq[j] = origin_order_id_seq[j] - 1;
          }
        }
      }
    }
    ...
    std::vector<int> feasible_lane_seq;
    for (const auto& order_id: origin_order_id_seq) {
      // 把从右向左的顺序转换成从左向右的顺序
      const int seq = link_total_lane_num - order_id + 1;
      if (seq > 0) {
        feasible_lane_seq.emplace_back(seq);
      }
    }
    ...
    int minVal_seq = feasible_lane_seq[0];
    int maxVal_seq = feasible_lane_seq[0];

    for (int num : feasible_lane_seq) {
      if (num < minVal_seq) {
        minVal_seq = num;
      }
      if (num > maxVal_seq) {
...
bool LDRouteInfoStrategy::CalculateFrontTargetLinkBaseFixDis(
  ...
  const double kFrontSearchDis = 500;
  double sum_dis;
...
bool LDRouteInfoStrategy::CalculateMergePreFeasibleLane(
  ...
  if (merge_pre_link_lane_vec.empty()) {
    ...
    merge_link_feasible_lane_order_id_seq.reserve(merge_link_feasible_lane.topo_lanes.size());
    for (const auto& feasible_lane: merge_link_feasible_lane.topo_lanes) {
      merge_link_feasible_lane_order_id_seq.emplace_back(feasible_lane.order_id);
    }
      ...
      for (int i = min_order; i > 0; i--) {
        for (const auto& [seq, lane] : first_merge_lane_info) {
          ...
          for (const auto& pre_lane_id :
            ...
            if (pre_lane->lane_transiton() !=
                    iflymapdata::sdpro::LTS_MERGE &&
...
bool LDRouteInfoStrategy::IsInvalidLane(const iflymapdata::sdpro::Lane* temp_lane) const {
  bool is_emergency_lane = IsEmergencyLane(temp_lane);
  bool is_merge_lane = temp_lane->lane_transiton() == iflymapdata::sdpro::LTS_MERGE;
...
bool LDRouteInfoStrategy::IsIgnoreMerge(
  ...
  // 1、在merge link的pre_link上车道数多的link认为是主路，车道数少的link认为是merge进来的；
...
bool LDRouteInfoStrategy::get_sdpromap_valid() {
  return ldmap_valid_;
}
