...
enum class ReferencePathType { MAP_LANE, REFINED_LANE, LANE_CHANGE, HISTORY_LINE };
...
class ReferencePathManager {
 public:
  ...
  // map lane reference
  std::shared_ptr<ReferencePath> get_reference_path_by_lane(int virtual_lane_id, bool create_if_not_exist = true);
 ...
 private:
  ...
  std::map<ReferencePathKeyType, std::shared_ptr<ReferencePath>> reference_paths_;
};
