...
void RouteInfo::Update() {
  ...
  ad_common::sdmap::SDMap sd_map;
  ad_common::hdmap::HDMap hd_map;
  bool is_hpp_scene = session_->get_scene_type() == common::HPP;
  if (!is_hpp_scene &&
      UpdateSdMap(local_view,&sd_map)) {
    UpdateRouteInfoForNOA(sd_map);
  } else if (UpdateStaticMap(local_view, &hd_map)){
    UpdateRouteInfoForHPP(hd_map);
  }
}  
...
void RouteInfo::UpdateRouteInfoForNOA(const ad_common::sdmap::SDMap& sd_map) {
  ...
  if (!UpdateEgoSegmentInfo(sd_map, segment, &nearest_s)) {
  ...
  // 计算距离上一个merge点的信息
  CaculateDistanceToLastMergePoint(sd_map, 
      current_segment, nearest_s, max_search_length);

  // 计算距离上一个split点的信息
  CaculateDistanceToLastSplitPoint(sd_map, 
      current_segment, nearest_s, max_search_length);
  
  // 计算到路线终点的距离
  CaculateDistanceToRoadEnd(sd_map, 
      current_segment, nearest_s, max_search_length);

  // 计算到最近收费站的距离
  CaculateDistanceToTollStation(sd_map, 
      current_segment, nearest_s, max_search_length);
}
...
void RouteInfo::CaculateRampInfo(
  ...
  const auto& ramp_info =
      sd_map.GetRampInfo(
      segment.id(), nearest_s, max_search_length);
...
void RouteInfo::CaculateMergeInfo(const ad_common::sdmap::SDMap& sd_map, const SdMapSwtx::Segment& segment,
                        const double nearest_s, const double max_search_length) {
      ...
      if (merge_info_temp.second > kEpsilon) {
        ...
        if (!is_find_first_merge_onfo) {
          if (merge_seg_last_seg->usage() == SdMapSwtx::RoadUsage::RAMP &&
              merge_seg->usage() != SdMapSwtx::RoadUsage::RAMP && route_info_output_.is_on_ramp) {
            route_info_output_.is_ramp_merge_to_road_on_expressway = true;
          }
          if (merge_seg_last_seg->usage() != SdMapSwtx::RoadUsage::RAMP &&
              merge_seg->usage() != SdMapSwtx::RoadUsage::RAMP &&
              !route_info_output_.is_on_ramp && route_info_output_.is_ego_on_expressway) {
          ...
          route_info_output_.first_merge_direction = MakesureMergeDirection(*merge_seg, sd_map);
          route_info_output_.distance_to_first_road_merge = merge_info_temp.second;
          is_find_first_merge_onfo = true;
          traverse_num++;
        } else if (is_find_first_merge_onfo) {
          route_info_output_.second_merge_direction = MakesureMergeDirection(*merge_seg, sd_map);
          route_info_output_.distance_to_second_road_merge = merge_info_temp.second;
          traverse_num++;
        }
...
void RouteInfo::CaculateSplitInfo(const ad_common::sdmap::SDMap& sd_map, const SdMapSwtx::Segment& segment,
                        const double nearest_s, const double max_search_length) {
    ...
    for (int i = 0; i < split_info.size(); i++) {
      ...
      if (split_segment && split_info[i].second > 0) {
        if (!is_find_first_split_info) {
          route_info_output_.distance_to_first_road_split = split_info[i].second;
          SplitSegInfo split_seg_info;
          split_seg_info = MakesureSplitDirection(*split_segment, sd_map);
          route_info_output_.first_split_direction = split_seg_info.split_direction;
          route_info_output_.split_seg_forward_lane_nums = split_seg_info.split_seg_forward_lane_nums;
          route_info_output_.split_next_seg_forward_lane_nums = split_seg_info.split_next_seg_forward_lane_nums;
          is_find_first_split_info = true;
          traverse_num++;
          route_info_output_.first_split_dir_dis_info = std::make_pair(
              static_cast<SplitRelativeDirection>(route_info_output_.first_split_direction),
              route_info_output_.distance_to_first_road_split);
        } else if (is_find_first_split_info) {
          route_info_output_.distance_to_second_road_split = split_info[i].second;
          SplitSegInfo split_seg_info;
          split_seg_info = MakesureSplitDirection(*split_segment, sd_map);
          route_info_output_.second_split_direction = split_seg_info.split_direction;
          traverse_num++;
        }
        if (traverse_num >= 2) {
          break;
  ...
  if (!split_info.empty()) {
    for (int i = 0; i < 2; i++) {
      if (i < split_info.size()) {
        ...
        route_info_output_.split_dir_dis_info_list.emplace_back(split_dir_dis_info);
      }
    }
  } else {
    route_info_output_.distance_to_first_road_split = NL_NMAX;
...
void RouteInfo::CaculateDistanceToLastMergePoint(const ad_common::sdmap::SDMap& sd_map, const SdMapSwtx::Segment& segment,
                      const double nearest_s, const double max_search_length) {
  ...
  if (!route_info_output_.is_on_ramp) {
    ...
    if (last_merge_seg && last_merge_seg->in_link().size() == 2) {
      ...
      if (merge_last_seg && merge_last_seg->usage() == SdMapSwtx::RAMP &&
        ...
        route_info_output_.sum_dis_to_last_merge_point = sum_dis_to_last_merge_point;
      }
    }
    if (route_info_output_.sum_dis_to_last_merge_point > dis_threshold_to_last_merge_point_) {
      is_accumulate_dis_to_last_merge_point_more_than_threshold_ = true;
    }
  }
  JSON_DEBUG_VALUE("sum_dis_to_last_merge_point", route_info_output_.sum_dis_to_last_merge_point);
}

void RouteInfo::CaculateDistanceToLastSplitPoint(const ad_common::sdmap::SDMap& sd_map, const SdMapSwtx::Segment& segment,
                      const double nearest_s, const double max_search_length) {
    ...
    if (last_split_seg != nullptr) {
      ...
      if (last_split_seg && last_split_seg->out_link().size() == 2) {
        if (last_split_seg->usage() != SdMapSwtx::RAMP) {
          route_info_output_.sum_dis_to_last_split_point = sum_dis_to_last_split_point;
        }
      }
    }
  }
  JSON_DEBUG_VALUE("sum_dis_to_last_split_point", route_info_output_.sum_dis_to_last_split_point);
...
void RouteInfo::CaculateDistanceToRoadEnd(const ad_common::sdmap::SDMap& sd_map, const SdMapSwtx::Segment& segment,
                      const double nearest_s, const double max_search_length) {
  double dis_to_end = NL_NMAX;
  int result = sd_map.GetDistanceToRouteEnd(segment.id(), nearest_s,
                                            dis_to_end);
  if (result == 0) {
  ...
  JSON_DEBUG_VALUE("distance_to_route_end", route_info_output_.distance_to_route_end);
}

void RouteInfo::CaculateDistanceToTollStation(const ad_common::sdmap::SDMap& sd_map, const SdMapSwtx::Segment& segment,
                      const double nearest_s, const double max_search_length) {
...
bool RouteInfo::UpdateEgoSegmentInfo (const ad_common::sdmap::SDMap& sd_map, const SdMapSwtx::Segment** segment, double* const nearest_s) {
  //获取当前的segment
  ...
  route_info_output_.is_on_ramp = current_segment->usage() == SdMapSwtx::RoadUsage::RAMP;
  route_info_output_.cur_seg_forward_lane_num = current_segment->forward_lane_num();
...
bool RouteInfo::UpdateSdMap(const LocalView &local_view,
  ...
  JSON_DEBUG_VALUE("sdmap_valid_", sdmap_valid)
  return sdmap_valid;
}
...
bool RouteInfo::UpdateStaticMap(const LocalView &local_view,
  ...
  JSON_DEBUG_VALUE("hdmap_valid", hdmap_valid)
  return hdmap_valid;
}

void RouteInfo::UpdateMLCInfoDecider(std::vector<std::shared_ptr<VirtualLane>> relative_id_lanes) {
  ...
  if (route_info_output_.is_nearing_ramp &&
    ...
    is_nearing_split = true;
       ...
       route_info_output_.distance_to_first_road_split < 
          route_info_output_.distance_to_first_road_merge) {
     is_nearing_split = true;
  } else if (route_info_output_.distance_to_first_road_split < nearing_split_dis_threshold) {
    is_nearing_split = true;
  }
  ...
  bool is_exit_lane_on_last_ramp_dir = 
      route_info_output_.last_split_seg_dir == RAMP_NONE ? false : 
      (route_info_output_.last_split_seg_dir == RAMP_ON_LEFT ? 
      !is_ego_on_leftest_lane : !is_ego_on_rightest_lane);
  const int cur_seg_forward_lane_num = route_info_output_.cur_seg_forward_lane_num;
  ...
  if (is_exit_lane_on_last_ramp_dir &&
    ...
    if (cur_seg_forward_lane_num == 1) {
      ...
      if (route_info_output_.last_split_seg_dir == RAMP_ON_RIGHT &&
          right_lane) {
        const auto& right_lane_left_boundary = right_lane->get_left_lane_boundary();
        const auto& right_lane_left_boundary_path = 
            virtual_lane_manager->MakeBoundaryPath(right_lane_left_boundary);
        const auto& right_lane_right_boundary = right_lane->get_right_lane_boundary();
        const auto& right_lane_right_boundary_path = 
            virtual_lane_manager->MakeBoundaryPath(right_lane_right_boundary);
        bool right_lane_left_boundary_is_dash_line = right_lane->is_dash_line(*session_, 
            LEFT_CHANGE, right_lane_left_boundary_path);
        bool right_lane_right_boundary_is_dash_line = right_lane->is_dash_line(*session_, 
            RIGHT_CHANGE, right_lane_right_boundary_path);
        const auto& current_lane_right_boundary = curret_lane->get_right_lane_boundary();
        const auto& current_lane_right_boundary_path = 
            virtual_lane_manager->MakeBoundaryPath(current_lane_right_boundary);
        bool current_lane_right_boundary_is_dash_line = curret_lane->is_dash_line(*session_, 
