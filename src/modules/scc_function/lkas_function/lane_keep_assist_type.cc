...
bool LKALineLeftIntervention(double tlc_to_line_threshold,
  ...
  double preview_distance_x =
      0;  // 在tlc_to_line_threshold秒后,车辆左前轮所处的纵向坐标,单位：m
  preview_distance_x =
      lkas_input->vehicle_info.veh_actual_speed * tlc_to_line_threshold +
      lkas_input->vehicle_info.common_wheel_base;

  double front_left_wheel_distance_to_line =
      0;  // 左前轮至左侧道线的距离,单位：m
  ...
  preview_distance_y = 0.5 * lkas_input->vehicle_info.veh_actual_speed *
...
bool LKARoadEdgeLeftIntervention(double tlc_to_roadedge_threshold,
  ...
  preview_distance_x =
      lkas_input->vehicle_info.veh_actual_speed * tlc_to_roadedge_threshold +
      lkas_input->vehicle_info.common_wheel_base;

  double front_left_wheel_distance_to_roadedge =
      0;  // 左前轮至左侧路沿的距离,单位：m
...
bool LKALineRightIntervention(double tlc_to_line_threshold,
                              planning::LkasInput *lkas_input) {
  double preview_distance_x =
      0.0;  // 在tlc_to_line_threshold秒后,车辆右前轮所处的纵向坐标,单位：m
  preview_distance_x =
      lkas_input->vehicle_info.veh_actual_speed * tlc_to_line_threshold +
      lkas_input->vehicle_info.common_wheel_base;

  double front_right_wheel_distance_to_line =
      0.0;  // 右前轮至右侧道线的距离,单位：m
...
bool LKARoadEdgeRightIntervention(double tlc_to_roadedge_threshold,
  ...
  double front_right_wheel_distance_to_roadedge =
      0;  // 右前轮至右侧路沿的距离,单位：m
