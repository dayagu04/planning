...
void LaneKeepAssistManager::Update() {
  // input update
  lkas_input_.road_info.left_line_valid = 0;       // 本车道左侧道线有效性 0:Invalid 1:Valid
  lkas_input_.road_info.left_line_type = 0;        // 本车道左侧车道线类型 0：虚线  1：实线
  ...
  lkas_input_.road_info.right_line_valid = 0;      // 本车道右侧道线有效性 0:Invalid 1:Valid
  lkas_input_.road_info.right_line_type = 0;       // 本车道右侧车道线类型 0：虚线  1：实线
  ...
  lkas_input_.road_info.left_roadedge_valid = 0;   // 本车道左侧路缘有效性 0:Invalid 1:Valid
  ...
  lkas_input_.road_info.right_roadedge_valid = 0;  // 本车道右侧路缘有效性 0:Invalid 1:Valid
  ...
  lkas_input_.road_info.lane_width_valid = 0;      // 当前车道宽度信息有效性 0:Invalid 1:Valid
  ...
  // 左侧边界结构体
  auto ptr_current_lane_left_boundary = ptr_current_lane->get_left_lane_boundary();
  // 判断边界是路沿还是道线
  ...
  if (lkas_input_.road_info.left_line_valid == 1)  // 判断为道线
  {
    ...
    if (left_line_type == 1 || left_line_type == 3 || left_line_type == 4 || left_line_type == 7) {
      lkas_input_.road_info.left_line_type = 0;  // 可跨越道线
    } else {
      lkas_input_.road_info.left_line_type = 1;  // 不可跨越道线
    }

    lkas_input_.road_info.left_line_c0 = ptr_current_lane_left_boundary.poly_coefficient(0);
    lkas_input_.road_info.left_line_c1 = ptr_current_lane_left_boundary.poly_coefficient(1);
    lkas_input_.road_info.left_line_c2 = ptr_current_lane_left_boundary.poly_coefficient(2);
    lkas_input_.road_info.left_line_c3 = ptr_current_lane_left_boundary.poly_coefficient(3);
  } else if (lkas_input_.road_info.left_roadedge_valid == 1)  // 判断为路沿
  {
    lkas_input_.road_info.left_roadedge_c0 = ptr_current_lane_left_boundary.poly_coefficient(0);
    lkas_input_.road_info.left_roadedge_c1 = ptr_current_lane_left_boundary.poly_coefficient(1);
    lkas_input_.road_info.left_roadedge_c2 = ptr_current_lane_left_boundary.poly_coefficient(2);
    lkas_input_.road_info.left_roadedge_c3 = ptr_current_lane_left_boundary.poly_coefficient(3);
  } else {
    /*do nothing*/
  }
  // 右侧边界
  auto ptr_current_lane_right_boundary = ptr_current_lane->get_right_lane_boundary();
  ...
  if (lkas_input_.road_info.right_line_valid == 1) {
    ...
    if ((right_line_type == 1) || (right_line_type == 3) || (right_line_type == 4) || (right_line_type == 6)) {
      lkas_input_.road_info.right_line_type = 0;
    } else {
      lkas_input_.road_info.right_line_type = 1;
    }
    lkas_input_.road_info.right_line_c0 = ptr_current_lane_right_boundary.poly_coefficient(0);
    lkas_input_.road_info.right_line_c1 = ptr_current_lane_right_boundary.poly_coefficient(1);
    lkas_input_.road_info.right_line_c2 = ptr_current_lane_right_boundary.poly_coefficient(2);
    lkas_input_.road_info.right_line_c3 = ptr_current_lane_right_boundary.poly_coefficient(3);
  } else if (lkas_input_.road_info.right_roadedge_valid == 1) {
    lkas_input_.road_info.right_roadedge_c0 = ptr_current_lane_right_boundary.poly_coefficient(0);
    lkas_input_.road_info.right_roadedge_c1 = ptr_current_lane_right_boundary.poly_coefficient(1);
    lkas_input_.road_info.right_roadedge_c2 = ptr_current_lane_right_boundary.poly_coefficient(2);
    lkas_input_.road_info.right_roadedge_c3 = ptr_current_lane_right_boundary.poly_coefficient(3);
  } else {
    /*do nothing*/
  }
  // 道路宽度
  ...
  lkas_input_.vehicle_info.veh_yaw_rate = ptr_ego_state_manager->ego_pose().theta * 0.174F;
  lkas_input_.vehicle_info.veh_yaw_rate =
      session_->mutable_environmental_model()->get_local_view().vehicel_service_output_info.yaw_rate();
  lkas_input_.vehicle_info.driver_hand_torque = ptr_ego_state_manager->driver_hand_torque();
  ...
  // ldw switch
  lkas_input_.vehicle_info.ldw_main_switch = TRUE;  // session_->mutable_environmental_model()
                                                    //->get_hmi_info()
                                                    //.ldw_main_switch();
  // ldp switch
  lkas_input_.vehicle_info.ldp_main_switch = TRUE;  // session_->mutable_environmental_model()
                                                    //  ->get_hmi_info()
                                                    //  .ldw_main_switch();
  // elk switch
  lkas_input_.vehicle_info.elk_main_switch = TRUE;  // session_->mutable_environmental_model()
                                                    //->get_hmi_info()
  // .elk_main_switch();
  // ldw sensitvity set
  lkas_input_.vehicle_info.ldw_tlc_level =
      session_->mutable_environmental_model()->get_hmi_info().ldw_set_sensitivity_level();

  lkas_input_.vehicle_info.common_front_over =
      session_->mutable_environmental_model()->vehicle_param().front_edge_to_center -
      session_->mutable_environmental_model()->vehicle_param().wheel_base;
  lkas_input_.vehicle_info.common_rear_over =
      session_->mutable_environmental_model()->vehicle_param().back_edge_to_center;
  lkas_input_.vehicle_info.common_wheel_base = session_->mutable_environmental_model()->vehicle_param().wheel_base;
  lkas_input_.vehicle_info.common_veh_width = session_->mutable_environmental_model()->vehicle_param().width;
  // update
  CalculateWheelToLine();
  JSON_DEBUG_VALUE("lkas_function::left_line_c0", lkas_input_.road_info.left_line_c0);
  JSON_DEBUG_VALUE("lkas_function::left_line_c1", lkas_input_.road_info.left_line_c1);
  JSON_DEBUG_VALUE("lkas_function::left_line_c2", lkas_input_.road_info.left_line_c2);
  JSON_DEBUG_VALUE("lkas_function::left_line_c3", lkas_input_.road_info.left_line_c3);
  JSON_DEBUG_VALUE("lkas_function::right_line_c0", lkas_input_.road_info.right_line_c0);
  JSON_DEBUG_VALUE("lkas_function::right_line_c1", lkas_input_.road_info.right_line_c1);
  JSON_DEBUG_VALUE("lkas_function::right_line_c2", lkas_input_.road_info.right_line_c2);
  JSON_DEBUG_VALUE("lkas_function::right_line_c3", lkas_input_.road_info.right_line_c3);
}
...
void LaneKeepAssistManager::CalculateWheelToLine() {
  ...
  lkas_input_.wheel_to_line.fl_wheel_distance_to_line =
      ...
      lkas_input_.road_info.left_line_c1 * lkas_input_.vehicle_info.common_wheel_base +
      lkas_input_.road_info.left_line_c2 * lkas_input_.vehicle_info.common_wheel_base *
          lkas_input_.vehicle_info.common_wheel_base +
      lkas_input_.road_info.left_line_c3 * lkas_input_.vehicle_info.common_wheel_base *
          lkas_input_.vehicle_info.common_wheel_base * lkas_input_.vehicle_info.common_wheel_base -
  ...
  lkas_input_.wheel_to_line.fl_wheel_distance_to_roadedge =
      ...
      lkas_input_.road_info.left_roadedge_c1 * lkas_input_.vehicle_info.common_wheel_base +
      lkas_input_.road_info.left_roadedge_c2 * lkas_input_.vehicle_info.common_wheel_base *
          lkas_input_.vehicle_info.common_wheel_base +
      lkas_input_.road_info.left_roadedge_c3 * lkas_input_.vehicle_info.common_wheel_base *
          lkas_input_.vehicle_info.common_wheel_base * lkas_input_.vehicle_info.common_wheel_base -
      Common_FrontCamera_PosYL;
  /*fr_wheel_distance_to_line*/
  lkas_input_.wheel_to_line.fr_wheel_distance_to_line =
      lkas_input_.road_info.right_line_c0 +
      lkas_input_.road_info.right_line_c1 * lkas_input_.vehicle_info.common_wheel_base +
      lkas_input_.road_info.right_line_c2 * lkas_input_.vehicle_info.common_wheel_base *
          lkas_input_.vehicle_info.common_wheel_base +
      lkas_input_.road_info.right_line_c3 * lkas_input_.vehicle_info.common_wheel_base *
          lkas_input_.vehicle_info.common_wheel_base * lkas_input_.vehicle_info.common_wheel_base +
  ...
  lkas_input_.wheel_to_line.fr_wheel_distance_to_roadedge =
      ...
      lkas_input_.road_info.right_roadedge_c1 * lkas_input_.vehicle_info.common_wheel_base +
      lkas_input_.road_info.right_roadedge_c2 * lkas_input_.vehicle_info.common_wheel_base *
          lkas_input_.vehicle_info.common_wheel_base +
      lkas_input_.road_info.right_roadedge_c3 * lkas_input_.vehicle_info.common_wheel_base *
          lkas_input_.vehicle_info.common_wheel_base * lkas_input_.vehicle_info.common_wheel_base +
      Common_FrontCamera_PosYR;
}
void LaneKeepAssistManager::Output() {
  ...
  */
  /*update ldw result*/
  lka_output_str.mutable_ldw_output_info()->set_ldw_state(lane_depart_warning_.get_ldw_state());
  ...
  /*update ldp result*/  /////////////////////////
  lka_output_str.mutable_ldp_output_info()->set_ldp_state(lane_depart_prevention_.get_ldp_state());

  if (lane_depart_prevention_.get_ldp_state() == 4) {
    lka_output_str.mutable_ldp_output_info()->set_ldp_left_intervention_flag(TRUE);
  } else {
    lka_output_str.mutable_ldp_output_info()->set_ldp_left_intervention_flag(FALSE);
  }
  if (lane_depart_prevention_.get_ldp_state() == 5) {
    lka_output_str.mutable_ldp_output_info()->set_ldp_right_intervention_flag(TRUE);
  } else {
    lka_output_str.mutable_ldp_output_info()->set_ldp_right_intervention_flag(FALSE);
  }
  /*update elk result*/  ////////////////////////
  lka_output_str.mutable_elk_output_info()->set_elk_state(emergency_lane_keep_.get_elk_state());
  if (emergency_lane_keep_.get_elk_state() == 4) {
    lka_output_str.mutable_elk_output_info()->set_elk_left_intervention_flag(TRUE);
  } else {
    lka_output_str.mutable_elk_output_info()->set_elk_left_intervention_flag(FALSE);
  }
  if (emergency_lane_keep_.get_elk_state() == 5) {
    lka_output_str.mutable_elk_output_info()->set_elk_right_intervention_flag(TRUE);
  } else {
    lka_output_str.mutable_elk_output_info()->set_elk_right_intervention_flag(FALSE);
  }
}
...
void LaneKeepAssistManager::set_lka_output_info() {
  ...
  */
  ...
  ldp_left_intervention_flag_ = lane_depart_prevention_.get_ldp_left_intervention_flag_info();
  ldp_right_intervention_flag_ = lane_depart_prevention_.get_ldp_right_intervention_flag_info();
  /*update elk result*/  ////////////////////////
  ...
  elk_left_intervention_flag_ = emergency_lane_keep_.get_elk_left_intervention_flag_info();
  elk_right_intervention_flag_ = emergency_lane_keep_.get_elk_right_intervention_flag_info();
}
