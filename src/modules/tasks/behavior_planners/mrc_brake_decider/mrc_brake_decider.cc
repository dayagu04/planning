...
MRCBrakeDecider::MRCBrakeDecider(const EgoPlanningConfigBuilder *config_builder,
...
bool MRCBrakeDecider::Execute() {
  ...
  auto mrc_output =
      session_->mutable_planning_context()->mutable_mrc_brake_decider_output();
...
bool MRCBrakeDecider::MRCBrakeProcess() {
  ...
  if (has_set_virtual_obs_ == false) {
    ...
    double obs_set_dis =
        std::min(reference_path_ptr->get_points().back().path_point.s() - 0.5,
                 ego_start_s + virtual_obs_dis);
    if (reference_path_ptr->get_reference_point_by_lon(obs_set_dis,
                                                       refpath_pt)) {
