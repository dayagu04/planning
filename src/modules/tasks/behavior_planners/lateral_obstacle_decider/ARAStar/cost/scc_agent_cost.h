...
class SccAgentCost : public BaseCost {
 public:
  SccAgentCost(
  ...
  ~SccAgentCost() = default;

  double MakeCost(Node3d& vertex) const;

 private:
  void NormalizeCost(double& cost) const;
  bool GetAreaCost(const SLBox2d& sl_box, const double ego_front_s,
  ...
  double GetNearingOverlapCost(const SLBox2d& sl_box, const double ego_front_s, const double ego_front_l,
