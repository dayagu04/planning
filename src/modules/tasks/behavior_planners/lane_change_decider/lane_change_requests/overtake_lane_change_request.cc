...
constexpr double kOvertakeLeadingVehicleRadicalLowSpeedDiffThreshold =
    2.78;                                                       // 10km/h
...
constexpr int kTruckConfirmFrames = 3;
...
void OvertakeRequest::Update(int lc_status) {
  ...
  left_feasible_lane_remain_distance_ = 1500.0;
  right_feasible_lane_remain_distance_ = 1500.0;

  if (llane != nullptr) {
    left_reference_path_ = reference_path_mgr->get_reference_path_by_lane(
        llane->get_virtual_id(), false);
    ILOG_DEBUG << "OvertakeRequest::Update: for left_lane: update:"
               << llane->get_virtual_id();
    const auto& feasible_lane_pair = llane->get_feasible_lane_distance();
    if (feasible_lane_pair.first) {
      left_feasible_lane_remain_distance_ = feasible_lane_pair.second;
    } else {
      left_feasible_lane_remain_distance_ = 0.0;
    }
  } else {
  ...
  if (rlane != nullptr) {
    ...
    ILOG_DEBUG << "OvertakeRequest::Update: for right_lane: update "
               << rlane->get_virtual_id();
    const auto& feasible_lane_pair = rlane->get_feasible_lane_distance();
    if (feasible_lane_pair.first) {
      right_feasible_lane_remain_distance_ = feasible_lane_pair.second;
    } else {
      right_feasible_lane_remain_distance_ = 0.0;
    }
  } else {
  ...
  ILOG_DEBUG << "request_type_:" << request_type_
...
void OvertakeRequest::setLaneChangeRequestByFrontSlowVehcile(int lc_status) {
  ...
  const double minimum_distance_nearby_ramp_to_surpress_overtake_for_baidu =
      2000.0;
  ...
  double lead_agent_longit_distance = 180.0;
  int lead_agent_id = -1;
  if (agent == nullptr) {
    if (origin_refline != nullptr) {
      ...
      for (const auto& current_agent : current_agents) {
        const auto obstacle_id = current_agent->agent_id();
        ...
        // 过滤自车道之外的障碍物
        bool is_out_current_lane =
            obstacle_iter->second->frenet_obstacle_boundary().l_start > 1.0 ||
            obstacle_iter->second->frenet_obstacle_boundary().l_end < -1.0;
        if (is_out_current_lane) {
          continue;
        }
        if (obstacle_iter->second->frenet_obstacle_boundary().s_start < ego_frenet_boundary.s_end) {
          continue;
        }
        if (obstacle_iter->second->frenet_obstacle_boundary().s_start < lead_agent_longit_distance) {
          lead_agent_longit_distance = obstacle_iter->second->frenet_obstacle_boundary().s_start;
          lead_agent_id = obstacle_id;
        }
      }
 
      if (lead_agent_id != -1) {
        agent = agent_manager->mutable_agent(lead_agent_id);
      }
    }
  } else {
    lead_agent_id = cipv_id;
  }
  ...
  if (enable_overtake_cross_line_large_agent) {
    ...
    // 2. 清理上一帧已消失的agent记录（避免哈希表无限膨胀）
    std::vector<int32_t> agents_to_erase;
    for (const auto & [ agent_id, count ] : truck_confirm_frame_count_map_) {
      if (current_frame_agent_ids.find(agent_id) ==
          current_frame_agent_ids.end()) {
        agents_to_erase.push_back(agent_id);
      }
    }
    for (int32_t id : agents_to_erase) {
      truck_confirm_frame_count_map_.erase(id);
    }
    ...
    for (const auto& current_agent : current_agents) {
      ...
      if (llane != nullptr) {
        IsTargetLaneExistTruck(current_agent, llane, true,
                               left_lane_exist_cross_line_truck_speed_);
      }
      if (rlane != nullptr) {
        IsTargetLaneExistTruck(current_agent, rlane, false,
                               right_lane_exist_cross_line_truck_speed_);
      }
      ...
      if (track_iter->second == nullptr) {
        continue;
      }
      auto& frame_count =
          truck_confirm_frame_count_map_[current_agent->agent_id()];
      if (current_agent->is_truck()) {
        frame_count += 1;
        frame_count = std::min(frame_count, kTruckConfirmFrames);
      } else {
        frame_count -= 1;
      ...
      if (track_iter->second->d_s_rel() <= leading_relative_dis &&
          current_agent->agent_id() != lead_agent_id) {
        ...
        double cutting_threshold =
            std::max(half_lane_width + 0.2 - agent_width * 0.5, 0.8);
        if (track_iter->second->frenet_l() > cutting_threshold && rlane &&
            llane) {
          enable_l_ = false;
        } else if (track_iter->second->frenet_l() < -cutting_threshold &&
                   rlane && llane) {
          enable_r_ = false;
        }
      }
  ...
  const double need_overtake_distance = leading_relative_dis + agent->length();
  ...
  JSON_DEBUG_VALUE("leading_vehicle_speed", leading_vehicle_speed);
  ...
  const bool is_trigger_left =
      ...
      !left_lane_exist_slow_cross_lane_truck &&
  ...
  const bool is_trigger_right =
      ...
      !right_lane_exist_slow_cross_lane_truck &&
  ...
  //       !left_lane_exist_slow_cross_lane_truck &&
  //       FeasibleLaneDistanceEnoughJudgment(left_route_traffic_speed,
  //                                          leading_vehicle_speed, llane,
  //                                          true, need_overtake_distance);
  ...
  JSON_DEBUG_VALUE("left_lane_is_on_navigation_route",
                   left_lane_is_on_navigation_route);
  JSON_DEBUG_VALUE("right_lane_is_on_navigation_route",
                   right_lane_is_on_navigation_route);
  ...
  if (trigger_left_overtake && current_time - tfinish_ >= 3.0 &&
    ...
    if (!IsDashEnoughForRepeatSegments(
      ...
      ILOG_DEBUG << "overtake_vehicle_id_: " << overtake_vehicle_id_
                 << "overtake_vehicle_speed_: " << overtake_vehicle_speed_;
    }
    ...
    ILOG_DEBUG << "[OvertakeRequest::update] " << __FUNCTION__ << ":"
               << __LINE__
               << " finish request, !trigger_left_overtake and "
                  "!trigger_right_overtake";
  }
...
bool OvertakeRequest::isSatisfyOvertakeCountMaintainCondition(
  ...
  double speed_threshold_percentage = 1.0;
  if (leading_agent->type() == agent::AgentType::TRUCK ||
      leading_agent->type() == agent::AgentType::BUS ||
  ...
  if (speed_diff >
      kOvertakeMaintainCountSpeedDiffThreshold * speed_threshold_percentage) {
    return true;
  }
  const double speed_ratio =
      (0.0 != reference_speed) ? (speed_diff / reference_speed) : 0.0;
  if (speed_ratio >
      kOvertakeMaintainCountRatioThreshold * speed_threshold_percentage) {
    return true;
  }
  return false;
}
...
void OvertakeRequest::updateRouteTrafficSpeed(
  ...
  *route_traffic_speed = 0.0;
  const auto& front_tracks_l = lane_tracks_manager_->front_tracks_l();
  const auto& front_tracks_r = lane_tracks_manager_->front_tracks_r();
  ...
  if (is_left) {
    for (const auto& tr : front_tracks_l) {
      ...
      if (tr->type() == iflyauto::ObjectType::OBJECT_TYPE_TRUCK ||
          tr->type() == iflyauto::ObjectType::OBJECT_TYPE_BUS ||
...
bool OvertakeRequest::isCouldOvertakeByRoute(
  ...
  if (is_left && left_lane_exist_truck_) {
    target_lane_exist_truck_speed_threshold_percentage =
        kTargetLaneExistTruckSpeedDiffThresholdPercentage;
  } else if (!is_left && right_lane_exist_truck_) {
    target_lane_exist_truck_speed_threshold_percentage =
        kTargetLaneExistTruckSpeedDiffThresholdPercentage;
  }

  const auto& lane_change_style =
      session_->environmental_model()
          .get_local_view()
          .function_state_machine_info.pilot_req.lane_change_style;
  double speed_threshold =
      config_.overtake_standard_left_lane_change_speed_threshold *
      speed_threshold_percentage;
  if (lane_change_style == iflyauto::LANE_CHANGE_STYLE_ASSISTIVE) {
    ...
    if (is_left) {
      ...
      speed_threshold =
          config_.overtake_standard_right_lane_change_speed_threshold *
          speed_threshold_percentage;
    }
  }
  ...
  // }
  const int total_lane_nums = left_lane_nums_ + right_lane_nums_ + 1;
  ...
  if (is_left) {
    left_overtake_speed_threshold = speed_threshold;
  } else {
    right_overtake_speed_threshold = speed_threshold;
  }
...
bool OvertakeRequest::isCouldOvertakeMaintainByRoute(
  ...
  if (lane_change_style == iflyauto::LANE_CHANGE_STYLE_ASSISTIVE) {
    if (leading_vehicle_speed >= kOvertakeLeadingVehicleHighSpeedThreshold) {
      ...
      speed_threshold = planning_math::lerp(
          config_.overtake_soft_lane_change_speed_threshold,
          kOvertakeLeadingVehicleLowSpeedThreshold,
          kOvertakeLeadingVehicleHighSpeedDiffThreshold,
          kOvertakeLeadingVehicleHighSpeedThreshold, leading_vehicle_speed);
    }
    ...
    if (leading_vehicle_speed >= kOvertakeLeadingVehicleHighSpeedThreshold) {
      ...
      speed_threshold = planning_math::lerp(
          config_.overtake_radical_lane_change_speed_threshold,
          kOvertakeLeadingVehicleLowSpeedThreshold,
          kOvertakeLeadingVehicleHighSpeedDiffThreshold,
          kOvertakeLeadingVehicleHighSpeedThreshold, leading_vehicle_speed);
    }
...
bool OvertakeRequest::checkLeftLaneChangeValidByObjects(
  ...
  for (const auto& id : left_potensial_objects) {
    ...
    if (iter != tracks_map.end()) {
      if (tracks_map.at(id)->d_s_rel() < -kPotensialObjectLonRange ||
          ...
          tracks_map.at(id)->frenet_l() > potensial_max_l ||
          tracks_map.at(id)->frenet_l() < 0.0) {
        continue;
      }
      ++potensial_counter;
  ...
  std::vector<std::shared_ptr<FrenetObstacle>>
      current_lane_cone_distribution_set;
  std::vector<std::shared_ptr<FrenetObstacle>>
      left_boundary_cone_distribution_set;
  std::vector<std::shared_ptr<FrenetObstacle>>
      left_boundary_cone_distribution_ordered_set;
  ...
  for (const auto& obj : ego_front_target_tracks) {
    if (obj->type() == iflyauto::OBJECT_TYPE_TRAFFIC_CONE) {
      ...
      if ((obj->frenet_l() > kLaneConeRangeThres) &&
          std::fabs(obj->frenet_l() - lane_width * 0.5) <
              kLaneBoundaryConeRangeThres) {
...
bool OvertakeRequest::checkRightLaneChangeValidByObjects(
  ...
  for (const auto& id : right_potensial_objects) {
    ...
    if (iter != tracks_map.end()) {
      if (tracks_map.at(id)->d_s_rel() < -kPotensialObjectLonRange ||
          tracks_map.at(id)->d_s_rel() > kPotensialObjectLonRange ||
          tracks_map.at(id)->frenet_l() < potensial_min_l ||
          tracks_map.at(id)->frenet_l() > 0.0) {
        continue;
      }
      ++potensial_counter;
  ...
  std::vector<std::shared_ptr<FrenetObstacle>>
      current_lane_cone_distribution_set;
  std::vector<std::shared_ptr<FrenetObstacle>>
      right_boundary_cone_distribution_set;
  std::vector<std::shared_ptr<FrenetObstacle>>
      right_boundary_cone_distribution_ordered_set;
...
bool OvertakeRequest::checkLaneChangeValidBySuprsSignal(const bool is_left) {
  ...
  for (const auto& id : target_side_objects_id_set) {
    ...
    const double dis =
        std::hypot(tracks_map.at(id)->obstacle()->x_center() - ego_pose.x,
                   tracks_map.at(id)->obstacle()->y_center() - ego_pose.y);
  ...
  *rear_required_space = 0.0;
  ...
  if (has_front_obs) {
    ...
    const bool front_safety_at_present = checkFrontSafetyAtPresent(
        ...
        kDecInCheckLaneChangeSafety,
        front_track_iter->second->obstacle()->acceleration(), ego_front_edge,
    ...
    if (!front_safety_at_present) {
      ...
      ILOG_DEBUG << "checkLaneChangeSafety(): Not safety for front vehicle at "
                    "present!";
      return false;
    }
  ...
  if (has_rear_obs) {
    ...
    const bool rear_safety_at_present = checkRearSafetyAtPresent(
        ...
        kDecInCheckLaneChangeSafety,
        rear_track_iter->second->obstacle()->acceleration(), ego_rear_edge,
...
void OvertakeRequest::selectTargetObstacleIds(
  ...
  auto isTruck = [&](const std::shared_ptr<FrenetObstacle>& veh) -> bool {
    return veh->type() == iflyauto::OBJECT_TYPE_TRUCK &&
           veh->length() > kMinLengthForTruck &&
           veh->width() > kMinWidthForTruck;
  };
  ...
  auto getHalfWidthOnLane = [&](const std::shared_ptr<FrenetObstacle>& veh,
    ...
    double veh_theta =
        veh->obstacle()->heading_angle() - ego_state->ego_pose().theta;
  ...
  for (int i = begin_index; i != end_index; i += step) {
    ...
    const bool is_select = selectObsByLateralDistance(obs_info->frenet_l(),
                                                      veh_half_width, buffer) &&
                           is_truck;
...
bool OvertakeRequest::checkFrontSafetyAtPresent(
  ...
  ILOG_DEBUG << "ego v:" << ego_v << "front v:" << veh_v
...
bool OvertakeRequest::isCancelOverTakingLaneChange(int lc_state) {
  ...
  if (leading_vehicle_iter != tracks_map.end()) {
    ...
    const auto lane_tracks_manager =
        session_->environmental_model().get_lane_tracks_manager();
    std::vector<std::shared_ptr<planning::FrenetObstacle>>
        ego_front_vehicle_id_array;
    ego_front_vehicle_id_array = lane_tracks_manager->front_tracks_c();
    double leading_vehicle_lateral_speed =
    ...
    if (ego_front_vehicle_id_array.size() >= 1) {
      ...
      if (ego_front_vehicle_id != -1) {
        ...
        if (front_leading_vehicle_iter != tracks_map.end()) {
          ...
          ILOG_DEBUG << "ego vehicle front_leading_vehicle id:"
                     << front_leading_vehicle_iter->second->id();
        }
      }
    }
    ...
    if (((lc_state == kLaneChangePropose || lc_state == kLaneKeeping) &&
      ...
      ILOG_DEBUG << "Cancel OverTakeLaneChange Dir: " << request_type_
                 ...
                 << "Front Leading Vehicle Distance:"
                 << front_leading_vehivle_long_distance;
      return true;
    }
    ...
    if (ego_left_front_vehicle_array.size() >= 1) {
      ...
      if (left_front_leading_vehicle_iter != tracks_map.end()) {
        ...
        ILOG_DEBUG << "ego_left_front_leading_vehivle_long_distance:"
                   << ego_left_front_leading_vehivle_long_distance;
      }
    }
    if (request_type_ == LEFT_CHANGE) {
    ...
    if (request_type_ == LEFT_CHANGE &&
        target_lane_exist_slow_front_veh_frame_num_ >= 5) {
    ...
    if (ego_right_front_vehicle_array.size() >= 1) {
      ...
      if (right_front_leading_vehicle_iter != tracks_map.end()) {
        ...
        ILOG_DEBUG << "right front leading vehicle long distance:"
                   << ego_right_front_leading_vehivle_long_distance;
      }
    }
...
bool OvertakeRequest::FeasibleLaneDistanceEnoughJudgment(
  ...
  double target_v = std::min(ego_state->ego_v_cruise(), lane_traffic_speed);
...
void OvertakeRequest::IsTargetLaneExistTruck(
  ...
  if (!target_lane_coord_ptr->XYToSL(agent_point, agent_frenet_point)) {
    ILOG_DEBUG << "ego on ref lane failed!";
    return;
  }

  // 与自车的纵向距离
  double d_s_rel = agent_frenet_point.x -
                   target_ref_line->get_frenet_ego_state().s() -
                   0.5 * vehicle_param.length - 0.5 * agent->length();
  ...
  // 计算侵入距离
  Point2D box_point;
  Point2D box_frenet_point;
  for (const auto& corner : agent->box().GetAllCorners()) {
    box_point.x = corner.x();
    box_point.y = corner.y();
    double point_intrusion_distance = 0.0;
    if (target_lane_coord_ptr->XYToSL(box_point, box_frenet_point)) {
      if (is_left) {
    ...
    if (point_intrusion_distance > intrusion_distance) {
      intrusion_distance = point_intrusion_distance;
    }
  }
  ...
  double extra_buffer_for_truck = -0.1;
  double extra_buffer_for_vru = -0.1;
  double lead_d_path_thr = base_safe_intrusoin_for_dynamic;

  // 障碍物类型
  if (agent->is_truck()) {
    lead_d_path_thr += extra_buffer_for_truck;
  } else if (agent->is_vru()) {
    lead_d_path_thr += extra_buffer_for_vru;
  }

  // 车道宽的影响,同样倾入距离的情况下，窄车道更容易跟车
  double lane_width_actor =
      interp(target_lane->width(), lane_width_bp_, lane_width_factor_);
  lead_d_path_thr = lead_d_path_thr * lane_width_actor;

  if (intrusion_distance >= lead_d_path_thr) {
    is_need_folow = true;
    if (agent->speed() < target_lane_exist_truck_speed) {
      target_lane_exist_truck_speed = agent->speed();
    }
  } else {
    // 针对cut_out或者横穿的障碍物，即远离自车道的障碍物，v_lat > 0,
    // 需要进一步优化 std::array<double, 5> x_cut_factor{0.2, 0.3, 0.4, 0.6,
    // 0.8}; std::array<double, 5> f_cut_factor{0, 0, 2, 6, 11}; double
    // cut_factor = interp(v_lat, x_cut_factor, f_cut_factor); follow_confidence
    // = std::fmax(
