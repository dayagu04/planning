...
bool MapRequest::CheckMLCEnable(const int lc_status) {
  ...
  const int current_lane_virtual_id = virtual_lane_mgr_->current_lane_virtual_id();
  ...
  const RequestType request_type = lc_map_decision < 0 ? LEFT_CHANGE : RIGHT_CHANGE;

  //2、当前状态可以生成变道请求，否则返回false
  const bool allow_generate =
  ...
  //4、判断虚线长度是否满足变道条件
  const RequestType target_direction = lc_map_decision < 0 ? LEFT_CHANGE : RIGHT_CHANGE;
  bool is_dash_enough = 
      IsDashEnoughForRepeatSegments(target_direction,
...
bool MapRequest::CheckTargetLaneLaneMarks(RequestType request_type) {
  ...
  target_path = reference_path_manager->
      get_reference_path_by_lane(target_lane->get_virtual_id());
  if (!target_path) {
    return false;
  }
  ...
  for (int i = 0; i < target_lane_marks.size(); ++i) {
    ...
    if (mark.begin < ego_s_on_target_lane &&
        mark.end > ego_s_on_target_lane) {
      seg_index = i;
      break;
    }
  }
...
bool MapRequest::CheckTargetLaneMergeDirection(RequestType request_type) {
  const auto& ego_lane_road_right_decider_output = session_->planning_context().ego_lane_road_right_decider_output();
  bool is_merge_region = ego_lane_road_right_decider_output.is_merge_region;
  bool cur_lane_is_continue = ego_lane_road_right_decider_output.cur_lane_is_continue;
  int merge_lane_virtual_id = ego_lane_road_right_decider_output.merge_lane_virtual_id;
