...
struct JointDecisionTrajectoryPoint {
  double t = 0.0;
  double s = 0.0;
  double x = 0.0;
  double y = 0.0;
...
struct LaneChangeDecisionInfo {
  int gap_front_agent_id = -1;
  int gap_rear_agent_id = -1;
  int origin_agent_id = -1;
  std::vector<TrajectoryPoint> ego_ref_traj;
  int target_lane_virtual_id = -100;
  int origin_lane_virtual_id = -100;  // just display
  // road
};
struct LaneChangeEgoTrajectory {
  std::vector<JointDecisionTrajectoryPoint> trajectory_points;
  ...
  JointDecisionTrajectoryPoint& operator[](size_t index) {
    return trajectory_points[index];
  }

  const JointDecisionTrajectoryPoint& operator[](size_t index) const {
    return trajectory_points[index];
  }
};

class LatLonJointDecisionOutput {
 public:
  LatLonJointDecisionOutput() = default;
  ~LatLonJointDecisionOutput() = default;

  const LaneChangeEgoTrajectory& GetLaneChangeEgoTrajectory() const {
    return ego_trajectory_;
  }
  LaneChangeEgoTrajectory& GetLaneChangeEgoTrajectory() {
    return ego_trajectory_;
  }
