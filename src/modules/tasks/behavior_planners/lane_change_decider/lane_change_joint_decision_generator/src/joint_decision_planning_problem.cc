...
void JointDecisionPlanningProblem::Init() {
  ...
  ilqr_core_ptr_->Init(std::make_shared<JointDecisionPlanningModel>(),
                       solver_config);
...
void JointDecisionPlanningProblem::SetBoundaryPaths(
...
uint8_t JointDecisionPlanningProblem::Update(
  ...
  ilqr_core_ptr_->SetDuTol(min_tol);

  ilqr_core_ptr_->SetStateAndInputSize(
      EGO_STATE_SIZE + OBS_STATE_SIZE * obs_num_,
      EGO_CONTROL_SIZE + OBS_CONTROL_SIZE * obs_num_);
  init_state_.resize(EGO_STATE_SIZE + OBS_STATE_SIZE * obs_num_);
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    cost_config_vec.at(i)[W_OBS_REF_Y] =
        planning_input.q_obs_ref_y() * obs_ref_decay_factor;
    cost_config_vec.at(i)[W_OBS_REF_THETA] =
        planning_input.q_obs_ref_theta() * obs_ref_decay_factor;
    cost_config_vec.at(i)[W_OBS_REF_DELTA] =
        planning_input.q_obs_ref_delta() * obs_ref_decay_factor;
    cost_config_vec.at(i)[W_OBS_REF_VEL] =
        planning_input.q_obs_ref_vel() * obs_ref_decay_factor;
    cost_config_vec.at(i)[W_OBS_REF_ACC] =
        planning_input.q_obs_ref_acc() * obs_ref_decay_factor;
    ...
    double obs_jerk_decay_factor = std::exp(-obs_jerk_decay_rate * i);
    cost_config_vec.at(i)[W_OBS_JERK] =
        planning_input.q_obs_jerk_weight() * obs_jerk_decay_factor;

    double obs_omega_decay_rate = 0.05;
    ...
    if (i >= 3) {
      ...
      const double peak_timestep = 3 + (N - 3) * peak_ratio;
      const double sigma = (N - 3) * 0.25;
      const double normalized_distance = (i - peak_timestep) / sigma;
      hard_halfplane_bound_decay =
          std::exp(-0.5 * normalized_distance * normalized_distance);
    }
  ...
  init_state_.block(0, 0, EGO_STATE_SIZE, 1)
      << planning_input.ego_init_state().x(),
  ...
  for (size_t i = 0; i < obs_num_; ++i) {
    init_state_.block(EGO_STATE_SIZE + OBS_STATE_SIZE * i, 0, OBS_STATE_SIZE, 1)
        << planning_input.obs_init_state(i).x(),
        planning_input.obs_init_state(i).y(),
        planning_input.obs_init_state(i).theta(),
        planning_input.obs_init_state(i).delta(),
        planning_input.obs_init_state(i).vel(),
        planning_input.obs_init_state(i).acc();
  }
  ...
  if (solver_condition >= iLqr::BACKWARD_PASS_FAIL) {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      u.resize(EGO_CONTROL_SIZE + OBS_CONTROL_SIZE * obs_num_);
      u.setZero();
      u_vec_[i] = u;
    }
