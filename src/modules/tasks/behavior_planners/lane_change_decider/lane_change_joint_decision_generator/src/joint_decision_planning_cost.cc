...
double ObsReferenceCostTerm::GetCost(const ilqr_solver::State& x,
  ...
  for (int i = 0; i < obs_num; ++i) {
    ...
    const int ref_delta_idx = GetObsRefStateIdx(i, obs_num, OBS_DELTA);
    const int ref_vel_idx = GetObsRefStateIdx(i, obs_num, OBS_VEL);
    const int ref_acc_idx = GetObsRefStateIdx(i, obs_num, OBS_ACC);

    obs_cost += 0.5 * cost_config_ptr_->at(W_OBS_REF_X) *
                (x[state_base_idx + OBS_X] - cost_config_ptr_->at(ref_x_idx)) *
                (x[state_base_idx + OBS_X] - cost_config_ptr_->at(ref_x_idx));
    obs_cost += 0.5 * cost_config_ptr_->at(W_OBS_REF_Y) *
                (x[state_base_idx + OBS_Y] - cost_config_ptr_->at(ref_y_idx)) *
                (x[state_base_idx + OBS_Y] - cost_config_ptr_->at(ref_y_idx));
    obs_cost +=
        0.5 * cost_config_ptr_->at(W_OBS_REF_THETA) *
        (x[state_base_idx + OBS_THETA] - cost_config_ptr_->at(ref_theta_idx)) *
        (x[state_base_idx + OBS_THETA] - cost_config_ptr_->at(ref_theta_idx));
    obs_cost +=
        0.5 * cost_config_ptr_->at(W_OBS_REF_DELTA) *
        (x[state_base_idx + OBS_DELTA] - cost_config_ptr_->at(ref_delta_idx)) *
        (x[state_base_idx + OBS_DELTA] - cost_config_ptr_->at(ref_delta_idx));
    obs_cost +=
        0.5 * cost_config_ptr_->at(W_OBS_REF_VEL) *
        (x[state_base_idx + OBS_VEL] - cost_config_ptr_->at(ref_vel_idx)) *
        (x[state_base_idx + OBS_VEL] - cost_config_ptr_->at(ref_vel_idx));
    obs_cost +=
        0.5 * cost_config_ptr_->at(W_OBS_REF_ACC) *
        (x[state_base_idx + OBS_ACC] - cost_config_ptr_->at(ref_acc_idx)) *
        (x[state_base_idx + OBS_ACC] - cost_config_ptr_->at(ref_acc_idx));
  }
  return obs_cost;
}
...
void ObsReferenceCostTerm::GetGradientHessian(
  ...
  for (int i = 0; i < obs_num; ++i) {
    ...
    lx(state_base_idx + OBS_X) +=
        cost_config_ptr_->at(W_OBS_REF_X) *
        (x[state_base_idx + OBS_X] - cost_config_ptr_->at(ref_x_idx));
    lx(state_base_idx + OBS_Y) +=
        cost_config_ptr_->at(W_OBS_REF_Y) *
        (x[state_base_idx + OBS_Y] - cost_config_ptr_->at(ref_y_idx));
    lx(state_base_idx + OBS_THETA) +=
        cost_config_ptr_->at(W_OBS_REF_THETA) *
        (x[state_base_idx + OBS_THETA] - cost_config_ptr_->at(ref_theta_idx));
    lx(state_base_idx + OBS_DELTA) +=
        cost_config_ptr_->at(W_OBS_REF_DELTA) *
        (x[state_base_idx + OBS_DELTA] - cost_config_ptr_->at(ref_delta_idx));
    lx(state_base_idx + OBS_VEL) +=
        cost_config_ptr_->at(W_OBS_REF_VEL) *
        (x[state_base_idx + OBS_VEL] - cost_config_ptr_->at(ref_vel_idx));
    lx(state_base_idx + OBS_ACC) +=
        cost_config_ptr_->at(W_OBS_REF_ACC) *
        (x[state_base_idx + OBS_ACC] - cost_config_ptr_->at(ref_acc_idx));
    ...
    lxx(state_base_idx + OBS_Y, state_base_idx + OBS_Y) +=
        cost_config_ptr_->at(W_OBS_REF_Y);
    lxx(state_base_idx + OBS_THETA, state_base_idx + OBS_THETA) +=
        cost_config_ptr_->at(W_OBS_REF_THETA);
    lxx(state_base_idx + OBS_DELTA, state_base_idx + OBS_DELTA) +=
        cost_config_ptr_->at(W_OBS_REF_DELTA);
    lxx(state_base_idx + OBS_VEL, state_base_idx + OBS_VEL) +=
        cost_config_ptr_->at(W_OBS_REF_VEL);
    lxx(state_base_idx + OBS_ACC, state_base_idx + OBS_ACC) +=
        cost_config_ptr_->at(W_OBS_REF_ACC);
  }
}
...
EgoThreeDiscSafeCostTerm::CalculateThreeDiscDistances(
  ...
  for (int i = 0; i < obs_num; ++i) {
    const int state_base_idx = EGO_STATE_SIZE + i * OBS_STATE_SIZE;
    double obs_x = x[state_base_idx + OBS_X];
    double obs_y = x[state_base_idx + OBS_Y];
    double obs_theta = x[state_base_idx + OBS_THETA];
...
double ObsJerkCostTerm::GetCost(const ilqr_solver::State& x,
  ...
  for (int i = 0; i < obs_num; ++i) {
    const int control_base_idx = EGO_CONTROL_SIZE + i * OBS_CONTROL_SIZE;
    double obs_jerk = u[control_base_idx + OBS_JERK];
    double weight = cost_config_ptr_->at(W_OBS_JERK);
    obs_jerk_cost += 0.5 * weight * obs_jerk * obs_jerk;
  }

  return obs_jerk_cost;
}
...
void ObsJerkCostTerm::GetGradientHessian(
  ...
  for (int i = 0; i < obs_num; ++i) {
    ...
    lu(control_base_idx + OBS_JERK) += weight * obs_jerk;
    luu(control_base_idx + OBS_JERK, control_base_idx + OBS_JERK) += weight;
  }
}
...
double ObsOmegaCostTerm::GetCost(const ilqr_solver::State& x,
  ...
  for (int i = 0; i < obs_num; ++i) {
    const int control_base_idx = EGO_CONTROL_SIZE + i * OBS_CONTROL_SIZE;
    double obs_omega = u[control_base_idx + OBS_OMEGA];
    double weight = cost_config_ptr_->at(W_OBS_OMEGA);
    obs_omega_cost += 0.5 * weight * obs_omega * obs_omega;
  }

  return obs_omega_cost;
}
...
void ObsOmegaCostTerm::GetGradientHessian(
  ...
  for (int i = 0; i < obs_num; ++i) {
    ...
    lu(control_base_idx + OBS_OMEGA) += weight * obs_omega;
    luu(control_base_idx + OBS_OMEGA, control_base_idx + OBS_OMEGA) += weight;
  }
}
...
void HardHalfplaneCostTerm::GetGradientHessian(
  ...
  for (const auto& result : results) {
    ...
    if (label_type == 1) {
      ...
      // lxx(EGO_X, state_base_idx + OBS_X) +=
      //     cross_weight * hess_coeff * ddist_dego_x * ddist_dobs_x;
      // lxx(state_base_idx + OBS_X, EGO_X) +=
      //     cross_weight * hess_coeff * ddist_dobs_x * ddist_dego_x;
      // lxx(EGO_X, state_base_idx + OBS_Y) +=
      //     cross_weight * hess_coeff * ddist_dego_x * ddist_dobs_y;
      // lxx(state_base_idx + OBS_Y, EGO_X) +=
      //     cross_weight * hess_coeff * ddist_dobs_y * ddist_dego_x;
      // lxx(EGO_Y, state_base_idx + OBS_X) +=
      //     cross_weight * hess_coeff * ddist_dego_y * ddist_dobs_x;
      // lxx(state_base_idx + OBS_X, EGO_Y) +=
      //     cross_weight * hess_coeff * ddist_dobs_x * ddist_dego_y;
      // lxx(EGO_Y, state_base_idx + OBS_Y) +=
      //     cross_weight * hess_coeff * ddist_dego_y * ddist_dobs_y;
...
SoftHalfplaneCostTerm::CalculateSoftHalfplane(const ilqr_solver::State& x) {
  ...
  for (int i = 0; i < obs_num; ++i) {
    ...
    const double obs_y = x[state_base_idx + OBS_Y];
    const double obs_theta = x[state_base_idx + OBS_THETA];
    const double obs_vel = x[state_base_idx + OBS_VEL];
    const double obs_length = cost_config_ptr_->at(GetObsLengthIdx(i, obs_num));
...
void SoftHalfplaneCostTerm::GetGradientHessian(
  ...
  for (const auto& result : results) {
    ...
    // const int label_type = result.label_type;
    // const int label_type = alpha > 0.99 ? 0 : result.label_type;// 反应时间内主要靠自车
    ...
    if (label_type == 1) {
      ...
      //
      // ∂violation/∂ego_x = +ego_normal_x
      // ∂violation/∂ego_y = +ego_normal_y
      // ∂violation/∂ego_vel = -(-v_obs*k) = v_obs*k  [来自s_target]
      // ∂violation/∂obs_x = -ego_normal_x
      // ∂violation/∂obs_y = -ego_normal_y
      // ∂violation/∂obs_vel = -(tau + 2*k*v_obs - k*v_ego)  [来自s_target]

      const int obs_state_idx = state_base_idx;
      const double ego_vel = x[EGO_VEL];
      ...
      // 速度交叉Hessian: ∂²violation/(∂v_ego·∂v_obs)
      // ∂(v_obs*k)/∂v_obs = k, ∂(-(tau + 2kv_obs - kv_ego))/∂v_ego = k
      // 所以交叉项 = k
      // if (s_target_term > 0.0) {
      //   const double vel_cross_hess = hess_coeff * k;
      //   lxx(EGO_VEL, obs_state_idx + OBS_VEL) +=
      //       ego_weight * obs_weight * vel_cross_hess;
      //   lxx(obs_state_idx + OBS_VEL, EGO_VEL) +=
      //       ego_weight * obs_weight * vel_cross_hess;
      // }
      ...
      // ∂violation/∂ego_x = -ego_normal_x
      // ∂violation/∂ego_y = -ego_normal_y
      // ∂violation/∂ego_vel = -(tau + 2*k*v_ego - k*v_obs)  [来自s_target]
      // ∂violation/∂obs_x = +ego_normal_x
      // ∂violation/∂obs_y = +ego_normal_y
      // ∂violation/∂obs_vel = -(-v_ego*k) = v_ego*k  [来自s_target]
      ...
      // 障碍物梯度
      // lx(obs_state_idx + OBS_X) += obs_weight * gradient_coeff *
      // ddist_dobs_x; lx(obs_state_idx + OBS_Y) += obs_weight * gradient_coeff
      // * ddist_dobs_y; lx(obs_state_idx + OBS_VEL) += obs_weight *
      // gradient_coeff * ddist_dobs_vel;
      ...
      // 速度交叉Hessian: ∂²violation/(∂v_ego·∂v_obs)
      // ∂(-(tau + 2kv_ego - kv_obs))/∂v_obs = k, ∂(v_ego*k)/∂v_ego = k
      // 但注意符号: ∂²s_target/(∂v_ego·∂v_obs) = -k
      // 所以 ∂²violation/(∂v_ego·∂v_obs) = -∂²s_target/(∂v_ego·∂v_obs) = k
      // if (s_target_term > 0.0) {
      //   const double vel_cross_hess = -hess_coeff * k;
      //   lxx(EGO_VEL, obs_state_idx + OBS_VEL) +=
      //       ego_weight * obs_weight * vel_cross_hess;
