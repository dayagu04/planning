...
bool CheckClustersConsecutiveDiffSlidingWindowImpl(
  ...
  for (const auto & [ cluster_id, construction_area ] : cluster_map) {
    ...
    for (size_t i = 0; i + 2 < ls.size(); ++i) {
      ...
      if (diff01 > JointDecisionSpeedLimit::kCAInvadeLatDisDiffThr &&
          diff12 > JointDecisionSpeedLimit::kCAInvadeLatDisDiffThr) {
        ...
        if (entering && abs_l0 > abs_l1 && abs_l1 > abs_l2 &&
            abs_l0 < JointDecisionSpeedLimit::kCAInvadeLatMaxDis) {
          return true;
        }
        if (!entering && abs_l0 < abs_l1 && abs_l1 < abs_l2 &&
            abs_l0 < JointDecisionSpeedLimit::kCAInvadeLatMinDis) {
          return true;
        }
      }
    }
...
JointDecisionSpeedLimit::JointDecisionSpeedLimit(
...
JointDecisionSpeedLimit::Result JointDecisionSpeedLimit::CalculateSpeedLimit() {
...
bool JointDecisionSpeedLimit::IsSSharpBend(
...
double JointDecisionSpeedLimit::JudgeCurvBySDProMap() const {
...
void JointDecisionSpeedLimit::CalculateCurveSpeedLimit() {
  ...
  std::vector<JointDecisionCurvInfo> preview_curv_info_vec;
  for (int idx = 0; idx * 2.0 < preview_x; idx++) {
    JointDecisionCurvInfo one_curv_info;
...
void JointDecisionSpeedLimit::CalculateMapSpeedLimit() {
  const auto& environmental_model = session_->environmental_model();
...
void JointDecisionSpeedLimit::CalculateIntersectionSpeedLimit() {
  const auto& environmental_model = session_->environmental_model();
  const auto ego_state_mgr = environmental_model.get_ego_state_manager();
  double v_ego = ego_state_mgr->ego_v();
...
void JointDecisionSpeedLimit::CalculatePOISpeedLimit() {
  const auto& environmental_model = session_->environmental_model();
  const auto& route_info_output =
      environmental_model.get_route_info()->get_route_info_output();
...
void JointDecisionSpeedLimit::CalculateSpeedLimitFromTFLDis() {
...
bool JointDecisionSpeedLimit::CheckClustersConsecutiveDiffSlidingWindow(
    const std::map<int, ConstructionAgentClusterArea>& cluster_map,
    const std::shared_ptr<planning_math::KDPath>& planned_kd_path,
    bool entering) const {
...
void JointDecisionSpeedLimit::CalculateConstructionZoneSpeedLimit() {
...
void JointDecisionSpeedLimit::CalculateFunctionFadingAwaySpeedLimit() {
