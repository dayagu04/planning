...
void JointDecisionObstaclesSelector::SelectLaneChangeObstacles(
  ...
  if (lc_info.gap_rear_agent_id != -1) {
    const auto* rear_agent = agent_manager->GetAgent(lc_info.gap_rear_agent_id);
  ...
  if (lc_info.gap_front_agent_id != -1) {
    const auto* front_agent =
        agent_manager->GetAgent(lc_info.gap_front_agent_id);
    if (front_agent != nullptr) {
      const auto& all_agents = agent_manager->GetAllCurrentAgents();
      for (const auto& agent : all_agents) {
        if (agent != nullptr &&
            agent->agent_id() == lc_info.gap_front_agent_id) {
...
void JointDecisionObstaclesSelector::SelectObstacles(
  ...
  key_obstacles_.erase(
      std::remove_if(key_obstacles_.begin(), key_obstacles_.end(),
                     [](const LaneChangeKeyObstacle& obs) {
                       return obs.init_s == std::numeric_limits<double>::max();
                     }),
      key_obstacles_.end());
}

void JointDecisionObstaclesSelector::CalculateAgentSLBoundary(
...
bool JointDecisionObstaclesSelector::JudgeOverlapWithPriorTrajectory(
  ...
  for (double relative_time = 0.0; relative_time <= kMaxTime;
    ...
    double longitudinal_threshold = kBaseLongitudinalThreshold + ego_vel * 0.3;
...
LaneChangeKeyObstacle JointDecisionObstaclesSelector::CreateKeyObstacle(
