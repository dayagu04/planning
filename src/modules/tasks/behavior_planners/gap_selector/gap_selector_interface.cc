...
void GapSelectorInterface::Store(
    const planning::framework::Frame *frame,
  ...
  auto &ref_path_mgr =
      frame->session()->environmental_model().get_reference_path_manager();
  auto &ego_state_mgr =
  ...
  // -------------------------- lane id
  gap_selector_input->set_origin_lane_id(
      frame->session()
          ->planning_context()
          .lat_behavior_state_machine_output()
          .origin_lane_virtual_id);
  gap_selector_input->set_target_lane_id(
      frame->session()
          ->planning_context()
          .lat_behavior_state_machine_output()
          .target_lane_virtual_id);
  gap_selector_input->set_current_lane_id(frame->session()
                                              ->environmental_model()
  ...
  //---------------------------- planning init state
  //-----------------------------------------
  gap_selector_input->set_cruise_vel(frame->session()
                                         ->environmental_model()
  ...
  auto virtual_lane_mgr =
      frame->session()->environmental_model().get_virtual_lane_manager();
  ...
  auto target_state =
      pipeline_context->coarse_planning_info.target_state == ROAD_LC_LCHANGE
          ? 1
          : (pipeline_context->coarse_planning_info.target_state ==
                     ROAD_LC_RCHANGE
                 ? 2
                 : 0);
  ...
  const auto map_gs_care_obstacles = frame->session()
                                         ->environmental_model()
  ...
  const auto map_origin_lane_obstacles =
      frame->session()
          ->environmental_model()
          .get_reference_path_manager()
          ->get_reference_path_by_lane(frame->session()
                                           ->environmental_model()
                                           .get_virtual_lane_manager()
                                           ->get_current_lane()
                                           ->get_virtual_id(),
...
void GapSelectorInterface::ReplayCollect(
    ...
    const GapSelectorStatus gap_status) {
  ...
  for (auto i = 0; i < front_gap_car_st_boundaries.size(); i++) {
    ...
    obj_pred_point->set_s(front_gap_car_st_boundaries[i].second.s());
    obj_pred_point->set_t(front_gap_car_st_boundaries[i].second.t());
  }
  // rear gap pred objs
  for (auto i = 0; i < rear_gap_car_st_boundaries.size(); i++) {
    planning::common::ObstaclePredicatedPoint *obj_pred_point =
        gap_selector_replay_info
            ...
            ->Add();
    obj_pred_point->set_s(rear_gap_car_st_boundaries[i].first.s());
    obj_pred_point->set_t(rear_gap_car_st_boundaries[i].first.t());
  }
