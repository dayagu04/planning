...
class GapSelector : public Task {
 public:
  GapSelector() {
    path_spline_.path_spline_status = GapSelectorPathSpline::NO_VALID;
  };
  explicit GapSelector(
      const EgoPlanningConfigBuilder *config_builder,
      const std::shared_ptr<TaskPipelineContext> &pipeline_context);
  virtual ~GapSelector() = default;

  bool Execute(planning::framework::Frame *frame) override;
  GapSelectorStatus Update(GapSelectorResult &gap_selector_result);

  void Preprocessor(planning::framework::Frame *frame,
                    std::shared_ptr<TaskPipelineContext> pipeline_context);
  GapSelectorStatus EnvHandle(const int target_state,
                              const Point2D &ego_cart_pose,
