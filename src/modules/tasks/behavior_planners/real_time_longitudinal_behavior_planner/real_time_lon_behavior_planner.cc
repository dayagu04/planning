...
void RealTimeLonBehaviorPlanner::Init() {
  ...
  // init v_cruise_slope_filter
  v_cruise_filter_.Init(config_.cruise_set_dec, config_.cruise_set_acc, 0.0,
                        config_.velocity_upper_bound, 0.1);
}
...
bool RealTimeLonBehaviorPlanner::Calculate() {
  ...
  LOG_DEBUG("RealTimeLonBehaviorPlanner cost time: [%f]ms \n",
            end_time - start_time);
  return true;
}
...
void RealTimeLonBehaviorPlanner::ConstructLonBehavInput() {
  ...
  const auto ego_v_cruise = ego_state_mgr->ego_v_cruise();
  const auto ego_v = ego_state_mgr->ego_v();
  // filter the v_cruise from HMI
  v_cruise_filter_.SetState(lon_decision_info.v_cruise());
  if (!dbw_status) {
    v_cruise_filter_.SetState(ego_v);
  } else {
    v_cruise_filter_.Update(ego_v_cruise);
  }
  lon_decision_info.set_v_cruise(v_cruise_filter_.GetOutput());

  auto ego_v_cruise_clip = lon_decision_info.v_cruise();
  auto ego_info = lon_behav_plan_input_->mutable_ego_info();
  ego_info->set_ego_v(ego_state_mgr->ego_v());
  ego_info->set_ego_cruise(std::max(ego_v_cruise_clip, 0.0));
