...
void SoftHalfplaneCostTerm::GetGradientHessian(
  ...
  for (const auto &result : results) {
    ...
    if (label_type == 1) {
      // OVERTAKE: s_current = (ego_rear - obs_front) · ego_normal
      // ∂s/∂ego_x = ego_normal_x
      // ∂s/∂ego_y = ego_normal_y
      // ∂s/∂ego_vel = -tau (影响s_target)
      // ∂s/∂obs_x = -ego_normal_x
      // ∂s/∂obs_y = -ego_normal_y
      ...
      // YIELD: s_current = (obs_rear - ego_front) · ego_normal
      // ∂s/∂ego_x = -ego_normal_x
      // ∂s/∂ego_y = -ego_normal_y
      // ∂s/∂obs_x = ego_normal_x
      // ∂s/∂obs_y = ego_normal_y
      // ∂s/∂obs_vel = -tau (影响s_target)
      ...
      const double ddist_dobs_x = normal_x;
      const double ddist_dobs_y = normal_y;
      const double ddist_dobs_vel = -tau;
      ...
      lxx(state_base_idx + OBS_Y, state_base_idx + OBS_X) +=
          obs_weight * hess_coeff * ddist_dobs_y * ddist_dobs_x;
      lxx(state_base_idx + OBS_X, state_base_idx + OBS_VEL) +=
          obs_weight * hess_coeff * ddist_dobs_x * ddist_dobs_vel;
      lxx(state_base_idx + OBS_VEL, state_base_idx + OBS_X) +=
          obs_weight * hess_coeff * ddist_dobs_vel * ddist_dobs_x;
      lxx(state_base_idx + OBS_Y, state_base_idx + OBS_VEL) +=
          obs_weight * hess_coeff * ddist_dobs_y * ddist_dobs_vel;
      lxx(state_base_idx + OBS_VEL, state_base_idx + OBS_Y) +=
          obs_weight * hess_coeff * ddist_dobs_vel * ddist_dobs_y;
