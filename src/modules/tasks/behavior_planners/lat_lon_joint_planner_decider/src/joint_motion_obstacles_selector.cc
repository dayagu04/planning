...
void JointMotionObstaclesSelector::SelectObstacles(
  ...
  // Stage 1: Add Lead One obstacle
  if (lead_one_id != -1) {
    for (const auto& agent : agents) {
      if (agent != nullptr && agent->agent_id() == lead_one_id) {
        key_obstacles_.emplace_back(
            CreateKeyObstacle(agent, ego_lane_coord, YIELD));
        break;
      }
    }
  }

  // Stage 2: Filter agents by distance range
  ...
  // Stage 3: Classify agents by prior trajectory overlap
  ...
  // Stage 4: Sort agents and select obstacles
  std::sort(front_agents.begin(), front_agents.end(),
            [&ego_lane_coord](const std::shared_ptr<agent::Agent>& a,
                              const std::shared_ptr<agent::Agent>& b) {
              double a_s = 0.0, a_l = 0.0, b_s = 0.0, b_l = 0.0;
              bool a_valid = ego_lane_coord->XYToSL(a->x(), a->y(), &a_s, &a_l);
              bool b_valid = ego_lane_coord->XYToSL(b->x(), b->y(), &b_s, &b_l);
              return (a_valid && b_valid) ? (a_s < b_s) : a_valid;
            });

  std::sort(side_agents.begin(), side_agents.end(),
            [&ego_lane_coord, ego_l](const std::shared_ptr<agent::Agent>& a,
                                     const std::shared_ptr<agent::Agent>& b) {
              double a_s = 0.0, a_l = 0.0, b_s = 0.0, b_l = 0.0;
              bool a_valid = ego_lane_coord->XYToSL(a->x(), a->y(), &a_s, &a_l);
              bool b_valid = ego_lane_coord->XYToSL(b->x(), b->y(), &b_s, &b_l);
              return (a_valid && b_valid)
                         ? (std::abs(a_l - ego_l) < std::abs(b_l - ego_l))
                         : a_valid;
            });

  auto add_obstacle = [&](const std::shared_ptr<agent::Agent>& agent) {
    if (agent == nullptr) {
      return;
    }
  ...
  int front_count = 0;
  for (const auto& agent : front_agents) {
  ...
  int side_count = 0;
  for (const auto& agent : side_agents) {
  ...
  // Stage 5: Clean up invalid obstacles
...
bool JointMotionObstaclesSelector::JudgeOverlapWithPriorTrajectory(
  ...
  for (double relative_time = 0.0; relative_time <= kMaxTime;
    ...
    if (relative_time < kEpsilon) {
      constexpr double kDirectRearLatBuffer = 0.3;
      bool is_directly_behind =
          is_rear_agent && (lateral_dist <= kDirectRearLatBuffer);
      ...
      if (!is_rear_agent && longitudinal_distance > 0) {
        ...
        if (std::abs(obs_center_l) < ego_half_width) {
