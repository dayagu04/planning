...
std::unordered_map<int32_t, int32_t>
    LongRefPathDecider::upper_bound_agent_observation_count_;
...
bool LongRefPathDecider::Execute() {
  ...
  // 0. reset
  Reset();
  // 1. update upper bound agent observation
  UpdateUpperBoundAgentObservation(session_, dt_, plan_points_num_);
  // 2. calculate s_ref
  target_maker_->Run();
  // 3. calculate bound
  ...
  // 5. update lon ref path
  UpdateLonRefPath();
  // 6. save long ref path
  SaveToSession();
  SaveToDebugInfo();
...
void LongRefPathDecider::UpdateLonRefPath() {
  ...
  for (unsigned int i = 0; i < plan_points_num_; i++) {
    ...
    s_soft_bound.upper = bound_maker_->s_soft_bound(t);
