...
bool BoundMaker::JudgeUpperBoundValid() {
  const auto& agent_headway_decider_output =
      session_->planning_context().agent_headway_decider_output();
  const auto& agents_headway_map =
      agent_headway_decider_output.agents_headway_Info();
  ...
  const auto& start_stop_decider_output =
      session_->planning_context().start_stop_decider_output();
  const auto virtual_zero_acc_curve = MakeVirtualZeroAccCurve();
  const double v_ego = ego_state_mgr->ego_v();
  constexpr double kLowSpeedThr = 5.0;  // 18kph
  if (v_ego > kLowSpeedThr) {
    return false;
  }
  ...
  if (st_graph != nullptr) {
    for (double t = 0.0; t < check_front_time; t += dt_) {
      ...
      const auto& next_upper_bound =
          st_graph->GetPassCorridorUpperBound(t + dt_);
      if (current_upper_bound.agent_id() == speed::kNoAgentId) {
        is_stable_cipv = false;
        break;
      }
  ...
  constexpr double kSrefInvalidNum = 8;
  bool is_need_change_s_upper_bound = false;
  for (int32_t i = 0; i < kSrefInvalidNum; i++) {
    const double t = i * dt_;
    const double vel = virtual_zero_acc_curve->Evaluate(1, t);
    const auto& agent_id = upper_bound_infos_[i].agent_id;
    double follow_time_gap = 1.5;
    ...
    double target_s_disatnce = std::max(
        vel * follow_time_gap + min_follow_distance_m_, min_follow_distance_m_);
    if (upper_bound_infos_[i].s < target_s_disatnce) {
      is_need_change_s_upper_bound = true;
    } else {
      is_need_change_s_upper_bound = false;
      break;
    }
  }

  return is_need_change_s_upper_bound;
}

void BoundMaker::MakeMinFollowDistance() {
  const double large_vehicle_min_follow_distance =
      speed_planning_config_.large_vehicle_min_follow_distance_gap;
  double min_follow_distance_lower =
      speed_planning_config_.lower_speed_min_follow_distance_gap;
  double cone_min_follow_distance =
      speed_planning_config_.cone_min_follow_distance_gap;
  const double traffic_light_min_follow_distance_gap =
      speed_planning_config_.traffic_light_min_follow_distance_gap;
  if (upper_bound_infos_[0].agent_id != -1 &&
      upper_bound_infos_[0].is_large_vehicle) {
    min_follow_distance_lower = large_vehicle_min_follow_distance;
  }
  if (upper_bound_infos_[0].agent_id != -1 &&
      upper_bound_infos_[0].type == agent::AgentType::TRAFFIC_CONE) {
    min_follow_distance_m_ = cone_min_follow_distance;
    return;
  }
  if (upper_bound_infos_[0].agent_id != -1 &&
      upper_bound_infos_[0].is_tfl_virtual_obs) {
    min_follow_distance_lower = traffic_light_min_follow_distance_gap;
  }
  const double min_follow_distance_upper =
      speed_planning_config_.high_speed_min_follow_distance_gap;
  const double low_speed_threshold =
      speed_planning_config_.low_speed_threshold_kmph / 3.6;
  const double high_speed_threshold =
      speed_planning_config_.high_speed_threshold_kmph / 3.6;
