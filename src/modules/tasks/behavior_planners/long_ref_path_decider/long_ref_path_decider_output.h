...
struct SafetyTargetUpperBoundInfo {
  double s = 0.0;
  double t = 0.0;
  double v = 0.0;
  int32_t agent_id = -1;
...
struct LonRefPathDeciderOutput {
  ...
  bool is_safety_target_lat_follow = false;
  bool is_safety_target_lon_cutin = false;
  std::vector<int32_t> follow_agent_ids;
  std::vector<SafetyTargetUpperBoundInfo> safe_target_upper_bound_infos;
};