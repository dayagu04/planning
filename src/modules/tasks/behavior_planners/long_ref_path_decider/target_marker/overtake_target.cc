...
OvertakeTarget::OvertakeTarget(const SpeedPlannerConfig& config,
  ...
  overtake_bounds_ =
      std::vector<OvertakeBound>(plan_points_num_, OvertakeBound());
  ...
  //   std::cout << "s target value" << target_value.s_target_val() <<
  //   std::endl;
  // }
...
void OvertakeTarget::MakeOvertakeBoundsWithStCorridor() {
  for (size_t i = 0; i < plan_points_num_; ++i) {
    ...
    auto lower_bound_point = session_->planning_context()
                                 .st_graph_helper()
                                 ->GetPassCorridorLowerBound(relative_t);
...
void OvertakeTarget::MakeOvertakeTarget(const FollowTarget& follow_target) {
  for (size_t i = 0; i < plan_points_num_; ++i) {
    ...
    double time_gap_with_overtake = planning_math::LerpWithLimit(
        kMaxOvertakeTimeGap, kMinOvertakeSpeedDiff, kMinOvertakeTimeGap,
        kMaxOvertakeSpeedDiff,
        speed_diff_with_overtake_agent);  // x0 + r * (x1 - x0);
    double time_gap_with_zero_acc_speed = planning_math::LerpWithLimit(
        kMaxOvertakeTimeGap, kMinOvertakeSpeed, kMinOvertakeTimeGap,
        kMaxOvertakeSpeed, speed);

    double proper_time_gap =
        std::fmin(time_gap_with_overtake, time_gap_with_zero_acc_speed);
    double s_target_for_overtake =
        overtake_bounds_[i].s + proper_time_gap * overtake_bounds_[i].v;
    // std::cout << "relative t:" << relative_t << std::endl;
    // std::cout << "proper time gap:" << proper_time_gap << std::endl;
    // std::cout << "speed_diff_with_overtake_agent: " <<
    // speed_diff_with_overtake_agent << std::endl; std::cout <<
    // "time_gap_with_overtake:" << time_gap_with_overtake << std::endl;
    // std::cout << "time_gap_with_zero_acc_speed:" <<
    // time_gap_with_zero_acc_speed << std::endl; std::cout <<
    // "overtake_bounds_[i].s :" << overtake_bounds_[i].s << std::endl;
    // std::cout << "overtake_bounds_[i].v :" << overtake_bounds_[i].v <<
    // std::endl; std::cout << "overtake t:" << overtake_bounds_[i].t <<
    // std::endl; std::cout << "overtake s target 1:" << s_target_for_overtake
    // << std::endl; std::cout << "overtake agent id:" <<
    // overtake_bounds_[i].overtake_agent_id << std::endl;

    // if s_target_for_overtake > follow_target s value
    if (follow_target.has_target(relative_t) &&
        follow_target.target_value(relative_t).s_target_val() <
            s_target_for_overtake) {
      auto upper_bound_point = session_->planning_context()
                                   .st_graph_helper()
                                   ->GetPassCorridorUpperBound(relative_t);
      double upper_bound_s = upper_bound_point.s();
      // make small follow time gap follow target
      double follow_s_target_distance = follow_target.MakeSlowerFollowSTarget(
          speed, upper_bound_s, kLowerFollowTimeGap);
      // std::cout << "follow_s_target_distance:" << follow_s_target_distance <<
      // std::endl;

      // make 60% gap from gap
      // std::cout << "upper_bound_s:" << upper_bound_s << std::endl;
      double s_target_with_corridor =
          0.75 * upper_bound_s + 0.25 * overtake_bounds_[i].s;
      // std::cout << "s_target_with_corridor:" << s_target_with_corridor <<
      // std::endl;
      double s_target_with_follow =
          std::fmin(follow_s_target_distance, s_target_with_corridor);
      // std::cout << "s_target_with_follow:" << s_target_with_follow <<
      // std::endl;
      s_target_for_overtake =
          std::fmin(s_target_with_follow, s_target_for_overtake);
      // std::cout << "s_target_for_overtake 2:" << s_target_for_overtake <<
      // std::endl;
    }
    ...
    // std::cout << "s_target_for_overtake 3:" << s_target_for_overtake <<
    // std::endl;
