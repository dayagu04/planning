...
bool GriddedPathTimeGraph::Search(
  ...
  double longit_risk_distance_between_obstacle =
      dp_poly_path_config_.obstacle_longit_risk_distance;
  Point2D ego_cart_pose{planning_init_point_.x, planning_init_point_.y};
  if (!active) {
  ...
  if (!current_lane_coord_->XYToSL(ego_cart_pose, ego_frenet_pose_)) {
    ILOG_DEBUG << "GriddedPathTimeGraph::Search() find ego pose in current "
                  "lane failed!";
  }

  total_length_s_ = ego_frenet_pose_.x + ego_state_manager->ego_v_cruise() *
    ...
    if (lead_agent->is_static()) {
      for (const auto& agent : agent_trajs) {
        if (agent.agent_id == lead_agent->agent_id() &&
          ...
          target_s = std::min(target_s,
                              agent.max_agent_box.min_x() - kStaticAgentBuffer);
          break;
        }
      }
    }
  ...
  longit_risk_distance_between_obstacle =
      std::max(longit_risk_distance_between_obstacle,
               dp_poly_path_config_.obstacle_longit_risk_distance);
  ...
  auto ego_init_state = spatio_temporal_union_plan_input.mutable_init_state();
  ...
  dp_search_paramms->set_total_length_t(
      gridded_path_time_graph_config_.total_length_t);
  dp_search_paramms->set_unit_t(gridded_path_time_graph_config_.default_unit_t);
  dp_search_paramms->set_dense_unit_s(
      gridded_path_time_graph_config_.dense_unit_s);
  dp_search_paramms->set_sparse_unit_s(
      gridded_path_time_graph_config_.sparse_unit_s);
  dp_search_paramms->set_unit_l(dp_poly_path_config_.lateral_sample_offset);
  dp_search_paramms->set_dimension_l(
      dp_poly_path_config_.sample_points_num_each_level);
  dp_search_paramms->set_dense_dimension_s(
      gridded_path_time_graph_config_.dense_dimension_s);
  dp_search_paramms->set_max_acceleration(max_acceleration_);
  dp_search_paramms->set_max_deceleration(max_deceleration_);
  dp_search_paramms->set_enable_use_parallel_calculate_cost(
      gridded_path_time_graph_config_.enable_use_parallel_calculate_cost);
  ...
  long_weight_params->set_spatial_potential_penalty(
      gridded_path_time_graph_config_.spatial_potential_penalty);
  long_weight_params->set_keep_clear_low_speed_penalty(
      gridded_path_time_graph_config_.keep_clear_low_speed_penalty);
  long_weight_params->set_exceed_speed_penalty(
      gridded_path_time_graph_config_.exceed_speed_penalty);
  long_weight_params->set_low_speed_penalty(
      gridded_path_time_graph_config_.low_speed_penalty);
  long_weight_params->set_reference_speed_penalty(
      gridded_path_time_graph_config_.reference_speed_penalty);
  long_weight_params->set_default_speed_cost(
      gridded_path_time_graph_config_.default_speed_cost);
  long_weight_params->set_accel_penalty(
      gridded_path_time_graph_config_.accel_penalty);
  long_weight_params->set_decel_penalty(
      gridded_path_time_graph_config_.decel_penalty);
  long_weight_params->set_positive_jerk_coeff(
      gridded_path_time_graph_config_.positive_jerk_coeff);
  long_weight_params->set_negative_jerk_coeff(
      gridded_path_time_graph_config_.negative_jerk_coeff);

  auto speed_limit_params =
      spatio_temporal_union_plan_input.mutable_speed_limit_params();
  speed_limit_params->Clear();
  speed_limit_params->set_straight_ramp_v_limit(
      speed_limit_config_.straight_ramp_v_limit_low);
  speed_limit_params->set_v_limit_near_ramp_zone(
      speed_limit_config_.v_limit_near_ramp_zone);
  speed_limit_params->set_brake_dis_near_ramp_zone(
      speed_limit_config_.brake_dis_near_ramp_zone);
  speed_limit_params->set_dis_near_ramp_zone(
      speed_limit_config_.dis_near_ramp_zone);
