...
public:
    ...
    double GetCost(const StPoint& point) const {
        if (point.t >= st_start.t && point.t <= st_end.t &&
            point.s >= st_start.s && point.s <= st_end.s) {
            return INF;  // 碰撞
        }
...
public:
    ...
    // 获取可达的s索引范围
    void GetReachableSRange(int time_idx, int current_s_idx,
                           int& min_s_idx, int& max_s_idx) const {
    ...
    void DynamicProgramming(double target_s) {
        ...
        for (size_t i = 0; i < num_time_steps_ - 1; ++i) {
            ...
            for (size_t j = 0; j < num_s_samples_; ++j) {
                ...
                // 获取可达的s索引范围
                int min_k, max_k;
                GetReachableSRange(static_cast<int>(i), static_cast<int>(j),
                                 min_k, max_k);
                ...
                for (int k = min_k; k <= max_k; ++k) {
                    ...
                    // 总代价
                    double total_cost = cost_matrix_[i][j].total_cost +
                                      edge_cost + obs_cost + spatial_cost;
...
int main() {
    // 创建Cost Table
    CostTable cost_table(MAX_T, MAX_S, DELTA_T, DELTA_S);

    // 设置障碍物 (ST坐标)
    std::vector<Obstacle> obstacles = {
        Obstacle(2.0, 30.0, 4.0, 35.0),   // 障碍物1
        Obstacle(5.0, 60.0, 7.0, 65.0)    // 障碍物2
    };
    cost_table.SetObstacles(obstacles);

    // 设定目标位移
    double target_s = 150.0;

    // 执行动态规划
    cost_table.DynamicProgramming(target_s);

    // 获取最优路径
    auto optimal_path = cost_table.GetOptimalPath();

    // 输出结果
    std::cout << "Optimal Speed Profile (t, s):\n";
    for (const auto& point : optimal_path) {
        double velocity = (point.t > 0) ? point.s / point.t : 0.0;
        std::printf("t=%.1fs, s=%.2fm, v=%.2fm/s\n",
                   point.t, point.s, velocity);
    }
