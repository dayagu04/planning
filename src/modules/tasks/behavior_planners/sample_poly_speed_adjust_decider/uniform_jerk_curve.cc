...
UniformJerkCurve::UniformJerkCurve(const double s0, const double v0,
  ...
  if (std::fabs(jerk) < kZeroEpsilon || is_over_speed_range) {
    ...
    bool is_acc_enable = (a0_ < acc_limit_upper_ || jerk < -kZeroEpsilon / 2.0) &&
...
void UniformJerkCurve::BuildZeroJerkTraj(){
  if ((v0_ > vel_limit_upper_ && a0_ > kZeroEpsilon / 2.0) ||
    ...
    double current_jerk = v0_ > vel_limit_upper_ ? jerk_limit_lower_ : jerk_limit_upper_;
    double t = std::fabs(a0_ / current_jerk);
    cofficients_.time_points.push_back(0.0);
    cofficients_.cofficients.push_back({s0_, v0_, a0_ / 2.0, current_jerk / 6.0});
    cofficients_.time_points.push_back(t);
    cofficients_.cofficients.push_back({CalcS(t), CalcV(t), 0.0, 0.0});//todo:consider the case that end_v0 is not vel_limit_upper_ or vel_limit_lower_
  } else {
    if (std::fabs(a0_) > kZeroEpsilon) {
      double uniform_vel =
          a0_ > kZeroEpsilon ? vel_limit_upper_ : vel_limit_lower_;
      double uniform_vel_time = std::fabs((uniform_vel - v0_) / a0_);//todo:consider the case that end_a0 is zero
      cofficients_.time_points.push_back(0.0);
      cofficients_.cofficients.push_back({s0_, v0_, a0_ / 2.0, 0.0});
      cofficients_.time_points.push_back(uniform_vel_time);
      cofficients_.cofficients.push_back({CalcS(uniform_vel_time), uniform_vel, 0.0, 0.0});
    }else{
      cofficients_.time_points.push_back(0.0);
...
void UniformJerkCurve::BuildUniformJerkTraj(const double jerk){
  ...
  if(t_hit_acc < t_hit_vel){
    ...
    cofficients_.cofficients.push_back({CalcS(t_hit_acc), CalcV(t_hit_acc), end_a0 / 2.0, 0.0});
