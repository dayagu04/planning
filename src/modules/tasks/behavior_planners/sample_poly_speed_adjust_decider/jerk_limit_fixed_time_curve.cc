// #include "jerk_limit_fixed_time_curve.h"
// #include <algorithm>
// #include <cmath>
// #include <iostream>
...
// bool JerkLimitedFixedTimeCurve::Update(const PointState &init_point_state,
...
//   double delt_v_3 = acc_cruise * t3 + velocity_param.j3 * t3_square / 2;
...
// PointState JerkLimitedFixedTimeCurve::UpdateTrajectory(
...
// PointState JerkLimitedFixedTimeCurve::UpdatePoint(const double x,
//                                                   const double v,
//                                                   const double a,
//                                                   const double j,
//                                                   const double t) {
...
// PositionParam JerkLimitedFixedTimeCurve::PositionTargetSolver(
...
// PointState JerkLimitedFixedTimeCurve::UpdateFinalTrajectory(
...
// bool JerkLimitedFixedTimeCurve::GenerateCurve(const double delta_t) {
...
// void JerkLimitedFixedTimeCurve::DebugString() const {
...
// double JerkLimitedFixedTimeCurve::GetLimitVelocityByFixedTime(
//     const double fixed_time, const bool is_upper_limit) {
//   double v_des = is_upper_limit ? state_limit_.v_max : state_limit_.v_min;
//   auto velocity_param =
//       VelocityTargetSolver(state_limit_, init_point_state_, v_des);
//   double acc_des =
//       velocity_param.j1 > 0 ? state_limit_.a_max : state_limit_.a_min;
//   double first_time = velocity_param.T1;
//   double cruise_time = velocity_param.T2 - velocity_param.T1;
//   double last_time = velocity_param.T3 - velocity_param.T2;
//   double v_end = 0.0;
//   if (velocity_param.T3 > fixed_time) {
//     return v_des;
//   } else if ((first_time + last_time) < fixed_time) {
//     v_end = v_des - (velocity_param.T3 - fixed_time) * acc_des;
//     return v_end;
//   } else {
//     double t1 = (init_point_state_.a + velocity_param.j3 * fixed_time) /
//                 (velocity_param.j3 - velocity_param.j1);
//     if (t1 < 0.0) {
//       v_end = init_point_state_.v + velocity_param.a * fixed_time +
//               0.5 * velocity_param.j3 * fixed_time * fixed_time;
//       return v_end;
//     } else if (t1 > fixed_time) {
//       v_end = init_point_state_.v + velocity_param.a * fixed_time +
//               0.5 * velocity_param.j1 * fixed_time * fixed_time;
//       return v_end;
//     } else {
//       double t2 = fixed_time - t1;
//       v_end = init_point_state_.v + velocity_param.a * fixed_time +
//               0.5 * velocity_param.j1 * t1 * t1 + velocity_param.j1 * t1 * t2 +
//               0.5 * velocity_param.j3 * t2 * t2;
//       return v_end;
//     }
//   }
// }

// PositionParam JerkLimitedFixedTimeCurve::PositionTargetSolverByFixedTime(const StateLimitParam &state_limit,
//   const PointState &init_state,const double t_end,const bool is_upper_limit){
//   double v_des = is_upper_limit ? state_limit.v_max : state_limit.v_min;
//   double v_cruise = is_upper_limit ? state_limit.v_min : state_limit.v_max;
//   auto first_p = VelocityTargetSolver(state_limit, init_state, v_cruise);
//   PointState cruise_state{0.0,v_cruise,0.0,0.0};
//   auto last_p = VelocityTargetSolver(state_limit, cruise_state, v_des);
//   if((first_p.T3 + last_p.T3) < t_end){
