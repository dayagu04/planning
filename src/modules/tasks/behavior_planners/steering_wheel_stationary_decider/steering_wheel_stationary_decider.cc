...
namespace {
  constexpr double kMinSteeringAngleError = 3.0;
  constexpr double kMaxExtraSteeringAngle = 20.0;
  constexpr double kControlSteeringAnglePercentage = 0.9;
};  // namespace
...
bool SteeringWheelStationaryDecider::Init() {
  static_steering_status_ = kStaticSteeringWait;
  static_steering_failed_reason_ = NO_FAILURE;
  is_need_start_slowly_ = false;
...
bool SteeringWheelStationaryDecider::Execute() {
  ...
  // is enable static steering
  if (!config_.enable_static_steering ||
      session_->is_hpp_scene()) {  // tmp close hpp steering
    Reset();
    return true;
  }
  // check dbw status
  if (!session_->environmental_model().GetVehicleDbwStatus()) {
    static_steering_status_ == kStaticSteeringWait;
    ResetWithReason(NOT_IN_FUNCTION);
    return true;
  }
  // main
  static_steering_failed_reason_ = NO_FAILURE;
  is_need_start_slowly_ = false;
  is_need_replan_ = false;
  switch (static_steering_status_) {
    case kStaticSteeringWait: {
      IsExecuteStaticSteering();
      break;
    }
    case kStaticSteeringExecution: {
      if (!IsCancelTurningSteering()) {
        IsReadyToDetour();
      }
      break;
    }
    case kStaticSteeringReady: {
      if (!IsCancelTurningSteering()) {
        IsDetourStarted();
      }
      break;
    }
    case kStaticSteeringComplete: {
      IsDetourFinished();
      break;
    }
    case kStaticSteeringCancel: {
      IsRestoreFinished();
      break;
    }
    default: { break; }
  ...
  JSON_DEBUG_VALUE("static_steering_status",
                   static_cast<int>(static_steering_status_))
  JSON_DEBUG_VALUE("static_steering_failed_reason",
                   static_cast<int>(static_steering_failed_reason_))
  return true;
}
...
bool SteeringWheelStationaryDecider::CalculateSteeringWheelAngle() {
  ...
  target_steering_angle_ =
    turning_direction_ * target_front_wheel_angle * steer_ratio_;  // rad
  // control can only achieve 95%
  target_steering_angle_ /= kControlSteeringAnglePercentage;
  // extra angle buffer
  target_steering_angle_ += turning_direction_ * (kMaxExtraSteeringAngle / 57.3);
  ...
  if (is_need_steering_wheel_stationary_ &&
      std::fabs(target_steering_angle_ - ego_steer_angle) <=
      (kMinSteeringAngleError / 57.3) &&
      std::fabs(ego_steer_angle - init_steer_angle) <=
      (kMinSteeringAngleError / 57.3)) {
    Reset();
    return false;
  }
...
bool SteeringWheelStationaryDecider::IsExecuteStaticSteering() {
  // ego state
  const auto &ego_state_manager =
      session_->environmental_model().get_ego_state_manager();
  double ego_vel = ego_state_manager->ego_v();
  double ego_steer_angle = ego_state_manager->ego_steer_angle();  // rad
  origin_steering_angle_ = ego_steer_angle;
  // double init_vel = ego_state_manager->planning_init_point().v;
  // double init_delta = ego_state_manager->planning_init_point().delta;
  // double init_steer_angle = init_delta * steer_ratio_;
  // trajectory length condition
  const auto &traj_points = session_->planning_context().planning_result().traj_points;
  if (traj_points.back().s - traj_points.front().s >= 1.0) {
    ResetWithReason(NOT_STATIONARY);
    return false;
  }
  // velocity condition
  if (ego_vel > 1e-3) {
  ...
  // steer angle condition
  // if (max_steer_angle_ - std::fabs(ego_steer_angle) < 1e-6) {
  //   Reset();
  //   return false;
  // }
  // obstalce condition
  ...
  if (target_obstacle == nullptr) {
    ResetWithReason(NO_TARGET_OBSTACLE);
    return false;
  }
  if (target_obstacle->rel_s() < config_.min_lon_dist_thr) {
    ResetWithReason(NO_TARGET_OBSTACLE);
    return false;
  }
  size_t last_target_target_obstacle_id = target_obstacle_id_;
  ...
  if (turning_direction_ > 0) {
    ...
    ResetWithReason(NO_TURNING_DIRECTION);
    return false;
  }
  if (!CheckLaneLineTypeByTurningDirection()){
    ResetWithReason(LANE_LINE_TYPE_SOLID);
    return false;
  }
  if (!CheckTurningSafetyForStaticObstacle(target_obstacle)){
    ResetWithReason(STEERING_SAFETY_CHECK_FAILED);
    return false;
  }
  ...
  if (min_turning_radius < min_inner_radius_  ||
      min_turning_radius > max_inner_radius_) {
    ResetWithReason(STEER_ANGLE_INVALID);
    return false;
  }
  // double triangle_side_length = (0.5 * ego_width_) + min_turning_radius;
  // double expected_outer_radius =
  ...
  target_steering_angle_ =
    turning_direction_ * target_front_wheel_angle * steer_ratio_;  // rad
  // control can only achieve 90%
  target_steering_angle_ /= kControlSteeringAnglePercentage;
  // extra angle buffer
  target_steering_angle_ += turning_direction_ * (kMaxExtraSteeringAngle / 57.3);
  if (std::fabs(target_steering_angle_) > max_steer_angle_) {
    target_steering_angle_ = turning_direction_ * max_steer_angle_;
  }
  // if (is_need_steering_wheel_stationary_ &&
  //     last_target_target_obstacle_id == target_obstacle_id_ &&
  ...
  // }
  is_need_steering_wheel_stationary_ = true;
  static_steering_status_ = kStaticSteeringExecution;
  return true;
}
...
void SteeringWheelStationaryDecider::GetTurningDirectionByRef() {
  ...
  const auto &soft_bounds_frenet_point =
      session_->planning_context().general_lateral_decider_output().second_soft_bounds_frenet_point;
  turning_direction_ = 0;
...
bool SteeringWheelStationaryDecider::CheckTurningSafetyForStaticObstacle(
    const std::shared_ptr<FrenetObstacle>& target_obstacle) {
  if (!target_obstacle->b_frenet_valid()) {
    return false;
  }
...
int SteeringWheelStationaryDecider::GetFrontNearestObstacleInLaneChangeState() {
  ...
  bool is_LANE_CHANGE =
      target_state == kLaneChangePropose ||
...
bool SteeringWheelStationaryDecider::IsCancelTurningSteering() {
  // check speed
  const auto &ego_state_manager =
      session_->environmental_model().get_ego_state_manager();
  double ego_vel = ego_state_manager->ego_v();
  ...
  if (ego_vel > 1e-3 &&
      (traj_points.back().s - traj_points.front().s >= 1.0)) {
    return false;
  }
  // check status
  if (is_static_lane_change_) {
    ...
    if (is_LC_CANCEL) {
      Restore();
      static_steering_status_ = kStaticSteeringCancel;
      return true;
    }
  } else if (is_static_lane_borrow_) {
    const auto &lane_borrow_decider_output =
        session_->planning_context()
...
bool SteeringWheelStationaryDecider::IsReadyToDetour() {
  // check status
  if (is_static_lane_change_) {
    const auto &coarse_planning_info =
        session_->planning_context()
  ...
  if (traj_points.back().s - traj_points.front().s < 1.0) {
    ...
    double ego_steer_angle =
        ego_state_manager->ego_steer_angle();  // rad
    if (std::fabs(ego_steer_angle) <
        std::fabs(target_steering_angle_) * kControlSteeringAnglePercentage) {
      return false;
    }
  } else {
    // not enough but go
    ...
    is_need_start_slowly_ = true;
    static_steering_status_ = kStaticSteeringReady;
    return true;
  }
  // check safety
  const auto &virtual_lane_manager =
  ...
  const auto &reference_path =
      virtual_lane_manager->get_current_lane()->get_reference_path();
  const auto &obs_map = reference_path->get_obstacles_map();
  auto it = obs_map.find(target_obstacle_id_);
  if (it != obs_map.end()) {
    if (!CheckTurningSafety(it->second)) {
      return false;
    }
  } else {
    // target obstacle has disappeared
    is_need_steering_wheel_stationary_ = false;
  }
  static_steering_status_ = kStaticSteeringReady;
  return true;
...
bool SteeringWheelStationaryDecider::IsDetourStarted() {
  is_need_start_slowly_ = true;
  // check traj
  const auto &traj_points =
      session_->planning_context().planning_result().traj_points;
  ...
  double ego_vel = ego_state_manager->ego_v();
  if (ego_vel > 1e-3) {
    static_steering_status_ = kStaticSteeringComplete;
    return true;
  }
  return false;
}

bool SteeringWheelStationaryDecider::IsDetourFinished() {
  is_need_steering_wheel_stationary_ = false;
  // // check speed
  // const auto &ego_state_manager =
  //     session_->environmental_model().get_ego_state_manager();
...
bool SteeringWheelStationaryDecider::IsRestoreFinished() {
  // check traj
  const auto &traj_points =
      session_->planning_context().planning_result().traj_points;
  if (traj_points.back().s - traj_points.front().s >= 1.0) {
  ...
  double ego_steer_angle =
      ego_state_manager->ego_steer_angle();  // rad
  if (std::fabs(target_steering_angle_ - ego_steer_angle) <
      10.0 / 57.3) {
    Reset();
    is_need_replan_ = true;
    static_steering_status_ = kStaticSteeringWait;
...
bool SteeringWheelStationaryDecider::UpdateOutput() {
  auto &steering_wheel_stationary_decider_output =
      session_->mutable_planning_context()
          ->mutable_steering_wheel_stationary_decider_output();
  steering_wheel_stationary_decider_output.static_steering_status =
      static_steering_status_;
  steering_wheel_stationary_decider_output.static_steering_failed_reason =
      static_steering_failed_reason_;
  steering_wheel_stationary_decider_output.is_need_steering_wheel_stationary =
      is_need_steering_wheel_stationary_;
  steering_wheel_stationary_decider_output.is_need_start_slowly =
      is_need_start_slowly_;
  steering_wheel_stationary_decider_output.is_static_lane_change =
      is_static_lane_change_;
  steering_wheel_stationary_decider_output.is_static_lane_borrow =
      is_static_lane_borrow_;
  steering_wheel_stationary_decider_output.is_static_lane_avoid =
      is_static_lane_avoid_;
  steering_wheel_stationary_decider_output.is_need_replan =
      is_need_replan_;
...
void SteeringWheelStationaryDecider::Restore() {
  is_need_steering_wheel_stationary_ = true;
  is_static_lane_change_ = false;
  is_static_lane_borrow_ = false;
  is_static_lane_avoid_ = false;
  is_need_start_slowly_ = false;
  is_need_replan_ = false;
  turning_direction_ = 0;
  target_steering_angle_ = origin_steering_angle_;
...
void SteeringWheelStationaryDecider::ResetWithReason(
    const StaticSteeringFailedReason failed_reason) {
  static_steering_failed_reason_ = failed_reason;
  is_need_steering_wheel_stationary_ = false;
  is_static_lane_change_ = false;
  is_static_lane_borrow_ = false;
  is_static_lane_avoid_ = false;
