...
class SteeringWheelStationaryDecider : public Task {
 ...
 private:
  ...
  bool CheckTurningSafetyForStaticObstacle(
      const std::shared_ptr<FrenetObstacle>& target_obstacle);
  ...
  bool IsExecuteStaticSteering();

  bool IsCancelTurningSteering();

  bool IsReadyToDetour();

  bool IsDetourStarted();

  bool IsDetourFinished();

  bool IsRestoreFinished();
  ...
  void ResetWithReason(const StaticSteeringFailedReason failed_reason);

 private:
  SteeringWheelStationaryDeciderConfig config_;
  StaticSteeringStatus static_steering_status_;
  StaticSteeringFailedReason static_steering_failed_reason_;
  bool is_need_steering_wheel_stationary_ = false;
  bool is_need_start_slowly_ = false;
  bool is_static_lane_change_ = false;
  bool is_static_lane_borrow_ = false;
  bool is_static_lane_avoid_ = false;
  bool is_need_replan_ = false;
  size_t target_obstacle_id_ = 0;
  double turning_direction_ = 0;
  double target_steering_angle_ = 0.0;  // rad
  double origin_steering_angle_ = 0.0;  // rad
