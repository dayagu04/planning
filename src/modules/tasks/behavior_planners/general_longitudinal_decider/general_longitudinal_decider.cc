...
BoundedConstantJerkTrajectory1d GeneralLongitudinalDecider::get_velocity_limit(
  ...
  // const auto &map_info_manager = frame_->mutable_session()
  //                                    ->mutable_planning_context()
  ...
  // double steer_angle = frame_->mutable_session()
  //                          ->mutable_planning_context()
  //                          ->ego_state()
  //                          ->ego_steer_angle();
  ...
  //     std::max(map_info_manager->map_velocity_limit(), kMinMapVelocityLimit);
  // double real_map_limit = map_info_manager->map_velocity_limit() *
  //                         config_.velocity_upper_bound_scale_rate;
  // map_velocity_limit =
  ...
  //   static constexpr double kVelocityLimitRads = 3.0;
  //   double mff_cruise_velocity = frame_->session()
  //                                    ->world_model()
  //                                    .get_vehicle_status()
  //                                    .velocity()
  //                                    .cruise_velocity()
  //                                    .value_mps();
  //   NLOGD("mff_cruise_velocity = %f", mff_cruise_velocity);
  //   map_velocity_limit =
  //       std::min(kVelocityLimitRads / 3.6, mff_cruise_velocity);
  //   user_velocity_limit = 0.83;
...
const double GeneralLongitudinalDecider::compute_max_lat_acceleration() const {
  ...
  // auto lc_acc_decay =
  //     (pipeline_context_->planning_target_state == CRUISE_CHANGE &&
  //      ego_velocity < kUrbanVelocityThld)
  //         ? kLowSpeedLaneChangeAccBuff
  //         : 0.0;
  // if (map_info_manager->is_on_highway()) {
...
void GeneralLongitudinalDecider::construct_longitudinal_obstacle_decision(
  ...
  //             mjson::Json("detect_initial_point_collision");
  //       }
  //     }
  //   }
  // }
  ...
  //   bool b_fast_car = obstacle_type != ObjectType::OFO and
  //                     obstacle_type != ObjectType::PEDESTRIAN &&
  ...
  //         NLOGI("[GeneralLongitudinalDecider]: keep stop for close leadone");
  //       }
  //     }
  //   }
...
void GeneralLongitudinalDecider::construct_longitudinal_outer_decision(
  ...
  //     coarse_planning_info.target_state == CRUISE_WAIT) {
...
void GeneralLongitudinalDecider::get_lon_decision_info(
  ...
  // int current_lane_virtual_id = frame_->mutable_session()
  //                                   ->mutable_planning_context()
  //                                   ->virtual_lane_manager()
  //                                   ->current_lane_virtual_id();
  // auto reference_path_manager = frame_->mutable_session()
  //                                   ->mutable_planning_context()
  //                                   ->reference_path_manager();
  // auto current_reference_path =
  //     reference_path_manager->get_reference_path_by_lane(
  //         current_lane_virtual_id);
  // // auto frenet_obstacles_map = current_reference_path->get_obstacles_map();
  ...
  //               "distance_to_right_road_border: %.2f, lateral_satisfy: %d, "
  //               "longitual_satisfy: %d",
  ...
  //           NLOGD("HHLDEBUGW s_l_satisfy: %d", s_l_satisfy);
  //         }
  //       }
  //     }
  //     NLOGD("HHLDEBUGW nearby_obstacle: %d",
  //           lon_decision_information.nearby_obstacle);
  ...
  //   NLOGD("HHLDEBUG: has_CIPV: %d",
  //         lon_decision_information.leadone_info.has_leadone);
  // }
  // if (lon_decision_information.cutin_info.has_cutin) {
  //   NLOGD("HHLDEBUG: has_cutin: %d",
...
double GeneralLongitudinalDecider::get_narrow_area_velocity_limit() {
  ...
  //     }
  //   }
  //   NLOGD("The path width is = : %f", width);
  // }
  ...
  // MDEBUG_JSON_ADD_ITEM(closest_distance, closest_distance, narrow_area_info)
  // MDEBUG_JSON_ADD_ITEM(suggest_velocity_limit, suggest_velocity_limit,
  //                      narrow_area_info)
...
double GeneralLongitudinalDecider::get_s_bound_by_target_parking_space() {
  ...
  // auto planning_init_point =
  //     reference_path_ptr_->get_frenet_ego_state().planning_init_point();
  // auto aimed_parking_space_id = frame_->mutable_session()
  //                                   ->mutable_planning_context()
  //                                   ->mutable_parking_status_info()
  //                                   ->parking_space_id();
  // NLOGI("aimed_parking_space_id: id= %d", aimed_parking_space_id);
  ...
  //     static_cast<int>(aimed_parking_space_id), target_parking_space_center,
  //     target_parking_space_coners);
  // if (!find_matched_parking_space) {
  //   NLOGI("Can not find target parking space !!!");
  ...
  //         (vehicle_param_.length / 2. - vehicle_param_.back_edge_to_center);
  //     if (reference_len < nearest_parking_location_s) {
  //       NLOGI("reference line length = %f < nearest parking location = %f",
  ...
  //       if (dis_to_target < confindence_longi_distance && dis_to_target > 0)
