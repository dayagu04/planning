...
GeneralLongitudinalDecider::GeneralLongitudinalDecider(const EgoPlanningConfigBuilder *config_builder,
...
bool GeneralLongitudinalDecider::Execute(planning::framework::Frame *frame) {
  ...
  generate_lon_decision_from_path(ego_planning_result.traj_points, refpath_points, ego_planning_info.obstacle_decisions,
                                  lon_ref_path);
  double time_2 = IflyTime::Now_ms();
  LOG_DEBUG("generate_lon_decision_from_path cost is [%f]ms:\n", time_2 - time_1);

  // execution leadone & cutin information
  auto &lon_decision_information = frame->mutable_session()->mutable_planning_context()->mutable_lon_decision_result();
  ...
  auto acc_function = frame->session()->mutable_planning_context()->adaptive_cruise_control_function();
  auto &acc_info = frame->mutable_session()->mutable_planning_context()->mutable_adaptive_cruise_control_result();
  acc_function->adaptive_cruise_control(lon_decision_information, acc_info, ego_planning_result);
  ...
  StartStopInfo &start_stop_result = frame->mutable_session()->mutable_planning_context()->mutable_start_stop_result();
  auto start_stop_function = frame->session()->mutable_planning_context()->start_stop();
  double stop_weight_dx_config = config_start_stop_.dx_ref_weight;
  if (start_stop_function->enable_start_stop()) {
    start_stop_function->go_trajectory(lon_decision_information, start_stop_info, start_stop_result);
  }
  ...
  bool enable_stop_flag =
      ((lon_yield_info_.keep_stop || start_stop_result.enable_stop) && !frame->session()->is_parking_scene());
  LOG_DEBUG(
      "HHLDEBUGDB start stop input: %d, enable stop_flag: %d, is_start: %d, "
      "is_stop: % d, enable_stop: % d \n",
      start_stop_info, enable_stop_flag, start_stop_result.is_start, start_stop_result.is_stop,
  ...
  for (size_t i = 0; i < ego_planning_result.traj_points.size() - 1; ++i) {
    auto delta_s = ego_planning_result.traj_points[i + 1].s - ego_planning_result.traj_points[i].s;
  ...
  // Step 4: 初始化纵向参考轨迹
  assert(config_.lon_num_step + 1 == static_cast<int>(ego_planning_result.traj_points.size()));
  const double s_rel = lon_decision_information.leadone_info.leadone_information.obstacle_s;
  bool v_direction =
  ...
  bool model_speed_up = acc_function->recognize_model_speed_up(ego_planning_result);
  // LOG_DEBUG("HHLDEBUGB model_speed_up: %d", model_speed_up);
  auto mrc_brake = frame->session()->mutable_planning_context()->mrc_condition();
  // MDEBUG_JSON_BEGIN_DICT(Mrc_v_ref_debug)
  mrc_brake->update_inline_brake_by_obstacle(lon_decision_information, planning_init_point);
  ...
  for (size_t i = 0; i < ego_planning_result.traj_points.size(); i++) {
    ...
    // adjust s_ref
    if (enable_stop_flag) {
      lon_ref_path.s_refs.emplace_back(ego_planning_result.traj_points[0].s, 1.0);
    } else {
      lon_ref_path.s_refs.emplace_back(ego_planning_result.traj_points[i].s, 1.0);
    }
    ...
    if (enable_stop_flag) {
      ...
      mrc_brake->mrc_brake_execute(lon_ref_path, ego_planning_result, planning_init_point);
    } else {
      if (acc_info.navi_speed_control_info.enable_v_cost && v_direction && !model_speed_up) {
    ...
    if (acc_update_s_bound) {
      ...
      s_bounds.emplace_back(WeightedBound{planning_init_point.frenet_state.s - 10.0,
                                          ego_planning_result.traj_points.back().s + s_buff, -1.0});
      lon_ref_path.bounds.emplace_back(s_bounds);
    } else {
      s_bounds.emplace_back(
          WeightedBound{planning_init_point.frenet_state.s - 10.0, ego_planning_result.traj_points.back().s, -1.0});
      lon_ref_path.bounds.emplace_back(s_bounds);
    }

    // adjust v_limit
    lon_ref_path.lon_bound_v.emplace_back(Bound{0.0, config_.velocity_upper_bound});
  ...
  // adjust jerk_bound
  acc_function->acc_update_jerk_bound(acc_info, lon_ref_path, ego_planning_result, planning_init_point);
  if (ego_planning_info.traffic_light_decision.stop_flag) {
    double stop_distance = std::max(0.1, ego_planning_info.traffic_light_decision.stop_distance);
    lon_yield_info_.min_ds = std::min(lon_yield_info_.min_ds, stop_distance);
    lon_yield_info_.min_ttc = std::min(lon_yield_info_.min_ttc, stop_distance / std::max(0.1, ego_v));
    lon_yield_info_.min_acc = std::min(lon_yield_info_.min_acc, -ego_v * ego_v / 2.0 / stop_distance);
    lon_yield_info_.min_jerk = std::min(lon_yield_info_.min_jerk, -ego_v * ego_v / 2.0 / stop_distance - ego_a);
  }
  bool is_emergency = lon_yield_info_.min_ttc < config_.ttc_thld ||
                      lon_yield_info_.min_jerk < std::min(0.0, planning_init_point.jerk - config_.delta_jerk_thld) ||
                      lon_yield_info_.min_acc < planning_init_point.a - config_.delta_acc_thld || mrc_condition_enable;
  size_t num_jerk_bound = std::min(config_.num_jerk_bound, lon_ref_path.lon_bound_jerk.size());
  const double kEgoAccThreshold{0.1};
  const double kEgoVelThreshold{2.0};
  const bool enable_jerk_bound_reshape =
      !is_emergency && !frame->session()->is_parking_scene() && ego_a < kEgoAccThreshold && ego_v > kEgoVelThreshold;
  if (enable_jerk_bound_reshape) {
    LOG_DEBUG("[GeneralLongitudinalDecider::execute] enable jerk bound reshape \n");
    for (size_t i = 0; i < num_jerk_bound; ++i) {
      double jerk_min_i = planning_init_point.jerk - config_.jerk_buffer - i * config_.delta_time * config_.jerk_rate;
      lon_ref_path.lon_bound_jerk[i].lower = std::min(lon_ref_path.lon_bound_jerk[i].upper - config_.jerk_buffer,
  ...
  auto stop_line_s = planning_init_point.frenet_state.s + ego_planning_info.traffic_light_decision.stop_distance;
  lon_ref_path.bounds.back().emplace_back(
      WeightedBound{std::numeric_limits<double>::min(), stop_line_s, -1.0, BoundInfo{0, "traffic light"}});
  ...
  if (frame_->session()->is_parking_scene()) {
    ...
    double destination_s = planning_init_point.frenet_state.s + distance_to_destination - stop_distance_to_destination;
    lon_ref_path.bounds.back().emplace_back(
        WeightedBound{std::numeric_limits<double>::min(), destination_s, -1.0, BoundInfo{0, "destination"}});
    // adjust s_limit by target parking space
    auto s_bound_upper = get_s_bound_by_target_parking_space();
    lon_ref_path.bounds.back().emplace_back(WeightedBound{std::numeric_limits<double>::min(), s_bound_upper, -1.0,
                                                          BoundInfo{0, "destination_parking_space"}});
    if (s_bound_upper < std::numeric_limits<double>::max() && s_bound_upper > destination_s) {
  ...
  for (auto &obstacle_decision : ego_planning_info.obstacle_decisions) {
    for (auto &position_decision : obstacle_decision.second.position_decisions) {
      ...
      for (size_t i = 0; i < lon_ref_path.t_list.size(); ++i) {
        ...
        if (std::fabs(t - position_decision.tp.t) < 1e-2) {
          ...
          lon_lead_bounds.insert(lon_lead_bounds.end(), position_decision.lon_lead_bounds.begin(),
                                 position_decision.lon_lead_bounds.end());

          for (auto &bound : position_decision.lon_bounds) {
            if (bound.weight < 0 and bound.upper < lon_ref_path.s_refs[i].first) {
              LOG_DEBUG(
                  "Decider bound obstacle_id: %d, t: %f, s_ref: %f, "
                  "upper_bound: %f\n",
                  obstacle_decision.first, t, lon_ref_path.s_refs[i].first, bound.upper);
            }
          }
        }
      }
...
BoundedConstantJerkTrajectory1d GeneralLongitudinalDecider::get_velocity_limit(const LonRefPath &lon_ref_path) {
  // NTRACE_CALL(9);

  // get curvature velocity limit
  const auto ego_state = frame_->session()->environmental_model().get_ego_state_manager();
  const auto &map_info_manager = frame_->session()->environmental_model().get_virtual_lane_manager();
  ...
  for (size_t i = 0; i < traj_points.size(); ++i) {
    ...
    if (i > 0) {
      s_accumulate +=
          planning::fast_hypot(traj_points[i].x - traj_points[i - 1].x, traj_points[i].y - traj_points[i - 1].y);
    }
  }
  LOG_DEBUG(
  ...
  double steer_curvature = std::tan(steer_angle / vehicle_param_.steer_ratio) / vehicle_param_.wheel_base;
  ...
  double v_limit_path_curv =
      std::max(config_.velocity_lower_bound,
               std::min(config_.velocity_upper_bound, std::sqrt(max_lat_acceleration / path_curvature)));
  double v_limit_curv =
      std::max(config_.velocity_lower_bound,
               std::min(config_.velocity_upper_bound, std::sqrt(max_lat_acceleration / max_curvature)));
  ...
  auto planning_init_point = reference_path_ptr_->get_frenet_ego_state().planning_init_point();
  const double ego_v = planning_init_point.v;
  const double ego_a = std::abs(planning_init_point.a) < 1e-2 ? 1e-2 : planning_init_point.a;
  const double ego_jerk = planning_init_point.jerk;
  const double b_square_minus_4ac = std::pow((2 * ego_v + v_limit_path_curv), 2) + 6 * ego_a * max_curv_s;
  // 曲率限速要低于当前速度 && 最大曲率距离>0 && 根号内值大于0
  const double kTimeDistanceThreshold = 0.5;
  const bool enable_vlimit_curv_dec =
      v_limit_path_curv < ego_v && max_curv_s > ego_v * kTimeDistanceThreshold && b_square_minus_4ac > 0.0;
  if (enable_vlimit_curv_dec) {
    time_to_brake = (-(2 * ego_v + v_limit_path_curv) + std::sqrt(b_square_minus_4ac)) / ego_a;
    time_to_brake = std::max(time_to_brake, 0.01);
    vlimit_jerk = 2 * (v_limit_path_curv - ego_v - ego_a * time_to_brake) / std::pow(time_to_brake, 2);
  }
  ...
  vel_limit_info_.v_limit_map = map_info_manager->get_current_lane()->velocity_limit();
  double map_velocity_limit = std::max(vel_limit_info_.v_limit_map, kMinMapVelocityLimit);
  double real_map_limit = vel_limit_info_.v_limit_map * config_.velocity_upper_bound_scale_rate;
  map_velocity_limit = map_velocity_limit * config_.velocity_upper_bound_scale_rate;
  ...
  bool disable_user_limit =
      (std::fabs(user_velocity_limit - real_map_limit) < 0.01 && (user_velocity_limit < kMinMapVelocityLimit));
  ...
  if (disable_user_limit) {
    ...
    final_velocity_limit = std::min(final_velocity_limit, std::min(user_velocity_limit, map_velocity_limit));
  }
  ...
  if (frame_->session()->is_parking_scene()) {
    ...
    map_velocity_limit = std::min(config_.velocity_limit_parking / 3.6, mff_cruise_velocity);
    ...
    if (reference_path_ptr_->get_reference_point_by_lon(frenet_ego_state.s() + kVelocityPreviewDistance, refpath_pt)) {
      user_velocity_limit = map_velocity_limit;
      if (std::fabs(refpath_pt.path_point.kappa) > 0.1) {
        user_velocity_limit = std::min((1.0 - std::fabs(refpath_pt.path_point.kappa)) * map_velocity_limit, 5 / 3.6);
    ...
    LOG_DEBUG("curvature: %f, curvature_velocity_limit:%f", refpath_pt.path_point.kappa, user_velocity_limit);
  }
  ...
  if (vlimit_jerk < kComfortBrakeJerk && time_to_brake > kTimeToBrakeThld) {
    enable_jerk =
        std::min(std::max(ego_jerk - kDeltaJerkThld, std::max(vlimit_jerk, kEmergencyBrakeJerk)), kComfortBrakeJerk);
    vlimit_acc = (std::pow(v_limit_path_curv, 2) - std::pow(ego_v, 2)) / (2 * max_curv_s);
  }

  BoundedConstantJerkTrajectory1d brake_traj(
      lon_ref_path.s_refs.front().first, frenet_ego_state.planning_init_point().v,
      std::max(frenet_ego_state.planning_init_point().a, vlimit_acc), enable_jerk, config_.delta_time);
  LOG_DEBUG(
      ...
      "enable_jerk: %f",
      final_velocity_limit, map_velocity_limit, user_velocity_limit, frenet_ego_state.planning_init_point().v,
      frenet_ego_state.planning_init_point().a, frenet_ego_state.velocity(), frenet_ego_state.acc(), ego_jerk,
      enable_jerk);
  // set traj v,a bound
  brake_traj.set_bound(final_velocity_limit, std::numeric_limits<double>::max(), std::numeric_limits<double>::lowest(),
...
const double GeneralLongitudinalDecider::compute_max_lat_acceleration() const {
  ...
  auto lc_acc_decay = ((pipeline_context_->planning_target_state == ROAD_LC_LCHANGE ||
  ...
  if (frame_->session()->environmental_model().is_on_highway()) {
    return config_.highway_max_lat_acceleration +
           std::min(kHighwayLatAccBuffMax, kHighwayLatAccRate * std::max(ego_velocity - kHighwayVelocityThld, 0.0));
  } else {
    return config_.urban_max_lat_acceleration - lc_acc_decay +
           std::min(kUrbanLatAccBuffMax, kUrbanLatAccRate * std::max(ego_velocity - kUrbanVelocityThld, 0.0));
  }
}

void GeneralLongitudinalDecider::set_velocity_acceleration_bound(LonRefPath &lon_ref_path) {
  // NTRACE_CALL(8);
  ...
  const double kAccUpperBound =
      std::max(config_.acceleration_lower_bound,
               config_.acceleration_upper_bound - config_.acceleration_curvature_ratio * curv_info.max_curvature);
  ...
  for (size_t i = 0; i < lon_ref_path.t_list.size(); ++i) {
    ...
    bound_a.upper = std::min(config_.acceleration_upper_bound, std::max(kAccUpperBound, ego_a + 1e-2));
    // LOG_DEBUG(
...
void GeneralLongitudinalDecider::generate_lon_decision_from_path(const TrajectoryPoints &lateral_trajectory_points,
  ...
  construct_longitudinal_obstacle_decisions(lateral_trajectory_points, refpath_points, obstacle_decisions,
                                            lon_ref_path);
  double time_2 = IflyTime::Now_ms();
  LOG_DEBUG(
      ...
      "[%f]ms:\n",
      time_2 - time_1);
  construct_longitudinal_outer_decision(lateral_trajectory_points, refpath_points,
                                        pipeline_context_->coarse_planning_info, obstacle_decisions);
}

bool GeneralLongitudinalDecider::check_longitudinal_ignore_obstacle(const std::shared_ptr<FrenetObstacle> obstacle) {
  ...
  if ((frame_->session()->is_parking_scene() && obstacle->type() != Common::ObjectType::OBJECT_TYPE_PEDESTRIAN)) {
    ...
    if (obstacle_sl_boundary.s_end + vehicle_param_.back_edge_to_rear_axis < ego_sl_boundary.s_end) {
      return true;
    }
  } else {
    if (obstacle_sl_boundary.s_end + kLonIgnoreDistance < ego_sl_boundary.s_end) {
      return true;
    }
  }
...
void GeneralLongitudinalDecider::construct_longitudinal_obstacle_decisions(const TrajectoryPoints &traj_points,
                                                                           const ReferencePathPoints &refpath_points,
                                                                           ObstacleDecisions &obstacle_decisions,
                                                                           LonRefPath &lon_ref_path) {
  // NTRACE_CALL(9);
  ...
  pipeline_context_->planning_result.extra_json["lon_decision_error_info"] = "none";
  auto ego_sl_boundary = reference_path_ptr_->get_ego_frenet_boundary();
  ...
  std::vector<LonObstalceYieldInfo> obstacle_info(traj_points.size(), {0, 1000, 1000});
  lon_ref_path.lon_obstacle_yield_info = std::move(obstacle_info);

  double construct_longitudinal_obstacle_decision_time_start = IflyTime::Now_ms();
  for (auto &obstacle : reference_path_ptr_->get_obstacles()) {
    ...
    if (obstacle->b_frenet_valid()) {
      ...
      LOG_DEBUG("construct_longitudinal_obstacle_decision== obstacle_id: [%d]:\n", obstacle_id);

      construct_longitudinal_obstacle_decision(traj_points, refpath_points, lon_overlap_path, obstacle,
  ...
  LOG_DEBUG("construct_longitudinal_obstacle_decision cost is %f]ms:\n",
            construct_longitudinal_obstacle_decision_time_end - construct_longitudinal_obstacle_decision_time_start);

  if (!lon_ref_path.lon_obstacle_yield_info.empty()) {
    for (size_t i = 0; i < lon_ref_path.lon_obstacle_yield_info.size(); i++) {
      auto iter = lon_ref_path.lon_obstacle_overlap_info.find(lon_ref_path.lon_obstacle_yield_info[i].yield_id);
      if (iter != lon_ref_path.lon_obstacle_overlap_info.end()) {
        lon_ref_path.lon_obstacle_overlap_info.erase(lon_ref_path.lon_obstacle_yield_info[i].yield_id);
      }
    }
...
void GeneralLongitudinalDecider::make_longitudinal_overlap_path(const TrajectoryPoints &traj_points,
                                                                std::vector<Polygon2d> &overlap_path) {
  ...
  auto make_polygon = [](const Vec2d &start, const Vec2d &end, double width) {
    ...
    return Polygon2d({left_begin_point, left_end_point, right_end_point, right_begin_point});
  };

  for (size_t i = 1; i < traj_points.size(); ++i) {
    auto &pre_point = traj_points[i - 1];
    auto &cur_point = traj_points[i];
    overlap_path.emplace_back(
        make_polygon(Vec2d(pre_point.s, pre_point.l), Vec2d(cur_point.s, cur_point.l), care_width));
  }

  auto s_end = traj_points.front().s + config_.lon_care_length;
  if (traj_points.back().s < s_end) {
    auto &last_point = traj_points.back();
    overlap_path.emplace_back(make_polygon(Vec2d(last_point.s, last_point.l), Vec2d(s_end, last_point.l), care_width));
  }
}

void GeneralLongitudinalDecider::construct_longitudinal_obstacle_decision(
    const TrajectoryPoints &traj_points, const ReferencePathPoints &refpath_points,
    const std::vector<planning_math::Polygon2d> &overlap_path, const std::shared_ptr<FrenetObstacle> obstacle,
  ...
  const bool is_cross_obj = obstacle_decision.rel_pos_type == ObsRelPosType::CROSSING;

  for (size_t i = 0; i < traj_points.size(); i++) {
    auto &traj_pt = traj_points[i];
    auto t = traj_pt.t;
    obstacle_overlap_with_ego.emplace_back(t, false);
    obstacle_overtake_lowers.emplace_back(t, std::numeric_limits<double>::min());
    obstacle_yield_uppers.emplace_back(t, std::numeric_limits<double>::max());

    Polygon2d obstacle_sl_polygon;
    auto ok = obstacle->get_polygon_at_time(t, reference_path_ptr_, obstacle_sl_polygon);
    if (not ok) {
      continue;
    }
    ...
    for (auto &overlap_path_polygon : overlap_path) {
      ...
      b_overlap = obstacle_sl_polygon.ComputeOverlap(overlap_path_polygon, &overlap_polygon);
      if (b_overlap) {
        auto points = overlap_polygon.GetAllVertices();
        overlap_points.insert(overlap_points.end(), points.begin(), points.end());
      }
    }

    // obstain all lon overlap info
    if ((overlap_points.empty() || i == traj_points.size() - 1) && !overlap_info.empty()) {
      lon_ref_path.lon_obstacle_overlap_info.emplace(obstacle->id(), overlap_info);
      overlap_info.clear();
    }
    ...
    ok = planning_math::Polygon2d::ComputeConvexHull(overlap_points, &care_overlap_polygon);
    if (not ok) {
      LOG_WARNING("Failed to get [%d]'s polygon", obstacle->id());
      continue;
    } else {
      overlap_info.emplace_back(
          LonObstacleOverlapInfo{care_overlap_polygon.min_x() - vehicle_param_.rear_axis_to_front_edge, t});
    }

    obstacle_overlap_with_ego.back().second = true;
    // calculate buffer
    if (t < kTOvertakeThreshold) {
      obstacle_overtake_lowers.back().second = std::numeric_limits<double>::max();
    } else {
      obstacle_overtake_lowers.back().second = care_overlap_polygon.max_x() + kSOvertakeBuffer;
    }

    obstacle_yield_uppers.back().second = care_overlap_polygon.min_x() - vehicle_param_.rear_axis_to_front_edge;

    if (i == 0) {
      // check initial collision
      if (!(care_overlap_polygon.min_x() - vehicle_param_.rear_axis_to_front_edge >
                reference_path_ptr_->get_frenet_ego_state().planning_init_point().frenet_state.s ||
            care_overlap_polygon.max_x() + vehicle_param_.back_edge_to_rear_axis <
                reference_path_ptr_->get_frenet_ego_state().planning_init_point().frenet_state.s)) {
        const auto &frenet_ego_state = reference_path_ptr_->get_frenet_ego_state();
        if (!frenet_ego_state.polygon().HasOverlap(obstacle_sl_polygon) &&
          ...
          pipeline_context_->planning_result.extra_json["lon_decision_error_info"] = "detect_initial_point_collision";
        }
      }
    }
  }
  ...
  for (size_t i = 0; i < traj_points.size(); ++i) {
    ...
    if (not exist) {
      ...
      if (lon_decision == LonObstacleDecisionType::YIELD and obstacle_yield_uppers[i].second < 1000) {
        need_create = true;
      } else if (lon_decision == LonObstacleDecisionType::OVERTAKE and obstacle_overtake_lowers[i].second > 0) {
  ...
  if (fabs(planning_math::NormalizeAngle(obstacle->frenet_relative_velocity_angle())) > M_PI / 4 * 3 &&
      obstacle->velocity() > 1.0) {
    ignore_relative_time = std::min(ignore_relative_time, config_.lon_max_ignore_relative_time);
  }

  auto ego_sl_boundary = reference_path_ptr_->get_ego_frenet_boundary();
  bool is_CIPV =
      !(obstacle_sl_boundary.l_start > ego_sl_boundary.l_end || obstacle_sl_boundary.l_end < ego_sl_boundary.l_start);
  auto ego_velocity = reference_path_ptr_->get_frenet_ego_state().velocity_s();
  auto distance_buff = is_CIPV ? std::min(5.0, std::max(0.0, ego_velocity - obstacle->frenet_velocity_s()) * 0.5) : 0.0;
  auto ini_ds = obstacle_sl_boundary.s_start - ego_sl_boundary.s_end;
  auto lat_overlap_ratio =
      is_CIPV ? std::min(1.0, std::max(0.0, std::min(ego_sl_boundary.l_end - obstacle_sl_boundary.l_start,
                                                     obstacle_sl_boundary.l_end - ego_sl_boundary.l_start)) /
                                  care_width)
              : 0.0;
  // keep stop before close leadone go away, ignore prediction
  const double ego_relative_heading = reference_path_ptr_->get_frenet_ego_state().heading_angle();
  const double ego_front_left_s = reference_path_ptr_->get_frenet_ego_state().corners().s_front_left;
  const double ego_front_left_l = reference_path_ptr_->get_frenet_ego_state().corners().l_front_left;
  const double ego_front_right_s = reference_path_ptr_->get_frenet_ego_state().corners().s_front_right;
  const double ego_front_right_l = reference_path_ptr_->get_frenet_ego_state().corners().l_front_right;
  const auto &frenet_obstacle_corners = obstacle->frenet_obstacle_corners();
  const double rad2deg = 180.0 / 3.1415926;
  const double min_radius =
      std::tan(vehicle_param_.max_steer_angle * rad2deg / vehicle_param_.steer_ratio) / vehicle_param_.wheel_base +
      vehicle_param_.width / 2.0;
  const bool unable_to_nudge_from_left =
      std::pow(frenet_obstacle_corners.s_rear_left - ego_front_right_s + min_radius * std::sin(ego_relative_heading),
               2) +
              std::pow(
                  frenet_obstacle_corners.l_rear_left - ego_front_right_l - min_radius * std::cos(ego_relative_heading),
                  2) <
          min_radius * min_radius ||
      std::pow(frenet_obstacle_corners.s_front_left - ego_front_right_s + min_radius * std::sin(ego_relative_heading),
               2) +
              std::pow(frenet_obstacle_corners.l_front_left - ego_front_right_l -
                           min_radius * std::cos(ego_relative_heading),
                       2) <
          min_radius * min_radius ||
      std::pow(frenet_obstacle_corners.s_rear_right - ego_front_right_s + min_radius * std::sin(ego_relative_heading),
               2) +
              std::pow(frenet_obstacle_corners.l_rear_right - ego_front_right_l -
                           min_radius * std::cos(ego_relative_heading),
                       2) <
          min_radius * min_radius ||
      std::pow(frenet_obstacle_corners.s_front_right - ego_front_right_s + min_radius * std::sin(ego_relative_heading),
               2) +
              std::pow(frenet_obstacle_corners.l_front_right - ego_front_right_l -
                           min_radius * std::cos(ego_relative_heading),
                       2) <
          min_radius * min_radius;
  const bool unable_to_nudge_from_right =
      std::pow(frenet_obstacle_corners.s_rear_right - ego_front_left_s - min_radius * std::sin(ego_relative_heading),
               2) +
              std::pow(
                  frenet_obstacle_corners.l_rear_right - ego_front_left_l + min_radius * std::cos(ego_relative_heading),
                  2) <
          min_radius * min_radius ||
      std::pow(frenet_obstacle_corners.s_front_right - ego_front_left_s - min_radius * std::sin(ego_relative_heading),
               2) +
              std::pow(frenet_obstacle_corners.l_front_right - ego_front_left_l +
                           min_radius * std::cos(ego_relative_heading),
                       2) <
          min_radius * min_radius ||
      std::pow(frenet_obstacle_corners.s_rear_left - ego_front_left_s - min_radius * std::sin(ego_relative_heading),
               2) +
              std::pow(
                  frenet_obstacle_corners.l_rear_left - ego_front_left_l + min_radius * std::cos(ego_relative_heading),
                  2) <
          min_radius * min_radius ||
      std::pow(frenet_obstacle_corners.s_front_left - ego_front_left_s - min_radius * std::sin(ego_relative_heading),
               2) +
              std::pow(
                  frenet_obstacle_corners.l_front_left - ego_front_left_l + min_radius * std::cos(ego_relative_heading),
                  2) <
          min_radius * min_radius;
  const bool too_close_to_nudge = unable_to_nudge_from_left && unable_to_nudge_from_right;
  LOG_DEBUG(
      "[GeneralLongitudinalDecider]: too close to nudge: %d  unable from left: "
      "% d unable from right: % d \n ",
      too_close_to_nudge, unable_to_nudge_from_left, unable_to_nudge_from_right);
  bool keep_stop = (pipeline_context_->planning_target_state != ROAD_LC_LCHANGE &&
                    pipeline_context_->planning_target_state != ROAD_LC_RCHANGE) &&
                   is_CIPV && ini_ds > 0.0 && ini_ds < kDisToCIPVThreshold && ego_velocity < kLowSpeedThreshold &&
                   too_close_to_nudge;
  ...
  for (size_t i = 0; i < obstacle_decision.position_decisions.size(); ++i) {
    ...
    if (t > ignore_relative_time or pos_decision.lon_decision != LonObstacleDecisionType::YIELD) {
      continue;
    }

    double frenet_point_s = obstacle->frenet_s();
    auto obstacle_point = obstacle->obstacle()->get_point_at_time(t);
    double relative_yaw = obstacle_point.velocity_direction - frenet_coord_->GetRefCurveHeading(frenet_point_s);
    double obstacle_v_lon = std::max(obstacle_point.v * std::cos(relative_yaw), 0.0);
    bool b_on_ego_path = std::min(std::min(fabs(obstacle_sl_boundary.l_start), fabs(obstacle_sl_boundary.l_end)),
    ...
    if (frame_->session()->is_parking_scene()) {
      ...
      distance_safe = kSafeDistanceInverse + std::fabs(relative_velocity) * kDistanceInverseTTC;
    }
    auto obstacle_type = obstacle->type();
    bool b_fast_car = obstacle_type != Common::ObjectType::OBJECT_TYPE_BICYCLE and
                      obstacle_type != Common::ObjectType::OBJECT_TYPE_PEDESTRIAN && obstacle_v_lon > ego_velocity &&
                      obstacle->obstacle()->acceleration() > 0.5;
    if (t < 0.3 && obstacle_yield_uppers[i].second < (5.0 + ego_s) && b_fast_car) {
      distance_safe = 1.0;
    }
    ...
    for (auto &t_upper_bound : obstacle_yield_uppers) {
      if (std::fabs(t - t_upper_bound.first) < 1e-2) {
        ...
        if (keep_stop) {
          yield_upper = std::min(yield_upper, obstacle_sl_boundary.s_start - vehicle_param_.rear_axis_to_front_edge);
          LOG_DEBUG("[GeneralLongitudinalDecider]: keep stop for close leadone");
        }
      }
    }

    pos_decision.lon_bounds.emplace_back(WeightedBound{std::numeric_limits<double>::min(), yield_upper - distance_safe,
                                                       -1, BoundInfo{obstacle->id(), "obstacle"}});

    for (size_t i = 0; i < traj_points.size(); ++i) {
      auto t_traj = traj_points[i].t;
      if (std::fabs(t_traj - t) < 1e-2 && lon_ref_path.lon_obstacle_yield_info[i].yield_upper > yield_upper) {
    ...
    lon_yield_info_.min_ttc =
        (obstacle_sl_boundary.s_start > ego_s && ego_velocity > obstacle->frenet_velocity_s())
            ? std::min(lon_yield_info_.min_ttc,
                       (obstacle_sl_boundary.s_start - ego_s) / (ego_velocity - obstacle->frenet_velocity_s()))
            : lon_yield_info_.min_ttc;
    if (yield_ds > 0.0) {
      if (obstacle_v_lon < ego_velocity) {
        double a_yield = (obstacle_v_lon * obstacle_v_lon - ego_velocity * ego_velocity) /
                         (2.0 * std::max(yield_ds - distance_safe, 0.5 * vehicle_param_.length));
        lon_yield_info_.min_acc = std::min(lon_yield_info_.min_acc, a_yield);
      } else if (yield_ds - distance_safe < ego_velocity * t) {
        double a_yield = 2.0 * (yield_ds - distance_safe - ego_velocity * t) / std::max(dt_square, t * t);
        lon_yield_info_.min_acc = std::min(lon_yield_info_.min_acc, a_yield);
      }
    }
    double jerk_yield =
        6.0 * (yield_ds - distance_safe - ego_velocity * t - 0.5 * ego_acc * t * t) / std::max(dt_cube, t * t * t);
    lon_yield_info_.min_jerk =
        jerk_yield < 0.0 ? std::min(lon_yield_info_.min_jerk, jerk_yield) : lon_yield_info_.min_jerk;

    // add ttc cost:
    const bool b_high_speed_diff = ego_velocity > 25.0 / 3.6 && ego_velocity > obstacle->frenet_velocity_s();
    const double t_ego = std::min(1.0, std::max(0.4, 2.0 * lat_overlap_ratio + 0.4 * t));
    const double ttc_pre =
        std::max(1.0, yield_upper - ego_sl_boundary.s_end) / std::max(0.1, ego_velocity - obstacle_v_lon);
    const double able_to_yield =
        yield_ds > distance_safe && obstacle_v_lon < ego_velocity &&
        (obstacle_v_lon * obstacle_v_lon - ego_velocity * ego_velocity) / 2.0 / (yield_ds - distance_safe) >
            config_.max_deceleration;
    const double ttc_buff = ((b_on_ego_path || is_cross_obj) && able_to_yield)
                                ? std::max(0.0, std::min(1.0, 0.5 * std::max(0.0, 4.0 - ttc_pre)))
                                : 0.0;
    const bool enable_ttc_cost = (b_high_speed_diff || b_on_ego_path || (is_cross_obj && able_to_yield));
    LOG_DEBUG(
        ...
        obstacle->id(), t, ttc_pre, ttc_buff, t_ego, enable_ttc_cost, able_to_yield, is_cross_obj, b_on_ego_path);
    if (enable_ttc_cost) {
...
void GeneralLongitudinalDecider::prepare_obstacle_decision(ObstacleDecisions &obstacle_decisions, int obstacle_id) {
  // NTRACE_CALL(10);
...
void GeneralLongitudinalDecider::prepare_obstacle_position_decision(ObstacleDecision &obstacle_decision,
...
void GeneralLongitudinalDecider::construct_longitudinal_outer_decision(const TrajectoryPoints &traj_points,
                                                                       const ReferencePathPoints &refpath_points,
  ...
  // Step 1) wait speed
  if (b_enable_lane_wait_adjust_speed && coarse_planning_info.target_state == ROAD_LC_LWAIT ||
          ...
          for (auto &traj_pt : traj_points) {
            ...
            for (auto &pos_decision : obstacle_decision.position_decisions) {
              if (std::fabs(pos_decision.tp.t - traj_pt.t) < 1e-2) {
                if (pos_decision.lon_decision == LonObstacleDecisionType::IGNORE) {
                  pos_decision.lon_decision = LonObstacleDecisionType::OVERTAKE;

                  Polygon2d obstacle_sl_polygon;
                  auto ok = obstacle->get_polygon_at_time(traj_pt.t, reference_path_ptr_, obstacle_sl_polygon);
                  if (not ok) {
                    continue;
                  }
                  pos_decision.lon_bounds.push_back(
                      WeightedBound{obstacle_sl_polygon.max_x() + vehicle_param_.back_edge_to_rear_axis,
                                    std::numeric_limits<double>::max(), 1.0});
                  LOG_DEBUG(
                      "wait_speed overtake obstacle_id:%d, t:%f, "
                      "lower_bound:%f \n",
                      overtake_obstacle_id, traj_pt.t,
                      obstacle_sl_polygon.max_x() + vehicle_param_.back_edge_to_rear_axis);
                }
              }
            }
          }
                ...
                if (pos_decision.lon_decision == LonObstacleDecisionType::IGNORE) {
                  ...
                  pos_decision.lon_bounds.push_back(
                      WeightedBound{std::numeric_limits<double>::min(),
                                    obstacle_sl_polygon.min_x() - vehicle_param_.rear_axis_to_front_edge, 1.0});
                  LOG_DEBUG(
                      "wait_speed yield obstacle_id:%d, t:%f,upper_bound: % f "
                      "\n",
                      yield_obstacle_id, traj_pt.t,
                      obstacle_sl_polygon.min_x() - vehicle_param_.rear_axis_to_front_edge);
                }
              }
            }
          }
...
void GeneralLongitudinalDecider::construct_refpath_points(const TrajectoryPoints &traj_points,
                                                          ReferencePathPoints &refpath_points) {
  // NTRACE_CALL(8);

  for (auto &traj_pt : traj_points) {
    ReferencePathPoint refpath_pt;
    auto success = reference_path_ptr_->get_reference_point_by_lon(traj_pt.s, refpath_pt);
    (void)success;
    // assert(success);
...
void GeneralLongitudinalDecider::construct_lon_decision_trajectory(const TrajectoryPoints &traj_points,
...
void GeneralLongitudinalDecider::get_lon_decision_info(LonDecisionInfo &lon_decision_information) {
  // NTRACE_CALL(8);

  auto &ego_planning_info = pipeline_context_->planning_info;
  double leadone_min_s_leadone = 10000.0;
  int current_lane_virtual_id =
      frame_->session()->environmental_model().get_virtual_lane_manager()->current_lane_virtual_id();
  auto reference_path_manager = frame_->session()->environmental_model().get_reference_path_manager();
  auto current_reference_path = reference_path_manager->get_reference_path_by_lane(current_lane_virtual_id);
  ...
  if (!frenet_obstacles_map.empty()) {
    const double ego_l_start = reference_path_ptr_->get_frenet_ego_state().boundary().l_start;
    const double ego_l_end = reference_path_ptr_->get_frenet_ego_state().boundary().l_end;
    for (auto &obstacle_decision : ego_planning_info.obstacle_decisions) {
      ...
      bool left_satisfy = frenet_obstacle->frenet_obstacle_boundary().l_start > ego_l_end &&
                          frenet_obstacle->frenet_velocity_l() > lateral_v_threshold;
      bool right_satisfy = frenet_obstacle->frenet_obstacle_boundary().l_end < ego_l_start &&
                           frenet_obstacle->frenet_velocity_l() < -lateral_v_threshold;
      if (!lon_decision_information.nearby_obstacle) {
        ...
        LOG_DEBUG(
            ...
            frenet_obstacle->frenet_obstacle_boundary().l_start, frenet_obstacle->frenet_obstacle_boundary().l_end,
            left_satisfy, right_satisfy, ego_l_start, ego_l_end, frenet_obstacle->frenet_s(), l_away_ego, ego_s,
            get_lon_success);
        if (!l_away_ego) {
          if (get_lon_success) {
            double distance_to_left_road_border = refpath_pt.distance_to_left_road_border;
            double distance_to_right_road_border = refpath_pt.distance_to_right_road_border;
            bool lateral_satisfy =
                (frenet_obstacle->frenet_obstacle_boundary().l_start > ego_l_end &&
                 frenet_obstacle->frenet_obstacle_boundary().l_start < distance_to_left_road_border) ||
                (frenet_obstacle->frenet_obstacle_boundary().l_end < ego_l_start &&
                 frenet_obstacle->frenet_obstacle_boundary().l_end > -distance_to_right_road_border);
            bool longitual_satisfy =
                (frenet_obstacle->frenet_s() - ego_s > -10.0) && (frenet_obstacle->frenet_s() - ego_s < 100.0);
            lon_decision_information.nearby_obstacle =
                lon_decision_information.nearby_obstacle || (lateral_satisfy && longitual_satisfy);
            LOG_DEBUG(
                "Distance_to_left_road_border: %.2f, "
                "distance_to_right_road_border: %.2f, lateral_satisfy: "
                "%d,longitual_satisfy: % d \n",
                distance_to_left_road_border, distance_to_right_road_border, lateral_satisfy, longitual_satisfy);
          } else {
            bool s_l_satisfy = (frenet_obstacle->frenet_obstacle_boundary().l_start > ego_l_end ||
                                frenet_obstacle->frenet_obstacle_boundary().l_end < ego_l_start) &&
                               (frenet_obstacle->frenet_s() - ego_s > -50.0) &&
                               (frenet_obstacle->frenet_s() - ego_s < 100.0);
            lon_decision_information.nearby_obstacle = lon_decision_information.nearby_obstacle || s_l_satisfy;
      ...
      if (!obstacle_decision.second.position_decisions.empty()) {
        has_lon_decision_overtake =
            obstacle_decision.second.position_decisions[0].lon_decision == LonObstacleDecisionType::OVERTAKE;
        has_lon_decision_yield =
            obstacle_decision.second.position_decisions[0].lon_decision == LonObstacleDecisionType::YIELD;
      } else {
        continue;
      }
      const bool satisfy_s_leadone =
          (frenet_obstacle->frenet_s() - ego_s - vehicle_param_.rear_axis_to_front_edge < leadone_min_s_leadone) &&
          frenet_obstacle->frenet_s() - ego_s - vehicle_param_.rear_axis_to_front_edge > 0;
      const bool satisfy_s_CIPV = frenet_obstacle->frenet_s() - ego_s - vehicle_param_.rear_axis_to_front_edge > 0;
      const bool polygen_out_lane = frenet_obstacle->frenet_obstacle_boundary().l_start > ego_l_end ||
                                    frenet_obstacle->frenet_obstacle_boundary().l_end < ego_l_start;
      LOG_DEBUG(
          "HHLDEBUG: l_start: %.2f, l_end: %.2f, ego_s: %.2f, "
          "ego_l_start:%.2f, ego_l_end: %.2f, half_ego_length: %.2f \n",
          frenet_obstacle->frenet_obstacle_boundary().l_start, frenet_obstacle->frenet_obstacle_boundary().l_end, ego_s,
      ...
      LOG_DEBUG(
          ...
          has_lon_decision_overtake, satisfy_s_leadone, satisfy_s_CIPV, polygen_out_lane);
      if (!polygen_out_lane) {
        ...
        if (satisfy_s_leadone) {
          lon_decision_information.leadone_info.leadone_information.obstacle_s =
              frenet_obstacle->frenet_s() - ego_s - vehicle_param_.rear_axis_to_front_edge;
          lon_decision_information.leadone_info.leadone_information.obstacle_id = obstacle_decision.first;
          lon_decision_information.leadone_info.leadone_information.obstacle_length = std::max(
              frenet_obstacle->frenet_obstacle_boundary().s_end - frenet_obstacle->frenet_obstacle_boundary().s_start,
              4.0);
          leadone_min_s_leadone = lon_decision_information.leadone_info.leadone_information.obstacle_s;
          lon_decision_information.leadone_info.leadone_information.obstacle_v = frenet_obstacle->velocity();
          if (frenet_obstacle->type() == Common::ObjectType::OBJECT_TYPE_PEDESTRIAN ||
              frenet_obstacle->type() == Common::ObjectType::OBJECT_TYPE_BICYCLE) {
            lon_decision_information.leadone_info.leadone_information.obstacle_type = 0;
          } else {
            lon_decision_information.leadone_info.leadone_information.obstacle_type = 1;
          }
        }
        // obtain CIPV information
        if (satisfy_s_CIPV) {
          lon_decision_information.CIPV_info.has_CIPV = true;
          CIPV_object.obstacle_s = frenet_obstacle->frenet_s() - ego_s - vehicle_param_.rear_axis_to_front_edge;
          ...
          lon_decision_information.CIPV_info.CIPV_information.emplace_back(CIPV_object);
        }
      }
      // obstain cutin information
        ...
        cutin_object.obstacle_s = frenet_obstacle->frenet_s() - ego_s - vehicle_param_.rear_axis_to_front_edge;
        cutin_object.obstacle_v = frenet_obstacle->velocity();
        cutin_object.obstacle_id = obstacle_decision.first;
        lon_decision_information.cutin_info.cutin_information.emplace_back(cutin_object);
      }
    }
  }
  ...
  if (lon_decision_information.leadone_info.has_leadone) {
    LOG_DEBUG(
        ...
        lon_decision_information.leadone_info.leadone_information.obstacle_length,
        lon_decision_information.nearby_obstacle);
  }
  if (lon_decision_information.CIPV_info.has_CIPV) {
    LOG_DEBUG("HHLDEBUG: has_CIPV: %d", lon_decision_information.leadone_info.has_leadone);
  }
  if (lon_decision_information.cutin_info.has_cutin) {
    LOG_DEBUG("HHLDEBUG: has_cutin: %d", lon_decision_information.cutin_info.has_cutin);
  }
  frenet_obstacles_map.clear();
}

bool GeneralLongitudinalDecider::check_obstacle_both_sides(const std::shared_ptr<FrenetObstacle> obstacle) {
  auto ego_sl_boundary = reference_path_ptr_->get_ego_frenet_boundary();
  ...
  if (obstacle_sl_boundary.s_end - kLonIgnoreDistance > ego_sl_boundary.s_start &&
...
void GeneralLongitudinalDecider::GenerateLonRefPathPB(const LonRefPath &lon_ref_path) {
  ...
  long_ref_path_pb->mutable_lon_lead_bounds()->Reserve(lon_ref_path.lon_lead_bounds.size());
  ...
  lon_bounds_jerk_pb->mutable_bound()->Reserve(lon_ref_path.lon_bound_jerk.size());
