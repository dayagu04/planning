...
class GeneralLongitudinalDecider : public Task {
 public:
  explicit GeneralLongitudinalDecider(const EgoPlanningConfigBuilder *config_builder,
 ...
 private:
  ...
  BoundedConstantJerkTrajectory1d get_velocity_limit(const LonRefPath &lon_ref_path);
  ...
  // 函数名需要统一
  void construct_longitudinal_obstacle_decisions(const TrajectoryPoints &traj_points,
                                                 const ReferencePathPoints &refpath_points,
                                                 ObstacleDecisions &obstacle_decisions, LonRefPath &lon_ref_path);

  // TBD(binwang33): refpath_points 入参没有被使用，需要去除
  // 函数名需要统一
  void construct_longitudinal_obstacle_decision(const TrajectoryPoints &traj_points,
                                                const ReferencePathPoints &refpath_points,
                                                const std::vector<planning_math::Polygon2d> &overlap_path,
                                                const std::shared_ptr<FrenetObstacle> obstacle,
                                                ObstacleDecision &obstacle_decision, LonRefPath &lon_ref_path);

  void make_longitudinal_overlap_path(const TrajectoryPoints &traj_points,
  ...
  // TBD(binwang33): refpath_points 入参没有被使用，需要去除
  // 函数名需要统一
  void construct_refpath_points(const TrajectoryPoints &traj_points, ReferencePathPoints &refpath_points);

  void construct_lon_decision_trajectory(const TrajectoryPoints &traj_points,
                                         TrajectoryPoints &lon_decision_traj_points);

  // outer decision
  void construct_longitudinal_outer_decision(const TrajectoryPoints &traj_points,
                                             const ReferencePathPoints &refpath_points,
                                             const CoarsePlanningInfo &coarse_planning_info,
                                             ObstacleDecisions &obstacle_decisions);

  static void prepare_obstacle_decision(ObstacleDecisions &obstacle_decisions, int obstacle_id);
  static void prepare_obstacle_position_decision(ObstacleDecision &obstacle_decision, const TrajectoryPoint &traj_pt);

  void generate_lon_decision_from_path(const TrajectoryPoints &lateral_trajectory_points,
                                       const ReferencePathPoints &refpath_points, ObstacleDecisions &obstacle_decisions,
