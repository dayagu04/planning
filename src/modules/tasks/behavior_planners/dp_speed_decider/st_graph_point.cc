...
namespace planning {
  ...
  // set value
  void STGraphPoint::Init(const uint32_t index_t, const uint32_t index_s,const STPoint& st_point){
    index_s_ = index_s;
    index_t_ = index_t;
    point_ = st_point;
  }
  void STGraphPoint::SetReferenceCost(const double reference_cost){reference_cost_ = reference_cost;}
  void STGraphPoint::SetObstacleCost(const double obstacle_cost){obstacle_cost_ = obstacle_cost;}
  void STGraphPoint::SetTotalCost(const double total_cost){total_cost_ = total_cost;}
  void STGraphPoint::SetPrePoint(const STGraphPoint& pre_point){pre_point_ = &pre_point;}// modify root 
