...
// cost coeffi
 ...
 public:
  ...
  bool DPSearchPath();
  bool FinedReferencePath(LaneBorrowDeciderOutput* lane_borrow_decider_output);
  ...
  void set_left_boundary(double left_boundary){sample_left_boundary_ = left_boundary;}
  void set_right_boundary(double right_boundary){sample_right_boundary_ = right_boundary;}
  // Getters
  ...
  const std::vector<StaticObstacleInfo>& obstacles_info() const { return obstacles_info_; }
  std::vector<StaticObstacleInfo>& mutable_obstacles_info() { return obstacles_info_; }
