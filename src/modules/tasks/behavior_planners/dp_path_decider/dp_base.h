...
class ComparableCost {
 public:
  ...
  ComparableCost(const bool has_collision, const bool out_boundary,
                 const double safety_cost, const double smooth_cost,
                 const double stitch_cost)
  ...
  int CompareTo(const ComparableCost &another) const {
    ...
    if ((has_collision_ || out_boundary_) &&
      ...
      return 1;
               ...
               (another.has_collision_ || another.out_boundary_)) {
      return -1;
    } else if (std::fabs(safety_cost_ + smooth_cost_ + stitch_cost_ -
                         another.safety_cost_ - another.smooth_cost_ -
                         another.stitch_cost_) < kEpsilion) {
      return 0;
    } else if (safety_cost_ + smooth_cost_ + stitch_cost_ >
               another.safety_cost_ + another.smooth_cost_ +
                   another.stitch_cost_) {
      return 1;
    } else {
      return -1;
    }
