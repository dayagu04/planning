...
bool LateralOffsetCalculator::update(
  ...
  if (is_LC_LWAIT || is_LC_RWAIT || is_LC_LBACK || is_LC_RBACK) {
...
void LateralOffsetCalculator::Reset() {
  lat_offset_ = 0.0;
  // TODO(clren)
}
...
void LateralOffsetCalculator::update_premove_path(
  ...
  bool is_LC_RBACK =
      (state == kLaneChangeCancel) && (lc_request_direction == RIGHT_CHANGE);
  if ((is_LC_LWAIT && (should_premove || accident_ahead)) || is_LC_LBACK) {
    premoving_ = true;
  ...
  } else if ((status == is_LC_RWAIT && (should_premove || accident_ahead)) ||
             status == is_LC_RBACK) {
    premoving_ = true;
...
bool LateralOffsetCalculator::update_avoidance_path(
  ...
  bool is_LC_RCHANGE =
      ((state == kLaneChangeExecution) || (state == kLaneChangeComplete)) &&
      (lc_request_direction == RIGHT_CHANGE);
  if (status == StateMachineLaneChangeStatus::kLaneKeeping ||
      status == StateMachineLaneChangeStatus::kLaneChangePropose ||
      status == StateMachineLaneChangeStatus::kLaneChangeExecution ||
      status == StateMachineLaneChangeStatus::kLaneChangeComplete ||
      status == StateMachineLaneChangeStatus::kLaneChangeCancel ||
      status == StateMachineLaneChangeStatus::kLaneChangeHold) {
...
bool LateralOffsetCalculator::update_planner_output() {
...
void LateralOffsetCalculator::calc_desired_path(
