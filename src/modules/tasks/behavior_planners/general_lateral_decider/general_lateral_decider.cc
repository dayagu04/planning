...
GeneralLateralDecider::GeneralLateralDecider(
  ...
  frenet_soft_bounds_second_.resize(config_.num_step + 1);
...
bool GeneralLateralDecider::InitInfo() {
  ...
  soft_bounds_.clear();
  soft_bounds_second_.clear();
  hard_bounds_.clear();
  frenet_soft_bounds_.assign(frenet_soft_bounds_.size(),
                             std::make_pair(0.0, 0.0));
  frenet_soft_bounds_second_.assign(frenet_soft_bounds_second_.size(),
                             std::make_pair(0.0, 0.0));
...
bool GeneralLateralDecider::Execute() {
  ...
  ExtractBoundary(frenet_soft_bounds_, frenet_soft_bounds_second_,
                  frenet_hard_bounds_, soft_bounds_info_,
                  soft_bounds_second_info_, hard_bounds_info_);
  ...
  GenerateLateralDeciderOutput(frenet_soft_bounds_, frenet_soft_bounds_second_,
                               frenet_hard_bounds_, soft_bounds_info_,
                               soft_bounds_second_info_, hard_bounds_info_,
                               general_lateral_decider_output);
  ...
  SaveLatDebugInfo(frenet_soft_bounds_, frenet_soft_bounds_second_, frenet_hard_bounds_, soft_bounds_info_,
                   soft_bounds_second_info_, hard_bounds_info_);
...
bool GeneralLateralDecider::ConstructReferencePathPoints(
  ...
  for (const auto &traj_point : traj_points) {
    ...
    if (coarse_planning_info.cart_ref_info.k_s_spline.get_x().size() > 0) {
      road_radius2 =
          1 / std::max(std::fabs(coarse_planning_info.cart_ref_info.k_s_spline(
                           traj_point.s)),
                       1e-6);
    }
...
void GeneralLateralDecider::GenerateRoadHardSoftBoundary() {
  ...
  soft_bounds_second_.resize(ref_traj_points_.size());
  for (size_t i = 0; i < ref_traj_points_.size(); i++) {
    ...
    // generate soft_bounds_second_ according to hard_bounds_ and soft_bounds_
  }
}
...
void GeneralLateralDecider::ExtendRoadSoftBound(
  ...
  if (is_lower) {
    for (int i = 0; i < bound_size; ++i) {
      ...
      while (!index_dq.empty() && soft_bounds_[index_dq.back()][0].lower <=
                                      soft_bounds_[i][0].lower) {
        index_dq.pop_back();
      }
      index_dq.push_back(i);
      if (i >= num_rear_extended_points - 1) {
        soft_bounds_[i - num_rear_extended_points + 1][0].lower =
            soft_bounds_[index_dq.front()][0].lower;
      }
      if (i == bound_size - 1) {
    ...
    for (int i = 0; i < bound_size; ++i) {
      ...
      while (!index_dq.empty() && soft_bounds_[index_dq.back()][0].upper >=
                                      soft_bounds_[i][0].upper) {
        index_dq.pop_back();
      }
      index_dq.push_back(i);
      if (i >= num_rear_extended_points - 1) {
        soft_bounds_[i - num_rear_extended_points + 1][0].upper =
            soft_bounds_[index_dq.front()][0].upper;
      }
      if (i == bound_size - 1) {
        int start_index = std::max(i - num_rear_extended_points + 1, 0);
        for (int j = start_index; j < bound_size; ++j) {
...
void GeneralLateralDecider::GenerateStaticObstacleDecision(
  ...
  for (size_t i = 0; i < ref_traj_points_.size(); i++) {
    ...
    double lat_buf_dis = CalStaticNudgeLatBufDis(
        ...
        is_side_obstacle, extra_lane_type_decrease_buffer, is_update_hard_bound,
...
double GeneralLateralDecider::CalStaticNudgeLatBufDis(
  ...
  double lat_buf_dis =
      general_lateral_decider_utils::CalDesireStaticLateralDistance(
          ...
          ego_frenet_state_.l(), obstacle, is_update_hard_bound, config_);
...
void GeneralLateralDecider::GenerateDynamicObstacleDecision(
  ...
  for (size_t i = 0; i < plan_history_traj_.size(); i++) {
    ...
    double lat_buf_dis = CalDynamicNudgeLatBufDis(
        ...
        is_same_side_obstacle_during_lane_change, is_update_hard_bound,
...
void GeneralLateralDecider::SaveLatDebugInfo(
  ...
  for (size_t i = 0; i < ref_traj_points_.size(); ++i) {
    ...
    auto soft_lower_bound_second_info =
        lat_debug_info_.mutable_soft_lower_bound_second_info_vec()->Add();
    soft_lower_bound_second_info->set_lower(frenet_soft_bounds_second[i].first);
    soft_lower_bound_second_info->mutable_bound_info()->set_id(
        soft_bounds_second_info[i].first.id);
    soft_lower_bound_second_info->mutable_bound_info()->set_type(
        BoundType2String(soft_bounds_second_info[i].first.type));

    auto soft_upper_bound_second_info =
        lat_debug_info_.mutable_soft_upper_bound_second_info_vec()->Add();
    soft_upper_bound_second_info->set_upper(frenet_soft_bounds_second[i].second);
    soft_upper_bound_second_info->mutable_bound_info()->set_id(
        soft_bounds_second_info[i].second.id);
    soft_upper_bound_second_info->mutable_bound_info()->set_type(
        BoundType2String(soft_bounds_second_info[i].second.type));
  }
...
void GeneralLateralDecider::GenerateLateralDeciderOutput(
  ...
  general_lateral_decider_output.soft_bounds_second = std::move(soft_bounds_second_);
  general_lateral_decider_output.hard_bounds = std::move(hard_bounds_);

  GenerateEnuBoundaryPoints(frenet_soft_bounds, frenet_soft_bounds_second, frenet_hard_bounds,
                            general_lateral_decider_output);
  ...
  auto &soft_bounds_second_frenet_output =
      general_lateral_decider_output.soft_bounds_second_frenet_point;
  ...
  auto &soft_bounds_second_info_output =
      general_lateral_decider_output.soft_bounds_second_info;
  ...
  for (size_t i = 0; i < ref_traj_points_.size(); ++i) {
    ...
    soft_bounds_second_frenet_output.emplace_back(frenet_soft_bounds_second[i]);
    soft_bounds_frenet_output.emplace_back(frenet_soft_bounds[i]);
    hard_bounds_info_output.emplace_back(hard_bounds_info[i]);
    soft_bounds_second_info_output.emplace_back(soft_bounds_second_info[i]);
...
bool GeneralLateralDecider::CheckLateralEmergencyAvoidSpace(
  ...
  // 判定自车离道路边缘的距离
  double max_lower_soft_bound_l = soft_bounds_[0][0].lower;
  double min_upper_soft_bound_l = soft_bounds_[0][0].upper;
  for (int i = 0; i < soft_bounds_.size(); ++i) {
    if (is_nudge_left) {
      if (soft_bounds_[i][0].bound_info.type == BoundType::ROAD_BORDER &&
          planning_init_point_l - soft_bounds_[i][0].lower <
              config_.emergency_avoid_safe_space_to_road_soft_bound_thr) {
        return false;
      }
      max_lower_soft_bound_l = soft_bounds_[i][0].lower > max_lower_soft_bound_l
                                   ? soft_bounds_[i][0].lower
                                   : max_lower_soft_bound_l;
    } else {
      if (soft_bounds_[i][0].bound_info.type == BoundType::ROAD_BORDER &&
          soft_bounds_[i][0].upper - planning_init_point_l <
              config_.emergency_avoid_safe_space_to_road_soft_bound_thr) {
        return false;
      }
      min_upper_soft_bound_l = soft_bounds_[i][0].upper < min_upper_soft_bound_l
                                   ? soft_bounds_[i][0].upper
