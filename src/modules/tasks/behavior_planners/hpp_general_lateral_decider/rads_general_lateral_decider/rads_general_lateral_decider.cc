#include "rads_general_lateral_decider.h"
...
#include "rads_general_lateral_decider_utils.h"
#include "log.h"
...
bool RADSGeneralLateralDecider::InitInfo() {
...
bool RADSGeneralLateralDecider::Execute() {
  LOG_DEBUG("=======RADSGeneralLateralDecider======= \n");
...
bool RADSGeneralLateralDecider::ExecuteTest(bool pipeline_test) {
  // pipeline test
  return true;
}

void RADSGeneralLateralDecider::UnitTest() {
  for (int i = 1; i < 10; i++) {
...
bool RADSGeneralLateralDecider::CalCruiseVelByCurvature(
...
void RADSGeneralLateralDecider::ConstructTrajPoints(
...
bool RADSGeneralLateralDecider::HandleAraPath(TrajectoryPoints &traj_points) {
...
void RADSGeneralLateralDecider::HandleAvoidScene(TrajectoryPoints &traj_points,
...
bool RADSGeneralLateralDecider::ConstructReferencePathPoints(
    ...
    if (config_.enable_last_lat_path) {
      ...
      for (size_t i = 0; i < ref_traj_points_.size(); ++i) {
        if (motion_planner_output.s_lat_vec.size() > 0) {
          ...
          if (is_ego_reverse_) {
            last_path_theta += M_PI;  // reverse 180 deg
          }
  ...
  if (ego_s <= plan_history_traj_tmp.front().s) {
    for (double t = 0; t <= plan_history_traj_tmp.back().t;
         t += config_.delta_t) {
      TrajectoryPoint pt =
          rads_general_lateral_decider_utils::GetTrajectoryPointAtTime(
  ...
  for (int i = 0; i < plan_history_traj_.size(); i++) {
    ...
    std::vector<int> match_indexes =
        rads_general_lateral_decider_utils::MatchRefTrajPoints(
...
void RADSGeneralLateralDecider::UpdateDistanceToRoadBorder() {
  const auto &vehicle_param =
      VehicleConfigurationContext::Instance()->get_vehicle_param();
  for (size_t i = 0; i < ref_traj_points_.size(); i++) {
...
void RADSGeneralLateralDecider::GenerateRoadAndLaneBoundary() {
...
void RADSGeneralLateralDecider::GenerateGroundLineAndParkingSpaceBoundary() {
  ...
  for (size_t i = 0; i < ref_traj_points_.size(); i++) {
    ...
    for (int direction = 0; direction < 2; direction++) {
      ...
      ObstacleBorderInfo parking_space_border =
          rads_general_lateral_decider_utils::GetNearestObstacleBorder(
...
void RADSGeneralLateralDecider::GenerateRoadHardSoftBoundary() {
  const auto &vehicle_param =
      VehicleConfigurationContext::Instance()->get_vehicle_param();
...
void RADSGeneralLateralDecider::GenerateLaneSoftBoundary() {
  const auto &vehicle_param =
      VehicleConfigurationContext::Instance()->get_vehicle_param();
  const auto &coarse_planning_info = session_->planning_context()
                                         .lane_change_decider_output()
...
void RADSGeneralLateralDecider::GetDesireRoadExtraBuffer(
...
void RADSGeneralLateralDecider::GetLateralTTCToRoad(
...
void RADSGeneralLateralDecider::GenerateObstaclesBoundary() {
...
void RADSGeneralLateralDecider::GenerateStaticObstaclesBoundary(
...
void RADSGeneralLateralDecider::GenerateStaticObstacleDecision(
  ...
  if (!is_update_hard_bound) {
    front_lon_buf_dis = rads_general_lateral_decider_utils::CalDesireLonDistance(
  ...
  for (size_t i = 0; i < ref_traj_points_.size(); i++) {
    ...
    double lat_buf_dis =
        rads_general_lateral_decider_utils::CalDesireStaticLateralDistance(
...
bool RADSGeneralLateralDecider::IsCutoutSideObstacle(
    const std::shared_ptr<FrenetObstacle> obstacle,
    Polygon2d &care_overlap_polygon) {
  if (plan_history_traj_.empty()) {
    return false;
...
double RADSGeneralLateralDecider::CalculateExtraDecreaseBuffer(
  ...
  if (rads_general_lateral_decider_utils::IsTruck(obstacle)) {
...
void RADSGeneralLateralDecider::GenerateDynamicObstaclesBoundary(
    const std::vector<std::shared_ptr<FrenetObstacle>>& obs_vec,
    ObstacleDecisions &obstacle_decisions) {
  for (auto &obstacle : obs_vec) {
...
void RADSGeneralLateralDecider::GenerateDynamicObstacleDecision(
    const std::shared_ptr<FrenetObstacle> obstacle,
  ...
  const double front_lon_buf_dis =
      rads_general_lateral_decider_utils::CalDesireLonDistance(
  ...
  for (size_t i = 0; i < plan_history_traj_.size(); i++) {
    ...
    double lat_buf_dis =
        rads_general_lateral_decider_utils::CalDesireLateralDistance(
...
void RADSGeneralLateralDecider::GenerateObstaclePreliminaryDecision(
...
void RADSGeneralLateralDecider::AddObstacleDecisionBound(
...
bool RADSGeneralLateralDecider::CheckObstacleNudgeDecision(
    const std::shared_ptr<FrenetObstacle> &obstacle) {
...
bool RADSGeneralLateralDecider::CheckObstacleCrossingCondition(
    const std::shared_ptr<FrenetObstacle> obstacle, bool &is_cross_obj) {
  return false;
}

void RADSGeneralLateralDecider::RefineConflictLatDecisions(
...
void RADSGeneralLateralDecider::ExtractBoundary(
...
void RADSGeneralLateralDecider::ProtectBoundByInitPoint(
...
void RADSGeneralLateralDecider::ExtractDynamicObstacleBound(
...
void RADSGeneralLateralDecider::ExtractStaticObstacleBound(
    const ObstacleDecision &obstacle_decision) {
  if (ref_traj_points_.size() <= 0) {
    return;
  }
...
void RADSGeneralLateralDecider::PostProcessBound(
    const double planning_init_point_l,
  ...
  while ((lower_index < bounds_size) && (upper_index < bounds_size)) {
    ...
    const int lower_priority =
        rads_general_lateral_decider_utils::GetBoundTypePriority(lower_type);
    const int upper_priority =
        rads_general_lateral_decider_utils::GetBoundTypePriority(upper_type);
    const double lower_weight =
        rads_general_lateral_decider_utils::GetBoundWeight(
...
void RADSGeneralLateralDecider::SaveLatDebugInfo(
...
void RADSGeneralLateralDecider::PostProcessReferenceTrajBySoftBound(
    const std::vector<std::pair<double, double>> &second_frenet_soft_bounds,
    const std::vector<std::pair<double, double>> &first_frenet_soft_bounds,
...
void RADSGeneralLateralDecider::GenerateLateralDeciderOutput(
    const std::vector<std::pair<double, double>> &second_frenet_soft_bounds,
    const std::vector<std::pair<double, double>> &first_frenet_soft_bounds,
    const std::vector<std::pair<double, double>> &frenet_hard_bounds,
    const std::vector<std::pair<BoundInfo, BoundInfo>> &second_soft_bounds_info,
...
void RADSGeneralLateralDecider::GenerateEnuBoundaryPoints(
    const std::vector<std::pair<double, double>> &second_frenet_soft_bounds,
    const std::vector<std::pair<double, double>> &first_frenet_soft_bounds,
    const std::vector<std::pair<double, double>> &frenet_hard_bounds,
    GeneralLateralDeciderOutput &general_lateral_decider_output) {
...
void RADSGeneralLateralDecider::GenerateEnuReferenceTraj(
    GeneralLateralDeciderOutput &general_lateral_decider_output) {
  const std::shared_ptr<KDPath> frenet_coord =
      reference_path_ptr_->get_frenet_coord();
  if (frenet_coord == nullptr) {
...
void RADSGeneralLateralDecider::GenerateEnuReferenceTheta(
    GeneralLateralDeciderOutput &general_lateral_decider_output) {
...
void RADSGeneralLateralDecider::SampleRoadDistanceInfo(
...
void RADSGeneralLateralDecider::CalcLateralBehaviorOutput() {
...
bool RADSGeneralLateralDecider::IsAgentPredLonOverlapWithPlanPath(
    const std::shared_ptr<FrenetObstacle> obstacle) {
...
bool RADSGeneralLateralDecider::IsLonOverlap(
    const std::shared_ptr<FrenetObstacle> obstacle) {
...
bool RADSGeneralLateralDecider::IsFarObstacle(
    const std::shared_ptr<FrenetObstacle> obstacle) {
...
bool RADSGeneralLateralDecider::IsRearObstacle(
    const std::shared_ptr<FrenetObstacle> obstacle) {
  return reference_path_ptr_->get_ego_frenet_boundary().s_start >
         obstacle->frenet_obstacle_boundary().s_end;
}

bool RADSGeneralLateralDecider::IsFilterForStaticObstacle(
    const std::shared_ptr<FrenetObstacle> obstacle) {
...
bool RADSGeneralLateralDecider::IsFilterForDynamicObstacle(
    const std::shared_ptr<FrenetObstacle> obstacle) {
  const auto &otype = obstacle->type();
  const auto ofusion_source = obstacle->obstacle()->fusion_source();
  if ((ofusion_source & OBSTACLE_SOURCE_CAMERA) == 0) {
...
void RADSGeneralLateralDecider::ConstructStaticObstacleTotalPolygons(
    std::array<std::vector<std::pair<int, Polygon2d>>, 2> &groundline_polygons,
  ...
  for (auto &obstacle : reference_path_ptr->get_parking_space()) {
    ...
    rads_general_lateral_decider_utils::MakePolygon(
