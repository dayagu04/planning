...
TaskPipelineHpp::TaskPipelineHpp(const EgoPlanningConfigBuilder *config_builder,
                                 framework::Frame *frame)
    : TaskPipeline(config_builder, frame) {
  name_ = "TaskPipelineHpp";
  config_ = config_builder->cast<EgoPlanningTaskPipelineHppConfig>();
  version_to_tasks_["v1"] = {TaskType::HPP_GENERAL_LATERAL_DECIDER,
...
bool TaskPipelineHpp::Run(const EgoPlanningCandidate &candidate) {
  // NTRACE_CALL(6);
...
void TaskPipelineHpp::CreatePlanningTasks(
