...
class Task {
 public:
  ...
  static std::shared_ptr<Task> Make(const TaskType &task_type, const EgoPlanningConfigBuilder *config_builder,
                                    const std::shared_ptr<TaskPipelineContext> &pipeline_context);
 ...
 protected:
  ...
  std::shared_ptr<FrenetCoordinateSystem> frenet_coord_ = nullptr;  // 这个是否还需要？
  std::string name_{};
