#include "realtime_longitudinal_motion_planner.h"
...
namespace planning {
RealtimeLongitudinalMotionPlanner::RealtimeLongitudinalMotionPlanner(
    const EgoPlanningConfigBuilder *config_builder,
    const std::shared_ptr<TaskPipelineContext> &pipeline_context)
    : Task(config_builder, pipeline_context) {
  config_ = config_builder->cast<RealTimeLonMotionPlannerConfig>();
...
void RealtimeLongitudinalMotionPlanner::Init() {
  // init planning problem
  planning_problem_ptr_ =
      std::make_shared<pnc::realtime_longitudinal_planning::
                           RealtimeLongitudinalMotionPlanningProblem>();
...
bool RealtimeLongitudinalMotionPlanner::Execute(
    planning::framework::Frame *frame) {
  LOG_DEBUG("=======RealtimeLonMotionPlanner======= \n");
  ...
  JSON_DEBUG_VALUE("RealTimeLonMotionCostTime", end_time - start_time);

  return true;
}

void RealtimeLongitudinalMotionPlanner::AssembleInput() {
...
void RealtimeLongitudinalMotionPlanner::Update() {
