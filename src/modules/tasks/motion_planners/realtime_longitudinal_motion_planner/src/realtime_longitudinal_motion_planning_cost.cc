...
void NonNegativeVelCost::GetGradientHessian(const State &x, const Control &,
                                            LxMT &lx, LuMT &, LxxMT &lxx,
                                            LxuMT &, LuuMT &) {
  if (x[VEL] < 0.0) {
...
double LonSVBoundCost::GetCost(const State &x, const Control &) {
...
void LonSVBoundCost::GetGradientHessian(const State &x, const Control &,
                                        LxMT &lx, LuMT &, LxxMT &lxx, LxuMT &,
                                        LuuMT &) {
  // get v limit by s-v discrete points
  const std::vector<double> sv_bounds_s = {
