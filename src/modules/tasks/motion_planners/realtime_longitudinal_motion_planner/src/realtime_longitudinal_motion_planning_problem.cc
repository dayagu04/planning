#include "realtime_longitudinal_motion_planning_problem.h"
...
void RealtimeLongitudinalMotionPlanningProblem::Init() {
  ...
  ilqr_core_ptr_->Init(
      std::make_shared<RealtimeLongitudinalMotionPlanningModel>(),
      solver_config);
...
uint8_t RealtimeLongitudinalMotionPlanningProblem::Update(
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    auto s =
        state_result->at(i)[pnc::realtime_longitudinal_planning::StateId::POS];
    auto v =
        state_result->at(i)[pnc::realtime_longitudinal_planning::StateId::VEL];
    auto a =
        state_result->at(i)[pnc::realtime_longitudinal_planning::StateId::ACC];

    double j = 0.0;

    if (i < N - 1) {
      j = control_result->at(
          i)[pnc::realtime_longitudinal_planning::ControlId::JERK];
    } else {
      j = control_result->at(
          i - 1)[pnc::realtime_longitudinal_planning::ControlId::JERK];
    }

    // post process for longitudinal motion planning, ensure non-negative vel
    if (v < 0.0 && i > 0) {
      s = state_result->at(
          i - 1)[pnc::realtime_longitudinal_planning::StateId::POS];
      v = 0.0;
      a = 0.0;
      j = 0.0;
    }
