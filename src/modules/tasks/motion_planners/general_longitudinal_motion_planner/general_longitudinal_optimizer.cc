...
bool LongitudinalOptimizerV3::Execute(planning::framework::Frame *frame) {
  ...
  //   return false;
  // }
  // auto config_builder = frame->mutable_session()
  //                           ->mutable_environmental_model()
  //                           ->config_builder(planning::common::SceneType::HIGHWAY);
  ...
  // if (!b_success) {
  //   NTRACE_FAIL("LongitudinalOptimizerV3::Execute failed");
  // }
  ...
  //     if (!b_success) {
  //       NTRACE_FAIL(
  //           "LongitudinalOptimizerV3::Execute failed with only map constraint");
  //     }
  //   }
...
bool LongitudinalOptimizerV3::optimize(
  ...
  // NLOGD("HHLDEBUGB in optimize ego_vel: %.2f, ego_acc: %.2f", ego_vel * 3.6,
  //       ego_acc);
  ...
  // PiecewiseProblem lon_problem(num_t, lon_ref_path.t_list, s_init_state, false);
  // auto mrc_brake = frame_->mutable_session()
  //                      ->mutable_planning_context()
  //                      ->mrc_condition();
  ...
  // pipeline_context_->planning_result.extra_json_raw["lon_motion_error_info"] =
  //     mjson::Json("none");
  // if (!success) {
  //   if (option.enable_log) {
  //     pipeline_context_->planning_result
  //         .extra_json_raw["lon_motion_error_info"] =
  //         mjson::Json("solver_failed");
  //   }
  // }
  ...
  //     std::to_string(option.use_raw_model_traj) + ", s:" +
  //     std::to_string(planning_init_point.frenet_state.s) + ", v:" +
  //     std::to_string(planning_init_point.v) + ", a:" +
  //     std::to_string(planning_init_point.a) + ", status:" +
  //     std::to_string(status) + ", success:";
  ...
  //   NLOGI("HHLDEBUG from model v: %.2f", v_ok * 3.6);
  //   s_last = s_ref;
  ...
  //   }
  //   if (s_lower > s_upper) {
  //     NLOGE("NP_DEBUG: Error! Invalid s bound!");
  //     check_invalid_speed_bound = true;
  //     if (option.enable_log) {
  //       pipeline_context_->planning_result
  //           .extra_json_raw["lon_motion_error_info"] =
  //           mjson::Json("invalid_s_bound");
  //     }
  //   }
  ...
  //       NLOGE("NP_DEBUG: Error! speed bound collide! %d %f [%f %f]", i, s[i],
  //             s_lower, s_upper);
  //       if (option.enable_log) {
  //         pipeline_context_->planning_result
  //             .extra_json_raw["lon_motion_error_info"] =
  //             mjson::Json("speed_bound_collide");
  //       }
  //     }
  //     if (a[i] > kMaxAcc || a[i] < kMinDec) {
  //       NLOGE("NP_DEBUG: Error! Invalid acc %f !", a[i]);
  //       if (option.enable_log) {
  //         pipeline_context_->planning_result
  //             .extra_json_raw["lon_motion_error_info"] =
  //             mjson::Json("speed_dynamic_check_failed_for_invalid_acc");
  //       }
  //     }
  //     if (std::abs(jerk[i]) > kMaxJerk) {
  //       NLOGE("NP_DEBUG: Error! Invalid jerk %f!", jerk[i]);
  //       if (option.enable_log) {
  //         pipeline_context_->planning_result
  //             .extra_json_raw["lon_motion_error_info"] =
  //             mjson::Json("speed_dynamic_check_failed_for_invalid_jerk");
  //       }
  //     }
  //   }
  ...
  //       v_lower, i < lon_bound_v.size() ? lon_bound_v[i].lower : -100.0, object)
  //   MDEBUG_JSON_ADD_ITEM(
  //       v_upper, i < lon_bound_v.size() ? lon_bound_v[i].upper : 100.0, object)
  //   MDEBUG_JSON_ADD_ITEM(
  //       a_lower, i < lon_bound_a.size() ? lon_bound_a[i].lower : -10.0, object)
  //   MDEBUG_JSON_ADD_ITEM(
  //       a_upper, i < lon_bound_a.size() ? lon_bound_a[i].upper : 10.0, object)
  //   MDEBUG_JSON_ADD_ITEM(v_ref, lon_ref_path.ds_refs[i].first, object)
  //   MDEBUG_JSON_ADD_ITEM(
  //       jerk_lower, i < lon_bound_jerk.size() ? lon_bound_jerk[i].lower : -10.0,
  //       object)
  //   MDEBUG_JSON_ADD_ITEM(
  //       jerk_upper, i < lon_bound_jerk.size() ? lon_bound_jerk[i].upper : 10.0,
  //       object)
  ...
  //                             std::to_string(s_lower_bound_info.id) + ",type:" +
  //                             s_lower_bound_info.type + ")")
  //   MDEBUG_TABLE_ADD_RAW_DATA(std::to_string(s_upper - init_s) + "(id:" +
  //                             std::to_string(s_upper_bound_info.id) + ",type:" +
  //                             s_upper_bound_info.type + ")")
  ...
  //     NLOGI("HHLDEBUG optimize s: %.2f, v: %.2f, a: %.2f, jerk: %.2f", s[i],
  //           v[i] * 3.6, a[i], jerk[i]);
  ...
  //   error_info << "lon_pjopt_fail:" << status;
  //   pipeline_context_->status_info.error_info = error_info.str();
  //   NTRACE_FAIL("LongitudinalOptimizerV3::optimize failed");
  //   return false;
  // }
...
void LongitudinalOptimizerV3::interpolate_frenet_lon(
  ...
  //     l[i] = interpolate(traj_points[j].l, traj_points[j + 1].l, ratio);
  //     heading_angle[i] =
  //         interpolate_angle(traj_points[j].heading_angle,
  //                           traj_points[j + 1].heading_angle, ratio);
  //     curvature[i] = interpolate(traj_points[j].curvature,
