#include "realtime_lateral_motion_planning_problem.h"
...
void RealtimeLateralMotionPlanningProblem::Init() {
  ...
  ilqr_core_ptr_->Init(std::make_shared<RealtimeLateralMotionPlanningModel>(),
                       solver_config);
  ...
  // disable soft bound for realtime motion planning
  //   ilqr_core_ptr_->AddCost(
...
uint8_t RealtimeLateralMotionPlanningProblem::Update(
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    planning_output_.mutable_x_vec()->Set(
        i, state_result->at(i)[pnc::realtime_lateral_planning::StateId::X]);
    planning_output_.mutable_y_vec()->Set(
        i, state_result->at(i)[pnc::realtime_lateral_planning::StateId::Y]);
    planning_output_.mutable_theta_vec()->Set(
        i, state_result->at(i)[pnc::realtime_lateral_planning::StateId::THETA]);
    planning_output_.mutable_delta_vec()->Set(
        i, state_result->at(i)[pnc::realtime_lateral_planning::StateId::DELTA]);
    planning_output_.mutable_acc_vec()->Set(
        i, kv2 * state_result->at(
                     i)[pnc::realtime_lateral_planning::StateId::DELTA]);

    if (i < N - 1) {
      planning_output_.mutable_omega_vec()->Set(
          i, control_result->at(
                 i)[pnc::realtime_lateral_planning::ControlId::OMEGA]);
      planning_output_.mutable_jerk_vec()->Set(
          i, kv2 * control_result->at(
                       i)[pnc::realtime_lateral_planning::ControlId::OMEGA]);
    } else {
      planning_output_.mutable_omega_vec()->Set(
