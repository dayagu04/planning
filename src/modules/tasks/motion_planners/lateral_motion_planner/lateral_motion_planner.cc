...
namespace planning {
LateralMotionPlanner::LateralMotionPlanner(const EgoPlanningConfigBuilder *config_builder,
...
void LateralMotionPlanner::Init() {
  ...
  planning_problem_ptr_ = std::make_shared<pnc::lateral_planning::LateralMotionPlanningProblem>();
...
void LateralMotionPlanner::GeneratePlanningInput() {
  ...
  // angle fix of difference between theta and ref_theta, such as [-179deg and 179deg]
  double angle_compensate = 0.0;
  const auto d_theta = ref_theta_vec.front() - planning_init_point.lat_init_state.theta();
  ...
  for (size_t i = 0; i < ref_theta_vec.size(); ++i) {
    if (i == 0) {
      planning_input_.mutable_ref_theta_vec()->Set(i, ref_theta_vec[i] + angle_compensate);
    } else {
      ...
      planning_input_.mutable_ref_theta_vec()->Set(i, ref_theta_vec[i] + angle_offset + angle_compensate);
    }
  }
  ...
  for (size_t i = 0; i < safe_bounds.size(); ++i) {
    ...
    planning_input_.mutable_soft_lower_bound_x0_vec()->Set(i, safe_lower_bound.x);
    planning_input_.mutable_soft_lower_bound_y0_vec()->Set(i, safe_lower_bound.y);
    planning_input_.mutable_soft_lower_bound_x1_vec()->Set(i, next_safe_lower_bound.x);
    planning_input_.mutable_soft_lower_bound_y1_vec()->Set(i, next_safe_lower_bound.y);

    planning_input_.mutable_soft_upper_bound_x0_vec()->Set(i, safe_upper_bound.x);
    planning_input_.mutable_soft_upper_bound_y0_vec()->Set(i, safe_upper_bound.y);
    planning_input_.mutable_soft_upper_bound_x1_vec()->Set(i, next_safe_upper_bound.x);
    planning_input_.mutable_soft_upper_bound_y1_vec()->Set(i, next_safe_upper_bound.y);
    ...
    planning_input_.mutable_hard_lower_bound_x0_vec()->Set(i, path_lower_bound.x);
    planning_input_.mutable_hard_lower_bound_y0_vec()->Set(i, path_lower_bound.y);
    planning_input_.mutable_hard_lower_bound_x1_vec()->Set(i, next_path_lower_bound.x);
    planning_input_.mutable_hard_lower_bound_y1_vec()->Set(i, next_path_lower_bound.y);

    planning_input_.mutable_hard_upper_bound_x0_vec()->Set(i, path_upper_bound.x);
    planning_input_.mutable_hard_upper_bound_y0_vec()->Set(i, path_upper_bound.y);
    planning_input_.mutable_hard_upper_bound_x1_vec()->Set(i, next_path_upper_bound.x);
    planning_input_.mutable_hard_upper_bound_y1_vec()->Set(i, next_path_upper_bound.y);

    static const double min_v_cruise = 0.5;
    planning_input_.set_ref_vel(std::max(lat_decider_output.v_cruise, min_v_cruise));
...
void LateralMotionPlanner::GeneratePlanningOutput() {
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    if (i == 0) {
      ...
      const double ds = std::hypot(x_vec[i + 1] - x_vec[i], y_vec[i + 1] - y_vec[i]);
      s += std::max(ds, 1e-3);
    }
    s_vec[i + 1] = s;
  }
