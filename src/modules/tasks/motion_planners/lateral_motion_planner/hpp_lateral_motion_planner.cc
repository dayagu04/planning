...
#include "hpp_lateral_motion_planner.h"
...
namespace planning {
HPPLateralMotionPlanner::HPPLateralMotionPlanner(
    const EgoPlanningConfigBuilder *config_builder, framework::Session *session)
    : BaseLateralMotionPlanner(config_builder, session) {
  name_ = "HPPLateralMotionPlanner";
  config_.horizon = static_cast<size_t>(
      config_builder->cast<HppGeneralLateralDeciderConfig>().num_step);
  InitInputAndOutput();
  Init();
};

void HPPLateralMotionPlanner::Init() {
...
bool HPPLateralMotionPlanner::Execute() {
  ILOG_DEBUG << "=======HPPLateralMotionPlanner=======";
  ...
  // handle input
  if (!HandleInputData()) {
    ILOG_DEBUG << "HPPLateralMotionPlanner AssembleInput failed";
    SaveDebugInfo();
    return false;
  }
  // update
  if (!Update()) {
    ILOG_DEBUG << "HPPLateralMotionPlanner Solve failed";
    SaveDebugInfo();
    return false;
  }
  // handle output
...
bool HPPLateralMotionPlanner::HandleInputData() {
  // reset
  ResetInputAndOutput();
  // assemble input
...
bool HPPLateralMotionPlanner::AssembleInput() {
...
bool HPPLateralMotionPlanner::Update() {
  const auto &vehicle_param =
      VehicleConfigurationContext::Instance()->get_vehicle_param();
