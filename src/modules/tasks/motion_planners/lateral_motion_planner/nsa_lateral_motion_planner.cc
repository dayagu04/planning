...
#include "nsa_lateral_motion_planner.h"
...
namespace planning {
NSALateralMotionPlanner::NSALateralMotionPlanner(
    const EgoPlanningConfigBuilder *config_builder, framework::Session *session)
    : BaseLateralMotionPlanner(config_builder, session) {
  name_ = "NSALateralMotionPlanner";
  Init();
};

void NSALateralMotionPlanner::Init() {
...
bool NSALateralMotionPlanner::Execute() {
  ILOG_DEBUG << "=======NSALateralMotionPlanner=======";
  ...
  // handle input
  if (!HandleInputData()) {
    ILOG_DEBUG << "NSALateralMotionPlanner AssembleInput failed";
    SaveDebugInfo();
    return false;
  }
  // update
  if (!Update()) {
    ILOG_DEBUG << "NSALateralMotionPlanner Solve failed";
    SaveDebugInfo();
    return false;
  }
  // handle output
...
bool NSALateralMotionPlanner::HandleInputData() {
  // reset
  ResetInputAndOutput();
  // assemble input
...
bool NSALateralMotionPlanner::AssembleInput() {
...
bool NSALateralMotionPlanner::Update() {
  const auto &vehicle_param =
      VehicleConfigurationContext::Instance()->get_vehicle_param();
