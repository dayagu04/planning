...
double ReferenceCostTerm::GetCost(const State &x, const Control & /*u*/) {
  return 0.5 * (cost_config_ptr_->at(W_REF_X) *
                    Square(x[X] - cost_config_ptr_->at(REF_X)) +
                cost_config_ptr_->at(W_REF_Y) *
...
void ReferenceCostTerm::GetGradientHessian(const State &x,
  ...
  lx(2) += -cost_config_ptr_->at(W_REF_THETA) *
...
double ContinuityCostTerm::GetCost(const State &x, const Control & /*u*/) {
  return 0.5 * (cost_config_ptr_->at(W_CONTINUITY_X) *
...
void ContinuityCostTerm::GetGradientHessian(const State &x,
                                            ...
                                            LuMT & /*lu*/, LxxMT &lxx,
                                            LxuMT & /*lxu*/, LuuMT & /*luu*/) {
  lx(0) += -cost_config_ptr_->at(W_CONTINUITY_X) *
           (cost_config_ptr_->at(CONTINUITY_X) - x[X]);
  lx(1) += -cost_config_ptr_->at(W_CONTINUITY_Y) *
           (cost_config_ptr_->at(CONTINUITY_Y) - x[Y]);
  lx(2) += -cost_config_ptr_->at(W_CONTINUITY_THETA) *
...
double LatAccCostTerm::GetCost(const State &x, const Control & /*u*/) {
  return 0.5 * cost_config_ptr_->at(W_ACC) *
         Square(cost_config_ptr_->at(CURV_FACTOR) *
...
void LatJerkCostTerm::GetGradientHessian(const State &x, const Control & /*u*/,
  ...
  lx(3) += 2. * cost_config_ptr_->at(W_JERK) * x[DELTA] *
           Square(cost_config_ptr_->at(CURV_FACTOR) * x[OMEGA] *
                  Square(cost_config_ptr_->at(REF_VEL))) *
           (1. + Square(x[DELTA]));
  ...
  lxx(3, 3) += 2. * cost_config_ptr_->at(W_JERK) *
               Square(cost_config_ptr_->at(CURV_FACTOR) * x[OMEGA] *
                      Square(cost_config_ptr_->at(REF_VEL))) *
               (1. + 3. * Square(x[DELTA]));
  const double lxx_delta_omega = 4. * cost_config_ptr_->at(W_JERK) * x[DELTA] *
                                 x[OMEGA] *
                                 Square(cost_config_ptr_->at(CURV_FACTOR) *
                                        Square(cost_config_ptr_->at(REF_VEL))) *
                                 (1. + Square(x[DELTA]));
...
double LatAccBoundCostTerm::GetCost(const State &x, const Control & /*u*/) {
  ...
  if (x[DELTA] > cost_config_ptr_->at(DELTA_BOUND)) {
    cost = 0.5 * cost_config_ptr_->at(W_ACC_BOUND) * k2v4 *
           Square(x[DELTA] - cost_config_ptr_->at(DELTA_BOUND));
...
void LatAccBoundCostTerm::GetGradientHessian(const State &x,
  ...
  if (x[DELTA] > cost_config_ptr_->at(DELTA_BOUND)) {
    lx(3) += cost_config_ptr_->at(W_ACC_BOUND) * k2v4 *
             (x[DELTA] - cost_config_ptr_->at(DELTA_BOUND));
...
double LatJerkBoundCostTerm::GetCost(const State &x, const Control & /*u*/) {
  ...
  if (x[OMEGA] > cost_config_ptr_->at(OMEGA_BOUND)) {
    cost = 0.5 * cost_config_ptr_->at(W_JERK_BOUND) * k2v4 *
           Square(x[OMEGA] - cost_config_ptr_->at(OMEGA_BOUND));
...
void LatJerkBoundCostTerm::GetGradientHessian(const State &x,
                                              const Control & /*u*/, LxMT &lx,
                                              LuMT & /*lu*/, LxxMT &lxx,
                                              LxuMT & /*lxu*/,
                                              LuuMT & /*luu*/) {
  const double k2v4 = Square(cost_config_ptr_->at(CURV_FACTOR)) *
                      SSquare(cost_config_ptr_->at(REF_VEL));
  if (x[OMEGA] > cost_config_ptr_->at(OMEGA_BOUND)) {
    lx(4) += cost_config_ptr_->at(W_JERK_BOUND) * k2v4 *
             (x[OMEGA] - cost_config_ptr_->at(OMEGA_BOUND));
...
void PathSoftCorridorCostTerm::GetGradientHessian(const State &x,
  ...
  if (a1 * x[X] + b1 * x[Y] + c1 < 0.) {
    lx(0) += cost_config_ptr_->at(W_SOFT_CORRIDOR) * a1 *
             (a1 * x[X] + b1 * x[Y] + c1) / d1;
    lx(1) += cost_config_ptr_->at(W_SOFT_CORRIDOR) * b1 *
             (a1 * x[X] + b1 * x[Y] + c1) / d1;
  ...
  if (a2 * x[X] + b2 * x[Y] + c2 > 0.) {
    lx(0) += cost_config_ptr_->at(W_SOFT_CORRIDOR) * a2 *
             (a2 * x[X] + b2 * x[Y] + c2) / d2;
    lx(1) += cost_config_ptr_->at(W_SOFT_CORRIDOR) * b2 *
             (a2 * x[X] + b2 * x[Y] + c2) / d2;
...
double LatSnapCostTerm::GetCost(const State & /*x*/, const Control &u) {
  const double k2v4 = Square(cost_config_ptr_->at(CURV_FACTOR)) *
                      SSquare(cost_config_ptr_->at(REF_VEL));
  return 0.5 * k2v4 *
         (cost_config_ptr_->at(W_SNAP) * Square(u[ControlId::OMEGA_DOT]));
}

void LatSnapCostTerm::GetGradientHessian(const State & /*x*/, const Control &u,
                                         LxMT & /*lx*/, LuMT &lu,
                                         LxxMT & /*lxx*/, LxuMT & /*lxu*/,
                                         LuuMT &luu) {
  const double k2v4 = Square(cost_config_ptr_->at(CURV_FACTOR)) *
