...
ALiLQRSolver::ALiLQRSolver() {}

void ALiLQRSolver::SimInit() {
  ...
  solver_config.constraint_num = 4;
  solver_config.max_al_iter = 10;
  solver_config.max_al_solve_time = 15.0;
#ifdef X86
  solver_config.max_al_solve_time = 60.0;
#endif
  solver_config.c_ilqr_enable = true;
  init_state_.resize(solver_config.state_size);
  //
  std::shared_ptr<DynamicModel> dynamic_model = std::make_shared<DynamicModel>();
  // STEP 1: init core with solver config
  al_ilqr_core_ptr_ = std::make_shared<ALiLQR>();
  al_ilqr_core_ptr_->Init(dynamic_model, solver_config);

  // STEP 2: add cost
  // 2-1: rear axle reference cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<ReferencePathCostTerm>());
  // 2-2: rear axle reference continuity cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<ContinuityCostTerm>());
  // 2-3: front axle reference cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<FrontReferencePathCostTerm>());
  // 2-4: virtual rear axle reference cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<VirtualReferencePathCostTerm>());
  // 2-5: lateral acc cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<LateralAccCostTerm>());
  // 2-6: lateral jerk cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<LateralJerkCostTerm>());
  // 2-7: lateral acc bound cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<LateralAccBoundCostTerm>());
  // 2-8: lateral jerk soft bound cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<LateralJerkBoundCostTerm>());
  // 2-9: path first soft corridor cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<PathFirstSoftCorridorCostTerm>());
  // 2-10: path second soft corridor cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<PathSecondSoftCorridorCostTerm>());
  // 2-11: path hard corridor cost
  al_ilqr_core_ptr_->AddCost(std::make_shared<PathHardCorridorCostTerm>());

  // STEP 3: init debug info, must run after add cost
  al_ilqr_core_ptr_->InitAdvancedInfo();
}

void ALiLQRSolver::Init(
...
uint8_t ALiLQRSolver::Update(
  ...
  // set cost config
  const size_t N = al_ilqr_core_ptr_->GetSolverConfigPtr()->horizon + 1;
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    // alilqr
    alilqr_config_vec.at(i)[W_ACC_HARDBOUND] = 1.0;
    alilqr_config_vec.at(i)[W_JERK_HARDBOUND] = 1.0;
    alilqr_config_vec.at(i)[W_POS_LOWER_HARDBOUND] = 1.0;
    alilqr_config_vec.at(i)[W_POS_UPPER_HARDBOUND] = 1.0;
    alilqr_config_vec.at(i)[L_ACC_HARDBOUND] = 0.0;
    alilqr_config_vec.at(i)[L_JERK_HARDBOUND] = 0.0;
    alilqr_config_vec.at(i)[L_POS_LOWER_HARDBOUND] = 0.0;
    alilqr_config_vec.at(i)[L_POS_UPPER_HARDBOUND] = 0.0;

    alilqr_config_vec.at(i)[PHI_SCALE] = 10.0;
    alilqr_config_vec.at(i)[CONSTRAINT_CONFIG_SIZE] = 4;
  }

  // set cost config
  al_ilqr_core_ptr_->SetCostConfig(cost_config_vec);
  al_ilqr_core_ptr_->SetAliLqrConfig(alilqr_config_vec);
  ...
  // al_ilqr_core_ptr_->Solve(init_state_);
  al_ilqr_core_ptr_->SolveForAliLqr(init_state_);

  // fail protection
  const uint8_t solver_condition =
      al_ilqr_core_ptr_->GetSolverInfoPtr()->solver_condition;
  const auto solver_config = al_ilqr_core_ptr_->GetSolverConfigPtr();
  ...
  if (solver_condition >= iLqr::BACKWARD_PASS_FAIL) {
    al_ilqr_core_ptr_->Simulation(init_state_, u_vec);
  }

  return solver_condition;
}

uint8_t ALiLQRSolver::SimUpdate(
    bool al_ilqr_enable, bool warm_start_enable,
    ...
    planning::common::LateralPlanningInput &planning_input) {
  // set cost config
  al_ilqr_core_ptr_->GetSolverConfigPtr()->max_iter = max_iter;
  const size_t N = al_ilqr_core_ptr_->GetSolverConfigPtr()->horizon + 1;
  std::vector<ilqr_solver::IlqrCostConfig> cost_config_vec;
  cost_config_vec.resize(N);
  ...
  // al_ilqr_core_ptr_->Solve(init_state_);
  al_ilqr_core_ptr_->ClearCost();
  if (al_ilqr_enable) {
    al_ilqr_core_ptr_->GetSolverConfigPtr()->warm_start_enable = warm_start_enable;
    al_ilqr_core_ptr_->GetSolverConfigPtr()->c_ilqr_enable = true;

    al_ilqr_core_ptr_->AddCost(
        std::make_shared<ReferencePathCostTerm>());  // reference cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<ContinuityCostTerm>());  // continuity cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<LateralAccCostTerm>());  // lateral acc cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<LateralJerkCostTerm>());  // lateral jerk cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<PathFirstSoftCorridorCostTerm>());  // path soft corridor cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<PathSecondSoftCorridorCostTerm>());  // path soft corridor cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<LateralAccHardBoundCostTerm>());  // lateral acc hard corridor cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<LateralJerkHardBoundCostTerm>());  // lateral jerk hard corridor cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<PathCorridorCostTerm>());  // path hard corridor cost

    al_ilqr_core_ptr_->InitAdvancedInfo();
    al_ilqr_core_ptr_->SolveForAliLqr(init_state_);
  } else {
    al_ilqr_core_ptr_->GetSolverConfigPtr()->warm_start_enable = warm_start_enable;
    al_ilqr_core_ptr_->GetSolverConfigPtr()->c_ilqr_enable = false;
    ...
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<LateralJerkCostTerm>());  // lateral jerk cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<LateralAccBoundCostTerm>());  // lateral acc bound cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<LateralJerkBoundCostTerm>());  // lateral jerk bound cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<PathFirstSoftCorridorCostTerm>());  // path soft corridor cost
    al_ilqr_core_ptr_->AddCost(
        std::make_shared<PathSecondSoftCorridorCostTerm>());  // path soft corridor cost
