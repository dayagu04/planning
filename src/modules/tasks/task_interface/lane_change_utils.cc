...
double CalcGapObjSafeDistance(const double ego_v, const double obj_v,
  ...
  //(1)当obj_a > 0，ego_v < obj_v; obj_a < 0，ego_v >
  // obj_v时，rel_dis的值单调变化，需考虑如果是减速情况，在4s范围内是否会出现倒车情况
  //(2)当obj_a > 0，ego_v > obj_v; obj_a < 0，ego_v < obj_v时,
  // rel_dis的值非单调变化，在二者速度相等的地方，rel_dis达到极值点，需使用极值点的时间计算相对距离
  double rel_dis = 0;
