...
TaskPipeline::TaskPipeline(const EgoPlanningConfigBuilder *config_builder, framework::Frame *frame)
    : pipeline_context_(std::make_shared<TaskPipelineContext>()) {
  frame_ = frame;
  config_ = config_builder->cast<EgoPlanningTaskPipelineConfig>();
}

std::shared_ptr<TaskPipeline> TaskPipeline::Make(const TaskPipelineType &task_pipeline_type,
