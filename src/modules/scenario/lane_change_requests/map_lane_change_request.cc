...
bool MapRequest::check_mlc_enable(double lc_map_tfinish) {
  ...
  int lc_map_decision =
      current_lane != nullptr ? virtual_lane_mgr_->lc_map_decision(current_lane) : 0;
  ...
  double delay_map = 0;
  double v_limit = session_->environmental_model()
...
void MapRequest::update(int lc_status, double lc_map_tfinish) {
  ...
  if (lane_change_lane_mgr_->has_origin_lane()) {
    ...
    LOG_DEBUG("has_origin_lane: %d, origin_lane_virtual_id_: %d", origin_lane_order_id_, origin_lane_virtual_id_);
  } else {
    // if olane disappeared, replace it with current_lane
    origin_lane_virtual_id_ = current_lane_virtual_id;
    origin_lane_order_id_ = virtual_lane_mgr_->get_current_lane()->get_order_id();
    LOG_DEBUG("no origin lane, origin_lane_order_id_ : %d, origin_lane_virtual_id_ : %d", origin_lane_order_id_, origin_lane_virtual_id_);
  }
    ...
    if (check_mlc_enable(lc_map_tfinish) == true && allow_generate == true) {
      if (lc_map_decision < 0) {
        ...
        if (!IsDashedLineEnough(LEFT_CHANGE, v_ego, virtual_lane_mgr_) &&
            ...
            virtual_lane_mgr_->distance_to_first_road_split() > 300 + 300 * std::fabs(lc_map_decision)) {
          Finish();
        ...
        if (!IsDashedLineEnough(RIGHT_CHANGE, v_ego, virtual_lane_mgr_) &&
            ...
            (lane_change_lane_mgr_->has_origin_lane() && lane_change_lane_mgr_->is_ego_on(olane)))) &&
            virtual_lane_mgr_->dis_to_ramp() > 300 + 300 * std::fabs(lc_map_decision)) {
          Finish();
          set_target_lane_virtual_id(current_lane_virtual_id);
          LOG_DEBUG("[MapRequest::update] : finish request, dashed not enough");
        }
