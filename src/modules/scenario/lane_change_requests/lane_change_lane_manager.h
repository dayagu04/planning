...
class LaneChangeLaneManager {
 public:
  LaneChangeLaneManager(std::shared_ptr<VirtualLaneManager> virtual_lane_mgr, planning::framework::Session* session);
  ...
  void reset_lc_lanes();
  void reset_origin_lane() { olane_virtual_id_ = virtual_lane_mgr_->current_lane_virtual_id(); }
  ...
  void set_target_lane(int target_lane_virtual_id) { tlane_virtual_id_ = target_lane_virtual_id; }

  bool has_target_lane() const { return virtual_lane_mgr_->has_lane(tlane_virtual_id_); }
  bool has_origin_lane() const { return virtual_lane_mgr_->has_lane(olane_virtual_id_); }
  bool has_fix_lane() const { return virtual_lane_mgr_->has_lane(flane_virtual_id_); }
  ...
  // 这里是否应该是返回const 指针引用？
  std::shared_ptr<VirtualLane> flane() { return virtual_lane_mgr_->get_lane_with_virtual_id(flane_virtual_id_); }
  std::shared_ptr<VirtualLane> tlane() { return virtual_lane_mgr_->get_lane_with_virtual_id(tlane_virtual_id_); }
  std::shared_ptr<VirtualLane> olane() { return virtual_lane_mgr_->get_lane_with_virtual_id(olane_virtual_id_); }
  ...
  void upload_fix_lane_virtual_id() { virtual_lane_mgr_->update_last_fix_lane_id(flane_virtual_id_); }
