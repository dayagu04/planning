...
void StateBase::process(Control &control, FsmContext &context) {
...

  {
    ...
    lat_behavior_common->set_current_state(state_machine_output.curr_state);
    ...
    lat_behavior_common->set_lc_invalid_reason(
        state_machine_output.lc_invalid_reason);
    lat_behavior_common->set_lc_back_reason(
        state_machine_output.lc_back_reason);
