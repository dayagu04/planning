...
EgoPlanningCandidate::EgoPlanningCandidate(planning::framework::Frame *frame) {
  ...
  auto config_builder = frame->session()->environmental_model().config_builder(frame_->session()->get_scene_type());
  config_ = config_builder->cast<EgoPlanningCandidateConfig>();
}

void EgoPlanningCandidate::set_coarse_planning_info(const StateTransitionContext &transition_context) {
  // Step 1) 计算state
  ...
  coarse_planning_info_.lane_change_request_source = transition_context.lane_change_request_source;
  ...
  if (transition_context.target_state == ROAD_LC_LWAIT || transition_context.target_state == ROAD_LC_RWAIT ||
    ...
    coarse_planning_info_.overtake_obstacles = transition_context.overtake_obstacles;
  ...
  frame_->mutable_session()->mutable_planning_context()->mutable_planning_result().use_refined_reference_path = false;
  coarse_planning_info_.reference_path = frame_->mutable_session()
  ...
  const auto &frenet_coord = coarse_planning_info_.reference_path->get_frenet_coord();
  ...
  for (size_t i = 0; i < point_size; ++i) {
    cart_ref_info.x_vec[i] = coarse_planning_info_.reference_path->get_points().at(i).path_point.x;
    cart_ref_info.y_vec[i] = coarse_planning_info_.reference_path->get_points().at(i).path_point.y;
    cart_ref_info.s_vec[i] = coarse_planning_info_.reference_path->get_points().at(i).path_point.s;
  }
  ...
  const auto &planning_init_point = coarse_planning_info_.reference_path->get_frenet_ego_state().planning_init_point();

  Eigen::Vector2d init_pos(planning_init_point.lat_init_state.x(), planning_init_point.lat_init_state.y());
  ...
  projection_spline.CalProjectionPoint(cart_ref_info.x_s_spline, cart_ref_info.y_s_spline, cart_ref_info.s_vec.front(),
                                       cart_ref_info.s_vec.back(), init_pos);
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    if (s_ref < cart_ref_info.s_vec.back()) {
      ...
      point.heading_angle =
          std::atan2(cart_ref_info.y_s_spline.deriv(1, s_ref), cart_ref_info.x_s_spline.deriv(1, s_ref));
    }
...
void EgoPlanningCandidate::set_last_planning_result(const std::shared_ptr<PlanningResult> &planning_result) {
...
bool EgoPlanningCandidate::pre_check() {
  ...
  if (coarse_planning_info_.target_state == ROAD_LC_LCHANGE || coarse_planning_info_.target_state == ROAD_LC_RCHANGE) {
    ...
    std::vector<double> ego_s_list = {ego_boundary.s_start, ego_boundary.s_end, frenet_ego_state.s()};
    for (auto s : ego_s_list) {
      ReferencePathPoint refpath_pt{};
      (void)reference_path->get_reference_point_by_lon(s, refpath_pt);
      auto ego_l = frenet_ego_state.l();
      if (ego_l < -refpath_pt.distance_to_right_road_border or ego_l > refpath_pt.distance_to_left_road_border) {
...
void EgoPlanningCandidate::refine(const std::shared_ptr<TaskPipeline> &task_pipeline) {
  success_ = false;
  ...
  if (not ok and last_planning_result_ != nullptr and last_planning_result_->use_backup_cnt <= 5) {
    auto cur_time = frame_->mutable_session()->mutable_environmental_model()->get_ego_state_manager()->navi_timestamp();
    ...
    if (0 < delta_time and delta_time < 1.0) {
      ...
      planning_result_.use_backup_cnt = last_planning_result_->use_backup_cnt + 1;
      success_ = true;
    }
  }

  if (success_) {
    planning_result_.timestamp =
        frame_->mutable_session()->mutable_environmental_model()->get_ego_state_manager()->navi_timestamp();
...
void EgoPlanningCandidate::interpolate_with_last_trajectory_points() {
  ...
  assert(last_planning_result_->target_lane_id == coarse_planning_info_.target_lane_id);

  auto curr_time = frame_->mutable_session()->mutable_environmental_model()->get_ego_state_manager()->navi_timestamp();
  ...
  for (int j = 0; j < backup_num_points; ++j) {
    ...
    for (; idx < last_traj_points.size() - 1; idx++) {
      if (last_traj_points[idx].t <= interpolate_t and interpolate_t <= last_traj_points[idx + 1].t) {
        break;
      }
    }
    ...
    traj_pt.t = t;
    traj_pt.x = planning_math::Interpolate(pre_pt.t, pre_pt.x, next_pt.t, next_pt.x, interpolate_t);
    traj_pt.y = planning_math::Interpolate(pre_pt.t, pre_pt.y, next_pt.t, next_pt.y, interpolate_t);
    traj_pt.heading_angle = planning_math::InterpolateAngle(pre_pt.t, pre_pt.heading_angle, next_pt.t,
                                                            next_pt.heading_angle, interpolate_t);
    traj_pt.curvature =
        planning_math::Interpolate(pre_pt.t, pre_pt.curvature, next_pt.t, next_pt.curvature, interpolate_t);
    traj_pt.v = planning_math::Interpolate(pre_pt.t, pre_pt.v, next_pt.t, next_pt.v, interpolate_t);
    traj_pt.a = planning_math::Interpolate(pre_pt.t, pre_pt.a, next_pt.t, next_pt.a, interpolate_t);
    traj_pt.s = planning_math::Interpolate(pre_pt.t, pre_pt.s, next_pt.t, next_pt.s, interpolate_t);
    traj_pt.l = planning_math::Interpolate(pre_pt.t, pre_pt.l, next_pt.t, next_pt.l, interpolate_t);
...
void EgoPlanningCandidate::copy_to_planning_context() {
  ...
  frame_->mutable_session()->mutable_planning_context()->mutable_planning_result() = planning_result_;
}
