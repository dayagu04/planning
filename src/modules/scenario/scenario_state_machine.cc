...
ScenarioStateMachine::ScenarioStateMachine(const EgoPlanningConfigBuilder *config_builder,
  ...
  lc_lane_mgr_ = std::make_shared<LaneChangeLaneManager>(virtual_lane_mgr, session);
  lc_req_mgr_ = std::make_shared<LaneChangeRequestManager>(session, config_builder, virtual_lane_mgr, lc_lane_mgr_);
}
...
bool ScenarioStateMachine::update(planning::framework::Frame *frame) {
  ...
  if (active) {
    ...
    if (scenario_ == SCENARIO_CRUISE) {
      // update lc_req_mgr_
      lc_req_mgr_->Update(fsm_context_.state, session_->environmental_model().IsOnRoute());
      gen_map_turn_signal();
...
void ScenarioStateMachine::update_start_move_dist_lane() {
  ...
  auto fix_reference_path =
      session_->environmental_model().get_reference_path_manager()->get_reference_path_by_lane(flane_virtual_id);
...
void ScenarioStateMachine::post_process() {
  RequestType turn_signal_this_frame = (map_turn_signal_ == NO_CHANGE) ? lc_req_mgr_->turn_signal() : map_turn_signal_;
  turn_signal_this_frame = (turn_signal_this_frame == NO_CHANGE) ? merge_split_turn_signal_ : turn_signal_this_frame;
  ...
  session_->mutable_planning_context()->mutable_planning_result().turn_signal = turn_signal_;

  LOG_DEBUG(
      "[ScenarioStateMachine] turn_signal_on_time_ %f map_turn_signal: %d, "
      "lc_turn_signal: %d, merge_split_turn_signal: %d, turn_signal_: %d",
      turn_signal_on_time_, (int)map_turn_signal_, (int)lc_req_mgr_->turn_signal(), (int)merge_split_turn_signal_,
      (int)turn_signal_);
}
...
bool ScenarioStateMachine::GapAvailable(RequestType direction, std::vector<int> &overtake_obstacles,
  ...
  auto current_reference_path = reference_path_manager->get_reference_path_by_current_lane();
  ...
  // 获取目标车道中的障碍物id序列
  auto tlane_lane_obstacles = target_lane->get_reference_path()->get_lane_obstacles_ids();
  for (auto &frenet_obstacle : frenet_obstacles) {
    if (std::count(tlane_lane_obstacles.begin(), tlane_lane_obstacles.end(), frenet_obstacle->id())) {
      double ego_s = frenet_ego_state.s();
      double obstacle_s = frenet_obstacle->frenet_s();
      if ((obstacle_s > ego_s) &&
          !IsFollowBufferEnough(frenet_obstacle->frenet_obstacle_boundary().s_start, frenet_ego_state.boundary().s_end,
                                frenet_obstacle->frenet_velocity_s(), frenet_ego_state.velocity_s())) {
      ...
      if ((obstacle_s < ego_s) &&
          !IsFollowBufferEnough(frenet_ego_state.boundary().s_start, frenet_obstacle->frenet_obstacle_boundary().s_end,
                                frenet_ego_state.velocity_s(), frenet_obstacle->frenet_velocity_s())) {
  ...
  // 4.排序障碍物: 由远至近
  auto tlane_obstacles = target_lane->get_reference_path()->get_lane_obstacles_ids();
  std::vector<std::shared_ptr<FrenetObstacle>> sorted_obstacles;
  for (auto &frenet_obstacle : frenet_obstacles) {
    if (std::count(tlane_obstacles.begin(), tlane_obstacles.end(), frenet_obstacle->id()) > 0) {
      sorted_obstacles.emplace_back(frenet_obstacle);
    }
  }
  std::sort(sorted_obstacles.begin(), sorted_obstacles.end(),
            [&](const std::shared_ptr<FrenetObstacle> &o1, const std::shared_ptr<FrenetObstacle> &o2) {
  ...
  for (auto &gap_info : gap_infos) {
    double sc_f = gap_info.s_front + gap_info.v_front * care_time - std::min(gap_info.v_front * time_headway, 5.0);
    double sc_r = gap_info.s_rear + gap_info.v_rear * care_time + std::min(gap_info.v_rear * time_headway, 5.0);
    double s_upper = s_ego_front + v_ego * care_time - 0.5 * acc * care_time * care_time;
    double s_lower = s_ego_rear + v_ego * care_time + 0.5 * acc * care_time * care_time;
    LOG_DEBUG(
        "[gap info] front id: [%d], rear id: [%d], sc_f: [%f], sc_r: [%f], "
        "s_upper: [%f],  s_lower: [%f] \n",
        gap_info.front_id, gap_info.rear_id, sc_f, sc_r, s_upper, s_lower);
    if ((s_upper < sc_f && s_lower > sc_r) || gap_info.s_rear < s_ego_rear - 20.0 || gap_info.rear_id == -1) {
...
bool ScenarioStateMachine::IsFollowBufferEnough(const double front_s, const double behind_s, const double front_v,
  ...
  if (fsm_context_.state == ROAD_LC_LCHANGE || fsm_context_.state == ROAD_LC_RCHANGE) {
    kMinSafeDistance = 2.0;
    kMaxEndurableAcc = 2.0;
  } else if (fsm_context_.state == ROAD_NONE || fsm_context_.state == ROAD_LC_LWAIT ||
             fsm_context_.state == ROAD_LC_RWAIT || fsm_context_.state == ROAD_LC_LBACK ||
...
void ScenarioStateMachine::compute_lc_valid_info(RequestType direction) {
  ...
  std::cout << "compute_lc_valid_info: flane_virtual_id: " << lc_lane_mgr_->flane_virtual_id()
            << " fix_lane->get_virtual_id(): " << fix_lane->get_virtual_id() << std::endl;
  std::cout << "compute_lc_valid_info: tlane_virtual_id: " << lc_lane_mgr_->tlane_virtual_id()
            << " target_lane->get_virtual_id(): " << target_lane->get_virtual_id() << std::endl;
  std::cout << "compute_lc_valid_info: clane_virtual_id: " << virtual_lane_manager->current_lane_virtual_id()
            << std::endl;
  auto reference_path_manager = session_->mutable_environmental_model()->get_reference_path_manager();
  auto fix_reference_path = reference_path_manager->get_reference_path_by_lane(lc_lane_mgr_->flane_virtual_id());
  auto target_reference_path = reference_path_manager->get_reference_path_by_lane(lc_lane_mgr_->tlane_virtual_id());
  // auto &frenet_obstacles = current_reference_path->get_obstacles();
  auto &frenet_ego_state = fix_reference_path->get_frenet_ego_state();
  ...
  double dist_mline =
      std::fabs(target_lane_frenet_ego_state.l()) - 1.8;  // TODO(Rui): use target_lane()->get_lane_width()
  ...
  auto &obstacle_manager = session_->mutable_environmental_model()->get_obstacle_manager();
  auto tlane_obstacles = target_lane->get_reference_path()->get_lane_obstacles_ids();

  for (auto &obstacle : lateral_obstacle->side_tracks()) {
    if (std::count(tlane_obstacles.begin(), tlane_obstacles.end(), obstacle.track_id) > 0) {
  ...
  for (auto &tr : side_target_tracks) {
    if (is_side_target_valid == true) {
      ...
      if (tr.d_rel < safety_dist && tr.d_rel > -5.0 - safety_dist && tr.v_rel > 100.0) {
        ...
        if (tr.d_rel > -5.0) {
          ...
          if (((tr.d_rel < interp(tr.v_rel, xp, fp) * safety_dist || tr.d_rel < mss ||
                (mss_t != mss && mss_t > -3.5)) &&
               (tr.d_rel >= 0 || v_ego >= 3)) ||
          ...
          if (v_ego < 17 && vel_sequence.size() > 1) {
            ...
            mss = std::min(std::max(temp1 * t_reaction + sign * temp2 * temp2 / (2. * a) + safety_dist, 3.0), 120.0);

            if (tr.type == 2 || tr.type == 3 || tr.type == 4) {
              mss = std::min(std::max(temp1 * (t_reaction + 1) + sign * temp2 * temp2 / (2 * a) + safety_dist, 3.0),
                             120.0);
            }

            for (size_t i = 0; i + 5 < vel_sequence.size(); i++) {
              mss_t = tr.d_rel + std::max(tr.v_rel - (vel_sequence[i] - v_ego) / 2.0, 0.0) * 0.1 * i;

              if (((mss_t > -7.0 || tr.d_rel > -5.0 - mss) && tr.v_lead > 1) || (tr.v_lead <= 1 && tr.d_rel > -5.0)) {
                is_side_target_valid = false;
                // lc_invalid_reason_ = "side view invalid";
                lc_invalid_track_.set_value(tr.track_id, tr.d_rel, tr.v_rel);
                break;
            ...
            int sign = temp < 0 ? -1 : 1;
            mss = std::min(std::max(temp * t_reaction + sign * temp * temp / (2 * a) + safety_dist, 3.0), 120.0);

            if (tr.type == 2 || tr.type == 3 || tr.type == 4) {
              mss =
                  std::min(std::max(temp * (t_reaction + 1) + sign * temp * temp / (2 * a) + safety_dist, 3.0), 120.0);
            }
          }

          if ((tr.d_rel > -5.0 - mss && tr.v_lead > 1) || (tr.v_lead <= 1 && tr.d_rel > -5.0)) {
            is_side_target_valid = false;
            // lc_invalid_reason_ = "side view invalid";
            lc_invalid_track_.set_value(tr.track_id, tr.d_rel, tr.v_rel);
          }
      ...
      if (tr.d_rel > -5.0) {
        ...
        if (tr.v_rel < 0.0) {
          if (v_ego < 17 && vel_sequence.size() > 6) {
            ...
            for (size_t i = 0; i + 5 < vel_sequence.size(); i++) {
              mss_t = tr.d_rel + std::min(tr.v_rel - (vel_sequence[i] - v_ego) / 2, 0.0) * 0.1 * i;
          ...
          if (v_ego < 17 && vel_sequence.size() > 1) {
            ...
            for (size_t i = 0; i + 5 < vel_sequence.size(); i++) {
              mss_t = tr.d_rel + (tr.v_rel - (vel_sequence[i] - v_ego) / 2) * 0.1 * i;
            ...
            mss = std::pow(std::max(2 - tr.v_rel, 0.0), 2) / (2 * a_aflc) + safety_dist;
          }
        }
...
LaneChangeStageInfo ScenarioStateMachine::decide_lc_valid_info(RequestType direction) {
  ...
  if (lane_change_stage_info_.gap_insertable) {
    ...
    LOG_DEBUG("arbitrator lc invalid reason %s ", lane_change_stage_info_.lc_invalid_reason.c_str());
    lc_valid_cnt_ = 0;
  }
...
void ScenarioStateMachine::compute_lc_back_info(RequestType direction) {
  ...
  int current_lane_virtual_id =
      session_->mutable_environmental_model()->get_virtual_lane_manager()->current_lane_virtual_id();
  auto reference_path_manager = session_->mutable_environmental_model()->get_reference_path_manager();
  std::cout << "compute_lc_back_info: flane_virtual_id: " << lc_lane_mgr_->flane_virtual_id()
            << " fix_lane->get_virtual_id(): " << fix_lane->get_virtual_id() << std::endl;
  std::cout << "compute_lc_back_info: tlane_virtual_id: " << lc_lane_mgr_->tlane_virtual_id()
            << " target_lane->get_virtual_id(): " << target_lane->get_virtual_id() << std::endl;
  auto fix_reference_path = reference_path_manager->get_reference_path_by_lane(lc_lane_mgr_->flane_virtual_id());
  auto target_reference_path = reference_path_manager->get_reference_path_by_lane(lc_lane_mgr_->tlane_virtual_id());
  // auto &frenet_obstacles = current_reference_path->get_obstacles();
  ...
  if (side_target_tracks.size() > 0) {
    ...
    for (auto &tr : side_target_tracks) {
      ...
      if (lane_change_stage_info_.lc_should_back == false &&
          ((direction == LEFT_CHANGE && lend < car_width + 0.3 - lane_width / 2 + tr.width / 2) ||
        ...
        if (tr.d_rel < safety_dist && tr.d_rel > -5.0 - safety_dist && tr.v_rel > 100.0) {
          ...
          if (tr.d_rel > -5.0) {
            ...
            if (((tr.d_rel < 0.5 * interp(tr.v_rel, xp, fp) * safety_dist || tr.d_rel < mss ||
                  (mss_t != mss && mss_t > -2)) &&
                 tr.v_lead >= 1) ||
            ...
            if (tr.v_rel >= 0) {
              ...
              if (std::fabs(virtual_lane_manager->lc_map_decision(fix_lane)) == 1) {
                mss = tr.v_rel * t_reaction / 2 * 0.7 + temp * temp / (2 * a) * 0.7 + safety_dist - 2 / (tr.v_rel + 1);

                if (tr.type == 2 || tr.type == 3 || tr.type == 4) {
                  mss = tr.v_rel * (t_reaction + 2) / 2 * 0.7 + temp * temp / 2 * 0.7 + safety_dist * 1.1 -
                        2 / (tr.v_rel + 1);
                }
              } else {
                mss = tr.v_rel * t_reaction / 2 + temp * temp / (2 * a) + safety_dist - 2 / (tr.v_rel + 1);
                if (tr.type == 2 || tr.type == 3 || tr.type == 4) {
                  mss = tr.v_rel * (t_reaction + 2) / 2 + temp * temp / 2 + safety_dist - 2 / (tr.v_rel + 1);
                }
              }

              if (tr.d_rel >
                      -5.0 - (mss -
                              std::max(std::max((std::max(l_ego - tr.d_max_cpath - 1.6, tr.d_min_cpath - l_ego - 1.6)),
                                                0.0) /
  ...
  if (((direction == LEFT_CHANGE && start_move_dist_lane_ != 0 && distance > -left_lane_width / 2 - move_thre) ||
       (direction == RIGHT_CHANGE && start_move_dist_lane_ != 0 && distance < right_lane_width / 2 + move_thre) ||
       fix_lane->get_virtual_id() == current_lane_virtual_id)) {  // TODO(Rui): &&
  ...
  if (front_target_tracks.size() > 0) {
    ...
    for (auto &tr : front_target_tracks) {
      if (lane_change_stage_info_.lc_should_back == false) {
        ...
        double lat_condi = std::max((std::max(l_ego - tr.d_max_cpath - 1.6, tr.d_min_cpath - l_ego - 1.6)), 0.0) /
                           (std::max(-tr.v_lat + 0.7, 0.3));
        if ((lat_condi < 1.5 && tr.d_rel < 1.0) ||
            (lat_condi <= 1.5 && tr.d_rel < 0.8 * (mss - std::max(lat_condi * tr.v_rel, 0.0)))) {
          lane_change_stage_info_.lc_should_back = true;
...
bool ScenarioStateMachine::check_lc_change_finish(RequestType direction) {
  auto &virtual_lane_mgr = session_->mutable_environmental_model()->get_virtual_lane_manager();
  auto clane = virtual_lane_mgr->get_current_lane();
  auto clane_virtual_id = virtual_lane_mgr->current_lane_virtual_id();

  if (clane == nullptr || !lc_lane_mgr_->has_target_lane() || !lc_lane_mgr_->has_fix_lane()) {
    LOG_ERROR("[check_lc_change_finish] invalid clane [%d] or tlane [%d] or flane", clane == nullptr,
              !lc_lane_mgr_->has_target_lane(), !lc_lane_mgr_->has_fix_lane());
    return false;
  }
  ...
  double v_ego = session_->mutable_environmental_model()->get_ego_state_manager()->ego_v();
  ...
  std::cout << "check_lc_change_finish: tlane_virtual_id: " << tlane_virtual_id
            << " clane_virtual_id: " << clane_virtual_id << " flane_virtual_id: " << flane_virtual_id << std::endl;
  auto target_reference_path = reference_path_mgr->get_reference_path_by_lane(tlane_virtual_id);
  auto frenet_ego_state = target_reference_path->get_frenet_ego_state();

  bool lc_change_finish{false};
  if (direction == RIGHT_CHANGE) {
    lc_change_finish =
        ((frenet_ego_state.l() < dist_threshold) && (std::fabs(frenet_ego_state.heading_angle()) < angle_threshold));
...
bool ScenarioStateMachine::check_lc_back_finish(RequestType direction) {
  ...
  std::shared_ptr<VirtualLaneManager> virtual_lane_mgr =
      session_->mutable_environmental_model()->get_virtual_lane_manager();
  auto flane = virtual_lane_mgr->get_lane_with_virtual_id(lc_lane_mgr_->flane_virtual_id());
  double half_lane_width = 1.6;  // HACK  TODO(Rui):get real fix_lane->width() / 2.
  double ego_width = session_->mutable_vehicle_config_context()->get_vehicle_param().width;
  double dist_threshold = half_lane_width - 0.5 * ego_width - 0.1;
  double angle_threshold = 0.01;

  std::shared_ptr<ReferencePathManager> reference_path_mgr =
      session_->mutable_environmental_model()->get_reference_path_manager();
  auto fix_reference_path = reference_path_mgr->get_reference_path_by_lane(lc_lane_mgr_->flane_virtual_id());
  auto frenet_ego_state = fix_reference_path->get_frenet_ego_state();
