...
bool PlanningComponent::Init() {
  ...
  auto radar_perception_objects_reader_ =
      planning_node_->CreateReader<RadarPerceptionObjects::RadarPerceptionObjectsInfo>(
          ...
          [this](const std::shared_ptr<RadarPerceptionObjects::RadarPerceptionObjectsInfo>
            ...
            std::cout << "receive radar_perception_objects_info "
                      << radar_perception_objects_info_msg->header().timestamp() << std::endl;
            std::lock_guard<std::mutex> lock(msg_mutex_);
  ...
  auto parking_fusion_info_reader_ = planning_node_->CreateReader<ParkingFusionInfo>(
      "/iflytek/fusion/parking_slot", [this](const std::shared_ptr<ParkingFusionInfo> &parking_fusion_info_msg) {
        std::lock_guard<std::mutex> lock(msg_mutex_);
