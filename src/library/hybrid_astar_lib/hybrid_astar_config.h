...
struct OpenSpaceSafeBuffer {
  ...
  std::array<double , 3> lat_safe_buffer_outside;
  std::array<double , 3> head_out_lat_safe_buffer_outside;
  double circle_path_extra_buffer_outside;

  // 车位内部的横向安全buffer，需要设定小一些
  std::array<double , 3> lat_safe_buffer_inside;
  double circle_path_extra_buffer_inside;

  // 纵向buffer不区分库内库外
  std::array<double , 3> lon_safe_buffer;
...
struct PlannerOpenSpaceConfig {
  ...
  double perpendicular_slot_head_out_node_step;
