...
bool HybridAStar::AnalyticExpansion(Node3d* current_node) {
  ...
  // gear check
  // for vertical space park in, the last rs segment gear must be reverse
  // todo, for other parking type, use different rules in parking scenerio
  // decider module.
  // size_t end_path_seg_id = reeds_shepp_to_end->segs_lengths.size() - 1;
  // if (reeds_shepp_to_end->segs_lengths[end_path_seg_id] > 0.0) {
  //   ILOG_INFO << " rs last path seg gear is d ";
  //   return false;
  // }
  ...
  rs_end_node_.Set(path, XYbounds_, config_, path.path_dist);
  if (!NodeInSearchBound(rs_end_node_.GetIndex())) {
    // ILOG_INFO << "node positiong is not expectation";
    return false;
  }

  rs_end_node_.SetPathType(AstarPathType::Reeds_Shepp);
  rs_end_node_.SetGearType(rs_path_.paths[0].gear);

  // collision check
  if (!RSPathCollisionCheck(current_node, &rs_path_)) {
    rs_end_node_.ClearPath();
  ...
  rs_end_node_.SetPre(current_node);

  if (!rs_end_node_.IsNodeValid()) {
...
bool HybridAStar::IsAllPathSegmentLongEnough(
...
bool HybridAStar::IsRsPathFirstSegmentLongEnough(
  ...
  if (same_gear_path_min_dist < rs_path_seg_advised_dist) {
    // ILOG_INFO << " rs path first seg len is small " << same_gear_path_min_dist;
    return false;
  }
...
bool HybridAStar::RSPathCollisionCheck(Node3d* current_node,
                                       const RSPath* reeds_shepp_to_end) {
  // length check
  ...
  bool is_valid = IsRSPathSafeByEDT(reeds_shepp_to_end, &rs_end_node_);
  // if (!is_valid) {
    // ILOG_INFO << "rs collision "
...
bool HybridAStar::ValidityCheckByConvex(Node3d* node) {
  ...
  for (size_t i = check_start_index; i < node_step_size; ++i) {
    ...
    RULocalPolygonToGlobal(&global_polygon, veh_local_polygon,
                                &global_pose);
...
void HybridAStar::GetPathByBicycleModel(NodePath* path, const double arc,
  ...
  for (int i = 0; i < path_point_num; ++i) {
    UpdatePoseBySamplingNumber(old_point, radius,
                               kinetics_model_number,
...
int HybridAStar::NextNodeGenerator(Node3d* new_node, Node3d* parent_node,
...
double HybridAStar::GenerateHeuristicCostByRsPath(
...
double HybridAStar::GenerateRefLineHeuristicCost(Node3d* next_node,
  ...
  double heading_cost =
      dist_to_go +
      std::fabs(GetThetaDiff(theta1, theta2)) * heading_cost_weight;
...
const bool HybridAStar::GenerateResult(HybridAStarResult* result) {
  Node3d* parent_node = nullptr;
  Node3d* child_node = &rs_end_node_;
...
bool HybridAStar::PlanOnce(const Pose2D& start, const Pose2D& end,
  ...
  //           << shrink_map_bounds_.y_min << " ,xmax " << shrink_map_bounds_.x_max
  ...
  while (!open_pq_.empty()) {
    ...
    for (size_t i = 0; i < next_node_num_; ++i) {
      NextNodeGenerator(&new_node, current_node, i);
      explored_node_num++;
...
void HybridAStar::DebugObstacleString() const {
  ...
  //   ILOG_INFO << "type " << static_cast<int>(obstacles_->obs_list[i].obs_type)
  ...
  //               << " center " << obstacles_->obs_list[i].polygons[j].center_pt.x
  //               << " " << obstacles_->obs_list[i].polygons[j].center_pt.y
  //               << "pt size" << obstacles_->obs_list[i].polygons[j].vertex_num;
  //   }
  // }
...
void HybridAStar::DebugRSPath(const RSPath* reeds_shepp_path) {
  ILOG_INFO << "rs seg size " << reeds_shepp_path->size
            << " total len " << reeds_shepp_path->total_length;
...
const std::vector<ad_common::math::Vec2d>& HybridAStar::GetChildNodeForDebug() {
  ILOG_INFO << "child node size" << child_node_debug_.size();
...
void HybridAStar::KineticsModel(const Pose2D* old_pose,
                                const double radius, Pose2D* pose,
                                const bool is_forward) {
