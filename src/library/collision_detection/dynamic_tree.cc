  ...
  if (root1->IsLeaf() && root2->IsLeaf()) {
    if (!(root1->aabb.overlap(root2->aabb))) {
      ...
      return callback(static_cast<CollisionObject *>(root1->userData),
                      static_cast<CollisionObject *>(root2->userData), cdata);
    }
  }
  if (!(root1->aabb.overlap(root2->aabb))) {
    return false;
...
bool b2DynamicTree::DistanceRecurse(const int32 root_id1, const int32 root_id2,
  ...
  if (root2->IsLeaf() ||
    ...
    if (d2 < d1) {
      if (d2 < min_dist) {
        if (DistanceRecurse(root1->child2, root_id2, tree, cdata, callback,
                            min_dist))
          return true;
      }

      if (d1 < min_dist) {
        if (DistanceRecurse(root1->child1, root_id2, tree, cdata, callback,
                            min_dist))
          return true;
      }
    } else {
    ...
    if (d2 < d1) {
      if (d2 < min_dist) {
        if (DistanceRecurse(root_id1, root2->child2, tree, cdata, callback,
                            min_dist))
          return true;
      }

      if (d1 < min_dist) {
        if (DistanceRecurse(root_id1, root2->child1, tree, cdata, callback,
                            min_dist))
          return true;
      }
    } else {
