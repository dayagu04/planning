...
namespace cdl {
real GJK2D::Distance(const ShapeProxy2D &P, const ShapeProxy2D &Q,
                     DistResult2D *result) {
  result->clear();
  std::size_t k = 0;
  real norm2Wmax = 0;
  ...
  do {
    squared_v = GetVectorDot(v, v);
    k++;
    p_idx = Support2D(P, -v);
    q_idx = Support2D(Q, v);
    w = P.vertices[p_idx] - Q.vertices[q_idx];
    /** first exit condition */
    if ((squared_v - GetVectorDot(v, w)) <= eps_rel * squared_v) {
      break;
    }
  ...
  result->distance = std::sqrt(GetVectorDot(v, v));
  result->iterations = k;
  ...
  if (result->distance > eps_rel) {
    ...
    /** line contact or point contact */
      ...
      if (GetVectorDot(p1 - p2, p1 - p2) < eps_tol ||
          GetVectorDot(q1 - q2, q1 - q2) < eps_tol) {
        result->status = CollisionStatus::POINT_CONTACT;
        getPointContactDirections(P, Q, result);
      } else {
        result->status = CollisionStatus::LINE_CONTACT;
        result->normal = GetCounterClockwiseNormal(simplex_.vertices.row(1) -
                                                   simplex_.vertices.row(0))
                             .normalized();
      }
    }
     ...
     * or penetration */
      ...
      /** line contact*/
        ...
        result->status = CollisionStatus::LINE_CONTACT;
        result->normal = GetCounterClockwiseNormal(
                             simplex_.vertices.row(2) -
...
real GJK2D::Distance2(const ShapeProxy2D &P, const ShapeProxy2D &Q,
                      DistResult2D *result) {
  result->clear();
  std::size_t k = 0;
  real norm2Wmax = 0;
  ...
  do {
    ...
    w = P.vertices[p_idx] - Q.vertices[q_idx];
    /** first exit condition */
    if ((squared_v - GetVectorDot(v, w)) <= eps_tol) {
      break;
    }
    ...
    if (GetVectorDot(v, v) <= eps_rel * eps_rel * norm2Wmax) {
      break;
    }
...
real GJK2D::Distance3(const ShapeProxy2D &P, const ShapeProxy2D &Q,
                      DistResult2D *result) {
  std::size_t k = 0;
  real norm2Wmax = 0;
  real tennorm = 0;
  ...
  do {
    real squared_v = GetVectorDot(v, v);
    k++;
    p_idx = Support2D(P, -v);
    q_idx = Support2D(Q, v);
    w = P.vertices[p_idx] - Q.vertices[q_idx];
...
bool GJK2D::Collision(const ShapeProxy2D &P, const ShapeProxy2D &Q) {
  ...
  do {
    ...
    if (GetVectorDot(v, w) > 0.0) {
      return false;
    } else {
      /** second exist condition, collision */
      if (simplex_.num == 2) {
        real a = GetVectorCross(simplex_.vertices.row(0), w);
        real b = GetVectorCross(simplex_.vertices.row(1), w);
        if (a * b <= 0.0) {
          return true;
        }
      }
...
void GJK2D::setShapeEdgeNormal(Vector2r *vertices, uint32_t &size,
                               Vector2r normals[]) {
  for (std::size_t i = 0; i < size; ++i) {
    std::size_t j = (i + 1 < size) ? i + 1 : 0;
    Vector2r edge = vertices[j] - vertices[i];
    normals[i] = GetClockwiseNormal(edge).normalized();
  }
}
...
bool GJK2D::GJKBasedRayCast(RayCastRequest *input, RayCastResult *output) {
  ...
  for (i = 0; i < max_iter; ++i) {
    ...
    vw = GetVectorDot(v, w);
    if (vw > 0.0) {
      vr = GetVectorDot(v, input->direction);
      if (vr <= 0.0) {
        output->status = RayCastStatus::NO_HIT;
        return false;
      } else {
...
bool GJK2D::shapeCast(ShapeCastResult *output, ShapeCastRequest *input) {
  ...
  for (iter = 0; iter < max_iter; ++iter) {
    ...
    if (simplex_.num > 0) {
      if (squared_v - GetVectorDot(p_moving, v) < eps_tol) {
        SubDistance(simplex_, v);
        if (v.norm() < eps_rel) {
          break;
        } else {
  ...
  output->clear();
  for (i = 0; i < simplex_.num; ++i) {
    output->collision_pointA +=
        simplex_.lambdas(i) * P.vertices[simplex_.pwids(i)];
    output->collision_pointB +=
        simplex_.lambdas(i) * Q.vertices[simplex_.qwids(i)];
  }
