...
// counterclockwise
struct ShapeProxy2D {
  PointType vertices[MAX_POLYGON_VERTEX_NUM];
  uint32_t size;
};
...
inline std::size_t Support2D(const ShapeProxy2D &body, const Vector2r &v)
{
    int32_t better = -1;
    real maxs;
    maxs = GetVectorDot(body.vertices[0], v);
    for (std::size_t i = 0; i < body.size; ++i)
    {
        real s = GetVectorDot(body.vertices[i], v);
        if (s > maxs)
        {
            maxs = s;
            better = i;
...
inline size_t getClimbDirection(const ShapeProxy2D &body, const Vector2r &v,
  ...
  while (next_id <= last_id) {
    next = GetVectorDot(v, body.vertices[next_id]);
    if (next > *value) {
      best = next;
      best_index = next_id;
      for (size_t i = next_id + 1; i < body.size; ++i) {
        *value = GetVectorDot(v, body.vertices[i]);
        if (*value < best) {
          return best_index;
        } else if (*value > best) {
          best = *value;
    ...
    last = GetVectorDot(v, body.vertices[last_id]);
    if (last > *value) {
      best = last;
      best_index = last_id;
...
inline size_t Support2D(const ShapeProxy2D &body, const Vector2r &v) {
  ...
  if (size >= 3) {
    ...
    next = GetVectorDot(v, body.vertices[1]);
    value = GetVectorDot(v, body.vertices[0]);
    last = GetVectorDot(v, body.vertices[size - 1]);
    real best;
    ...
    last = GetVectorDot(v, body.vertices[0]);
    next = GetVectorDot(v, body.vertices[1]);
...
class GJK2D {
 public:
  ...
  bool localMinimum1(const ShapeProxy2D &body, const Vector2r &v) {
    ...
    int32_t better = 0;
    std::size_t i;
    real max0 = GetVectorDot(body.vertices[0], v);
    real s;

    for (i = 1; i < body.size; ++i) {
      s = GetVectorDot(body.vertices[i], v);
      if (s > max0) {
        max0 = s;
        better = i;
      } else if (s == max0) {
  ...
  bool localMinimum2(const ShapeProxy2D &P, const ShapeProxy2D &Q,
    ...
    /** judge the edge linked by the two support points whether contains the
        origin. */
    if (GetVectorDot(v, next_support_p) < 0.0 &&
        ifly_fequal(GetVectorCross(v, next_support_p), 0.0)) {
      v = GetCounterClockwiseNormal(v - next_support_p);
      return true;
    }
    return false;
  }
   ...
   */
  void localMinimum(const ShapeProxy2D &P, const ShapeProxy2D &Q, Vector2r &v) {
    std::size_t vp_idx, vq_idx;
    /** local minimum1: add deflection */
    if (localMinimum1(P, v) && localMinimum1(Q, -v)) {
      v += GetCounterClockwiseNormal(v) * eps_deflection;
  ...
  void getPointContactDirections(const ShapeProxy2D &P, const ShapeProxy2D &Q,
    ...
    switch (simplex_.num) {
      ...
      case 3: {
        ...
        real p_lower = atan2(last_cur_vector_p(1), last_cur_vector_p(0));
        real p_upper = atan2(next_cur_vector_p(1), next_cur_vector_p(0));

        if (p_upper < p_lower) {
          p_upper = p_upper + cdl::constants::pi() * 2.0;
        }

        real q_lower = atan2(cur_last_vector_q(1), cur_last_vector_q(0));
        real q_upper = atan2(cur_next_vector_q(1), cur_next_vector_q(0));
  ...
  void S1D2d(Simplex2D &simplex, Vector2r &v) {
    ...
    /** a region */
    real d_ab_a = -GetVectorDot(a, e_ab);
    if (d_ab_a <= 0.0f) {
      simplex.num = 1;
    ...
    /** b region */
    real d_ab_b = GetVectorDot(b, e_ab);
    if (d_ab_b <= 0.0f) {
      v = b;
      simplex.num = 1;
      simplex.lambdas(0) = 1.0;
  ...
  enum struct VertexRegion { C_VERTEX, B_VERTEX, A_VERTEX };
  ...
  void S2D2dVertexRegion(const Vector2r &vertex, const Vector2r &left,
    ...
    if (GetVectorDot(left_edge, right_edge) < 0.0) {
      real v_left_e, left_e, inv_e;
      /** determine the voronoi region */
      v_left_e = GetVectorDot(vertex, left_edge);
      if (v_left_e > 0.0) {
        /** c-ac edge, b-ab edge, or a-ba edge */
        left_e = -GetVectorDot(left, left_edge);
      ...
      v_right_e = GetVectorDot(vertex, right_edge);
      if (v_right_e > 0.0) {
        /** c-BC edge, b-cb edge, a-ca edge */
        right_e = -GetVectorDot(right, right_edge);
