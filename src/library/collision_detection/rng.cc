...
namespace cdl {
RNG::RNG()
    : generator_(Seed::getNextSeed()), uniDist_(0, 1), normalDist_(0, 1) {}
...
int32 RNG::uniformInt(int32 lower_bound, int32 upper_bound) {
  int32 r =
      (int32)floor(uniformReal((real)lower_bound, (real)(upper_bound) + 1.0));
...
int RNG::halfNormalInt(int r_min, int r_max, real focus) {
  int r =
      (int)std::floor(halfNormalReal((real)r_min, (real)(r_max) + 1.0, focus));
...
void RNG::ball(real r_min, real r_max, real& x, real& y, real& z) {
  ...
  auto c = uniform01();
  auto r =
      std::pow(a * std::pow(r_max, 3) + (1 - a) * std::pow(r_min, 3), 1 / 3.0);
