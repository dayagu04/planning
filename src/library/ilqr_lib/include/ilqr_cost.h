...
class BaseCostTerm {
 public:
  ...
  virtual void GetGradientHessian(const State &x, const Control &u, LxMT &lx, LuMT &lu, LxxMT &lxx, LxuMT &lxu,
                                  LuuMT &luu) = 0;
  virtual void GetDiffGradientHessian(const State &x, const Control &u, LxMT &lx, LuMT &lu, LxxMT &lxx, LxuMT &lxu,
                                      LuuMT &luu);
  virtual std::string GetCostString() = 0;
  virtual uint8_t GetCostId() = 0;
  void SetConfig(IlqrCostConfig *cost_config_ptr) { cost_config_ptr_ = cost_config_ptr; }
