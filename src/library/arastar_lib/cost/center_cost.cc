...
CenterCost::CenterCost(const double weight, const double ego_wheel_base,
    ...
    : BaseCost(BaseCost::CostType::CENTER, weight), ego_wheel_base_(ego_wheel_base), ego_lane_(ego_lane){
...
double CenterCost::MakeCost(Node3D& vertex) const {
  ...
  if (!ego_lane_->XYToSL(front_axis_position.x(), front_axis_position.y(), &ego_front_s,
                         &ego_front_l)) {
  ...
  // double cost = std::max(max_l, std::pow(2, max_l));  use square to make the cost more sensitive
