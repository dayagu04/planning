...
real EPA2D::originToEdgeDistance(const Vector2r& point1,
  ...
  Vector2r orthogonal = GetClockwiseNormal(point2 - point1);
  orthogonal.normalize();
  return GetVectorDot(point1, orthogonal);
}
...
void EPA2D::enableCounterClockwise(Simplex2D& simplex_) {
  ...
  real aCrossB = GetVectorCross(ab, ac);
...
real EPA2D::pointToEdgeDistance(const Vector2r& point, const Vector2r& edge_p0,
  ...
  Vector2r orthogonal = GetCounterClockwiseNormal(edge_p1 - edge_p0);
  orthogonal.normalize();
  return fabs(GetVectorDot(edge_p0 - point, orthogonal));
}
...
void EPA2D::zeroOrderSimplexExpanding(const ShapeProxy2D& P,
  ...
  for (size_t i = 0; i < P.size; ++i) {
    for (size_t j = 0; j < Q.size; ++j) {
      ...
      diff = point - point_a;
      if (GetVectorDot(diff, diff) < 1e-10) {
        continue;
      } else {
...
void EPA2D::firstOrderSimplexExpanding(const ShapeProxy2D& P,
  ...
  Vector2r orthogonal =
      GetClockwiseNormal(point_b - point_a); /** generate clockwise simplex */
  ...
  if (pointToEdgeDistance(point, point_a, point_b) < cdl::constants::eps()) {
    ...
    real d12_2 = -GetVectorDot(point_a, e12);
    real d12_1 = GetVectorDot(point_b, e12);
  ...
  Vector2r orthogonal_right_hand = GetCounterClockwiseNormal(point_b - point_a);
...
void EPA2D::Penetration(const ShapeProxy2D& P, const ShapeProxy2D& Q,
  ...
  for (i = 0; i < max_iter; ++i) {
    ...
    dir = GetCounterClockwiseNormal(point_1 - point_2);
    ...
    dir.normalize();
    projection = GetVectorDot(new_vertex, dir);
    if (projection - closest_distance < eps_tol) {
      ...
      real d12_2 = -GetVectorDot(point_1, e12);
      real d12_1 = GetVectorDot(point_2, e12);
