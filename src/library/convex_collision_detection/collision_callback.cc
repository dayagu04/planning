...
namespace cdl {
static cdl::ShapeProxy2D bodyA, bodyB;
static cdl::GJK2D gjk_solver;
static cdl::DistResult2D gjk_result;
static cdl::EPA2D epa_solver;
static Contact ct;
...
std::size_t collide(const CollisionObject *o1, const CollisionObject *o2,
  ...
  if (o1->collisionGeometry()->getNodeType() == GEOM_CONVEX) {
    const Convex *obj1 = static_cast<const Convex *>(o1->collisionGeometry());
  ...
  if (o2->collisionGeometry()->getNodeType() == GEOM_CONVEX) {
    const Convex *obj2 = static_cast<const Convex *>(o2->collisionGeometry());
...
real distance(const CollisionObject *o1, const CollisionObject *o2,
  ...
  if (request.enable_signed_distance_request) {
    if (gjk_result.status == cdl::CollisionStatus::PENETRATION) {
      epa_solver.Penetration(bodyA, bodyB, gjk_solver.simplex_, gjk_result);
    }
  }
