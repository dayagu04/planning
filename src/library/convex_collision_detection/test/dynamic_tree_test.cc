...
#include "./../morton2d.h"
#include "./../rng.h"
#include "./../convex_collision_detection/dynamic_tree.h"
...
TEST(test_b2DynamicTree, test_radixSort) {
  ...
  for (i = 0; i < DYNAMIC_TREE_SIZE_TEST; i++) {
    cdl::RNG rng;
    Vector2r c(rng.uniformReal(lowBound, upBound),
               rng.uniformReal(lowBound, upBound));
    Vector2r d(rng.uniformReal(lowBound, upBound),
               rng.uniformReal(lowBound, upBound));
    AABB aabb(c, d);
    node[i].aabb = aabb;
  }
  ...
  for (i = 0; i < DYNAMIC_TREE_SIZE_TEST; ++i) {
    node[i].code = coder(node[i].aabb.center()); /**< generate morton code */
  }
