...
void drawObjectAndCollision(cdl::ShapeProxy2D &shape1,
                            cdl::ShapeProxy2D &shape2,
  ...
  if (result.status == cdl::RayCastStatus::HIT) {
    // display hit spot
...
TEST(GJKBASED_RAYCAST, HIT_rng) {
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    if (cdl::GetVectorDot(flag, flag) > 0.00001) {
      std::cout << "------------------- " << std::endl;
      printf("not match between hit spot and hit lambda \n");
      std::cout << "hit spot " << result.hit_spot << std::endl;
      std::cout << "hit lambda "
