...
int32_t collision_point_is_overlap(const cdl::ShapeCastRequest &input,
                                   const cdl::ShapeCastResult &result) {
  if (!result.collided) {
    return 1;
  }
  ...
  if (cdl::GetVectorDot(diff, diff) < cdl::constants::eps()) {
    return 1;
  }
  printf("diff %.4f \n", cdl::GetVectorDot(diff, diff));
  return 0;
}
...
TEST(shapecast_with_dir, test1) {
  ...
  planning::GJK2DInterface interface;

  interface.ShapeCastByDirection(&is_collision, &dist_a, &dist_b, &pos_a,
                                 &pos_b, &poly_a, &dir_a, &poly_b, &dir_b);
...
TEST(shape_case_test_viz2d, viz2d_start)
{
    ...
    start_a.x = 1003963.497227;
    start_a.y = 3407264.221959;
    start_a.theta = 0.01;

    end_a.x = 1003973.497098;
    end_a.y = 3407264.272719;
    end_a.theta = 0.01;

    start_b.x = 1003976.054427;
    start_b.y = 3407267.279113;
    start_b.theta = 4.72;

    end_b.x = 1003976.122999;
    end_b.y = 3407257.279348;
    end_b.theta = 4.72;

    ret = GenerateRectPolygon_base_1point(
            &poly_a_start, &start_a, length, width);
    EXPECT_EQ(ret, 0);

    ret = GenerateRectPolygon_base_1point(&poly_a_end, &end_a, length, width);
    EXPECT_EQ(ret, 0);
    ...
    ret = GenerateRectPolygon_base_1point(&poly_b_end, &end_b, length, width);
    EXPECT_EQ(ret, 0);
#else
    ...
    UpdatePolygonValue(
            &poly_a_start, NULL, false, false, MAX_FLOAT64);
    ...
    UpdatePolygonValue(
            &poly_a_end, NULL, false, false, MAX_FLOAT64);
    ...
    UpdatePolygonValue(
            &poly_b_start, NULL, false, false, MAX_FLOAT64);
    ...
    UpdatePolygonValue(
            &poly_b_end, NULL, false, false, MAX_FLOAT64);
#endif
    ...
    interface.ShapeCast(&is_collision, &dist_a, &dist_b, &pos_a, &pos_b,
    ...
    Pose2Dbase_pose;
    base_pose.x = poly_a_start.vertexes[0].x;
    base_pose.y = poly_a_start.vertexes[0].y;
    base_pose.theta = 0.0;
