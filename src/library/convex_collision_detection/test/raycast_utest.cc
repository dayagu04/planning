...
TEST(GJKBASED_RAYCAST, HIT2) {
  ...
  EXPECT_EQ(cdl::RayCastStatus::HIT, result.status);
  /** travesal version */
...
TEST(GJKBASED_RAYCAST, no_hit) {
  ...
  EXPECT_EQ(cdl::RayCastStatus::NO_HIT, result.status);
  /** travesal version */
  gjk_solver.traversalRayCast(&input, &result);
...
TEST(GJKBASED_RAYCAST, source_is_inside) {
  ...
  EXPECT_EQ(cdl::RayCastStatus::SOURCE_IS_INSIDE, result.status);
  EXPECT_EQ(result.hit_edge_id, 1);
...
TEST(raycast, raycast_20211026_bug)
{
    ...
    UpdatePolygonValue(&poly, NULL, false, false, MAX_FLOAT64);
    ...
    interface.RaycastCollisoinCheck(&src, &dir, 20, &poly, &is_collision,
            &collision_point, &hit_edge_id, &ratio, NULL);

    Pose2D base_pose;
    base_pose.x = poly.vertexes[0].x;
    base_pose.y = poly.vertexes[0].y;
    base_pose.theta = 0.0;
    ...
    CvtPosGlobalToLocal(&start_local, &src, &base_pose);
    Position2D end_local, end_global;

    end_global.x = src.x+dir.x;
    end_global.y = src.x+dir.y;

    CvtPosGlobalToLocal(&end_local, &end_global, &base_pose);
...
using namespace cdl;
void printStatus(const cdl::RayCastResult &result) {
  switch (result.status) {
    case cdl::RayCastStatus::SOURCE_IS_INSIDE:
      printf("source is inside \n");
      break;
    case cdl::RayCastStatus::NO_HIT:
      printf("no hit \n");
      break;
    case cdl::RayCastStatus::HIT:
      printf("hit \n");
      break;
    case cdl::RayCastStatus::WILL_HIT:
      printf("will hit \n");
      break;
    default:
      break;
...
TEST(GJKBASED_TRAVERSAL_RAYCAST, WILL_HIT) {
  ...
  EXPECT_EQ(cdl::RayCastStatus::WILL_HIT, result.status);
...
TEST(SHAPECAST, SOURCE_IS_INSIDE) {
  cdl::ShapeCastRequest input;
  cdl::ShapeCastResult output;
  cdl::GJK2D gjk_solver;
  uint32_t size;
  real error = 0.002;

  Position2D P_pos[4] = {{-76996.317957, 4389958.369791},
                         {-76994.318135, 4389958.343173},
                         {-76994.285528, 4389960.792956},
                         {-76996.285351, 4389960.819574}};

  Position2D Q_pos[4] = {{-76993.861329, 4389955.599311},
