...
void spline::make_monotonic() {
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    double EPS = get_eps();
    if (std::fabs(avg) <= EPS && (std::fabs(m_b[i]) > EPS || std::fabs(m_b[i + 1]) > EPS)) {
      modified = true;
      m_b[i] = 0.0;
      m_b[i + 1] = 0.0;
...
std::vector<double> solve_linear(double a, double b) {
  ...
  if (std::fabs(b) <= get_eps()) {
    if (std::fabs(a) <= get_eps()) {
      // 0*x = 0
      x.resize(1);
