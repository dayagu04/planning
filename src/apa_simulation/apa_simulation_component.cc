...
bool ApaSimulationComponent::Init() {
  ...
  const std::string apa_sim_config_path = "/asw/planning/res/conf/apa_sim_config.pb.txt";
  common::util::GetProtoFromFile(apa_sim_config_path, &apa_sim_config_);

  const std::string apa_sim_parking_fusion_info_path = "/asw/planning/res/conf/apa_sim_parking_fusion_info.pb.txt";
  common::util::GetProtoFromFile(apa_sim_parking_fusion_info_path, &parking_fusion_info_msg_);
  ...
  // 2.定义收发topics
  auto planning_output_reader = simulation_node_->CreateReader<PlanningOutput>(
      "/iflytek/planning/plan", [this](const std::shared_ptr<PlanningOutput>& planning_msg) {
  ...
  localization_estimate_writer_ =
      simulation_node_->CreateWriter<LocalizationEstimate>("/iflytek/localization/ego_pose");
  parking_fusion_info_writer_ = simulation_node_->CreateWriter<ParkingFusionInfo>("/iflytek/fusion/parking_slot");
  vehicle_service_output_info_writer_ =
      simulation_node_->CreateWriter<VehicleServiceOutputInfo>("/iflytek/vehicle_service");
  func_state_machine_writer_ = simulation_node_->CreateWriter<FuncStateMachine>("/iflytek/system_state/soc_state");

  key_input_thread_ = std::make_shared<std::thread>(&ApaSimulationComponent::GetKeyInput, this);
...
void ApaSimulationComponent::MockLocalizationAndVehicleService() {
  ...
  if (last_planning_gear_ != cur_planning_output_.gear_command().gear_command_value()) {
    traj_pt_index_ = 0;
  }
  ...
  if (cur_planning_output_.trajectory().trajectory_points_size() > 1) {
    traj_pt_index_ = std::min(traj_pt_index_, cur_planning_output_.trajectory().trajectory_points_size() - 1);
    const auto& traj_points = cur_planning_output_.trajectory().trajectory_points();
  ...
  localization_estimate_msg.mutable_pose()->set_type(LocalizationOutput::Pose::LOCATION_LOCAL);
  auto local_position = localization_estimate_msg.mutable_pose()->mutable_local_position();
  local_position->set_x(ego_x);
  local_position->set_y(ego_y);
  local_position->set_z(0.0);
  auto euler_angles = localization_estimate_msg.mutable_pose()->mutable_euler_angles();
  ...
  last_planning_gear_ = cur_planning_output_.gear_command().gear_command_value();
  last_ego_x_ = ego_x;
  last_ego_y_ = ego_y;
