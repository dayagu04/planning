 ...
 *	\date 2012
 *
 *	Solution analysis class, based on a class in the standard version of the
 *qpOASES
 */
...
returnValue SolutionAnalysis::getHessianInverse(QProblem *qp,
   ...
   *
   * KKT * x =
   *   [delta_g_cov', delta_lbA_cov', delta_ubA_cov', delta_lb_cov',
   * delta_ub_cov]'
   *
  ...
  for (run3 = 0; run3 < NVMAX; run3++) {
    ...
    returnvalue = qp->hotstart_determineStepDirection(
        FR_idx, FX_idx, AC_idx, delta_g_cov, delta_lbA_cov, delta_ubA_cov,
        delta_lb_cov, delta_ub_cov, Delta_bC_isZero, Delta_bB_isZero, delta_xFX,
...
returnValue SolutionAnalysis::getHessianInverse(QProblemB *qp,
  ...
  for (run3 = 0; run3 < NVMAX; run3++) {
    ...
    /* Evaluation of the step: */
    returnvalue = qp->hotstart_determineStepDirection(
        FR_idx, FX_idx, delta_g_cov, delta_lb_cov, delta_ub_cov,
...
returnValue SolutionAnalysis::getVarianceCovariance(QProblem *qp,
  ...
  for (run3 = 0; run3 < KKT_DIM; run3++) {
    ...
    for (run1 = 0; run1 < NVMAX; run1++) {
      ...
      delta_lb_cov[run1] =
          K[run3 + (NVMAX + run1) * KKT_DIM]; /*  ROW WISE LOADING OF THE */
      delta_ub_cov[run1] =
          K[run3 + (NVMAX + run1) * KKT_DIM]; /*  INTERMEDIATE RESULT K   */
    }
    for (run1 = 0; run1 < NCMAX; run1++) {
