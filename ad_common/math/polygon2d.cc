...
void Polygon2d::BuildFromPoints() {
  ...
  // assert(area_ >= 0.0);
  if (std::isnan(area_)) {
    // std::cout << "polygon area_ is nan !" << std::endl;
  }
...
bool Polygon2d::ComputeOverlap(const Polygon2d &other_polygon,
  ...
  for (auto point : other_polygon.points()) {
    if (std::isnan(point.x()) || std::isnan(point.y())) {
      // std::cout << "polygon overlap func: other point is nan" << std::endl;
    }
  }
  for (auto point : points_) {
    if (std::isnan(point.x()) || std::isnan(point.y())) {
      // std::cout << "polygon overlap func: ego point is nan" << std::endl;
    }
  }
