...
CameraStructContainerInterfaceClientImpl::CameraStructContainerInterfaceClientImpl(const uint32_t instanceId)
...
CameraStructContainerInterfaceClientImpl::~CameraStructContainerInterfaceClientImpl()
{
    if (workFlag_) {
        Stop();
    }
...
bool CameraStructContainerInterfaceClientImpl::Init()
{
    if (findServFlag_) {
        return true;
    }
    ara::core::String insIdStr = std::to_string(instanceIdx_);
    serviceHandle_ = CameraStructContainerInterfaceProxy::StartFindService([this](
        ara::com::ServiceHandleContainer<CameraStructContainerInterfaceProxy::HandleType> handles,
        const ara::com::FindServiceHandle findServiceHandle) {
            CameraStructContainerInterfaceCallback(std::move(handles), findServiceHandle);
            },
    ...
    CameraStructContainerInterfaceProxy::StopFindService(serviceHandle_);
    return false;
}

void CameraStructContainerInterfaceClientImpl::CameraStructContainerInterfaceCallback(
    ara::com::ServiceHandleContainer<CameraStructContainerInterfaceProxy::HandleType> handles,
    const ara::com::FindServiceHandle findServiceHandle)
{
    ...
    for (auto singleHandle : handles) {
        ...
        if (singleHandle.GetInstanceId().ToString() == insIdStr) {
            ...
            std::call_once(callFlag_, [&singleHandle, this]() {
                this->proxyPtr_ = std::make_unique<CameraStructContainerInterfaceProxy>(singleHandle);
            });
            break;
        }
    }
...
void CameraStructContainerInterfaceClientImpl::Stop()
{
    workFlag_ = false;
    if (findServFlag_) {
        CameraStructContainerInterfaceProxy::StopFindService(serviceHandle_);
...
void CameraStructContainerInterfaceClientImpl::EmReportExec()
{
...
void CameraStructContainerInterfaceClientImpl::RegisterEventNotifyHandler(const eventHandlerType handler)
{
    if (proxyPtr_ == nullptr) {
...
void CameraStructContainerInterfaceClientImpl::RegisterEventNotifyHandler(const eventHandlerType handler, const std::shared_ptr<ara::com::ThreadGroup>& threadGroup)
{
    if (proxyPtr_ == nullptr) {
        LOG_SPACE::GetLoggerIns("CLNT")->LogError() << "Failed to find the service instance.";
        return;
...
void CameraStructContainerInterfaceClientImpl::PushEventDataToContainer(const eventDataType&& data)
{
...
RecvEventType CameraStructContainerInterfaceClientImpl::GetEventOneData(const uint32_t freshDataTime)
{
    if (!workFlag_) {
...
RecvEventType CameraStructContainerInterfaceClientImpl::GetEventOneDataBlocking(const uint32_t blockTimeout)
{
...
std::vector<RecvEventType> CameraStructContainerInterfaceClientImpl::GetEventNdata(const size_t n)
{
    if (!workFlag_) {
        LOG_SPACE::GetLoggerIns("CLNT")->LogWarn() << "Data receive got stopFlag";
        return {};
