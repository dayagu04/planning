...
#ifndef _IFLYAUTO_COMMON_H_INCLUDE_
#define _IFLYAUTO_COMMON_H_INCLUDE_
...
#define INPUT_HISTORY_TIMESTAMP_MAX_NUM 32
...
// LLH坐标类型
typedef enum {
  LLH_TYPE_UNKNOW = 0,  // 未知
  LLH_TYPE_WGS84 = 1,   // WGS84坐标系
  LLH_TYPE_GCJ02 = 2    // GCJ-02坐标系
} _ENUM_PACKED_ LLHType;
...
typedef enum {
  ...
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_AROUND_VIEW_PERCEPTION_OCCUPANCY_OBJ,
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_AROUND_VIEW_PERCEPTION_3D_OBJ,
  ...
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_PANORAMA_VIEW_CAMERA_PERCEPTION_OCCUPANCY_GRID,
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_PANORAMA_VIEW_CAMERA_PERCEPTION_SPACE_GRID,
  ...
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_FUSION_USS_PERCEPTION,
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_FUSION_OCCUPANCY_OBJ,
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_FUSION_PARKING_SLOT,
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_FUSION_GROUND_LINE,
  ...
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_SDPRO,
  ...
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_FMA_DEGRADED,
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_ENGINEER_INNER,
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_CAMERA_RESTART_STATUS,
  INPUT_HISTORY_TIMESTAMP_SOURCE_TYPE_ADAS_FUNCTION,
} _ENUM_PACKED_ InputHistorySourceType;

typedef struct {
  InputHistorySourceType input_type;  // 数据来源模块
  uint32 seq;                         // 数据源的msg_header.seq，用于离线排查整个链路的数据情况
  uint64 stamp;                       // 数据源的sensor_meta.stamp(微秒)，若不存在sensor_meta.stamp，则使用msg_header.stamp(微秒)，用于在线排查整个链路的数据情况。发现传感器路延时较大，开发者需要记得
} _STRUCT_ALIGNED_ InputHistoryInfo;

// 通用消息附加信息
typedef struct {
  uint64 start_time;  // 仅表示模块本帧运算开始的时间     (微秒)
...
} _STRUCT_ALIGNED_ MsgMeta;

// 传感器消息附加信息
typedef struct {
  uint64 stamp; // 传感器硬件数据产生时刻，ISP为曝光中点时刻，CAN为进入域控MCU时刻 (微秒)
} _STRUCT_ALIGNED_ SensorMeta;
...
// 通用消息头
typedef struct {
  uint32 seq;    // 消息序号，算法输出为输出时递增，传感器为SOC发出时递增
  uint64 stamp;  // 消息发送时刻 (微秒)
} _STRUCT_ALIGNED_ MsgHeader;
...
typedef enum {
  ...
  OBJECT_TYPE_SPECIAL_VEHICLE = 39,       // 特殊车辆，包括救护车、消防车等
  ...
  OBJECT_TYPE_OCC_VEGETATION = 61,      // 植被
  OBJECT_TYPE_OCC_DRIVEABLE_AREA = 62,  // 可行驶区域
  OBJECT_TYPE_OCC_NON_DRIVEABLE_AREA = 63, // 不可行驶区域

  OBJECT_TYPE_OCC_NUM = 64,
...
// 通用车位材质类型
typedef enum {
  PARKING_SLOT_MATERIAL_MARKLINE = 0,     // 划线车位
  PARKING_SLOT_MATERIAL_STONE = 1,        // 砖石车位
  PARKING_SLOT_MATERIAL_SPACE = 2,        // 空间车位
} _ENUM_PACKED_ ParkingSlotMaterial;
...
typedef enum {
  ...
  SENSOR_TYPE_FRONT_30_CAMERA = 16,        // 前视30度相机
...
typedef struct {
  ...
  */
  ...
  boolean is_dangerous;                 // 是否是危险障碍物
  float32 dangerous_conf;               // 危险障碍物得分
...
} _ENUM_PACKED_ RearViewMirrorSignalType;
...
typedef enum {
  ...
  LaneDrivableDirection_DIRECTION_STRAIGHT_RIGHT = 2,         // 直行+右转
  LaneDrivableDirection_DIRECTION_RIGHT = 3,                  // 右转
  LaneDrivableDirection_DIRECTION_STRAIGHT_LEFT = 4,          // 直行+左转
  LaneDrivableDirection_DIRECTION_LEFT = 5,                   // 左转
  LaneDrivableDirection_DIRECTION_UTURN_LEFT = 6,             // 掉头+左转
  LaneDrivableDirection_DIRECTION_UTURN_RIGHT = 7,            // 掉头+右转
  LaneDrivableDirection_DIRECTION_STRAIGHT_LEFT_RIGHT = 8,    // 直行+左转+右转
  LaneDrivableDirection_DIRECTION_STRAIGHT_UTURN_LEFT = 9,    // 直行+左掉头
  LaneDrivableDirection_DIRECTION_STRAIGHT_UTURN_RIGHT = 10,  // 直行+右掉头
  LaneDrivableDirection_DIRECTION_STRAIGHT_OFF_ROUTE = 11,    // 直行+左转+左掉头
  LaneDrivableDirection_DIRECTION_LEFT_UTURN = 12,            // 左掉头
  LaneDrivableDirection_DIRECTION_RIGHT_UTURN = 13,           // 右掉头
  LaneDrivableDirection_DIRECTION_NO_UTURN = 14,              // 禁止掉头
  LaneDrivableDirection_DIRECTION_LEFT_RIGHT = 15,            // 左转+右转
  ...
  LaneDrivableDirection_DIRECTION_X = 19, 
  LaneDrivableDirection_DIRECTION_STRAIGHT_UTURNLEFT_RIGHT = 20, // 直行+左掉头+右转
  LaneDrivableDirection_DIRECTION_UTURNLEFT_RIGHT = 21,       // 左掉头+右转
...
typedef enum {
  ...
  LANETYPE_OPPOSITE = 42,                 // 对向车道
  LANETYPE_DIVERSION_BELT = 43,           // 自车压导流带
...
} _ENUM_PACKED_ StaticFusionResourceType;
...
#endif  //_IFLYAUTO_COMMON_H_INCLUDE_
