...
TEST(test_viz2d, viz2d_init_image) {
  ...
  ret = viz2d_init_window(&image_handle, &font, win_name, win_columns, win_rows,
                          origin_columns_index, origin_rows_index, resolution,
                          background_color);
  EXPECT_EQ(ret, 0);
  ...
  resolution = 0.05;
  viz2d_init_window(&image_handle, &font, win_name, win_columns, win_rows,
                    origin_columns_index, origin_rows_index, resolution,
                    background_color);
...
TEST(test_viz2d, multiply_image_windows) {
  ...
  for (i = 0; i < 3; i++) {
    ret =
        viz2d_init_window(&image_handle[i], &font, win_name[i], win_columns[i],
                          win_rows[i], origin_columns_index[i],
                          origin_rows_index[i], resolution, background_color);
    EXPECT_EQ(ret, 1);
