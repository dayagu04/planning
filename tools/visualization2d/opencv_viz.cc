...
static int cv_font_names[viz2d_font_num] = {
...
int viz2d_draw_xy_axis(viz2d_image *img_handle) {
  ...
  y_end.y = ifly_round(origin_row_index + 20);
  ...
  cvLine(img, x_end, x_end_left_tail, CVUI_SCALAR(viz2d_colors_green), 1, CV_AA,
         0);
  ...
  cvLine(img, y_end, y_end_left_tail, CVUI_SCALAR(viz2d_colors_green), 1, CV_AA,
         0);
...
int viz2d_init_window(viz2d_image *img_handle, const viz2d_font_setting *font,
                      const char *win_name, int columns, int rows,
                      int origin_column_index, int origin_row_index,
                      double resolution, viz2d_color background_color) {
...
int viz2d_get_cvfont(CvFont *cvfont, const viz2d_font_setting *font_setting) {
  ...
  cvInitFont(cvfont, cv_font_names[font_type], font_setting->hscale,
             font_setting->vscale, font_setting->shear, font_setting->thickness,
             font_setting->line_type);
...
int viz2d_get_index(viz2d_image *img_handle, CvPoint *index, double x,
  ...
  index_x = ifly_floor(y / resolution) + origin_column_index;
  index_y = ifly_floor(x / resolution) + origin_row_index;
  if (index_x < 0) {
    index_x = 0;
...
int viz2d_draw_circle(viz2d_image *img_handle, double x, double y,
  ...
  resolution = img_handle->resolution;
  R = std::fmax((int)ifly_round(radius / resolution), 1);
  img = img_handle->image;
...
int viz2d_draw_line(viz2d_image *image_handle, double x1, double y1, double x2,
                    double y2, int thickness, viz2d_color color_index) {
  int ret;
...
int viz2d_draw_int_var_text(viz2d_image *img_handle, const char *description,
                            int var, double width_percentage,
                            double height_percentage, viz2d_color color_index) {
  int ret;
  CvFont font;
  IplImage *img;
  CvScalar color;
  ...
  img = img_handle->image;
  text_start_pt.x = ifly_round(img_handle->columns * width_percentage);
  text_start_pt.y = ifly_round(img_handle->rows * height_percentage);
...
int viz2d_draw_arrow(viz2d_image *img_handle, const Pose2D *ego_pose,
  ...
  ret = viz2d_get_index(img_handle, &center, ego_pose->x, ego_pose->y);

  /* theta conversation from ego frame to image frame */
  arrow_heading_theta = M_PI / 2 - ego_pose->theta;
  arrow_heading_theta = IflyUnifyTheta(arrow_heading_theta, 0);

  center_end.x = center.x;
  center_end.x += delta_length * ifly_cos(arrow_heading_theta);
  center_end.y = center.y;
  center_end.y += delta_length * ifly_sin(arrow_heading_theta);

  arrow_angle = M_PI / 4;
  delta_theta = arrow_heading_theta - arrow_angle;
  delta_theta = IflyUnifyTheta(delta_theta, 0);
  end_left.x = center.x;
  end_left.x -= arrow_slope_length * ifly_cos(delta_theta);
  end_left.y = center.y;
  end_left.y -= arrow_slope_length * ifly_sin(delta_theta);

  delta_theta = arrow_heading_theta + arrow_angle;
  delta_theta = IflyUnifyTheta(M_PI / 2 - delta_theta, 0);
  end_right.x = center.x;
  end_right.x -= arrow_slope_length * ifly_sin(delta_theta);
  end_right.y = center.y;
  end_right.y -= arrow_slope_length * ifly_cos(delta_theta);
...
int viz2d_draw_polygon_with_thickness(viz2d_image *image_handle,
  ...
  for (i = 0; i < polygon->vertex_num; i++) {
    ...
    CvtPosGlobalToLocal(&cur_local_position, &cur_position, base_pose);
    ...
    CvtPosGlobalToLocal(&next_local_position, &next_position, base_pose);
...
int viz2d_draw_filled_polygon(viz2d_image *viz2d, const Polygon2D *polygon,
                              viz2d_color color, const Pose2D *base_pose_global,
  ...
  if (ref_pose_is_map_ref_frame) {
    ...
    ret = GlobalPolygonToRULocal(&local_poly, polygon, base_pose_global);
  }

  for (i = 0; i < polygon->vertex_num; i++) {
    pt = local_poly.vertexes[i];
    viz2d_get_index(viz2d, &points[i], pt.x, pt.y);
  }
...
int viz_draw_filled_box(viz2d_image *viz2d, const Position2D *center,
  ...
  CvtPosGlobalToLocal(&local_center, center, base_pose_global);
